...
static int aeApiAddEvent(evEventBase *eventLoop, int fd, int mask) {
     ...
     */
    fullmask = mask | eventLoop->IOEvents[fd].events;
...
static void aeApiDelEvent(evEventBase *eventLoop, int fd, int mask) {
    ...
    if (pfd != -1) {
        ...
        if (state->pending_masks[pfd] == EV_NONE)
    ...
    fullmask = eventLoop->IOEvents[fd].events;
    if (fullmask == EV_NONE) {
        /*
