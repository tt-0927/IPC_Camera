...
#define NETWORK_G711AAUDIO_STREAM 1000005
#define NETWORK_H264_STREAM 1000002
#define NETWORK_H265_STREAM 1000003

#define NETWORK_NET_HEARTBIT_CMD 30032

#define NETWORK_TIME_OUT	(7)			//超时时间，低于4s的话，对接原来的sdk的话，会出现超时，因为原来的sdk心跳间隔为3s多一点，
...
/* 发送数据初始化协议头 */
typedef int(*network_HeadInit)(network_Handle_t handle, void *pHeadBuf,\
				int nBodySize,int nCode,void *user);
/* 接受数据校验协议头 */
typedef int(*network_HeadCheck)(network_Handle_t handle, void *pHeadBuf,\
...
typedef struct network_Inparam_ClientNet_
{
	...
	/* 下面三个自定义头，务必同时存在值 */
	network_HeadInit headInit;		//数据头的组装
	network_HeadCheck headCheck;	//数据头的校验
	int headSize;					//数据头大小
