...
int mpeg4_aac_adts_save(const struct mpeg4_aac_t* aac, size_t payload, uint8_t* data, size_t bytes)
{
	...
	assert(aac->sampling_frequency_index >= 0 && aac->sampling_frequency_index <= 0xc);
#endif
	if(aac->profile <= 0 || aac->profile >= 31)
	{
		return FLV_PROFILE_ERROR;
	}
	if(aac->channel_configuration < 0 || aac->channel_configuration > 7)
	{
		return FLV_CHANNEL_CONFIG_ERROR;
	}
	if(aac->sampling_frequency_index < 0 || aac->sampling_frequency_index > 0xc)
	{
		return FLV_SAMPLE_FREQUENCY_ERROR;
	}
