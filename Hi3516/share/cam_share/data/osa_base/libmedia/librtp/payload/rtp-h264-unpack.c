...
struct rtp_decode_h264_t
{
	...
	uint8_t* ptr;
	int size/*record currect decode size*/, capacity/*current malloc size*/;

	int flags;		//whether the package is lost(RTP_PAYLOAD_FLAG_PACKET_LOST)
};
...
#endif
			unpacker->flags = RTP_PAYLOAD_FLAG_PACKET_LOST;
			return RTP_LOSTPACK; // packet discard
		}
#ifdef DEBUG_ASSERT
...
static int rtp_h264_unpack_input(void* p, const void* packet, int bytes)
{
	...
	{
		printf("seq is not continue!! preSeq[%d] nowSeq[%d]\n",unpacker->seq,pkt.rtp.seq);
		unpacker->flags = RTP_PAYLOAD_FLAG_PACKET_LOST;
	...
	case 0: // reserved
	case 31: // reserved
		//assert(0);	//zhangjb del 2018/12/5
		printf("rtp nal[%d] is not support!!\n",(nal&0x1F));
		return RTP_NALUNDEFINED; // packet discard
