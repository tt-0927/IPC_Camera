...
//sha256生成32字节码
#define B 64
#define L (SHA256_BLOCK_SIZE)
#define K (SHA256_BLOCK_SIZE * 2)
#define I_PAD 0x36
#define O_PAD 0x5C
...
{
	unsigned short i = 0;
	unsigned char kh[SHA256_BLOCK_SIZE] = {0};
	...
	sha256(tmpdata, key_len+B, output);//把kx和输入数据拼接起来算一次sha256

#else

	SHA256_CTX octx;
	sha256_init(&octx);
	sha256_update(&octx, kx, B);
	sha256_update(&octx, data, data_len);
	sha256_final(&octx, output);
	...
	memcpy(tmpdata,kx,B);
	memcpy(&tmpdata[B],output,SHA256_BLOCK_SIZE);
	sha256(tmpdata, SHA256_BLOCK_SIZE+B, output);//把kx和上一步生成的32B数据拼接起来再算一次sha256，输出结果。
}
