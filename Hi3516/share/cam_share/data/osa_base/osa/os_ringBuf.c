...
int OS_ringBufCreate(OS_ringBufHndl *hndl, int buf_num)
{
    ...
    {
        hndl->bufInfo[i] = NULL;
        OS_quePut(&hndl->emptyQue, i, OS_TIMEOUT_FOREVER);
    }
    return status;
}
...
int OS_ringBufSwitchFull(OS_ringBufHndl *hndl, Int64 *bufId, Uint32 timeout)
{
    int status = OS_SOK;
    Int64 newBufId = 0;
    status = OS_ringBufGetEmpty(hndl, &newBufId, OS_TIMEOUT_NONE);
...
int OS_ringBufSwitchEmpty(OS_ringBufHndl *hndl, Int64 *bufId, Uint32 timeout)
{
    int status = OS_SOK;
    Int64 newBufId = 0;
    status = OS_ringBufGetFull(hndl, &newBufId, OS_TIMEOUT_NONE);
    if (status==OS_SOK)
    {
        if (*bufId != OS_RING_BUF_ID_INVALID)
        {
...
//从缓冲区中取数据
static Uint32 __ring_buffer_get(OS_streamRingBufHndl *ring_buf, unsigned char * buffer, Uint32 size)
{
...
//向缓冲区中存放数据
static Uint32 __ring_buffer_put(OS_streamRingBufHndl *ring_buf, unsigned char *buffer, Uint32 size)
{
    if(ring_buf == NULL || (buffer == NULL))
    {
        printf("__ring_buffer_put this argument is null!!!\n");
        return 0;
    }

    // printf("11size:%d , ring_buf->size:%d, in:%d, out:%d\n", size,  ring_buf->size, ring_buf->in, ring_buf->out);
    Uint32 len = 0;
    ...
    ring_buf->in += size;
    // printf("22size:%d , ring_buf->size:%d\n", size,  ring_buf->size);
    return size;
}
...
Uint32 OS_streamRingbufferPut(OS_streamRingBufHndl *hndl, unsigned char *buffer, Uint32 size)
{
    if(((hndl == NULL) || (hndl->init != 1)) || (buffer == NULL))
    {
        printf("OS_streamRingbufferPut this argument is null!!!\n");
        return 0;
    }
...
//从缓冲区丢弃数据
static Uint32 __ring_buffer_throw(OS_streamRingBufHndl *ring_buf, Uint32 size)
{
    if(ring_buf == NULL)
    {
        // printf("this argument is null!!!\n");
