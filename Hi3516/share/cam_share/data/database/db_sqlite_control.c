...
sqlite3* db_sql_init(char *achFileName)
{
	...
	if (nRet < 0)
	{
		dlog(LOG_ERROR,"DB sqlite3_open failed::%s\n",sqlite3_errmsg(pSqlHandle));
		return NULL;
	}
    return pSqlHandle;
}
...
int db_backup_data(sqlite3 *pSqlHandle, int *pIsSave,char *pSqlFileBackup)
{
	...
	{
		dlog(LOG_DEBUG, "%s is not Exist.\n", pSqlFileBackup);
	}

	if (!nHaveBackup && 0 == *pIsSave)
	{
		dlog(LOG_DEBUG, "No %s and main DB,Need create!!", pSqlFileBackup);
		return 0;
	}
	...
	dlog(LOG_DEBUG, "backup sqlite3_open success nRc=%d", nRc);
...
int db_create_table(sqlite3 *pSqlHandle,char *achTableName,char *achSqlData)
{
    ...
    {
		dlog(LOG_ERROR,"DB db_create_table  data is NULL\n");
        return -1;
    }
	...
	// dlog(LOG_DEBUG,"db_create_table[%s] nRet[%d]\n",achTableName,nRet);
	// if (nRet != SQLITE_OK)
	// {
	// 	if(pErrMsg)
	// 	{
	// 		dlog(LOG_DEBUG,"DB sql table[%s] is no! error[%s]\n",achTableName,pErrMsg);
...
int db_control_sql(sqlite3 *pSqlHandle,char *achSqlData)
{
    ...
    {
		dlog(LOG_ERROR,"DB db_control_sql  is NULL\n");
        return -1;
    }
	char *pErrMsg = NULL;
	int nRet = 0;
	...
	{
		//异常，回滚，撤销失败的动作
		dlog(LOG_ERROR,"Error,sqlite3_exec failed!! %s nRet=%d error[%s]\n",achSqlData,nRet,pErrMsg);
		sqlite3_exec(pSqlHandle,"rollback transaction",0,0,&pErrMsg);
		return -1;
	}
    dlog(LOG_DEBUG,"DB perform sql[%s] is succful!\n",achSqlData);
	//提交事务
...
int db_get_sqlData(sqlite3 *pSqlHandle,char *achSqlData,char ***ppOutData,int *nTotal,int *nLine)
{
    if(pSqlHandle == NULL || achSqlData == NULL)
    {
		dlog(LOG_ERROR,"db_get_sqlData is NULL\n");
        return -1;
    }
	...
	if (nRet != SQLITE_OK)
	{
		dlog(LOG_DEBUG,"sqlite3_get_table nRet[%d]\n",nRet);
        if(pErrMsg)
		{
			dlog(LOG_ERROR,"DB sql=%s  <---> Error=%s\n", achSqlData, pErrMsg);
			sqlite3_free(pErrMsg);
		}
	...
	*nLine = nColumn;
	...
	//dlog(LOG_DEBUG,"DB sql=%s  <---> total=%d\n",achSqlData, nRow);
...
int db_char_escape(char *pEsData,int nLen,char *pData)
{
	if(pData == NULL)
	{
        dlog(LOG_TRACE, "db_char_escape return");
		return 0;
	}
