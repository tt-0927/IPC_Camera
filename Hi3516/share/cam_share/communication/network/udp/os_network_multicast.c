/**
 * @FilePath     : os_network_multicast.c
 * @Author       : luoyongkang
 * @Date         : 2021-03-29 14:13:10
 * @LastEditors  : zhouzirui
 * @LastEditTime : 2025-05-12 14:25:54
 * @Description  : UDP接口文件
 */
...
static int os_network_socket(NetworkMulticast_S*  stpnetwork_info)
{
	...
	{
		if (setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, &stpnetwork_info->nsend_opt, sizeof(int)) < 0)
		{
			printf("setsockopt error=%d(%s)!!!\n", errno, strerror(errno));
			goto error;
		}
...
int os_networkbroadcast_send(const char *pdata, int nlenght, NetworkMulticast_S* pNetwork_info)
{
	...
	while( nsend_lenght < nlenght )
	{
		...
		{
			perror("error:");
			printf("os_networkbroadcast_send error nRet[%d]\n",nret);
			break;
		}
		if(nret == -1)
		{
...
void* mucast_asynchronoussend_thr(void* param)
{
	...
	while( !stpnetwork_info->nExit )
	{
		...
		if(nstatus == 0)
		{
			...
			if(abuf != NULL)
			{
				nRet = sendto(stpnetwork_info->nfd, abuf, stpnetwork_info->nasynchronous_sendsize, 
...
void* os_networkmulticast_recv(void* network_info1)
{
	...
	while (!stpnetwork_info->nExit)
	{
		...
		stpnetwork_info->stuser_recv_handle.lenght = 
		           recvfrom(stpnetwork_info->nfd, stpnetwork_info->stuser_recv_handle.data,
		                    nrecv_size, 0, (struct sockaddr *)&stpnetwork_info->stuser_recv_handle.stuRemote,
				            (socklen_t*)&stpnetwork_info->stuser_recv_handle.nRemoteLen);
		if(stpnetwork_info->stuser_recv_handle.lenght < 0) 
			continue;
...
int os_networkmulticast_init(NetworkMulticast_S* stpnetwork_info)
{
	...
	return OS_thrCreate(&stpnetwork_info->recv_thr, os_networkmulticast_recv,OS_JOINABLE,OS_THR_STACK_SIZE_DEFAULT, stpnetwork_info );
}
