...
Boolean OutputSocket::connectToDestination(struct sockaddr_storage const& addressAndPort) {
  if (fIsConnected) {
    // Already connected, check if it's to the same address
    // For simplicity, we'll reconnect if needed
    // (UDP connect can be called multiple times to change destination)
  }
  
  if (connectSocket(env(), socketNum(), addressAndPort)) {
    fIsConnected = True;
    return True;
  }
...
Boolean OutputSocket::write(struct sockaddr_storage const& addressAndPort, u_int8_t ttl,
  ...
  // If socket is connected, use send() instead of sendto() for better error handling
  if (fIsConnected) {
    // For connected socket, we can use send() directly (no need to specify address)
    // But we still need to handle TTL for multicast
    if (addressAndPort.ss_family == AF_INET && IsMulticastAddress(addressAndPort)) {
      // For multicast, we still need to set TTL
...
#define TTL_TYPE u_int8_t
#endif
	TTL_TYPE ttlValue = (TTL_TYPE)ttl;
	if (setsockopt(socketNum(), IPPROTO_IP, IP_MULTICAST_TTL,
		       (const char*)&ttlValue, sizeof ttlValue) < 0) {
	  env().setResultErrMsg("setsockopt(IP_MULTICAST_TTL) error");
	  return False;
	}
    ...
    writeSuccess = writeSocketConnected(env(), socketNum(), buffer, bufferSize);
  } else {
    // Not connected, use the original sendto() method
    if ((unsigned)ttl == fLastSentTTL) {
      // Optimization: Don't do a 'set TTL' system call again
      writeSuccess = writeSocket(env(), socketNum(), addressAndPort, buffer, bufferSize);
    } else {
      writeSuccess = writeSocket(env(), socketNum(), addressAndPort, ttl, buffer, bufferSize);
      fLastSentTTL = (unsigned)ttl;
    }
  }
...
void Groupsock::addDestination(struct sockaddr_storage const& addr, Port const& port,
  ...
  // For unicast addresses, connect the socket to the destination for better error handling
  // This allows using send() instead of sendto(), which can help prevent crashes
  // Only connect if this is the first destination (or if we want to support multiple destinations,
  // we would need separate sockets for each)
  if (fDests != NULL && fDests->fNext == NULL && !IsMulticastAddress(addr)) {
    // This is the first (and only) destination, and it's unicast - connect to it
    struct sockaddr_storage destAddrAndPort = addr;
    setPortNum(destAddrAndPort, port.num());
    if (!connectToDestination(destAddrAndPort)) {
      if (DebugLevel >= 1) {
	env() << *this << ": failed to connect to destination: "
	      << env().getResultMsg() << "\n";
      }
      // Continue anyway - we'll fall back to sendto()
    }
  }
}
