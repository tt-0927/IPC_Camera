...
int setupStreamSocket(UsageEnvironment& env, Port port, int domain, int dscp = 0,
...
Boolean connectSocket(UsageEnvironment& env,
		      int socket, struct sockaddr_storage const& addressAndPort);
    // Connect a UDP socket to a specific destination address and port.
    // This allows using send() instead of sendto() for better error handling.

Boolean writeSocketConnected(UsageEnvironment& env,
			      int socket, unsigned char* buffer, unsigned bufferSize);
    // Send data on a connected UDP socket using send() instead of sendto().
