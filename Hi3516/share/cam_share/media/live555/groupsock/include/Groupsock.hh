...
protected:
  ...
  // Connect the socket to a destination address (for UDP, this allows using send() instead of sendto())
  Boolean connectToDestination(struct sockaddr_storage const& addressAndPort);
  
  // Check if socket is connected
  Boolean isConnected() const {return fIsConnected;}
...
private:
  ...
  Boolean fIsConnected;  // Track if socket is connected to a destination
};
