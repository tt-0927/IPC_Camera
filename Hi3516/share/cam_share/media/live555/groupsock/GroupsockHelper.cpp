...
int setupStreamSocket(UsageEnvironment& env, Port port, int domain, int dscp,
  ...
  /* 添加 QoS 设置：DSCP 标记（示例为 EF 优先级，DSCP=46） */
  // int dscp = 46 << 2;  // EF = 0xB8（二进制 10111000）
  printf("设置媒体dscp的qos策略(0~63): %d\n", dscp);
  int dscpValue = dscp << 2;
  if (setsockopt(newSocket, IPPROTO_IP, IP_TOS,
     (const char*)&dscpValue, sizeof dscpValue) < 0) {
...
Boolean connectSocket(UsageEnvironment& env,
		      int socket, struct sockaddr_storage const& addressAndPort) {
  do {
    SOCKLEN_T dest_len = addressSize(addressAndPort);
    if (connect(socket, (struct sockaddr const*)&addressAndPort, dest_len) < 0) {
      char tmpBuf[100];
      sprintf(tmpBuf, "connectSocket(%d) error: ", socket);
      socketErr(env, tmpBuf);
      break;
    }
...
Boolean writeSocketConnected(UsageEnvironment& env,
			      int socket, unsigned char* buffer, unsigned bufferSize) {
  do {
    int bytesSent = send(socket, (char*)buffer, bufferSize, 0);
    if (bytesSent != (int)bufferSize) {
      char tmpBuf[100];
      sprintf(tmpBuf, "writeSocketConnected(%d), send() error: wrote %d bytes instead of %u: ", socket, bytesSent, bufferSize);
      socketErr(env, tmpBuf);
      break;
    }
