...
/* 当前连接的客户端个数 */
static volatile int gs_nClientCount = 0;
/* 互斥锁，用于保护客户端个数的更新操作 */
static std::mutex gs_connectMutex;
...
RTSPServer* RTSPServer::createNew(UsageEnvironment& env,
                                  ...
                                  Authenticator::AuthAlgorithm authAlgorithm)
{
    ...
    return new RTSPServer(env, ourSocketIPv4, ourSocketIPv6, ourPort, authDatabase, reclamationSeconds, authAlgorithm);
}
...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort)
{
    ...
    if (fHTTPServerSocketIPv4 >= 0 || fHTTPServerSocketIPv6 >= 0)
    {
        ...
        envir().taskScheduler().turnOnBackgroundReadHandling(fHTTPServerSocketIPv4,
                                                             incomingConnectionHandlerHTTPIPv4,
                                                             this);
                                             ...
                                             char const* urlSuffix)
{
    // default implementation
    // info ------------------ zhouzr ------------------
    char clientIP[INET6_ADDRSTRLEN] = { 0 }; // 存储IP字符串的缓冲区
    ...
    {
        struct sockaddr_in6* s6 = (struct sockaddr_in6*) &clientAddr;
        inet_ntop(AF_INET6, &(s6->sin6_addr), clientIP, INET6_ADDRSTRLEN);
    }
    else
    {
        strcpy(clientIP, "Unknown Address Family");
    }
    live_log("客户端 IP: %s", clientIP);

    if (handshakeAuthCallback)
    {
        handshakeAuthCallback(clientIP);
    }
    else
    {
        live_log("未设置客户端连接握手回调函数");
    }
    return True;
    // info ------------------ zhouzr ------------------
}
...
RTSPServer::RTSPServer(UsageEnvironment& env,
                       ...
                       unsigned reclamationSeconds,
                       Authenticator::AuthAlgorithm authAlgorithm)
    : GenericMediaServer(env, ourSocketIPv4, ourSocketIPv6, ourPort, reclamationSeconds), fHTTPServerSocketIPv4(-1),
      fHTTPServerSocketIPv6(-1), fHTTPServerPort(0),
      ...
      fOurConnectionsUseTLS(False), fWeServeSRTP(False), fAuthAlgorithm(authAlgorithm)
{
}
// info ------------------ zhouzr ------------------
...
void RTSPServer ::unnoteTCPStreamingOnSocket(int socketNum, RTSPClientSession* clientSession, unsigned trackNum)
{
    if (socketNum < 0)
        return;
    streamingOverTCPRecord* sotcpHead = (streamingOverTCPRecord*) fTCPStreamingDatabase->Lookup(
...
RTSPServer::RTSPClientConnection ::RTSPClientConnection(RTSPServer& ourServer,
                                                        ...
                                                        struct sockaddr_storage const& clientAddr,
                                                        Boolean useTLS)
    : GenericMediaServer::ClientConnection(ourServer, clientSocket, clientAddr, useTLS), fOurRTSPServer(ourServer),
      fClientInputSocket(fOurSocket), fClientOutputSocket(fOurSocket), fPOSTSocketTLS(envir()),
      fAddressFamily(clientAddr.ss_family), fIsActive(True), fRecursionCount(0), fOurSessionCookie(NULL),
      fScheduledDelayedTask(0), fCurrentAuthenticator(ourServer.fAuthAlgorithm)
{
    ...
    {
        strcpy(clientIP, "UnknownAF");
    }
    live_log("新客户端[%s]连接", clientIP);

    // fOurRTSPServer.registerClientConnection(clientIP, this);
    // info ------------------ zhouzr ------------------
    resetRequestBuffer();
}
// info ------------------ zhouzr ------------------
...
RTSPServer::RTSPClientConnection::~RTSPClientConnection()
{
    ...
    {
        // We were being used for RTSP-over-HTTP tunneling. Also remove ourselves from the 'session cookie' hash table
        // before we go:
    ...
    // info ------------------ lianghy ------------------
    char clientIP[INET6_ADDRSTRLEN] = "unknown";
    ...
    // fOurRTSPServer.unregisterClientConnection(clientIP, this);
    // info ------------------ lianghy ------------------
    closeSocketsRTSP();
}
...
{
    // By default, we implement "GET_PARAMETER" (on the entire server) just as a 'no op', and send back a dummy
    // response. (If you want to handle this type of "GET_PARAMETER" differently, you can do so by defining a subclass
    // of "RTSPServer" and "RTSPServer::RTSPClientConnection", and then reimplement this virtual function in your
    // subclass.)
...
{
    // By default, we implement "SET_PARAMETER" (on the entire server) just as a 'no op', and send back an empty
    // response. (If you want to handle this type of "SET_PARAMETER" differently, you can do so by defining a subclass
    // of "RTSPServer" and "RTSPServer::RTSPClientConnection", and then reimplement this virtual function in your
    // subclass.)
...
void RTSPServer::RTSPClientConnection ::DESCRIBELookupCompletionFunction(void* clientData,
...
void RTSPServer::RTSPClientConnection::handleCmd_redirect(char const* urlSuffix)
{
    snprintf((char*) fResponseBuffer,
             ...
             fCurrentCSeq,
             dateHeader(),
             fOurRTSPServer.rtspURLPrefix(fClientInputSocket),
             urlSuffix);
}
...
#endif
    // Construct a response to the "OPTIONS" command that notes that our special headers (for RTSP-over-HTTP tunneling)
    // are allowed:
    snprintf((char*) fResponseBuffer,
             sizeof fResponseBuffer,
             "HTTP/1.0 200 OK\r\n"
...
{
    // Record ourself as having this 'session cookie', so that a subsequent HTTP "POST" command (with the same 'session
    // cookie') can find us:
...
{
    // Use the "sessionCookie" string to look up the separate "RTSPClientConnection" object that should have been used
    // to handle an earlier HTTP "GET" request:
    if (fOurRTSPServer.fClientConnectionsForHTTPTunneling == NULL)
    {
        fOurRTSPServer.fClientConnectionsForHTTPTunneling = HashTable::create(STRING_HASH_KEYS);
    }
    RTSPServer::RTSPClientConnection*
        prevClientConnection = (RTSPServer::RTSPClientConnection*) (fOurRTSPServer.fClientConnectionsForHTTPTunneling
                                                                        ->Lookup(sessionCookie));
    if (prevClientConnection == NULL || prevClientConnection == this)
    {
        // Either there was no previous HTTP "GET" request, or it was on the same connection; treat this "POST" request
        // as bad:
    ...
    prevClientConnection->changeClientInputSocket(fClientInputSocket, fInputTLS, extraData, extraDataSize);
...
void RTSPServer::RTSPClientConnection::resetRequestBuffer()
{
    ...
    fLastCRLF = &fRequestBuffer[-3]; // hack: Ensures that we don't think we have end-of-msg if the data starts with
                                     // <CR><LF>
...
void RTSPServer::RTSPClientConnection::closeSocketsRTSP()
{
    ...
    // 恢复连接计数
    {
        ...
        if (gs_nClientCount > 0)
        {
            gs_nClientCount--;
			live_log("恢复连接计数至[%d]", gs_nClientCount);
        }
    }
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead)
{
    ...
    do
    {
        ...
        if (fClientOutputSocket != fClientInputSocket && numBytesRemaining == 0)
        {
            ...
            if (numBytesToDecode > 0)
            {
                ...
                // Copy the new decoded bytes in place of the old ones (we can do this because there are fewer decoded
                // bytes than original):
        ...
        if (parseSucceeded)
        {
#ifdef DEBUG
            fprintf(stderr,
                    "parseRTSPRequestString() succeeded, returning cmdName \"%s\", urlPreSuffix \"%s\", urlSuffix "
                    "\"%s\", CSeq \"%s\", Content-Length %u, with %d bytes following the message.\n",
                    cmdName,
                    urlPreSuffix,
                    urlSuffix,
                    cseq,
            ...
            {
                // The special "*" URL means: an operation on the entire server.  This works only for GET_PARAMETER and
                // SET_PARAMETER:
        ...
        else
        {
...
#endif
            // The request was not (valid) RTSP, but check for a special case: HTTP commands (for setting up
            ...
            *fLastCRLF = '\r';
            if (parseSucceeded)
            {
#ifdef DEBUG
                fprintf(stderr,
                        "parseHTTPRequestString() succeeded, returning cmdName \"%s\", urlSuffix \"%s\", sessionCookie "
                        "\"%s\", acceptStr \"%s\"\n",
                        cmdName,
                        urlSuffix,
                        sessionCookie,
                ...
                {
                    // There was no "x-sessioncookie:" header.  If there was an "Accept: application/x-rtsp-tunnelled"
                    // header, then this is a bad tunneling request.  Otherwise, assume that it's an attempt to access
                    // the stream via HTTP.
                ...
                {
                    // We might have received additional data following the HTTP "POST" command - i.e., the first
                    // Base64-encoded RTSP command. Check for this, and handle it if it exists:
        ...
        // If so, move them to the front of our buffer, and keep processing it, because it might be a following,
        // pipelined request.
    ...
    --fRecursionCount;
    // If it has a scheduledDelayedTask, don't delete the instance or close the sockets. The sockets can be reused in
    // the task.
    if (!fIsActive && fScheduledDelayedTask <= 0)
    {
        if (fRecursionCount > 0)
            ...
            delete this;
        // Note: The "fRecursionCount" test is for a pathological situation where we reenter the event loop and get
        // called recursively while handling a command (e.g., while handling a "DESCRIBE", to get a SDP description). In
        // such a case we don't want to actually delete ourself until we leave the outermost call.
    }
}
                                                            ...
                                                            char const* fullRequestStr)
{
    ...
    do
    {
        ...
        // and response string:
        if (!parseAuthorizationHeader(fullRequestStr, username, realm, nonce, uri, response) || username == NULL ||
            realm == NULL || strcmp(realm, fCurrentAuthenticator.realm()) != 0 || nonce == NULL ||
        ...
        if (fCurrentAuthenticator.getAuthAlgorithm() == Authenticator::ALGO_MD5_SHA256)
        {
            int len = strlen(response);
            if (len == 32)
            {
                // MD5 校验
                char const* ourResponse = fCurrentAuthenticator.computeDigestMD5Response(cmdName, uri);
            ...
            {
                // SHA256 校验
                char const* ourResponse = fCurrentAuthenticator.computeDigestSHA256Response(cmdName, uri);
                success = (strcmp(ourResponse, response) == 0);
                fCurrentAuthenticator.reclaimDigestResponse(ourResponse);
            }
        }
    ...
    // info ------------------ zhouzr ------------------
    // 如果我们到这里，说明用户认证失败了。
    // 发送回 "401 Unauthorized" 响应，并附带一个新的随机 nonce。
    ...
    if (fCurrentAuthenticator.getAuthAlgorithm() == Authenticator::ALGO_MD5_SHA256)
    {
        ...
        snprintf((char*) fResponseBuffer,
                 ...
                 "CSeq: %s\r\n"
                 "%s"
                 "WWW-Authenticate: Digest realm=\"%s\", nonce=\"%s\", algorithm=MD5, stale=false\r\n"
                 "WWW-Authenticate: Digest realm=\"%s\", nonce=\"%s\", algorithm=SHA-256, stale=false\r\n\r\n",
                 fCurrentCSeq,
                 dateHeader(),
    ...
    else
    {
        snprintf((char*) fResponseBuffer,
                 ...
                 "WWW-Authenticate: Digest realm=\"%s\", nonce=\"%s\", algorithm=%s, stale=false\r\n\r\n",
                 fCurrentCSeq,
                 dateHeader(),
                 fCurrentAuthenticator.realm(),
                 fCurrentAuthenticator.nonce(),
                 fCurrentAuthenticator.getAlgorithmString());
        // printf("fResponseBuffer = %s", (char*)fResponseBuffer);
    }
    // info ------------------ zhouzr ------------------
    if (!isInitial401)
...
RTSPServer::RTSPClientSession ::RTSPClientSession(RTSPServer& ourServer, u_int32_t sessionId)
    : GenericMediaServer::ClientSession(ourServer, sessionId), fOurRTSPServer(ourServer), fIsMulticast(False),
      fStreamAfterSETUP(False), fTCPStreamIdCount(0), fNumStreamStates(0), fStreamStates(NULL)
{
}
...
void RTSPServer::RTSPClientSession ::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
                                                     ...
                                                     char const* fullRequestStr)
{
    // Normally, "urlPreSuffix" should be the session (stream) name, and "urlSuffix" should be the subsession (track)
    // name. However (being "liberal in what we accept"), we also handle 'aggregate' SETUP requests (i.e., without a
    // track name), in the special case where we have only a single track.  I.e., in this case, we also handle:
...
void RTSPServer::RTSPClientSession ::handleCmd_SETUP_afterLookup1(ServerMediaSession* sms)
{
    ...
    {
        concatenatedStreamName = new char[strlen(fURLPreSuffix) + strlen(fURLSuffix) +
...
void RTSPServer::RTSPClientSession ::handleCmd_SETUP_afterLookup2(ServerMediaSession* sms)
{
    ...
    do
    {
        if (sms == NULL)
        {
            ...
            {
                // The client asked for a stream that doesn't exist (and this session descriptor has not been used
                // before):
                fOurClientConnection->handleCmd_notFound();
            }
            else
            {
                // The client asked for a stream that doesn't exist, but using a stream id for a stream that does exist.
                // Bad request:
        ...
        else
        {
            ...
            {
                // The client asked for a stream that's different from the one originally requested for this stream id.
                // Bad request:
                fOurClientConnection->handleCmd_bad();
                break;
            }
        ...
        {
            // This is the first "SETUP" for this session.  Set up our array of states for all of this session's
            // subsessions (tracks):
            ...
            for (unsigned i = 0; i < fNumStreamStates; ++i)
            {
                ...
                fStreamStates[i].streamToken = NULL; // for now; it may be changed by the "getStreamParameters()" call
                                                     // that comes later
            }
        ...
        parseTransportHeader(fFullRequestStr,
                             streamingMode,
                             streamingModeString,
             ...
             fOurClientConnection->fClientOutputSocket != fOurClientConnection->fClientInputSocket))
        {
            ...
            //     1/ TCP streaming was requested, but with no "interleaving=" fields.  (QuickTime Player sometimes does
            //     this.), or 2/ TCP streaming was not requested, but we're doing RTSP-over-HTTP tunneling (which
            //     implies TCP streaming).
        ...
        subsession->getStreamParameters(fOurSessionId,
                                        fOurClientConnection->fClientAddr,
                                        clientRTPPort,
                                        clientRTCPPort,
                                                             ...
                                                             char const* fullRequestStr)
{
    ...
    //   or "urlPreSuffix" and "urlSuffix" are both nonempty, but when concatenated, (with "/") form the session
    //   (stream) name.
                                                        ...
                                                        ServerMediaSubsession* subsession)
{
    ...
    // (Without this optimization, however, this object would still get reclaimed later, as a result of a 'liveness'
    // timeout.)
    Boolean noSubsessionsRemain = True;
    for (i = 0; i < fNumStreamStates; ++i)
    {
                                                    ...
                                                    char const* fullRequestStr)
{
    ...
    {
        // Use this information, plus the stream's duration (if known), to create our own "Range:" header, for the
        // response:
    ...
    for (i = 0; i < fNumStreamStates; ++i)
    {
            ...
            || subsession == fStreamStates[i].subsession)
        {
            ...
            unsigned rtpInfoSize = rtpInfoFmtSize + strlen(prevRTPInfo) + 1 + rtspURLSize + strlen(urlSuffix) +
...
GenericMediaServer::ClientConnection* RTSPServer::createNewClientConnection(int clientSocket,
                                                                            struct sockaddr_storage const& clientAddr)
{
    std::lock_guard<std::mutex> lock(gs_connectMutex);

    if (gs_nClientCount >= MAX_CONNECT_NUM)
    {
        /* 拒绝新连接，关闭socket */
        ::closeSocket(clientSocket);
        live_log("连接数已达上限 (%d)，拒绝新连接", MAX_CONNECT_NUM);
        return NULL;
    }

    gs_nClientCount++;
	live_log("新增客户端连接，当前连接数[%d]", gs_nClientCount);
...
// info ------------------ lhy ------------------
// ========= 连接管理方法实现 =========
void RTSPServer::registerClientConnection(const char* clientIP, RTSPClientConnection* conn)
{
    auto it = fClientConnectionsByIP.find(clientIP);
    if (it == fClientConnectionsByIP.end())
    {
        fClientConnectionsByIP[clientIP] = std::list<RTSPClientConnection*>();
    }
    fClientConnectionsByIP[clientIP].push_back(conn);
}

void RTSPServer::unregisterClientConnection(const char* clientIP, RTSPClientConnection* conn)
{
    auto ipIt = fClientConnectionsByIP.find(clientIP);
    if (ipIt != fClientConnectionsByIP.end())
    {
        ipIt->second.remove(conn);
        if (ipIt->second.empty())
        {
            fClientConnectionsByIP.erase(ipIt);
        }
    }
}

void RTSPServer::closeConnectionsByIP(const char* targetIP)
{
    if (!targetIP || strlen(targetIP) == 0)
        return;

    auto ipIt = fClientConnectionsByIP.find(targetIP);
    if (ipIt == fClientConnectionsByIP.end())
        return;

    // 复制连接列表（避免在迭代过程中修改原始列表）
    auto connections = ipIt->second;

    for (auto conn : connections)
    {
        // 安全关闭连接
        if (conn->fClientInputSocket != -1)
        {
            // 1. 从任务调度器中注销套接字
            envir().taskScheduler().disableBackgroundHandling(conn->fClientInputSocket);

            // 2. 关闭套接字
            ::close(conn->fClientInputSocket);
            conn->fClientInputSocket = -1;
        }

        // 关闭输出套接字
        if (conn->fClientOutputSocket != -1 && conn->fClientOutputSocket != conn->fClientInputSocket)
        {
            envir().taskScheduler().disableBackgroundHandling(conn->fClientOutputSocket);
            ::close(conn->fClientOutputSocket);
            conn->fClientOutputSocket = -1;
        }

        // 3. 标记连接为非活动状态
        conn->fIsActive = False;

        // 4. 强制回收媒体资源
        // conn->reclaimStreamStates();
        printf("lhy debug [%s]:[%s]:[%d]\n", __FILE__, __func__, __LINE__);
    }
}
