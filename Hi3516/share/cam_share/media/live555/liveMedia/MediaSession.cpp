...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription, int turnAudio) {
  ...
  while (sdpLine != NULL) {
	...
	if (strstr(sdpLine, "AAC")!= NULL)
	{
		subsession->audio_type = 0;
	}
	else
	{
		subsession->audio_type = 1;
	}
	int tmp_a = 0;
 	const char *aduio = strstr(sdpLine, "m=audio");
	if(aduio != NULL)
	{
		const char *result = strstr(aduio, "a=rtpmap");
    	if(result)
    	{	
        	sscanf(result, "a=rtpmap:%d MPEG4-GENERIC/%d/%d",
        		&add, &(subsession->sample_rate), &tmp_a);
       	 printf("**********\n sample_rate:%d audio_type:%d \n****************\n", subsession->sample_rate, subsession->audio_type);
    	}
	}
	if(subsession->audio_type == 1)
	{
		subsession->sample_rate = 8000;
	}
#endif
