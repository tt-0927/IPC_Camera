...
private:
    ...
    double m_frameRate; /* 使用 double 类型以支持分数帧率（如 0.25fps） */
    int m_nSynchronize;
};
...
H264or5VideoStreamFramer ::H264or5VideoStreamFramer(int hNumber,
    ...
    : MPEGVideoStreamFramer(env, inputSource), fHNumber(hNumber), fIncludeStartCodeInOutput(includeStartCodeInOutput),
      fInsertAccessUnitDelimiters(insertAccessUnitDelimiters), fLastSeenVPS(NULL), fLastSeenVPSSize(0),
      fLastSeenSPS(NULL), fLastSeenSPSSize(0), fLastSeenPPS(NULL), fLastSeenPPSSize(0)
{
    ...
    fFrameRate = 30.0; // We assume a frame rate of 30 fps, unless we learn otherwise (from parsing a VPS or SPS NAL
                       // unit)
}
...
H264or5VideoStreamParser ::H264or5VideoStreamParser(int hNumber,
                                                    ...
                                                    FramedSource* inputSource,
                                                    Boolean includeStartCodeInOutput)
    : MPEGVideoStreamParser(usingSource, inputSource), fHNumber(hNumber),
      fOutputStartCodeSize(includeStartCodeInOutput ? 4 : 0), fHaveSeenFirstStartCode(False),
      fHaveSeenFirstByteOfNALUnit(False), fParsedFrameRate(0.0), cpb_removal_delay_length_minus1(23),
      dpb_output_delay_length_minus1(23), CpbDpbDelaysPresentFlag(0), pic_struct_present_flag(0),
      DeltaTfiDivisor(hNumber == 264 ? 2.0 : 1.0)
{
    // sigmastar default value
    // according to H264 and H265 spec, if not fill sei data, then frame_field_info_present_flag is zero. so need to set
    // DeltaTfiDivisor to 2.0 in H264 and 1.0 in H265
    if (fHNumber == 264)
    {
        DeltaTfiDivisor = 2.0;
    }
    ...
    m_frameCount = 0;
    m_lost = 0;
    m_frameRate = 25.0; /* 使用 double 类型初始化 */
    count_video = 0;
    continue_count_lost = 0;
    gettimeofday(&m_FirstTime, NULL);
    m_nSynchronize = inputSource->m_nSynchronize;
}
...
Boolean H264or5VideoStreamParser::usuallyBeginsAccessUnit(u_int8_t nal_unit_type)
{
    return fHNumber == 264
               ...
               : (nal_unit_type >= 32 && nal_unit_type <= 35) || (nal_unit_type == 39) ||
                     (nal_unit_type >= 41 && nal_unit_type <= 44) || (nal_unit_type >= 48 && nal_unit_type <= 55);
}
...
void H264or5VideoStreamParser ::analyze_video_parameter_set_data(unsigned& num_units_in_tick, unsigned& time_scale)
{
    ...
    for (i = vps_sub_layer_ordering_info_present_flag ? 0 : vps_max_sub_layers_minus1; i <= vps_max_sub_layers_minus1;
         ++i)
    {
...
static int dealvideo_frame_timestamp(int d_value, int continue_count, int* continue_count_lost, int lostframe)
{
    ...
    try
    {
        ...
        if (haveSeenEOF())
        {
            ...
            if (fHNumber == 264)
            {
                ...
                fprintf(stderr,
                        "Parsed trailing %d-byte NAL-unit (nal_ref_idc: %d, nal_unit_type: %d (\"%s\"))\n",
                        trailingNALUnitSize,
                        nal_ref_idc,
                        nal_unit_type,
            ...
            { // 265
                ...
                fprintf(stderr,
                        "Parsed trailing %d-byte NAL-unit (nal_unit_type: %d (\"%s\"))\n",
                        trailingNALUnitSize,
                        nal_unit_type,
        ...
        else
        {
            ...
            while (next4Bytes != 0x00000001 && (next4Bytes & 0xFFFFFF00) != 0x00000100)
            {
                ...
                {
                    // Common case: 0x00000001 or 0x000001 definitely doesn't begin anywhere in "next4Bytes", so we save
                    // all of it:
            ...
            // Assert: next4Bytes starts with 0x00000001 or 0x000001, and we've saved all previous bytes (forming a
            // complete NAL unit). Skip over these remaining bytes, up until the start of the next NAL unit:
        ...
        if (fHNumber == 264)
        {
            ...
            u_int8_t nal_ref_idc = (fFirstByteOfNALUnit & 0x60) >> 5;
            fprintf(stderr,
                    "Parsed %d-byte NAL-unit (nal_ref_idc: %d, nal_unit_type: %d (\"%s\"))\n",
...
#ifdef DEBUG
                    fprintf(stderr,
                            "\tThis \"Video Parameter Set\" NAL unit contained no frame rate information, so we use a "
...
#ifdef DEBUG
                    fprintf(stderr,
                            "\tThis \"Sequence Parameter Set\" NAL unit contained no frame rate information, so we use "
                            "a default frame rate of %f fps\n",
                            usingSource()->fFrameRate);
#endif
                }
            }
        ...
        { // Supplemental enhancement information (SEI)
            ...
            // fprintf(stderr, "[DBG-Framer] SEI NAL parsed, size=%d\n", curFrameSize() - fOutputStartCodeSize);
        }

        usingSource()->setPresentationTime();
        // fprintf(stderr,
        //         "[DBG-Framer] NAL type=%u, size=%d, PTS=%ld.%06ld, frameRate=%.4f\n",
        //         nal_unit_type,
        //         curFrameSize() - fOutputStartCodeSize,
        //         (long) usingSource()->fPresentationTime.tv_sec,
        //         (long) usingSource()->fPresentationTime.tv_usec,
        //         usingSource()->fFrameRate);
#ifdef DEBUG
        ...
        else
        {
            ...
            u_int8_t const& next_nal_unit_type = fHNumber == 264 ? (firstBytesOfNextNALUnit[0] & 0x1F)
            ...
            if (isVCL(next_nal_unit_type))
            {
                ...
                u_int8_t const byteAfter_nal_unit_header = fHNumber == 264 ? firstBytesOfNextNALUnit[1]
...
#endif
            // fprintf(stderr,
            //         "[DBG-Framer] *** AccessUnit END *** NAL=%u, pictureCount=%u, frameRate=%.4f\n",
            //         nal_unit_type,
            //         usingSource()->fPictureCount + 1,
            //         usingSource()->fFrameRate);
            // 只有 VCL NAL（视频编码层）才标记为 Access Unit 结束
            // SP S/PPS/SEI 不属于视频编码层，应该忽略
		...
		else
		{
...
#else
            // fixme zhouzr /* 使用double进行声明，适配分数帧率。 */
            // live_log("m_frameRate:%f", m_frameRate);
            /* 直接使用从VUI解析出的double类型帧率值
             * m_frameRate 已经是 double 类型，直接来自 usingSource()->fFrameRate
             * 这是从VPS/SPS中通过 time_scale / (DeltaTfiDivisor * num_units_in_tick) 计算得到的
             */
            double safeFrameRate = m_frameRate;

            /* 确保后续所有除法安全 */
            if (safeFrameRate <= 0.0)
            {
                safeFrameRate = 1.0;
            }
            // 【修改建议】在计算 nErrorTime 之前，对 m_nexttime 进行修正
            // 因为 m_nexttime 是收到下一帧头部的时间，所以它比当前帧理论结束时间晚了一个帧间隔
            // 只有在低帧率且非EOF情况下才需要这么做

            // 定义帧间隔 (微秒)
            long frameIntervalUs = (long) (1000000.0 / safeFrameRate);

            // 创建一个修正后的对比时间
            struct timeval corrected_wall_time = m_nexttime;

            // 如果 safeFrameRate 很低 (例如 <= 10)，说明 Look-ahead 延迟不可忽略
            if (safeFrameRate <= 10.0 && safeFrameRate > 0)
            {
                // 将墙钟时间回退一帧，以匹配当前帧的 PTS
                if (corrected_wall_time.tv_usec >= frameIntervalUs)
                {
                    corrected_wall_time.tv_usec -= frameIntervalUs;
                }
                else
                {
                    corrected_wall_time.tv_sec -= 1;
                    corrected_wall_time.tv_usec += (1000000 - frameIntervalUs);
                }
            }

            /**
             * 时间戳同步校正逻辑
             * 修改说明：移除 safeFrameRate > 10.0 的限制条件，让低帧率场景也能正确处理
             * 原问题：当无音频(m_nSynchronize=0)或低帧率时，时间戳校正被跳过，导致分数帧率异常
             */
            if (m_nSynchronize == 1 && safeFrameRate >= 1.0)
            {
                /* 每帧对应的流时间（毫秒） */
                int frameIntervalMs = (int) (1000.0 / safeFrameRate);
                ...
                /* 累计流时间（毫秒） */
                static int accumulatedStreamTimeMs = 0;

                /* 每处理一帧，累加该帧对应的流时间 */
                accumulatedStreamTimeMs += frameIntervalMs;

                /* 原来语义：每秒触发一次统计 / 校正逻辑 */
                // if((count_video++) % safeFrameRate == 0)
                /* 现在改为：累计流时间达到 1000ms 时触发 */
                if (accumulatedStreamTimeMs >= 1000)
                {
                    accumulatedStreamTimeMs -= 1000;
                    int nStreamTime = (int) (1000.0 / safeFrameRate);
                    // live_log("safeFrameRate:%f", safeFrameRate);
                    /* 确保 nStreamTime 不为 0 */
                    if (nStreamTime <= 0)
                    {
                        nStreamTime = 1;
                    }
                    int nErrorTime = (usingSource()->fPresentationTime.tv_sec - corrected_wall_time.tv_sec) * 1000 +
                                     (usingSource()->fPresentationTime.tv_usec - corrected_wall_time.tv_usec) / 1000;

                    // live_log("nStreamTime:%d nErrorTime:%d", nStreamTime, nErrorTime);
                    int lostframe = 1;
                    int nFrameLost = 0;
                    /* 正常帧率，保留原有逻辑 */
                    if (nStreamTime > 0)
                    {
                        nFrameLost = nErrorTime / nStreamTime;
                    }
                    else
                    {
                        nFrameLost = 0;
                    ...
                    // live_log("nFrameLost:%d", nFrameLost);
                    count_video = 1;
                    /* 原有逻辑：仅在正常帧率下生效 */
                    if (1 == dealvideo_frame_timestamp(nFrameLost, 5, &continue_count_lost, lostframe))
                    {
                        usingSource()->fPresentationTime = m_nexttime;
                    }
                }
            ...
            /**
             * 低帧率场景（< 1 fps）的特殊处理
             * 对于分数帧率（如 1/16 fps），直接使用当前墙钟时间作为 PresentationTime
             * 避免时间戳累积计算导致的巨大跳跃
             */
            if (safeFrameRate < 1.0)
            {
                /* 分数帧率场景：使用当前帧的 PresentationTime，避免使用上一帧的 m_nexttime */
                usingSource()->fPresentationTime = usingSource()->fPresentationTime;
            }
#endif
            ...
            // fixme zhouzr /* 使用适配分数帧率的值进行计算 */
            double nextFraction = nextPT.tv_usec / 1000000.0 + 1.0 / safeFrameRate;
            // double nextFraction = nextPT.tv_usec/1000000.0 + 1/usingSource()->fFrameRate;
            // live_log("nextFraction:%f", nextFraction);
