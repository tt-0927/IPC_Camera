...
::doSpecialFrameHandling(unsigned fragmentationOffset,
  ...
  if (fragmentationOffset == 0) {
    ...
    if (startCode == VIDEO_SEQUENCE_HEADER_START_CODE) {
      ...
      envir() <<  "start code "<< "VIDEO_SEQUENCE_HEADER_START_CODE\n";
	...
	FFC = ((next4Bytes&0x00000003)<<1) | ((byte8&0x80)>>7);
      ...
      envir() <<  "start code "<< "PICTURE_START_CODE\n";
      ...
      if (lastCodeByte <= 0xAF) {
        ...
        envir() <<  "get last code "<< "had got 0xaf\n";
      } else {
	// This is probably a GOP header; we don't do anything with this
        envir() <<  "GOP header"<< "we don't do anything with this\n";
      }
    } else {
