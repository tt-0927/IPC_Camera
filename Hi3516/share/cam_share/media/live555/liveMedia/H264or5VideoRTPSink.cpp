...
H264or5VideoRTPSink::~H264or5VideoRTPSink()
{
    ...
    delete[] fSPS;
    delete[] fPPS;
    stopPlaying(); // call this now, because we won't have our 'fragmenter' when the base class destructor calls it
                   // later.
                                                 ...
                                                 unsigned /*numRemainingBytes*/)
{
    ...
    /* 为每个输出 RTP 包重写当前帧时间戳，确保同一 Access Unit 内所有 NAL/FU 分片使用同一个 timestamp。 */
...
H264or5Fragmenter::H264or5Fragmenter(int hNumber,
                                     ...
                                     unsigned inputBufferMax,
                                     unsigned maxOutputPacketSize)
    : FramedFilter(env, inputSource), fHNumber(hNumber), fInputBufferSize(inputBufferMax + 1),
