...
int GenericMediaServer::setUpOurSocket(UsageEnvironment& env, Port& ourPort, int domain, int dscp) {
  int ourSocket = -1;
  ...
  do {
    ...
    ourSocket = setupStreamSocket(env, ourPort, domain, dscp, True, True);
        // later fix to support IPv6
    if (ourSocket < 0) break;
     
    // Make sure we have a big send buffer:
    if (!increaseSendBufferTo(env, ourSocket, 4*1024*1024)) break; //50
...
void GenericMediaServer::incomingConnectionHandlerOnSocket(int serverSocket) {
  ...
  increaseSendBufferTo(envir(), clientSocket, 4*1024*1024);//50
