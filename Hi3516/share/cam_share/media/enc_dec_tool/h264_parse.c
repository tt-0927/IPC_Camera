...
/**
 视频可用性信息(Video usability stuInformation)解析
 @param bs sps_bit_stream数据
 @param stuInfo sps解析之后的信息数据及结构体
 @see E.1.1 VUI parameters syntax
 */
void vui_para_parse(sps_bit_stream *bs, SpsInfo_S *stuInfo)
{
    ...
    if (timing_info_present_flag) {
        ...
        stuInfo->fps = (UINT)((float)time_scale / (float)num_units_in_tick);
        if (fixed_frame_rate_flag) {
            stuInfo->fps = stuInfo->fps/2;
        }
    }
...
INT h264_parse_sps(const BYTE *data, UINT dataSize, SpsInfo_S *stuInfo)
{
    if (!data || dataSize <= 0 || !stuInfo) 
    {
        return 0;
    }    
    INT ret = 0;
    ...
    if (nal_unit_type == 0x7) {     //Nal SPS Flag
        ...
        stuInfo->level_idc = u(&bs, 8);
        
        ue(&bs);    //seq_parameter_set_id
        
        UINT chroma_format_idc = 1;     //摄像机出图大部分格式是4:2:0
        if (stuInfo->profile_idc == 100 || stuInfo->profile_idc == 110 || stuInfo->profile_idc == 122 ||
            stuInfo->profile_idc == 244 || stuInfo->profile_idc == 44 || stuInfo->profile_idc == 83 ||
            stuInfo->profile_idc == 86 || stuInfo->profile_idc == 118 || stuInfo->profile_idc == 128 ||
            stuInfo->profile_idc == 138 || stuInfo->profile_idc == 139 || stuInfo->profile_idc == 134 || stuInfo->profile_idc == 135) {
        ...
        stuInfo->width = (INT)(pic_width_in_mbs_minus1 + 1) * 16;
        stuInfo->height = (INT)(2 - frame_mbs_only_flag) * (pic_height_in_map_units_minus1 + 1) * 16;
        ...
        if (frame_cropping_flag) {
            ...
            stuInfo->width -= crop_unit_x * (frame_crop_left_offset + frame_crop_right_offset);
            stuInfo->height -= crop_unit_y * (frame_crop_top_offset + frame_crop_bottom_offset);
        }
        
        UINT vui_parameters_present_flag = u(&bs, 1);
        if (vui_parameters_present_flag) {
            vui_para_parse(&bs, stuInfo);
        }
...
// 解析 H.264 编码流中的 NALU 头部
void parseNALUHeader(const uint8_t* data, int length) {
    if (length < 1) {
        // 数据长度不足，无法解析
        return;
    }
    ...
    // 打印解析结果
    printf("NALU Header:\n");
    printf("Forbidden Zero Bit: %d\n", forbiddenZeroBit);
    printf("NALU Reference IDC: %d\n", nalRefIdc);
    printf("NALU Unit Type: %d\n", nalUnitType);
}



// 解析PTS
uint64_t parsePTS(const uint8_t* nalData, size_t nalSize) 
{
    uint64_t pts = 0;
    size_t offset = 0;

    // 查找NAL单元头部，头部长度可能为1或2字节
    while (offset < nalSize)
    {
        if (nalData[offset++] == 0x00) 
        {
            if (offset < nalSize && nalData[offset++] == 0x00) 
            {
                if (offset < nalSize && nalData[offset++] == 0x01) 
                {
                    break;
                }
            }
    ...
    // 解析NAL单元头部
    uint8_t nalType = nalData[offset] & 0x1F;
    if (nalType == 5 || nalType == 1 || nalType == 2) 
    {
        // 关键帧或非关键帧
        offset++; // 跳过NAL类型字节

        // 解析帧时间戳
        uint8_t ftsFlag = nalData[offset] >> 7;
        if (ftsFlag) 
        {
            uint8_t nri = (nalData[offset] >> 5) & 0x03;
            uint8_t ftsLen = (nalData[offset] & 0x1F) + 1;

            offset++; // 跳过帧类型和帧时间戳长度字节

            // 解析帧时间戳
            for (int i = 0; i < ftsLen; i++) 
            {
                pts = (pts << 8) | nalData[offset + i];
            }
        }
    }
