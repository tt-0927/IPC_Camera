...
/*******************************************************************************
<功能描述>
wmemove_s 函数将 src 指向对象的 n 个字符复制到 dest 指向的对象中，和memmove_s一样，但是支持宽字符。
...
void show_error_codes() {
    printf(
        ...
        "EINVAL = 22\t源缓冲区src为null，或者目标缓冲区dest为null，或者都为null\n"
        "EINVAL_AND_RESET = -22\t源缓冲区src为null，但目标缓冲区不为null，清空dest\n"
        "EOVERLAP_AND_RESET = -35\t源缓冲区src和目标缓冲区dest出现重叠部分，提示使用remove_s并清空dest\n"
    );
}
// 测试用例
void test_case(const char *desc, wchar_t *d, size_t dm, const wchar_t *s, size_t c, errno_t expect) {
   // 初始化为非0值 FF 00 00 00
    wchar_t buf[16] = {0xFFFF};  // 宽字符初始化
    printf("\n[%s]:dest=%p, destMax=%zu, src=%p, count=%zu\n",desc, d, dm, s, c);
    errno_t ret = wmemcpy_s(d ? buf : d, dm, s, c);
    printf("ret=%d (expect %d)\n ", ret, expect);
    if (d) {
        printf("Buffer: ");
        for (int i = 0; i < 4 && buf[i] != L'\0'; i++) {  
            printf("%lc", buf[i]);
        }
        puts("");
        for (int i = 0; i < 4; i++) {
...
//特殊用例（重叠情况）
void test_overlap(const char *desc, wchar_t *d, size_t dm, const wchar_t *s, size_t c, errno_t expect) {
    printf("\n[%s]:dest=%p, destMax=%zu, src=%p, count=%zu\n",desc, d, dm, s, c);
    errno_t ret = wmemcpy_s(d , dm, s, c);
    printf("ret=%d (expect %d)\n ", ret, expect);
    if (d) {
        printf("Buffer: ");
        for (int i = 0; i < 4 && d[i] != L'\0'; i++) {  
            printf("%lc", d[i]);
        }
        puts("");
        for (int i = 0; i < 4; i++) {
...
int main() {
   ...
   // 宽字符字符串
    ...
    // 重叠用例
    strcpy(buf, "12345678"); 
    test_overlap("源缓冲区和目标缓冲区有重叠", buf+2, 8, buf, 5, EOVERLAP_AND_RESET);
    // Windows特殊场景测试
