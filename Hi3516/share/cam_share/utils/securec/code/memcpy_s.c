...
{
    printf("enter SecMemcpyError \n ");
    //目标缓冲区=0或超出限制
    if (destMax == 0 || destMax > SECUREC_MEM_MAX_LEN) {
        SECUREC_ERROR_INVALID_RANGE("memcpy_s");  //打印错误信息
        return ERANGE;
    }
    //源或者目标缓冲区为null（例外：当destmax=0时，dest可以为null）
    if (dest == NULL || src == NULL) {
        SECUREC_ERROR_INVALID_PARAMTER("memcpy_s");
        if (dest != NULL) {    //仅当dest不为null时，清空dest
...
{
    /**如果字节数大于定义的64限度，则还是使用memcpy或者memcpy_opt */
 ...
 */
        ...
        /**如果字节数满足64以内，使用SECUREC_SMALL_MEM_COPY 宏来进行拷贝，具体实现见宏定义部分 */
...
/*******************************************************************************
<功能描述>
*memcpy_s 函数将 src 指向对象的 n 个字符复制到 dest 指向的对象中。
*
<输入参数>
dest         目标缓冲区
destMax      目标缓冲区大小
...
EOVERLAP_AND_RESET      =-35   源缓冲区src和目标缓冲区dest出现重叠部分，提示使用memove_s并清空dest
...
errno_t memcpy_s(void *dest, size_t destMax, const void *src, size_t count)
{
     ...
     */
    /* 兼容Windows 的 fread API 会传入 destMax=0xFFFFFFFF，跳过长度检查 ，所以这里允许destMax=0xFFFFFFFF可以超出长度上线*/
...
#else
     /*否则严格检查destMax书否超出长度上限，并检查源和目标缓冲区是否null，是否内存重叠*/
    if (SECUREC_LIKELY(count <= destMax && dest != NULL && src != NULL &&
                       ...
                       ((dest > src && (const void *)((const unsigned char *)src + count) <= dest) ||
                        (src > dest && (void *)((unsigned char *)dest + count) <= src)))) {
#endif
#if SECUREC_MEMCOPY_WITH_PERFORMANCE //最高性能定制实现
        SecDoMemcpyOpt(dest, src, count);  //（硬件加速/SIMD指令等极致优化）
#else
#ifdef SECUREC_USE_ASM
        (void)memcpy_opt(dest, src, count);    //（平台相关的汇编优化，如x86 SSE指令）
#else
        (void)memcpy(dest, src, count);
#endif
#endif
    ...
    /*不满足上面的if则返回错误处理函数，且所有错误均清零目标缓冲区（防御敏感数据泄漏）*/
