...
// 2025.7.26
 #define SECUREC_WRITE_STRING(src, len, stream, outChars) do { \
            const SecChar *_src = (src); /* 转换为指针避免数组问题 */ \
            size_t _len = (len); \
            FILE *_stream = (stream); \
            int *_out = (outChars); \
            if (_src && _stream && _out) { \
                for (size_t _i = 0; _i < _len; ++_i) { \
                    if (fputc(_src[_i], _stream) == EOF) break; \
                    ++(*_out); \
                } \
            } \
        } while(0)
...
//2025.7.19
#define SECUREC_WRITE_MULTI_CHAR(_ch, _count, _stream, _outChars) do { \
...
#define SECUREC_CHAR(c) (c)  // 去除非常量计算
