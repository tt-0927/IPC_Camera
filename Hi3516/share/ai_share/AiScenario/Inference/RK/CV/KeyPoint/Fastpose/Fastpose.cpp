...
InferenceV1_0_NS::CFastpose::CFastpose(std::string strModelPath)
    : CCVInferenceRK(strModelPath)
{
}

InferenceV1_0_NS::CFastpose::~CFastpose()
{
}

/* 推理数据 */
bool InferenceV1_0_NS::CFastpose::inference(
    ...
    std::vector<float> &vOutData)
{
    ...
    /* 后处理 */
    /* 预分配空间以提高效率 */
    int nPointNums = m_vOutputAttrs[0].dims[1]*m_vOutputAttrs[0].dims[2];
    vOutData.reserve(nPointNums);
    float* pOutput = (float*) m_pOutputs[0].buf;
    std::copy(pOutput, pOutput + nPointNums, std::back_inserter(vOutData));
    /* 必须释放 */
