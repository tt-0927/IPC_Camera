...
std::vector<cSTrack*> cBYTETracker::joint_stracks( std::vector<cSTrack*> &tlista,  std::vector<cSTrack> &tlistb)
{
    std::map<int, int> exists;
    std::vector<cSTrack*> res;
...
std::vector<cSTrack> cBYTETracker::sub_stracks( std::vector<cSTrack> &tlista,  std::vector<cSTrack> &tlistb)
{
    std::map<int, cSTrack> stracks;
    for (int i = 0; i < tlista.size(); i++)
    {
        stracks.insert(std::pair<int, cSTrack>(tlista[i].track_id, tlista[i]));
    }
    for (int i = 0; i < tlistb.size(); i++)
    {
        int tid = tlistb[i].track_id;
    ...
    std::vector<cSTrack> res;
    std::map<int, cSTrack>::iterator  it;
...
void cBYTETracker::remove_duplicate_stracks( std::vector<cSTrack> &resa,  std::vector<cSTrack> &resb,  std::vector<cSTrack> &stracksa,  std::vector<cSTrack> &stracksb)
{
...
void cBYTETracker::linear_assignment( std::vector< std::vector<float> > &cost_matrix, int cost_matrix_size, int cost_matrix_size_size, float thresh,
...
std::vector< std::vector<float> > cBYTETracker::ious( std::vector< std::vector<float> > &atlbrs,  std::vector< std::vector<float> > &btlbrs)
{
...
std::vector< std::vector<float> > cBYTETracker::iou_distance( std::vector<cSTrack*> &atracks,  std::vector<cSTrack> &btracks, int &dist_size, int &dist_size_size)
{
...
std::vector< std::vector<float> > cBYTETracker::iou_distance( std::vector<cSTrack> &atracks,  std::vector<cSTrack> &btracks)
{
    std::vector< std::vector<float> > atlbrs, btlbrs;
    for (int i = 0; i < atracks.size(); i++)
    {
...
double cBYTETracker::lapjv(const  std::vector< std::vector<float> > &cost,  std::vector<int> &rowsol,  std::vector<int> &colsol,
...
cv::Scalar cBYTETracker::getColor(int nIdx)
{
    nIdx += 3;
