...
/* 构造函数 -- 初始化变量 */
RK_FACE_FEATURE::RK_FACE_FEATURE()
{
    /* 模型的位置 */
    m_pModelName = "./weights/MB_facenet.rknn";
    /* 输入视频流的大小和通道 */
    nImgWidth = 160;
    nImgHeight = 160;
    nImgChannel = 3;
...
RK_FACE_FEATURE::RK_FACE_FEATURE(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *RK_FACE_FEATURE::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
int RK_FACE_FEATURE::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
int RK_FACE_FEATURE::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int RK_FACE_FEATURE::RknnDetectDestory()
{
...
/* bgr格式视频流的识别 */
int RK_FACE_FEATURE::DetectFaceRgb(cv::Mat aInputImg,std::vector<float> &OutputVector)
{
    if (!aInputImg.empty())
    {
	 if (aInputImg.cols != 160 || aInputImg.rows != 160) {
		// 计算缩放比例
		double scale = std::min(160.0 / aInputImg.cols, 160.0 / aInputImg.rows);
		// 根据缩放比例调整图像大小
		cv::resize(aInputImg, aInputImg, cv::Size(), scale, scale, cv::INTER_LINEAR);
	 }
        /* 计算函数使用的时间 */
	...
	for(int i=0;i<128;i++)
	{
	    OutputVector.push_back(((float*)aOutputs[0].buf)[i]);
	}	
