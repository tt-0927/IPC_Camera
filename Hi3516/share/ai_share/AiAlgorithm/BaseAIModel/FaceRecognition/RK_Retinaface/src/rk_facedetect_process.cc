		...
		int nWidth,std::vector<int> &vBox,DetectParam_S stDetectParam,int nModelNeed)
{
	...
	for (const auto& value : indexArray) {
	     if(value!=-1)
	     {
	          if(pInput0[vNeedVector[value]*4+0]>=nWidth || pInput0[vNeedVector[value]*4+2]>=nWidth || pInput0[vNeedVector[value]*4+1]>nHeight || pInput0[vNeedVector[value]*4+3]>nHeight) continue;
		for(int i=0;i<4;i++)
