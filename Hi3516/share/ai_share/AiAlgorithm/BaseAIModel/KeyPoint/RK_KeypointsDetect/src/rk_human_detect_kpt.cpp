/*
*  File Name: RK_HUMAN_DETECT_KPT.cpp
...
/* 构造函数 -- 初始化变量 */
RK_HUMAN_DETECT_KPT::RK_HUMAN_DETECT_KPT()
{
    /* 模型的位置 */
...
RK_HUMAN_DETECT_KPT::RK_HUMAN_DETECT_KPT(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *RK_HUMAN_DETECT_KPT::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
int RK_HUMAN_DETECT_KPT::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
int RK_HUMAN_DETECT_KPT::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int RK_HUMAN_DETECT_KPT::RknnDetectDestory()
{
...
/* bgr格式视频流的识别 */
int RK_HUMAN_DETECT_KPT::DetectKptRgb(cv::Mat aInputImg,std::vector<float> &vPoints)
{
    if (!aInputImg.empty())
    {
        /* 计算函数使用的时间 */
        ...
        post_process_kpt((float *)aOutputs[0].buf, (float *)aOutputs[1].buf, (float *)aOutputs[2].buf, nModelWidth, nModelHeight,
