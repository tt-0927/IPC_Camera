...
*  Author: wcp
*  description : 获取输入的视频流,然后返回std::vector<float>的结果
*  Modify date: 2023年11月30日
*/
 
#include <chrono>
#include "rk_mask_detect.h"

/* 构造函数 -- 初始化变量 */
MASK_DETECT_DETECT::MASK_DETECT_DETECT()
{
    /* 模型的位置 */
...
MASK_DETECT_DETECT::MASK_DETECT_DETECT(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *MASK_DETECT_DETECT::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
int MASK_DETECT_DETECT::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
int MASK_DETECT_DETECT::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int MASK_DETECT_DETECT::RknnDetectDestory()
{
...
/* bgr格式视频流的识别 */
int MASK_DETECT_DETECT::DetectMaskRgb(cv::Mat aInputImg,std::vector<float> &vPoints)
{
    if (!aInputImg.empty())
    {
        /* 计算函数使用的时间 */
