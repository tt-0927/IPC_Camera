...
/* 显示的相关配置 */
std::vector<char*> classes = {"BridgeRgb" , "", "", "", "", "", "", ""};
...
// 行人检测
CRkBridgeFracture demo("./weights/BridgeFracture.rknn");
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    while (!g_bExit)
    {
		...
		for (int i = 0; i < vPoints.size()/6; i++)
		{
            ...
            putText(imgShow, text, cv::Point(x1, y1 - 5), cv::FONT_HERSHEY_SIMPLEX, 0.7, classColor[int(vPoints[6*i+5])],2);
		}
...
void AI()
{
	while (true)
	{
		...
		demo.DetectRkBridgeRgb(aSimgPart,vAIPoints);
