...
/* 构造函数 -- 初始化变量 */
CRkBridgeFracture::CRkBridgeFracture()
{
    /* 模型的位置 */
    m_pModelName = "./weights/BridgeFracture.rknn";
    /* 输入视频流的大小和通道 */
    nImgWidth = 640;
    nImgHeight = 640;
    nImgChannel = 3;
...
CRkBridgeFracture::CRkBridgeFracture(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *CRkBridgeFracture::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
int CRkBridgeFracture::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
int CRkBridgeFracture::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int CRkBridgeFracture::RknnDetectDestory()
{
...
/* rgb格式视频流的识别 */
int CRkBridgeFracture::DetectRkBridgeRgb(cv::Mat aInputImg, std::vector<float> &vPoints)
{
    if (!aInputImg.empty())
    {
        /* 计算函数使用的时间 */
        ...
        // std::cout << "桥梁检测模型推理一次使用的时间：" << duration.count() << "毫秒--"<< std::endl;
        ...
        // std::cout << "桥梁检测后处理推理一次使用的时间：" << duration_.count() << "毫秒--"<< std::endl;
