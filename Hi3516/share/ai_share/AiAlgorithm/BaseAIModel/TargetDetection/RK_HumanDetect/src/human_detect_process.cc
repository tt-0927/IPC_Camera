...
#include "human_detect_process.h"
...
{ // input --> (1,18,20,20)
  ...
  for (int a = 0; a < 3; a++)
  {
    for (int i = 0; i < grid_h; i++)
    {
      ...
      {
        // (5+1)*(i3)+4
        int offset = (a*6+4)*grid_len + i * grid_w + j;
        float box_confidence = sigmoid(input[offset]);
        if (box_confidence >= threshold)
        {
          
          offset = i * grid_w + j;  
          float box_x = sigmoid(input[offset+(a*6+0)*grid_len]) * 2.0 - 0.5;
          float box_y = sigmoid(input[offset+(a*6+1)*grid_len]) * 2.0 - 0.5;
          float box_w = sigmoid(input[offset+(a*6+2)*grid_len]) * 2.0;
          float box_h = sigmoid(input[offset+(a*6+3)*grid_len]) * 2.0;
