/*
*  File Name: RK_STUDENT_BEHAVIOR_V1.cpp
*  Created on: 2023年10月9日
*  Author: hhj
*  description : 学生行为分析 
    RK_STUDENT_BEHAVIOR_V1::StudentBehaviorRgb
        char* aInputImg: input image
        std::vector<float> &vPoints output result format: [x1,y1, x2, y2, conf, classid]
*  Modify date: 2023年10月9日
*/
...
/* 构造函数 -- 初始化变量 */
RK_STUDENT_BEHAVIOR_V1::RK_STUDENT_BEHAVIOR_V1()
{
    /* 模型的位置 */
...
RK_STUDENT_BEHAVIOR_V1::RK_STUDENT_BEHAVIOR_V1(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *RK_STUDENT_BEHAVIOR_V1::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
int RK_STUDENT_BEHAVIOR_V1::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
int RK_STUDENT_BEHAVIOR_V1::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int RK_STUDENT_BEHAVIOR_V1::RknnDetectDestory()
{
...
/* bgr格式视频流的识别 */
int RK_STUDENT_BEHAVIOR_V1::StudentBehaviorRgb(char* aInputImg,std::vector<float> &vPoints)
{
    if (aInputImg)
    {
        /* 计算函数使用的时间 */
        auto start_time = std::chrono::high_resolution_clock::now();
        aInputs[0].buf = (void*) aInputImg;
        ...
        // std::cout << "学生行为分析模型推理一次使用的时间：" << duration.count() << "毫秒--"<< std::endl;
       
        // 后处理
        post_process_v1((float *)aOutputs[0].buf, (float *)aOutputs[1].buf, (float *)aOutputs[2].buf, nModelWidth, nModelHeight,
