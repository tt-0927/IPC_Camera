/*
*  Created on: 2024年01月23日
*  Author: wcp
*  description : 获取输入的视频流,然后返回std::vector<float>的结果
*  Modify date: 2024年01月23日
*/
...
/* 构造函数 -- 初始化变量 */
HEAD_DETECT_DETECT::HEAD_DETECT_DETECT()
{
    /* 模型的位置 */
...
HEAD_DETECT_DETECT::HEAD_DETECT_DETECT(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *HEAD_DETECT_DETECT::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
int HEAD_DETECT_DETECT::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
int HEAD_DETECT_DETECT::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int HEAD_DETECT_DETECT::RknnDetectDestory()
{
...
/* bgr格式视频流的识别 */
int HEAD_DETECT_DETECT::DetectHeadRgb(cv::Mat aInputImg,std::vector<float> &vPoints)
{
    if (!aInputImg.empty())
    {
        /* 计算函数使用的时间 */
