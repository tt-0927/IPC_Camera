...
*  Author: wcp
*  description : 玩手机检测	
*  Modify date: 2023年11月14日
*/
 
#include <chrono>
#include "rk_phone_detect.h"

/* 构造函数 -- 初始化变量 */
RK_PHONE_DETECT::RK_PHONE_DETECT()
{
    /* 模型的位置 */
...
RK_PHONE_DETECT::RK_PHONE_DETECT(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *RK_PHONE_DETECT::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
int RK_PHONE_DETECT::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
int RK_PHONE_DETECT::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int RK_PHONE_DETECT::RknnDetectDestory()
{
...
/* rgb格式视频流的识别 */
int RK_PHONE_DETECT::DetectPhoneRgb(cv::Mat aInputImg,std::vector<float> &vPoints)
{
    if (!aInputImg.empty())
    {
        /* 计算函数使用的时间 */
