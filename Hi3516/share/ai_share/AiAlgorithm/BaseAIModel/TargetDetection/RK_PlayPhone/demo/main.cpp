...
/* 显示的相关配置 */
std::vector<char*> classes = {"Phone"};
std::vector<cv::Scalar> classColor = {
    cv::Scalar(255, 0, 0),       // 红色
    cv::Scalar(255, 255, 0),     // 亮青色
    cv::Scalar(255, 0, 255),     // 亮紫色
    cv::Scalar(128, 128, 255),   // 淡粉色
    cv::Scalar(128, 255, 128),   // 淡绿色
    cv::Scalar(0, 128, 255),     // 淡橙色
    cv::Scalar(0, 255, 255),     // 亮黄色
    cv::Scalar(255, 0, 128)      // 淡紫色
};

// 行人检测
RK_PHONE_DETECT demo("./weights/PhoneRelu.rknn");
...
void AI()
{
	while (true)
	{
		...
		demo.DetectPhoneRgb(aSimgPart,vAIPoints);
