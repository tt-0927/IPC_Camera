/*
 * @FilePath     : StuBehaviorAnalyzerV1.cpp
 * @Author       : yanzeh yanzeh@kfb.cn
 * @Date         : 2024-01-08 15:13:54
 * @LastEditors  : yanzeh yanzeh@kfb.cn
 * @LastEditTime : 2024-01-23 14:21:39
...
CStuBehaviorAnalyzerV1::CStuBehaviorAnalyzerV1(BehaviorAnalyzerInParam_S stInParam)
    : CBehaviorAnalyzerBase(stInParam)
{
    if (nullptr == m_pAlgorithm)
    {
        m_pAlgorithm = new RK_STUDENT_BEHAVIOR_V1(const_cast<char*>(stInParam.stNeedParam.strModelPath.c_str()));

        RK_STUDENT_BEHAVIOR_V1* pAlgorithm = static_cast<RK_STUDENT_BEHAVIOR_V1*>(m_pAlgorithm);

        pAlgorithm->fBoxThreshold = stInParam.stExParam.fConfidenceThreshold;
    }
}

CStuBehaviorAnalyzerV1::~CStuBehaviorAnalyzerV1()
{
    if (m_pAlgorithm)
    {
        delete static_cast<RK_STUDENT_BEHAVIOR_V1*>(m_pAlgorithm);
        m_pAlgorithm = nullptr;
    }
}

/* 分析数据 */
BlError_E CStuBehaviorAnalyzerV1::dataAnalysis(
    ...
    std::list<BehaviorAnalyzerResult_S>& listOutInfo)
{
    ...
    if (pchData == nullptr)
    {
        nRet = pAlgorithm->StudentBehaviorRgb(stMediaDataInfo.pchData, vecPos);
    }
    else
    {
        nRet = pAlgorithm->StudentBehaviorRgb(pchData, vecPos);
    }
    ...
    {
        enRetCode = OK;
        if (vecPos.size() <= ((i * DATA_GROUP_SIZE) + 4))
        {
            break;
        }
        ...
        switch (nBehavior)
        {
            case 0:
            {
                stItemInfo.enBehavior = LOW_HEAD_WRITING;
                break;
            }
            case 1:
            {
            ...
            case 5:
            {
                stItemInfo.enBehavior = STAND;
                break;
            }
            case 6:
            {
                stItemInfo.enBehavior = GROUP_DISCUSSION;
                break;
            }
            case 7:
            {
                stItemInfo.enBehavior = TEACHER_GUIDANCE;
                break;
            }
            default:
            {
                dlog(LOG_ERROR, "分析出来的行为异常 [%d]", nBehavior);
                enRetCode = NOK;
                break;
            }
        }
