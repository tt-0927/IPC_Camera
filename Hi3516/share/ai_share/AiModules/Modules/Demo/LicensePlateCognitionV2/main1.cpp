            ...
            { /* ---------------------------------- 显示函数 ----------------------------------  */
                for (int i = 0; i < vOutData.size(); i++)
                {
                    cv::rectangle(aSrcImg,
                                  ...
                                  cv::Scalar(255, 0, 0), 2);
                    std::string text = vOutData[i].licensePlateNumber + " " + vOutData[i].licensePlateColor + " " + vOutData[i].licensePlateType;
                    cv::putText(aSrcImg, text, cv::Point(vOutData[i].fX, vOutData[i].fY - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, getColor(0), 2);
                    std::cerr << "ID: " << vOutData[i].licensePlateNumber << std::endl;
                }
            }
