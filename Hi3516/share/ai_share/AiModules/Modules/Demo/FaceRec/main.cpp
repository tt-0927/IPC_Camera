...
/* 结果容器 */
std::vector<FaceDetect_NS::Result_S> vFaceDetects; /* 检测结果 */
std::vector<std::vector<float>> vFaceFeatures;     /* 特征提取结果 */
/* ===================================================== AI需要的全局变量配置 ============================================================= */
...
{
    /* ---------------------- 人脸检测模型初始化 ------------------------ */
    FaceDetect_NS::InParam_S stDInParam;
    stDInParam.strModelPath = "./RK3588_Face_Detect_V2.0.rknn";
    /* 初始化车牌识别检测 */
    FaceDetect_NS::CFaceDetectV2_0 *detectDemo = new FaceDetect_NS::CFaceDetectV2_0(stDInParam);
    ...
    /* ---------------------- 特征提取模型初始化 ------------------------ */
    FaceFeature_NS::InParam_S stFInParam;
    stFInParam.strModelPath = "./RK3588_Face_Feature_V1.0.rknn";
    /* 初始化车牌识别检测 */
    FaceFeature_NS::CFaceFeatureV1_0 *featureDemo = new FaceFeature_NS::CFaceFeatureV1_0(stFInParam);
    bFlag = featureDemo->init();
    if (!bFlag)
    {
        printf("特征提取模型初始化失败\n");
        exit(0);
    ...
    FaceDetect_NS::InData_S stInDetectData;
    stInDetectData.stParam.fBoxThreshold = 0.7;
    while (true)
    {
        if (aSrcImg.empty())
        {
        ...
        /* =============================== AI算法 ======================================== */
        /* 人脸检测 */
        stInDetectData.inMat = aSrcImg;
        std::vector<FaceDetect_NS::Result_S> vDetectOutData;
        bFlag = detectDemo->process(stInDetectData, vDetectOutData);
        if (!bFlag)
        {
            printf("人脸检测推理失败\n");
        }
        ...
        std::chrono::duration<double, std::milli> duration01 = end01 - start;
        std::cout << "DetectTime: " << duration01.count() << " ms" << std::endl;

        /* ================================ 开始计时 ===================================== */
        auto start1 = std::chrono::high_resolution_clock::now();
        std::vector<std::vector<float>> vm_FaceFeatures;
        for (int nF = 0; nF < vDetectOutData.size(); nF++)
        {
            int x1 = std::min(aSrcImg.cols*1.0f,std::max(0.0f, vDetectOutData[nF].fX1));
            int y1 = std::min(aSrcImg.rows*1.0f,std::max(0.0f, vDetectOutData[nF].fY1));
            int x2 = std::min(aSrcImg.cols*1.0f,std::max(0.0f, vDetectOutData[nF].fX2));
            int y2 = std::min(aSrcImg.rows*1.0f,std::max(0.0f, vDetectOutData[nF].fY2));

            /* 人脸特征提取 */
            FaceFeature_NS::InData_S stInData;
            /* 人脸裁剪 */
            aSrcImg(cv::Rect(x1, y1, x2 - x1, y2 - y1)).copyTo(stInData.inMat);
            std::vector<float> vFeatureOutData;
            bFlag = featureDemo->process(stInData, vFeatureOutData);
            if (!bFlag)
            {
                printf("特征提取推理失败\n");
            }
            vm_FaceFeatures.push_back(vFeatureOutData);
        }
        /* =============================== 输出推理事件 ================================== */
        auto end1 = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> duration1 = end1 - start1;
        std::cout << "FeatureTime: " << duration1.count() * 1.0 / vDetectOutData.size() << " ms/脸" << std::endl;

        if (vDetectOutData.size() != 0)
        {
            vFaceDetects = vDetectOutData;
            vFaceFeatures = vm_FaceFeatures;
        }
        /* =============================== 输出推理事件 ================================== */
...
{
    for (int i = 0; i < vFaceDetects.size(); i++)
    {
        cv::rectangle(aImgShow,
                      cv::Point(vFaceDetects[i].fX1, vFaceDetects[i].fY1),
                      cv::Point(vFaceDetects[i].fX2, vFaceDetects[i].fY2),
                      cv::Scalar(255, 0, 0), 2);
        for (int nP = 0; nP < vFaceDetects[i].vPoint.size() / 2; nP++)
        {
            int nPx = vFaceDetects[i].vPoint[nP * 2 + 0];
            int nPy = vFaceDetects[i].vPoint[nP * 2 + 1];
            cv::circle(aImgShow,
                       cv::Point(nPx, nPy), 5, cv::Scalar(0, 255, 0), -1);
        }
    }
    // for (int i = 0; vFaceFeatures.size()>0 && i < vFaceFeatures[0].size(); i++)
    // {
    //     printf("%f ", vFaceFeatures[0][i]);
    // }
    // printf("\n");
    // printf("--------------------------------- 特征长度[%d] ----------------------------\n ", vFaceDetects.size());
}
