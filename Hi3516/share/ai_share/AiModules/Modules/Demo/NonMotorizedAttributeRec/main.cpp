...
/* 结果容器 */
typedef struct _AttributeResult_
{
...
std::vector<AttributeResult_S> vGlobalOutData; /* 全局的结果 */
/* ===================================================== AI需要的全局变量配置 ============================================================= */
...
void AI()
{
    ...
    stInParam.strModelPath = "./RK3588_PersonMotorNomotor_Detect_V1.0.rknn";
    NonMotorizedAttribute_NS::InParam_S stInAttributeParam;
    stInAttributeParam.strModelPath = "./RK3588_NonMotorized_Attribute_V1.0.rknn";
    /* 初始化检测模型 */
    PMNMDetect_NS::CPMNMDetectV1_0 *demo = new PMNMDetect_NS::CPMNMDetectV1_0(stInParam);
    ...
    /* 初始化属性识别模型 */
    NonMotorizedAttribute_NS::CNonMotorizedAttributeV1_0 *demoAttribute = new NonMotorizedAttribute_NS::CNonMotorizedAttributeV1_0(stInAttributeParam);
    bFlag = demoAttribute->init();
    if(!bFlag)
    {
        printf("属性识别模型初始化失败\n");
        exit(0);
    ...
    NonMotorizedAttribute_NS::InData_S stInAttributeData;
    stInAttributeData.stParam.fConfidence = 0.3;
    ...
    while (true)
    {
        ...
        std::vector<PMNMDetect_NS::Result_S> vOutData;
        std::vector<AttributeResult_S> vFinalData;
        std::cout << "11111: 检测推理开始！！---" << std::endl;
        std::cout << "输入图片的宽高为：" << aSrcImg.cols << "x" << aSrcImg.rows << std::endl;
        bFlag = demo->process(stInData, vOutData);
        std::cout << "22222: 检测推理开始！！---" << std::endl;
        if (!bFlag)
        {
            printf("推理失败\n");
        }
        ...
        if(vOutData.size()!=0)
        {
            ...
            {   
                if(vOutData[i].nID == 2)
                {
                    cv::Rect rect(cv::Point(vOutData[i].fX1, vOutData[i].fY1),
                                  cv::Point(vOutData[i].fX2, vOutData[i].fY2));
                    cv::Mat NonMotorizedImage = aSrcImg(rect);
                    // cv::cvtColor(outImage, outImage, cv::COLOR_BGR2RGB);

                    stInAttributeData.inMat = NonMotorizedImage;
                    std::vector<NonMotorizedAttribute_NS::Result_S> vOutAttributeData;
                    // std::cout << "11111: 属性推理开始！！---" << i << std::endl;
                    bFlag = demoAttribute->process(stInAttributeData, vOutAttributeData);
                    // std::cout << "22222: 属性推理结束！！---" << i << std::endl;
                    if (!bFlag)
                    {
                        printf("属性识别推理失败\n");
                    }
                    AttributeResult_S NonmotorizedAttribute;
                    NonmotorizedAttribute.fX1 = vOutData[i].fX1;
                    NonmotorizedAttribute.fY1 = vOutData[i].fY1;
                    NonmotorizedAttribute.fX2 = vOutData[i].fX2;
                    NonmotorizedAttribute.fY2 = vOutData[i].fY2;
                    for (size_t j = 0; j < vOutAttributeData.size(); j++)
                    {
                        NonmotorizedAttribute.strAttributeName.push_back(std::string(vOutAttributeData[j].strName) + ":" + std::to_string(vOutAttributeData[j].fConfidence));
                        // printf("属性：%s, 置信度: %.2f!\n", vOutAttributeData[j].strName, vOutAttributeData[j].fConfidence);
                    }
                    vFinalData.push_back(NonmotorizedAttribute);
                    n += 1;
                }
            }
            std::cout << "非机动车数量: " << n << std::endl;
            vGlobalOutData = vFinalData;
        }
        auto end2 = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> duration2 = end2 - start2;
        std::cout << "AttributeInferAllTime: " << duration2.count() << " ms" << std::endl;
...
void showImg(cv::Mat &aImgShow)
{
    for (int i = 0; i < vGlobalOutData.size(); i++)
    {
        cv::rectangle(aImgShow,
                      ...
                      cv::Scalar(255, 0, 0), 2);
        // char conf[10];
		// sprintf(conf, "%d %.2f",  vGlobalOutData[i].nID,  vGlobalOutData[i].fBoxConfidence);
        float fY = 20.0;
        for (int j = 0; j < 2; j++)
        {
            cv::putText(aImgShow, vGlobalOutData[i].strAttributeName[j], cv::Point(vGlobalOutData[i].fX2, vGlobalOutData[i].fY1+fY), cv::FONT_HERSHEY_SIMPLEX, 0.7, getColor(0), 2);
            // printf("属性：%s, 置信度: %.2f!\n", vOutAttributeData[j].strName, vOutAttributeData[j].fConfidence);
            fY += 20.0;
            // std::cerr << "种类: " << vGlobalOutData[i].strAttributeName[j] << std::endl;
        }
        // cv::putText(aImgShow, vGlobalOutData[i].strAttributeName, cv::Point(vGlobalOutData[i].fX2, vGlobalOutData[i].fY1), cv::FONT_HERSHEY_SIMPLEX, 0.7, getColor(0), 2);
    }
}
