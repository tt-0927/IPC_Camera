...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    /* ---------------------- 模型初始化 ------------------------ */
    HeadDetect_NS::InParam_S stInParam;
    stInParam.strModelPath = "./RK3588_Head_Detect_V1.0.rknn";
    /* 初始化模型 */
    HeadDetect_NS::CHeadDetectV1_0 *demo = new HeadDetect_NS::CHeadDetectV1_0(stInParam);
    ...
    HeadDetect_NS::InData_S stInData;
    ...
    while (!g_bExit)
    {
        ...
        if (nstatus == 0 && naddr != 0)
        {
            ...
            std::vector<HeadDetect_NS::Result_S> vOutData;
            { /*---------------------------------- 模型推理 ------------------------------------ */
                /* ================================ 开始计时 ===================================== */
            ...
            { /* ---------------------------------- 显示函数 ----------------------------------  */
                for (int i = 0; i < vOutData.size(); i++)
                {
                    ...
                    std::string conf = std::to_string(vOutData[i].nID) + " " + std::to_string(vOutData[i].fBoxConfidence);
                    cv::putText(aSrcImg, conf, cv::Point(vOutData[i].fX1, vOutData[i].fY1 - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, getColor(0), 2);
