 ...
 */
#include <algorithm>
#include "FaceLandmarkV2_0.hpp"
#include "SaveImage.hpp"

#define nResultNum 16
#define nLandmarkNum 2

FaceLandmark_NS::CFaceLandmarkV2_0::CFaceLandmarkV2_0(InParam_S stInParam)
    : m_stInParam(stInParam)
{
}

FaceLandmark_NS::CFaceLandmarkV2_0::~CFaceLandmarkV2_0()
{
    unInit();
}

/* 初始化 */
bool FaceLandmark_NS::CFaceLandmarkV2_0::init()
{
    bool bRet = false;

    /* 人脸检测模型初始化 */
    m_pFaceDetect = new Inference_NS::CScrfd(m_stInParam.strDetectModelPath);
    if (m_pFaceDetect)
    {
...
/* 反初始化 */
bool FaceLandmark_NS::CFaceLandmarkV2_0::unInit()
{
    if (m_pFaceDetect)
    {
...
/* 处理数据 */
bool FaceLandmark_NS::CFaceLandmarkV2_0::process(
    InData_S stInData,
    std::vector<OutData_S> &stOutData)
{
    ...
    for (int nIndex = 0; nIndex < (int)vOutData.size() / nResultNum; nIndex++)
    {
        ...
        float fW = fX2 - fX1;
        float fH = fY2 - fY1;
        float fCx = (fX1 + fX2)*0.5;
        float fCy = (fY1 + fY2)*0.5;
        if (fW <= 0 || fH <= 0)
        {
            continue;
        }
        // float fMaxSize = std::max(fW, fH)*1.1;
        // fX1 = fCx - fMaxSize * 0.5;
        // fY1 = fCy - fMaxSize * 0.5;
        // fX2 = fCx + fMaxSize * 0.5;
        // fY2 = fCy + fMaxSize * 0.5;
        ...
        if ((vLandmarkOutData.size() % nLandmarkNum) !=0)
        {
            printf("算法推理结果异常\n");
            return false;
        }
        ...
        {
            float fX = (vOutData[nIndex * nResultNum + 6 + 2*nPoint] - m_nDetectXOffset) / m_fDetectResizeScale;
            float fY = (vOutData[nIndex * nResultNum + 7 + 2*nPoint] - m_nDetectYOffset) / m_fDetectResizeScale;
            std::vector<float> vPoint;
            vPoint.push_back(fX);
            vPoint.push_back(fY);
            stOneOutData.vvPoints.push_back(vPoint);
        }
        for (int nL = 0; nL < vLandmarkOutData.size() / 2; nL++)
        {
            ...
            std::vector<float> vLandmark;
            vLandmark.push_back(fPx);
            vLandmark.push_back(fPY);
            stOneOutData.vvLandmarks.push_back(vLandmark);
        }
...
/* 处理数据 */
bool FaceLandmark_NS::CFaceLandmarkV2_0::resizeAndPadImage(
    cv::Mat inputImage,
    int nLimitWidth,
    int nLimitHeight,
    int &nXOffset,
