...
using namespace PlayPhoneDetect_NS;

PlayPhoneDetect_NS::CPlayPhoneDetectV1_0::CPlayPhoneDetectV1_0(InParam_S stInParam)
    : m_stInParam(stInParam)
{
}

PlayPhoneDetect_NS::CPlayPhoneDetectV1_0::~CPlayPhoneDetectV1_0()
{
    unInit();
}

/* 初始化 */
bool PlayPhoneDetect_NS::CPlayPhoneDetectV1_0::init()
{
    CStatisticsTimer runTime("玩手机识别初始化耗时");
    bool bRet = false;
...
/* 反初始化 */
bool PlayPhoneDetect_NS::CPlayPhoneDetectV1_0::unInit()
{
    CStatisticsTimer runTime("玩手机识别反初始化耗时");
...
/* 处理数据 */
bool PlayPhoneDetect_NS::CPlayPhoneDetectV1_0::process(
    InData_S stInData,
    ...
    OutData_S *stOutData)
{
    ...
    if (access("/test_PlayPhoneDetect", F_OK) == 0)
	{
        for (int i = 0; i < vBoxDatas.size(); i++)
        {
		
            printf("===================>玩手机识别  当前目标[%d] 类别class[%d]  置信度[%.2f] <===================\n",
            ...
            // Modules_NS::saveImage(stInData.inMat, "/mnt/test_PlayPhoneDetect");
		
	    }	
    }
    
	// 标记当前帧是否识别到了玩手机
	bool bDetectPlayPhone = false;
    for (const auto& box : vBoxDatas)
    {
        DetectResult_S result;
        result.fConfidence = box.fConfidence;
        result.nClassId = box.nLabel;
        // printf("===================>玩手机检测  类别class[%d]  置信度[%.2f] 置信度阈值[%.2f]<===================\n",
        // result.nClassId,result.fConfidence,stInData.stParam.stPlayPhoneParam.fConfidence);
		if (result.nClassId == PLAY_PHONE && result.fConfidence > stInData.stParam.stPlayPhoneParam.fConfidence) 
		{
			bDetectPlayPhone = true; 
		} 
    }
	int enType = 0;
	if(stInData.stParam.stPlayPhoneParam.bEnable)
	{
		if (bDetectPlayPhone) 
		{
            m_nPlayPhoneFrameCount++;
            if (m_nPlayPhoneFrameCount >= stInData.stParam.stPlayPhoneParam.nDetectFrame) 
            {
                Result_S stResult;
                stResult.bPlayPhone = true;
                vecResult.push_back(stResult);

                printf("【报警】识别到了玩手机！\n");
                enType |= 0x02;
                m_nPlayPhoneFrameCount = 0;
            }
		} 
