/**
 * @file ParkingDetectV1_0.cpp
 * @author xiejh (xiejh@kfb.cn)
 * @date 2025-03-05
 * 
 * @brief 停车检测
 */
#include "ParkingDetectV1_0.hpp"
#include "SaveImage.hpp"
#include "StatisticsTimer.hpp"

/* 一组数据的大小 */
#define DATA_GROUP_SIZE 6
using namespace ParkingDetect_NS;

ParkingDetect_NS::CParkingDetectV1_0::CParkingDetectV1_0(InParam_S stInParam)
    : m_stInParam(stInParam)
{
}

ParkingDetect_NS::CParkingDetectV1_0::~CParkingDetectV1_0()
{
    unInit();
}

/* 初始化 */
bool ParkingDetect_NS::CParkingDetectV1_0::init()
{
...
/* 反初始化 */
bool ParkingDetect_NS::CParkingDetectV1_0::unInit()
{
...
/* 处理数据 */
bool ParkingDetect_NS::CParkingDetectV1_0::process(
    InData_S stInData,
    ...
    OutData_S *stOutData)
{
    ...
    for (const auto& box : vBoxDatas)
    {
        ...
        if (stInData.stParam.stParkingParam.bEnable)
        {
            bAreaRet = isIntersecting(vRectPolygon,
                                        stInData.stParam.stParkingParam.vecPoints);
        }
    ...
    /* 跟踪算法 */
    {
        ...
        /* 获取停车检测参数 */
        int nParkingTimeThreshold = stInData.stParam.nParkingTimeThreshold;

        /* 分析结果 */
        /* 遍历当前帧检测到的目标 */
        bool bParkingDetected = false; /* 标记是否检测到停车 */
        for (const auto &track : vecStracks)
        {
            int nVehicleId = track.track_id;
            ...
            if (m_mapVehicle.count(nVehicleId))
            {
                ...
                m_mapVehicle[nVehicleId].nexistTime++;  /* 增加停留时间 */
            ...
            else
            {
                ...
                newVehicle.nexistTime = 1; /* 初始停留时间 */
                newVehicle.isUsed = true;  /* 标记为当前帧检测到 */
                m_mapVehicle[nVehicleId] = newVehicle;
            }

            /* 判断是否触发停车报警 */
            if (m_mapVehicle[nVehicleId].nexistTime >= nParkingTimeThreshold)
            {
                bParkingDetected = true; /* 标记检测到停车 */
            }
        }
...
/* 处理数据 */
bool ParkingDetect_NS::CParkingDetectV1_0::resizeAndPadImage(cv::Mat inputImage, cv::Mat &outputImage)
{
...
bool ParkingDetect_NS::CParkingDetectV1_0::resizeAndPadImage2(cv::Mat inputImage, cv::Mat &outputImage)
{
    int imageWidth = inputImage.cols;
    int imageHeight = inputImage.rows;
    m_fResizeScale = static_cast<float>(m_nLimitWidth) / std::max(imageWidth, imageHeight);
...
/* 判断两个多边形是否有交点 */
bool ParkingDetect_NS::CParkingDetectV1_0::isIntersecting(
