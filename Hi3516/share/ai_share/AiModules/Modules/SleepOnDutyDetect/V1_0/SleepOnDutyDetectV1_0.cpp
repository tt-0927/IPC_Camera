/**
 * @file SleepOnDutyDetectV1_0.cpp
...
using namespace SleepOnDutyDetect_NS;

SleepOnDutyDetect_NS::CSleepOnDutyDetectV1_0::CSleepOnDutyDetectV1_0(InParam_S stInParam)
    : m_stInParam(stInParam)
{
}

SleepOnDutyDetect_NS::CSleepOnDutyDetectV1_0::~CSleepOnDutyDetectV1_0()
{
    unInit();
}

/* 初始化 */
bool SleepOnDutyDetect_NS::CSleepOnDutyDetectV1_0::init()
{
...
/* 反初始化 */
bool SleepOnDutyDetect_NS::CSleepOnDutyDetectV1_0::unInit()
{
...
/* 处理数据 */
bool SleepOnDutyDetect_NS::CSleepOnDutyDetectV1_0::process(
    InData_S stInData,
    ...
    OutData_S *stOutData)
{
    ...
    if (access("/test_SleepOnDutyDetect", F_OK) == 0)
	{
        for (int i = 0; i < vBoxDatas.size(); i++)
        {
		
            printf("===================>睡岗识别  当前目标[%d] 类别class[%d]  置信度[%.2f] <===================\n",
            ...
            // Modules_NS::saveImage(stInData.inMat, "/mnt/SleepOnDuty_test");
    ...
    for (const auto& box : vBoxDatas)
    {
        ...
        // printf("===================>睡岗检测  类别class[%d]  置信度[%.2f] 睡岗置信度阈值[%.2f]<===================\n",
        // result.nClassId,result.fConfidence,stInData.stParam.stSleepOnDutyParam.fConfidence);
		if (result.nClassId == Sleep && result.fConfidence > stInData.stParam.stSleepOnDutyParam.fConfidence) 
		{
			bDetectSleep = true; 
		} 
    }
	int enType = 0;
	if(stInData.stParam.stSleepOnDutyParam.bEnable)
	{
		if (bDetectSleep) 
		{
            m_nSleepOnDutyFrameCount++;
            if (m_nSleepOnDutyFrameCount >= stInData.stParam.stSleepOnDutyParam.nDetectFrame) 
            {
                Result_S stResult;
                stResult.bSleepOnDuty = true;
                vecResult.push_back(stResult);

                printf("【报警】识别到了睡岗！\n");
                enType |= 0x02;
                m_nSleepOnDutyFrameCount = 0;
            }
		} 
