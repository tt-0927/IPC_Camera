/**
 * @file FaceQualityAssessmentV1_0.cpp
...
FaceQualityAssessment_NS::CFaceQualityAssessmentV1_0::CFaceQualityAssessmentV1_0(InParam_S stInParam)
    : m_stInParam(stInParam)
{
}

FaceQualityAssessment_NS::CFaceQualityAssessmentV1_0::~CFaceQualityAssessmentV1_0()
{
    unInit();
}

/* 初始化 */
bool FaceQualityAssessment_NS::CFaceQualityAssessmentV1_0::init()
{
    bool bRet = false;
...
/* 反初始化 */
bool FaceQualityAssessment_NS::CFaceQualityAssessmentV1_0::unInit()
{
    if (m_pImageFeature)
    {
...
/* 处理数据 */
bool FaceQualityAssessment_NS::CFaceQualityAssessmentV1_0::process(
    InData_S stInData,
    float &fResult)
{
    fResult = 0.0;
    ...
    int numToAverage = std::min(10, numFeatures);
    for (int i = 0; i < numToAverage; i++)
    {
        fResult += vClsDatas[0].vFeature[i];
    }
    
    if (numToAverage > 0)
    {
        fResult /= numToAverage;
    }
...
/* 处理数据 */
bool FaceQualityAssessment_NS::CFaceQualityAssessmentV1_0::resizeAndPadImage(cv::Mat inputImage, cv::Mat &outputImage)
{
