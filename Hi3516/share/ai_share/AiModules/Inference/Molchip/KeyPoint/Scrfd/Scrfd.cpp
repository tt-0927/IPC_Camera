...
Inference_NS::CScrfd::CScrfd(std::string strConfigPath)
    : CCVInferenceMOL(strConfigPath)
{
    /* 后处理初始化 */
    m_postProcess = new PostProcess_NS::cScrfdPostProcess;
}

Inference_NS::CScrfd::~CScrfd()
{
    if (m_postProcess)
    {
        /* 后处理去初始化 */
...
/* 推理数据 */
bool Inference_NS::CScrfd::inference(
...
/* 设置参数 */
bool Inference_NS::CScrfd::setParam(float fBoxThreshold, float fNmsThreshold)
{
...
bool Inference_NS::CScrfd::checkModelProConfig()
{
    /* 读取json文件 */
