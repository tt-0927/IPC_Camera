    ...
    std::vector<Inference_NS::BoxData_S> &vBoxDatas)
{
    ...
    /* 后处理 */
    m_nCLASS_NUM = m_stModelDesc.ioDesc.out[1].tensor.dims[1];
    int nFlLen = m_stModelDesc.ioDesc.out[0].tensor.dims[1] / 4;
    /* 模型输出头 为9 的后处理 */
    if ((m_stModelDesc.ioDesc.outputNum / 3) == 3)
    {
    ...
    /* 模型输出头 为12 的后处理 */
    else if ((m_stModelDesc.ioDesc.outputNum / 3) == 4)
    {
