...
int main(int argc, char **argv)
{
    ...
    /* 读取数据--模型的输入的图片大小是：宽：640，高：640，通道：3 */
    // cv::Mat image = cv::imread(argv[2], cv::IMREAD_UNCHANGED);

    // cv::Mat resized_image;
    // cv::resize(image, resized_image, cv::Size(640, 640), 0, 0, cv::INTER_LINEAR);

    // std::vector<float> chw_data(3 * 640 * 640);
    // // HWC (BGR) -> CHW (BGR float)
    // for (int c = 0; c < 3; ++c) {
    //     for (int h = 0; h < 640; ++h) {
    //         for (int w = 0; w < 640; ++w) {
    //             float pixel = resized_image.at<cv::Vec3f>(h, w)[c];  // OpenCV keeps BGR order
    //             chw_data[c * 640 * 640 + h * 640 + w] = pixel;
    //         }
    //     }
    // }

    // Inference_NS::InputData_S stInputData;
    // stInputData.pData = chw_data.data();
    // stInputData.nDataSize = static_cast<int>(chw_data.size())* sizeof(float);

    // Inference_NS::InputData_S stInputData;
    // stInputData.pData = resized_image.data;
    // stInputData.nDataSize = resized_image.total() * resized_image.elemSize();
