 ...
 * @LastEditTime : 2025-04-21 16:59:11
 * @Description  :
 */
...
int CCertManage::setIssuerSubjectName(const CertIssuerSubjectInfo_S &stInfo, uint8_t *pName, size_t *pNameLen, size_t maxLen)
{
    ...
    *pNameLen = 0;
    ...
    /*向名称中添加州或省名称属性*/
    dlog_check_return(x509_name_add_state_or_province_name(pName, pNameLen, maxLen, ASN1_TAG_PrintableString, (uint8_t *)stInfo.strStateOrProvince.c_str(), stInfo.strStateOrProvince.size()), TRUE);
    /*向名称中添加地区名称属性*/
    dlog_check_return(x509_name_add_locality_name(pName, pNameLen, maxLen, ASN1_TAG_PrintableString, (uint8_t *)stInfo.strLocality.c_str(), stInfo.strLocality.size()), TRUE);
    /*向名称中添加组织名称属性*/
    dlog_check_return(x509_name_add_organization_name(pName, pNameLen, maxLen, ASN1_TAG_PrintableString, (uint8_t *)stInfo.strOrganization.c_str(), stInfo.strOrganization.size()), TRUE);
    /*向名称中添加部门名称属性*/
    dlog_check_return(x509_name_add_organizational_unit_name(pName, pNameLen, maxLen, ASN1_TAG_PrintableString, (uint8_t *)stInfo.strOrganizationalUnit.c_str(), stInfo.strOrganizationalUnit.size()), TRUE);
    /*向名称中添加通用名称属性*/
    dlog_check_return(x509_name_add_common_name(pName, pNameLen, maxLen, ASN1_TAG_PrintableString, (uint8_t *)stInfo.strCommonName.c_str(), stInfo.strCommonName.size()), TRUE);
...
int CCertManage::generateCSR(CertParams_S &stParams, std::string &strCsrDer)
{
    ...
    dlog_debug("生成证书请求");
    /*生成证书请求*/
    dlog_check_return(x509_req_sign_to_der(
                            stParams.nX509Version,
                            aSubjectName, szSubjectNameLen,
                            stParams.stCertSubjectInfo.pKey,
                            attrs_buf, attrs_len,
                            OID_sm2sign_with_sm3,
                            stParams.stCertSubjectInfo.pKey,
                            strSignerId.c_str(),strSignerId.size(),
                            &p, &szCsrLen),
                        TRUE);
...
int CCertManage::generateCertificate(CertParams_S &stParams)
{
    ...
    dlog_check_return(x509_exts_add_basic_constraints(aExts, &szExtsLen, sizeof(aExts), X509_critical, stParams.bIsCa, stParams.nPathLen), TRUE);
    ...
    /*利用随机数生成序列号*/
    dlog_check_return(randomNumber_get(aSerial, SM_CERT_SERIAL_LENGTH_DEFAULT), OK);
    //! 签名者ID 后续设置为企业标识符，而非默认值。在SM2签名中，iD参与生成Z值（哈希中间值），使签名只能被特定标识符对应的公钥验证 
    std::string strSignerId = SM2_DEFAULT_ID;
    /*指向存储证书缓冲区指针*/
    std::string *pCertDer;
    ...
    /*将证书信息编码为 DER 格式并签名*/
    dlog_check_return(x509_cert_sign_to_der(
                          stParams.nX509Version,
                          aSerial, sizeof(aSerial),                // 随机序列号
                          OID_sm2sign_with_sm3,                    // 签名算法标识
                          aIssuerName, szIssuerNameLen,            // 颁发者名称
                          timeNotBefore, timeNotAfter,             // 有效起始日期,有效终止日期
                          aSubjectName, szSubjectNameLen,          // 主体名称
                          stParams.stCertSubjectInfo.pKey,         // 主体公钥指针
                          nullptr, 0,                              // 颁发者唯一ID
                          nullptr, 0,                              // 主体唯一ID
                          aExts, szExtsLen,                        // 扩展项
                          stParams.stCertIssuerInfo.pKey,          // 签名私钥指针
                          strSignerId.c_str(), strSignerId.size(), // 默认SM2 ID定义
                          &p, &szCertLen),                         // 存放X.509证书缓冲区
                      TRUE);
...
int CCertManage::revokeCertificate(const std::string &serial_number,
                                     int reason_code,
                                     std::string &crl_der)
{
...
int CCertManage::readCertificate_from_file()
{
    ...
    dlog_check_return(x509_cert_print(stderr, 0, 4, "签名证书", reinterpret_cast<const uint8_t *>(strSigningCertDer.c_str()), strSigningCertDer.size()), TRUE);
