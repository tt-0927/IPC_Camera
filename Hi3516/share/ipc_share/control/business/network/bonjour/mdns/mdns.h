...
typedef ssize_t mdns_ssize_t;
	...
	typedef struct
	{
		int sockets[MDNS_SOCKETS_COUNT]; // 保存打开的socket列表
		int num_sockets;				 // 有效socket数量
		char *service_name;				 // 服务名称（调整后的格式）
		service_t service_data;			 // 服务记录数据
		void *buffer;					 // 网络缓冲区
