 ...
 * @LastEditors  : EasonLu
 * @LastEditTime : 2025-05-15 11:13:50
 * @FilePath     : BlockQueue.hpp
...
#endif
/* 无上限队列标记 */
#ifndef BLOCK_UNLIMITED_QUEUE
#define BLOCK_UNLIMITED_QUEUE -1
#endif
...
public:
    ...
    bool push(const T &item, bool bForce = false)
    {
        ...
        else if (m_queueData.size() + 1 >= m_nMaxSize)
        {
            /* 超过队列最大值则加入失败 */
            if (!bForce)
            {
     ...
     * @note
     */
    bool pop(T &item, int nTimeOut = BLOCK_WAIT)
    {
        auto bRet = peek(item, nTimeOut);
        if (bRet)
        {
            /* 获取到数据则移除队列中该元素 */
            m_queueData.pop();
    ...
    /**
     * @brief   查看队列首部元素，但不移除，可以设置阻塞等待
     * @param   [T &] item - 用于接收队首元素的引用
     * @param   [int] nTimeOut - 超时时间（毫秒）选项，不设置默认为阻塞
     * @return  [bool] 如果在指定时间内队列非空则返回 true，否则返回 false
     * @author  EasonLu
     * @note
     */
    bool peek(T &item, int nTimeOut = BLOCK_WAIT)
    {
        std::unique_lock<std::mutex> lock(m_mutexData);
        if (BLOCK_WAIT == nTimeOut)
        {
            /* 等待队列非空或退出标志被设置 */
