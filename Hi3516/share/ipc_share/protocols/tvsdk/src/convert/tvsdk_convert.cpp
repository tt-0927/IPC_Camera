...
{
    std::memset(&dst, 0, sizeof(dst));
    /* SDK 将预览地址和图像参数拆分为嵌套结构，保持 IPC 侧数据语义不变。 */
    std::strncpy(dst.stRtspUrl.szRtspMainUrl,
                 src.stRtspUrl.strRtspMainUrl.c_str(),
                 sizeof(dst.stRtspUrl.szRtspMainUrl) - 1);
    std::strncpy(dst.stRtspUrl.szRtspSubUrl,
                 src.stRtspUrl.strRtspSubUrl.c_str(),
