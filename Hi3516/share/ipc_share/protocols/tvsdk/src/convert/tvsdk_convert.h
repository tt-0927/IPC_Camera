...
/**
 * @brief 将 IPC 人脸抓拍叠加配置转换为 SDK 配置。
 * @param [in] src IPC 人脸抓拍叠加配置。
 * @param [out] dst SDK 人脸抓拍叠加配置。
 * @return 无。
 */
void FillFaceCaptureOverlayInfo(const Alarm::OverlayInfo_S &src,
                                NET_TV_FACE_CAPTURE_OVERLAY_INFO_S &dst);
/**
 * @brief 将 SDK 人脸抓拍叠加配置转换为 IPC 配置。
 * @param [in] src SDK 人脸抓拍叠加配置。
 * @param [out] dst IPC 人脸抓拍叠加配置。
 * @return 无。
 */
void ToFaceCaptureOverlayInfo(const NET_TV_FACE_CAPTURE_OVERLAY_INFO_S &src,
 ...
 * @return 无。
 */
void ToLinkageList(const NET_TV_LINKAGE_LIST_S& stSource,
