 ...
 * @LastEditTime : 2026-08-11 11:11:20
...
bool is_start_code(const uint8_t *pData, int nSize, int nPos, int &nStartCodeLen)
{
    ...
    if (nPos + 4 < nSize && pData[nPos] == 0x00 && pData[nPos + 1] == 0x00 && pData[nPos + 2] == 0x00 && pData[nPos + 3] == 0x01)
    {
        nStartCodeLen = 4;
        return true;
    }
...
bool annexb_to_mp4(const uint8_t *pData, int nSize, std::vector<uint8_t> &vMp4Data)
{
    ...
    vMp4Data.clear();
    /* perf: 输出长度通常接近输入长度，预留容量避免每个NAL插入时反复扩容。 */
    if (vMp4Data.capacity() < static_cast<std::size_t>(nSize))
    {
        vMp4Data.reserve(static_cast<std::size_t>(nSize));
    }
    int nPos = 0;
    while (nPos < nSize - 4)
    {
