...
static int load_ini_write_file(const char *file, char *buf,int *file_size)
{
    FILE *in = NULL;
    int i=0;
    *file_size =0;
                    ...
                    if(';' != p[j]) //skip over comment
                    {
                        while(j < i && p[j]!='=') {
                            ...
                            if('=' == p[j] ) {
                                ...
                                if(strncmp(key,p+newline_start,strlen(key))==0 && size_t(j-newline_start) == strlen(key))
                                {
                                    //find the key ok
                                    *key_s = newline_start;
                                    *key_e = j-1;
...
*/
int ini_read_profile_char( const char *section, const char *key,char *value,
...
*/
int ini_read_profile_int( const char *section, const char *key,int *nValue ,int default_value,
     const char *file)
{
    char value[32] = {0};

    if(!ini_read_profile_char(section,key,value, sizeof(value),NULL,file))
    {
...
int ini_write_profile_char(const char *section, const char *key, const char *value, const char *file)
{
    ...
    if(!load_ini_write_file(file,buf,&file_size))
    {
        sec_s = -1;
    }
    else
...
int ini_write_profile_int(const char *section, const char *key, int value, const char *file)
{
    ...
    return ini_write_profile_char(section, key, sVal,file);
}
...
void CIni::load(std::string filename)
{
    m_fileName = filename;
}

int CIni::read(std::string section, std::string key, std::string &value, std::string defultValue)
{
    char achTmpBuff[256] = {0};
    int nRet = ini_read_profile_char(section.c_str(), key.c_str(), achTmpBuff, sizeof(achTmpBuff), defultValue.c_str(), m_fileName.c_str());
    value = achTmpBuff;
    return nRet;
}
int CIni::read(std::string section, std::string key, int &value, int nDefultValue)
{
    return ini_read_profile_int(section.c_str(), key.c_str(), &value, nDefultValue , m_fileName.c_str());
}
int CIni::write(std::string section, std::string key, std::string value)
{
    return ini_write_profile_char(section.c_str(), key.c_str(), value.c_str(), m_fileName.c_str());
}
int CIni::write(std::string section, std::string key, int value)
{
    return ini_write_profile_int(section.c_str(), key.c_str(), value, m_fileName.c_str());
}
