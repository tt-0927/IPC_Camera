    ...
    FILE *file, osd_logo *video_logo)
{
    ...
    video_logo->height = (td_u32)bmp_info->bmp_header.bi_height;
    ...
    (td_void)fseek(file, (td_s32)bmp_file_header->bf_off_bits, 0);
    ...
    FILE *file, osd_logo *video_logo, osd_color_format fmt)
{
    td_u8 *orig_bmp_buf = NULL;
    td_u16 bpp = 0;
    td_u32 bmp_data_stride = 0;
    td_u32 bmp_data_size = 0;
    td_s32 ret = TD_SUCCESS;
    bmp_data_size_info data_size_info = {0};

    if (bmp_info != TD_NULL && video_logo != TD_NULL) {
        updata_osd_logo_size_info(bmp_info, fmt, video_logo);
    } else {
        printf("bmp info or video logo can not be zero!\n");
        return TD_FAILURE;
    }
    ...
    if (bmp_file_header == TD_NULL) {
        printf("bmp file header can not be zero!\n");
        free(orig_bmp_buf);
        return TD_FAILURE;
    }
    (td_void)fseek(file, (td_s32)bmp_file_header->bf_off_bits, 0);
    ...
    FILE *file, osd_logo *video_logo, osd_color_format fmt)
{
    ...
    data_size_info.height = (td_u32)bmp_info->bmp_header.bi_height;
...
td_s32 create_surface_by_bit_map(const td_char *file_name, osd_surface *surface, td_u8 *virt)
{
    ...
    surface->height = (td_u16)logo.height;
    surface->width = (td_u16)logo.width;
    surface->stride = (td_u16)logo.stride;
    ...
    const canvas_size_info *canvas_size)
{
    ...
    surface->height = (td_u16)canvas_size->height;
    surface->width = (td_u16)canvas_size->width;
    surface->stride = (td_u16)canvas_size->stride;
