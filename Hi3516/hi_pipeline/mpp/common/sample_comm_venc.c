...
td_s32 sample_comm_venc_get_gop_attr(ot_venc_gop_mode gop_mode, ot_venc_gop_attr *gop_attr)
{
    switch (gop_mode) {
        ...
        case OT_VENC_GOP_MODE_SINGLE_SP:
            gop_attr->gop_mode = OT_VENC_GOP_MODE_SINGLE_SP;
            gop_attr->single_sp.ip_qp_delta = 4; /* 4 is a number */
            gop_attr->single_sp.sp_qp_delta = 2; /* 2 is a number */
            gop_attr->single_sp.sp_interval = 0; /* 0 is a number */
            break;
...
static td_s32 sample_comm_venc_set_rc_param(ot_venc_chn venc_chn)
{
    td_s32 ret;
    ...
    if (chn_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H264_ABR) {
        ...
        rc_param.h264_avbr_param.min_qp = 28; /* 28,  recommended parameters */
        rc_param.h264_avbr_param.min_i_qp = 26; /* 26,  recommended parameters */
        ...
        rc_param.h265_avbr_param.min_qp = 28; /* 28,  recommended parameters */
        rc_param.h265_avbr_param.min_i_qp = 26;  /* 26,  recommended parameters */
        ...
        rc_param.svac3_avbr_param.min_qp = 28; /* 28,  recommended parameters */
        rc_param.svac3_avbr_param.min_i_qp = 26;  /* 26,  recommended parameters */
    ...
    td_u32 frame_rate)
{
    ...
    h264_avbr.max_bit_rate = ((td_u64)2048 * (pic_width * pic_height)) / (1920 * 1080); /* 2048: 2M 1920, 1080: FHD */
    ...
    td_u32 frame_rate)
{
    ...
    h265_avbr.max_bit_rate = ((td_u64)2048 * (pic_width * pic_height)) / (1920 * 1080); /* 2048: 2M 1920, 1080: FHD */
    ...
    td_u32 frame_rate)
{
    ...
    svac3_avbr.max_bit_rate = ((td_u64)2048 * (pic_width * pic_height)) / (1920 * 1080); /* 2048: 2M 1920, 1080: FHD */
...
td_s32 sample_comm_venc_create(ot_venc_chn venc_chn, sample_comm_venc_chn_param *chn_param)
{
    ...
    if ((ret = sample_comm_venc_set_rc_param(venc_chn)) != TD_SUCCESS) {
...
{
    if (snprintf_s(stream_proc_info->file_name[index], FILE_NAME_LEN, FILE_NAME_LEN - 1,
        "/stream_chn%d%s", index, stream_proc_info->file_postfix) < 0) {
        return TD_FAILURE;
    }

    if (realpath("./", stream_proc_info->real_file_name[index]) == TD_NULL) {
        sample_print("chn[%d] stream file path error\n", venc_chn);
        return TD_FAILURE;
    }

    if (strncat_s(stream_proc_info->real_file_name[index], VENC_PATH_MAX, stream_proc_info->file_name[index],
        strlen(stream_proc_info->file_name[index])) < 0) {
        return TD_FAILURE;
    }
...
static td_s32 sample_comm_set_file_name_svc_t(sample_comm_venc_stream_proc_info *stream_proc_info, td_s32 index)
{
    ...
    if (realpath("./", stream_proc_info->real_file_name[index]) == TD_NULL) {
        sample_print("stream file path error\n");
        return TD_FAILURE;
    }
