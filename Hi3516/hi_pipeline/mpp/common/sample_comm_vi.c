...
#define SC533HAI_FULL_LINES_STD 1650
...
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_sc4336p_10bit_2m_nowdr_attr = {
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
...
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_sc4336p_10bit_2m_nowdr_dev0_attr = {
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_1920, HEIGHT_1080},
...
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_sc4336p_10bit_2m_nowdr_dev1_attr = {
    .devno = 1,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_1920, HEIGHT_1080},
...
static combo_dev_attr_t g_mipi_4lane_chn0_sensor_sc533hai_10bit_4m_nowdr_attr = {
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_2880, HEIGHT_1620},
...
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_sc533hai_10bit_5m_nowdr_attr = {
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_2880, HEIGHT_1620},
...
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_sc533hai_10bit_5m_nowdr_dev0_attr = {
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_2880, HEIGHT_1620},
...
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_sc533hai_10bit_5m_nowdr_dev1_attr = {
    .devno = 1,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_2880, HEIGHT_1620},
...
static td_u32 sample_comm_vi_get_sensor_pixel_rate_by_type(sample_sns_type sns_type)
{
    ...
    switch (sns_type) {
        ...
        case SC533HAI_MIPI_5M_30FPS_10BIT:
            pixel_rate = sample_comm_vi_calc_pipe_pixel_rate(864, 2, 10, 1); /* 864Mbps * 2lane / 10bit */
            break;
        case SC533HAI_MIPI_5M_30FPS_10BIT_WDR2TO1:
            pixel_rate = sample_comm_vi_calc_pipe_pixel_rate(864, 4, 10, 2); /* 864Mbps * 4lane / 10bit / 2vc */
            break;
        default:
            break;
    }
                                                            ...
                                                            combo_dev_attr_t *combo_attr)
{
    switch (sns_type) {
        ...
        case SC533HAI_MIPI_5M_30FPS_10BIT:
            if (vi_dev == 0) {
                (td_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                    &g_mipi_2lane_chn0_sensor_sc533hai_10bit_5m_nowdr_dev0_attr, sizeof(combo_dev_attr_t));
            } else if (vi_dev == 1) { /* dev1 */
                (td_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                    &g_mipi_2lane_chn0_sensor_sc533hai_10bit_5m_nowdr_dev1_attr, sizeof(combo_dev_attr_t));
            }
            break;
    ...
    combo_dev_attr_t *combo_attr)
{
    ...
    switch (sns_type) {
        ...
        case SC4336P_MIPI_2M_30FPS_10BIT:
            if (vi_dev == 0) {
                (td_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                    &g_mipi_2lane_chn0_sensor_sc4336p_10bit_2m_nowdr_dev0_attr, sizeof(combo_dev_attr_t));
            } else if (vi_dev == 1) { /* dev1 */
                (td_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
...
td_void sample_comm_vi_get_full_lines_std_by_sns_type(sample_sns_type sns_type, td_u32 *full_lines_std)
{
    switch (sns_type) {
        case SC4336P_MIPI_4M_30FPS_10BIT:
        case GC4023_MIPI_4M_30FPS_10BIT:
        ...
        case OS04D10_MIPI_4M_30FPS_10BIT:
            *full_lines_std = OS04D10_FULL_LINES_STD;
            break;
        case SC450AI_MIPI_4M_30FPS_10BIT:
            *full_lines_std = SC450AI_FULL_LINES_STD;
            break;
        case SC500AI_MIPI_5M_30FPS_10BIT:
            *full_lines_std = SC500AI_FULL_LINES_STD;
            break;
        case SC533HAI_MIPI_5M_30FPS_10BIT:
            *full_lines_std = SC533HAI_FULL_LINES_STD;
            break;
        default:
            *full_lines_std = SC4336P_FULL_LINES_STD;
            break;
                                             ...
                                             sample_vi_pipe_info pipe_info[])
{
    ...
    for (i = 0; i < bind_pipe->pipe_num; i++) {
        ...
        pipe_info[i].pipe_attr.pixel_format                   = OT_PIXEL_FORMAT_RGB_BAYER_10BPP; // 12->10
...
td_s32 sample_comm_vi_start_mipi_rx(const sample_sns_info *sns_info, const sample_mipi_info *mipi_info)
{
    td_s32 ret;
...
static td_s32 sample_comm_vi_set_grp_info(const sample_vi_grp_info *grp_info)
{
    ...
    for (i = 0; i < grp_info->distribute_grp_num; i++) {
        ret = ss_mpi_vi_set_distribute_grp_attr(grp_info->distribute_grp[i], &grp_info->distribute_grp_attr[i]);
        if (ret != TD_SUCCESS) {
            sample_print("vi set distribute grp attr failed!\n");
            return TD_FAILURE;
        }
    }
...
static td_s32 sample_comm_vi_init_one_pipe_attach(ot_vi_pipe vi_pipe, const sample_vi_pipe_info *pipe_info)
{
    td_s32 ret;

    if (pipe_info->attach_pool != OT_VB_INVALID_POOL_ID) {
        ret = ss_mpi_vi_set_pipe_vb_src(vi_pipe, OT_VB_SRC_USER);
        if (ret != TD_SUCCESS) {
            sample_print("vi set pipe(%d) vb_src user failed with %#x!\n", vi_pipe, ret);
            return TD_FAILURE;
        }
        ret = ss_mpi_vi_attach_pipe_vb_pool(vi_pipe, pipe_info->attach_pool);
        if (ret != TD_SUCCESS) {
            sample_print("vi attach pipe(%d) vb_pool failed with %#x!\n", vi_pipe, ret);
            return TD_FAILURE;
        }
    }
...
static td_s32 sample_comm_vi_init_one_pipe(ot_vi_pipe vi_pipe, const sample_vi_pipe_info *pipe_info)
{
    ...
    if (pipe_info->set_early_end_mode == TD_TRUE) {
        ...
        if (ret != TD_SUCCESS) {
            sample_print("vi set pipe(%d) interrupt_attr failed with %#x!\n", vi_pipe, ret);
            goto start_pipe_failed;
        }
    }

    if (sample_comm_vi_init_one_pipe_attach(vi_pipe, pipe_info) != TD_SUCCESS) {
        sample_print("pipe(%d) attach vb failed!\n", vi_pipe);
        goto start_pipe_failed;
    }
    ...
    td_bool is_master_pipe)
{
    ...
    if (pipe_info->low_delay_info.enable == TD_TRUE) {
        ...
        if (ret != TD_SUCCESS) {
            sample_print("set pipe(%d) low delay failed.[0x%x]\n", vi_pipe, ret);
            return ret;
        }
    }
    if (pipe_info->frame_interrupt_attr.interrupt_type != OT_FRAME_INTERRUPT_START) {
        ...
        if (ret != TD_SUCCESS) {
            sample_print("set pipe(%d) frame interrupt attr failed.[0x%x]\n", vi_pipe, ret);
            return ret;
        }
    }
...
td_s32 sample_comm_vi_switch_isp_mode(const sample_vi_cfg *vi_cfg)
{
    ...
    sample_common_isp_switch_set_exposure(vi_cfg);
    for (i = 0; i < dev_num; i++) {
...
static td_bool sample_common_vi_is_frame_pts_suitable(td_u64 pts1, td_u64 pts2)
{
    ...
    pts_diff = (pts1 >= pts2) ? (pts1 - pts2) : (pts2 - pts1);
