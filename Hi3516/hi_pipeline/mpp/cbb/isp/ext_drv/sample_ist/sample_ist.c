...
static td_slong sample_ist_ioctl(struct file *file, td_u32 cmd, td_ulong arg)
{
    ...
    if (read_user_linear_space_valid((td_u8 *)(td_uintptr_t)arg, sizeof(node_index)) == 0) {
        sync_task_info_trace("address is invalid!\n");
        return -EFAULT;
    }

    ret = osal_copy_from_user(&node_index, (td_u32 __user *)(td_uintptr_t)arg, sizeof(node_index));
    if (ret) {
        return -1;
    }
