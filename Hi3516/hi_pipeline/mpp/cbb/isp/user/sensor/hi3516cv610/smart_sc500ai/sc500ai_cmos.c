...
/**
 * @brief    : cmos 设置sensor上下左右翻转
 * @param    {ot_vi_pipe} vi_pipe VI 物理PIPE号
 * @param    {ot_isp_sns_mirrorflip_type} sns_mirror_flip sensor mirror-flip 枚举
 * @return   {td_void}
 */
static td_void cmos_mirror_flip(ot_vi_pipe vi_pipe, ot_isp_sns_mirrorflip_type sns_mirror_flip)
{
    /* sc500ai_mirror_flip_addr 0x3221 */
    td_u32 sc500ai_mirror_flip_addr = 0x3221;
    td_s32 ret;
    td_u8 data;
    cis_info *cis = TD_NULL;
    ...
    {
    case ISP_SNS_NORMAL:
        /* reg[6:5] 2'b00 flip off; reg[2:1] 2'b00 mirror off */
        data = 0x00;
        break;

    case ISP_SNS_MIRROR:
        /* reg[6:5] 2'b00 flip off; reg[2:1] 2'b11 mirror on */
        data = 0x06;
        break;

    case ISP_SNS_FLIP:
        /* reg[6:5] 2'b11 flip on; reg[2:1] 2'b00 mirror off */
        data = 0x60;
        break;

    case ISP_SNS_MIRROR_FLIP:
        /* reg[6:5] 2'b11 flip on; reg[2:1] 2'b11 mirror on */
        data = 0x66;
        break;

    default:
        isp_err_trace("unknown mirror flip mode!\n");
        return;
    }

    ret = cis_write_reg(&cis->i2c, sc500ai_mirror_flip_addr, data);
    if (ret != TD_SUCCESS)
    {
