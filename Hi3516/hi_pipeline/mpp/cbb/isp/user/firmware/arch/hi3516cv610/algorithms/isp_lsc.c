...
static td_void lsc_get_lut_index(td_u8 cur_blk, isp_lsc *lsc, isp_lsc_usr_cfg *usr_reg_cfg)
{
    ...
    while ((width_sum_of_blk <= first_point_pos_x) && (first_point_pos_x != 0) &&
           (blk_col_start < OT_ISP_LSC_GRID_COL - 2)) { /* 2: delta_len = OT_ISP_LSC_GRID_COL - 1 */
        blk_col_start++;
    ...
    /* 2: delta_len = OT_ISP_LSC_GRID_COL - 1 */
    while ((width_sum_of_blk < last_point_pos_x) && (blk_col_end < OT_ISP_LSC_GRID_COL - 2)) {
        blk_col_end++;
...
static td_void lsc_get_gain_lut_initialize(ot_vi_pipe vi_pipe, const isp_lsc *lsc, isp_lsc_usr_cfg *usr_reg_cfg)
{
    ...
    if (sns_dft->key.bit1_lsc) {
        ...
        def_gain = g_mesh_gain_def[lsc->mesh_scale];
...
{
    td_u16 i;
    td_u16 lut_size = (td_u16)(sizeof(td_u16) * OT_ISP_LSC_GRID_POINTS);
    ot_isp_cmos_default   *sns_dft  = TD_NULL;
    const ot_isp_cmos_lsc *cmos_lsc = TD_NULL;
    ...
    for (i = 0; i < OT_ISP_MLSC_X_HALF_GRID_NUM; i++) {
        ot_ext_system_isp_mesh_shading_xgrid_write(vi_pipe, (td_u8)i, lsc->delta_x[i]);
    }

    for (i = 0; i < OT_ISP_MLSC_Y_HALF_GRID_NUM; i++) {
        ot_ext_system_isp_mesh_shading_ygrid_write(vi_pipe, (td_u8)i, lsc->delta_y[i]);
    }
...
static td_void lsc_image_size(ot_vi_pipe vi_pipe, td_u8 blk_num, isp_lsc  *lsc, td_bool cmos_en)
{
    ...
    if (cmos_en) {
        ...
        lsc_geometric_grid_size(lsc->delta_x, lsc->inv_x,
            (td_u16)isp_ctx->block_attr.frame_rect.width >> 1, OT_ISP_LSC_GRID_COL);
        lsc_geometric_grid_size(lsc->delta_y, lsc->inv_y,
            (td_u16)isp_ctx->block_attr.frame_rect.height >> 1, OT_ISP_LSC_GRID_ROW);
    }
    ...
    for (i = 0; i < blk_num; i++) {
        ...
        lsc->cur_width[i]       = (td_u16)block_rect.width;
        lsc->first_point_pos_x[i] = (td_u16)block_rect.x;
    }
}
