...
static td_void csc_dyna_regs_config(const ot_isp_csc_matrx *csc_attr, isp_csc_dyna_cfg *dyna_reg_cfg)
{
    ...
    for (i = 0; i < OT_ISP_CSC_COEF_NUM; i++) {
        dyna_reg_cfg->csc_coef[i] = (td_s16)(csc_attr->csc_coef[i] * norm_coeff1 / norm_coeff2);
    }
...
static td_void csc_ext_regs_initialize(ot_vi_pipe vi_pipe)
{
    ...
    /* write DC components */
    for (i = 0; i < OT_ISP_CSC_DC_NUM; i++) {
        ot_ext_system_csc_dcin_write(vi_pipe, i,  (td_u16)(csc_attr.csc_in_dc[i]));
        ot_ext_system_csc_dcout_write(vi_pipe, i, (td_u16)(csc_attr.csc_out_dc[i]));
    }

    /* write coefficients */
    for (i = 0; i < OT_ISP_CSC_COEF_NUM; i++) {
        ot_ext_system_csc_coef_write(vi_pipe, i, (td_u16)(csc_attr.csc_coef[i]));
    }
...
td_void isp_csc_read_ext_regs(ot_vi_pipe vi_pipe, isp_csc *csc)
{
    ...
    if (csc->update) {
        ...
        for (i = 0; i < OT_ISP_CSC_COEF_NUM; i++) {
            csc->mpi_cfg.csc_magtrx.csc_coef[i] = (td_s16)ot_ext_system_csc_coef_read(vi_pipe, i);
        }
...
static td_s32 csc_cal_matrix(ot_vi_pipe vi_pipe, isp_csc *csc, ot_isp_csc_matrx *csc_coef)
{
    ...
    for (i = 0; i < OT_ISP_CSC_DC_NUM; i++) {
        ...
        ot_ext_system_csc_dcout_write(vi_pipe, i, (td_u16)(csc_coef->csc_out_dc[i]));
    }

    if (csc->mpi_cfg.color_gamut != OT_COLOR_GAMUT_USER) {
        for (i = 0; i < OT_ISP_CSC_COEF_NUM; i++) {
            ot_ext_system_csc_coef_write(vi_pipe, i, (td_u16)(csc_coef->csc_coef[i]));
        }
    }
