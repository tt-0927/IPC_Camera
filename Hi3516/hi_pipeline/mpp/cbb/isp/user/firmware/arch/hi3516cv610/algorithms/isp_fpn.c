...
{
    td_s32 ret;
    td_u8 fpn_work_mode_tmp = fpn_work_mode;
    if (correct_enable == TD_FALSE) {
        fpn_work_mode_tmp = FPN_MODE_NONE;
    }

    ret = ioctl(isp_get_fd(vi_pipe), ISP_FPN_WORK_MODE_SET, &fpn_work_mode_tmp);
    if (ret != TD_SUCCESS) {
        isp_err_trace("ISP[%d] set isp fpn work mode %u failed!\n", vi_pipe, fpn_work_mode_tmp);
        return ret;
    }
...
static td_u32 fpn_get_sensor_iso(ot_vi_pipe vi_pipe, const isp_usr_ctx *isp_ctx)
{
    ...
    if (aibnr_mode) {
        ...
        sensor_iso = (td_u32)(((td_u64)isp_ctx->linkage.sync_iso_buf[delay_index] << 8) / /* left shift 8 bit */
