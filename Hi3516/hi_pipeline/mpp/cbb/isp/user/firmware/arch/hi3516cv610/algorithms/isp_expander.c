...
static td_void isp_expander_reg_calc(isp_expander_usr_cfg *usr_reg_cfg, const ot_isp_expander_attr *expander_attr)
{
    ...
    for (i = 1; i < (td_u32)(expander_point_num + 1); i++) {
        x_lut[i] = (td_u16)(expander_attr->knee_point_coord[i - 1].x * 2); /* 2 knee_point */
        y_lut[i] = (td_u32)(expander_attr->knee_point_coord[i - 1].y);
    }
    ...
    for (i = 0; i < OT_ISP_EXPANDER_REG_NODE_NUM; i++) {
        ...
        usr_reg_cfg->lut[i] =
            (td_u32)linear_inter((td_s32)i, x_lut[idx_low], y_lut[idx_low], x_lut[idx_high], y_lut[idx_high]);
    }
