...
static td_void dpc_image_size(ot_vi_pipe vi_pipe, isp_defect_pixel *dp)
{
    ...
    for (i = 0; i < block_num; i++) {
        ...
        dp->offset[i] = (td_u16)block_rect.x;
        dp->offset_for_split[i] = (td_u16)((i == 0) ? 0 : (dp->offset[i] + isp_ctx->block_attr.over_lap));
    }

    dp->offset_for_split[block_num] = (td_u16)(isp_ctx->block_attr.frame_rect.width);
    dp->offset[block_num] = (td_u16)(isp_ctx->block_attr.frame_rect.width);
}
...
static td_s32 isp_dpc_cmos_param_def(ot_vi_pipe vi_pipe, isp_defect_pixel *dp)
{
    ...
    if (sns_dft->key.bit1_dpc) {
        ...
        for (i = 0; i < ISP_DPC_MAX_CHN_NUM; ++i) {
            for (j = 0; j < OT_ISP_AUTO_ISO_NUM; j++) {
                dp->auto_attr[i].strength[j] = (td_u8)g_dpc_strength[j];
                dp->auto_attr[i].blend_ratio[j] = (td_u8)g_dpc_blend_ratio[j];
            }
        }
    }
    return TD_SUCCESS;
...
static td_void dpc_usr_cfg(ot_vi_pipe vi_pipe, isp_defect_pixel *dp, isp_reg_cfg *reg_cfg)
{
    ...
    for (i = 0; i < reg_cfg->cfg_num; i++) {
        ...
        for (j = 0; j < ISP_DPC_MAX_CHN_NUM; j++) {
            ...
            dyna_cfg->sup_twinkle_thr_min[j] = (td_s8)(clip3(
                dyna_cfg->sup_twinkle_thr_max[j] - dp->sup_twinkle_slope[j], DPC_TWINKLE_THR_MIN, DPC_TWINKLE_THR_MAX));
    ...
    td_u16 *blend_ratio, td_u8 i)
{
    ...
    iso1 = (td_s32)get_iso(iso_index_lower);
    iso2 = (td_s32)get_iso(iso_index_upper);
    ...
    if (dpc_fw_cfg->op_type[i] == OT_OP_MODE_MANUAL) {
        ...
        *strength = (td_u16)linear_inter((td_s32)iso,
                                         iso1, (td_s32)dpc->strength[iso_index_lower],
                                         iso2, (td_s32)dpc->strength[iso_index_upper]);

        *blend_ratio = (td_u16)linear_inter((td_s32)iso,
                                            iso1, (td_s32)dpc->blend_ratio[iso_index_lower],
                                            iso2, (td_s32)dpc->blend_ratio[iso_index_upper]);
    }
...
static td_void isp_dynamic_set(td_u32 iso, isp_dpc_dyna_cfg *dpc_hw_cfg, isp_defect_pixel *dpc_fw_cfg, td_u8 i)
{
    ...
    alpha1_rb = (td_u8)((blend_ratio > ISP_DPC_BLEND_RATIO_MAX_THR) ? ISP_DPC_BLEND_RATIO_MAX_THR : blend_ratio);
...
static td_void isp_dpc_normal_mode(isp_defect_pixel *dp, isp_reg_cfg *reg_cfg, isp_usr_ctx *isp_ctx)
{
    ...
    dp->dpcc_mode = (td_u16)(isp_dpc_calc_mode(ISP_DPCC_MODE, dp->static_enable, dp->dynamic_enable));
