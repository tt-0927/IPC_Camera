...
static td_u32 get_max_data(const td_u32 *data, td_u32 length)
{
    td_u32 i;
...
static td_u32 get_min_data(const td_u32 *data, td_u32 length)
{
    td_u32 i;
                                ...
                                const ot_isp_mlsc_calibration_cfg *lsc_cali_cfg)
{
    ...
    lsc_cali_result->mesh_scale = (td_u8)(lsc_cali_cfg->mesh_scale);
    ...
    isp_lsc_grid_step *lsc_grid_step_xy, ot_isp_mesh_shading_table *lsc_cali_result)
{
    ...
    for (i = 1; i <= half_grid_size_x; i++) {
        ...
        lsc_cali_result->x_grid_width[i - 1] = (td_u16)lsc_grid_step_xy->grid_width_step[i];
    }
    ...
    isp_lsc_grid_step *lsc_grid_step_xy, ot_isp_mesh_shading_table *lsc_cali_result)
{
    ...
    for (j = 1; j <= half_grid_size_y; j++) {
        ...
        lsc_cali_result->y_grid_width[j - 1] = (td_u16)lsc_grid_step_xy->grid_height_step[j];
    }
