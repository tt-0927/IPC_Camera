...
static inline td_bool vreg_check_isp_vreg_id(td_u32 base_addr)
{
    return (td_bool)(isp_get_vreg_id(base_addr) < OT_ISP_MAX_PIPE_NUM);
}
...
{
    if (vreg_check_isp_vreg_base(base_addr) && vreg_check_isp_vreg_id(base_addr)) {
...
static td_void vreg_munmap_reg_specific(ot_vi_pipe vi_pipe)
{
...
static td_u32 get_slave_addr_offset(td_u32 base_addr)
{
    ...
    addr_id = (td_u8)get_slave_id(base_addr);
