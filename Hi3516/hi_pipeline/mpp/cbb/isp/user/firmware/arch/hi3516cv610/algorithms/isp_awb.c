...
#define ISP_CC_SWITCH_GAIN   4095
#endif
...
#ifdef CONFIG_OT_ISP_DRC_SWITCH_SUPPORT
static td_bool g_switch_en[OT_ISP_MAX_PIPE_NUM] = {0};
#endif
typedef struct {
    td_u8 crop_en;
    td_u16 overlap;
...
#ifdef CONFIG_OT_ISP_DRC_SWITCH_SUPPORT
static td_void awb_switch_set_gain(ot_vi_pipe vi_pipe, isp_usr_ctx *isp_ctx, isp_awb_reg_dyn_cfg *awb_reg_dyn_cfg)
{
    if (g_switch_en[vi_pipe] == TD_TRUE && isp_ctx->wdr_attr.wdr_mode != OT_WDR_MODE_NONE &&
        is_online_mode(isp_ctx->block_attr.running_mode)) {
        awb_reg_dyn_cfg->be_cc_en = 1;
        awb_reg_dyn_cfg->be_cc_r_gain = ISP_CC_SWITCH_GAIN;
        awb_reg_dyn_cfg->be_cc_g_gain = ISP_CC_SWITCH_GAIN;
        awb_reg_dyn_cfg->be_cc_b_gain = ISP_CC_SWITCH_GAIN;
    }
}
#endif
static td_void awb_res_regs_default(ot_vi_pipe vi_pipe, td_u8 i, td_u8 block_num, isp_awb_reg_dyn_cfg *awb_reg_dyn_cfg)
{
    ...
    overlap = (td_u16)(isp_ctx->block_attr.over_lap);
...
#ifdef CONFIG_OT_ISP_DRC_SWITCH_SUPPORT
    awb_switch_set_gain(vi_pipe, isp_ctx, awb_reg_dyn_cfg);
#endif
    ...
    ot_ext_system_awb_crop_height_write(vi_pipe, (td_u16)(isp_ctx->block_attr.frame_rect.height));
    ot_ext_system_awb_crop_width_write(vi_pipe, (td_u16)(isp_ctx->block_attr.frame_rect.width));
}
...
static td_void awb_res_read_ext_regs(ot_vi_pipe vi_pipe, isp_reg_cfg *reg_cfg)
{
    ...
    awb_crop.overlap = (td_u16)(isp_ctx->block_attr.over_lap);
    ...
    awb_zone_row = (td_u8)ot_ext_system_awb_vnum_read(vi_pipe);
    awb_zone_col = (td_u8)ot_ext_system_awb_hnum_read(vi_pipe);
...
static td_void awb_image_mode_set(ot_vi_pipe vi_pipe, isp_reg_cfg *reg_cfg)
{
    ...
    height = (td_u16)(isp_ctx->block_attr.frame_rect.height);
    width = (td_u16)(isp_ctx->block_attr.frame_rect.width);
    awb_zone_col = (td_u8)ot_ext_system_awb_hnum_read(vi_pipe);
    awb_zone_row = (td_u8)ot_ext_system_awb_vnum_read(vi_pipe);

    if (awb_zone_col * OT_ISP_AWB_MIN_WIDTH > width) {
        awb_zone_col = (td_u8)(width / OT_ISP_AWB_MIN_WIDTH);
    }

    if (awb_zone_row * OT_ISP_AWB_MIN_HEIGHT > height) {
        awb_zone_row = (td_u8)(height / OT_ISP_AWB_MIN_HEIGHT);
    }
                                ...
                                td_u16 color_matrix[OT_ISP_COLOR_SECTOR_NUM][OT_ISP_CCM_MATRIX_SIZE])
{
    ...
    for (i = 0; i < OT_ISP_BAYER_CHN_NUM; i++) {
        td_u16 gain = (td_u16)(MIN2(isp_awb_rshift8(white_balance_gain[i]), ISP_AWB_GAIN_MAX_THR));
...
static td_void awb_result_check(ot_isp_awb_result *awb_result)
{
    ...
    if ((awb_result->white_balance_gain[0] == 0) && (awb_result->white_balance_gain[1] == 0) &&
        (awb_result->white_balance_gain[2] == 0) && (awb_result->white_balance_gain[3] == 0)) { /* index 2 3 */
        isp_err_trace("awb result white_balance_gain should not be all 0 \n");
    }
...
static td_void awb_cfg_regs(ot_vi_pipe vi_pipe, ot_isp_awb_result *awb_result, isp_reg_cfg *reg_cfg)
{
    ...
    /* FE */
    for (i = 0; i < OT_ISP_BAYER_CHN_NUM; i++) {
        td_u16 gain = (td_u16)(MIN2(isp_awb_rshift8(awb_result->white_balance_gain[i]), ISP_AWB_GAIN_MAX_THR));
...
static td_s32 isp_awb_init(ot_vi_pipe vi_pipe, td_void *reg_cfg)
{
    ...
    awb_param.awb_width = (td_u16)(isp_ctx->block_attr.frame_rect.width);
    awb_param.awb_height = (td_u16)(isp_ctx->block_attr.frame_rect.height);
    ...
    for (i = 0; i < OT_ISP_MAX_REGISTER_ALG_LIB_NUM; i++) {
        if (isp_ctx->awb_lib_info.libs[i].used) {
            ...
            if (lib->awb_regsiter.awb_exp_func.pfn_awb_init != TD_NULL) {
                ...
                awb_result_check(&awb_init_result);
            }
        }
    }
...
static td_u16 stat_blc_dg(td_u16 stat_value, td_u32 offset, td_u32 dgain, td_u32 dgain_shift)
{
    if ((td_u32)stat_value <= offset) {
        return 0;
    } else {
        return (td_u16)(MIN2(((((td_u32)stat_value - offset) * dgain) >> dgain_shift), 0xFFFF));
    }
}
...
static td_void isp_awb_stat_lsc_process(td_u8 vi_pipe, isp_awb_lsc_lut *lsc_lut, ot_isp_awb_info *awb_info)
{
    ...
    for (i = 0; i < OT_ISP_AWB_ZONE_ORIG_ROW; i++) {
        ...
        for (j = 0; j < OT_ISP_AWB_ZONE_ORIG_COLUMN; j++) {
            ...
            gain_r = (td_s32)(mesh_gain_unit + (((td_u32)gain_r * lsc_lut->strength) >> mul_shift));
            gain_g = (td_s32)(mesh_gain_unit + (((td_u32)gain_g * lsc_lut->strength) >> mul_shift));
            gain_b = (td_s32)(mesh_gain_unit + (((td_u32)gain_b * lsc_lut->strength) >> mul_shift));
...
static td_bool isp_awb_set_overlap_info(ot_vi_pipe vi_pipe, td_u8 stitch_pipe_num, ot_isp_awb_info *awb_info)
{
    ...
    td_u8 awb_zone_row = (td_u8)ot_ext_system_awb_vnum_read(vi_pipe);
    td_u8 awb_zone_col = (td_u8)ot_ext_system_awb_hnum_read(vi_pipe);
                                            ...
                                            isp_stat *stat_info, ot_isp_awb_info *awb_info)
{
    ...
    isp_awb_stat_lsc_process((td_u8)vi_pipe, &lsc_lut, awb_info);
...
static td_s32 isp_awb_run(ot_vi_pipe vi_pipe, const td_void *stat_info, td_void *reg_cfg, td_s32 rsv)
{
    ...
    awb_param.awb_zone_row = (td_u8)ot_ext_system_awb_vnum_read(vi_pipe);
    awb_param.awb_zone_col = (td_u8)ot_ext_system_awb_hnum_read(vi_pipe);
...
static td_s32 isp_awb_ctrl(ot_vi_pipe vi_pipe, td_u32 cmd, td_void *value)
{
...
#ifdef CONFIG_OT_ISP_DRC_SWITCH_SUPPORT
        g_switch_en[vi_pipe] = TD_TRUE;
        awb_image_mode_set(vi_pipe, &reg_cfg->reg_cfg);
        g_switch_en[vi_pipe] = TD_FALSE;
#else
        awb_image_mode_set(vi_pipe, &reg_cfg->reg_cfg);
#endif
    }
    ...
    if ((cmd == OT_ISP_WDR_MODE_SET)) {
        ...
        if (isp_ctx->block_attr.block_num == isp_ctx->block_attr.pre_block_num) {
            g_switch_en[vi_pipe] = TD_TRUE;
            isp_awb_reg_dyn_cfg *awb_reg_dyn_cfg =  &reg_cfg->reg_cfg.alg_reg_cfg[0].awb_reg_cfg.awb_reg_dyn_cfg;
            awb_switch_set_gain(vi_pipe, isp_ctx, awb_reg_dyn_cfg);
            g_switch_en[vi_pipe] = TD_FALSE;
        }
#endif
