...
static td_void isp_af_ext_regs_default(ot_vi_pipe vi_pipe)
{
    ...
    af_cfg->config.crop.width = (td_u16)(isp_ctx->block_attr.frame_rect.width / 8 * 8);   /* alignment 8byte */
    af_cfg->config.crop.height = (td_u16)(isp_ctx->block_attr.frame_rect.height / 2 * 2);   /* alignment 2byte */

    af_cfg->config.fe_crop.x = 0;
    af_cfg->config.fe_crop.y = 0;
    af_cfg->config.fe_crop.width = (td_u16)(isp_ctx->sys_rect.width / 8 * 8);   /* alignment 8byte */
    af_cfg->config.fe_crop.height = (td_u16)(isp_ctx->sys_rect.height / 2 * 2);   /* alignment 2byte */
...
static td_void isp_af_blk_crop(isp_af_reg_cfg *af_reg_cfg, isp_usr_ctx *isp_ctx, td_u8 index, td_u8 block_num)
{
    ...
    if (index == 0) {
        if (block_num > 1) {
            ...
            af_reg_cfg->crop_h_size = (td_u16)(block_rect.width - overlap);
        } else {
            af_reg_cfg->crop_pos_x  = 0;
            af_reg_cfg->crop_h_size = (td_u16)(block_rect.width);
        }
        ...
        af_reg_cfg->crop_pos_x   = overlap;
        af_reg_cfg->crop_h_size  = (td_u16)(block_rect.width - (overlap << 1));
    }
}
...
static td_void isp_af_be_blk_cfg(ot_vi_pipe vi_pipe, isp_reg_cfg *reg_cfg, isp_usr_ctx *isp_ctx)
{
    ...
    td_u8  window_hnum = (td_u8)ot_ext_af_window_hnum_read(vi_pipe);
    td_u8  window_vnum = (td_u8)ot_ext_af_window_vnum_read(vi_pipe);
...
static td_void isp_af_image_res_write(ot_vi_pipe vi_pipe, isp_reg_cfg *reg_cfg)
{
    ...
    ot_ext_af_crop_h_size_write(vi_pipe, (td_u16)(isp_ctx->block_attr.frame_rect.width / 8 * 8));  /* alignment 8byte */
    ot_ext_af_crop_v_size_write(vi_pipe, (td_u16)(isp_ctx->block_attr.frame_rect.height / 2 * 2)); /* alignment 2byte */

    ot_ext_af_fe_crop_pos_x_write(vi_pipe, 0);
    ot_ext_af_fe_crop_pos_y_write(vi_pipe, 0);
    ot_ext_af_fe_crop_h_size_write(vi_pipe, (td_u16)(isp_ctx->sys_rect.width / 8 * 8));  /* alignment 8byte */
    ot_ext_af_fe_crop_v_size_write(vi_pipe, (td_u16)(isp_ctx->sys_rect.height / 2 * 2)); /* alignment 2byte */

    ot_ext_af_input_h_size_write(vi_pipe, (td_u16)(isp_ctx->block_attr.frame_rect.width));
    ot_ext_af_input_v_size_write(vi_pipe, (td_u16)(isp_ctx->block_attr.frame_rect.height));
    isp_af_be_blk_cfg(vi_pipe, reg_cfg, isp_ctx);
