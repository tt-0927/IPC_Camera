...
static td_void ge_read_extregs(ot_vi_pipe vi_pipe)
{
    ...
    if (ge->coef_update_en) {
        ...
        ge->cmos_ge.slope       = (td_u8)ot_ext_system_ge_slope_read(vi_pipe);
        ge->cmos_ge.sensi_slope = (td_u8)ot_ext_system_ge_sensitivity_read(vi_pipe);
...
static td_void ge_dyna_fw(td_u32 iso, isp_ge *ge, isp_ge_dyna_cfg *ge_dyna_reg_cfg)
{
    ...
    td_s32 iso_low  = (td_s32)get_iso(index_lower);
    td_s32 iso_high = (td_s32)get_iso(index_upper);

    for (i = 0; i < ge->ge_chn_num; i++) {
        ge_dyna_reg_cfg->ge_ct_th3[i] = clip_max((td_u16)linear_inter((td_s32)iso,
            iso_low, ge->cmos_ge.threshold[index_lower],
            iso_high, ge->cmos_ge.threshold[index_upper]) << ISP_GE_MV_BIT, (td_u16)(1 << ge->bit_depth) - 1);

        ge_dyna_reg_cfg->ge_ct_th1[i] = clip_max((td_u32)linear_inter((td_s32)iso,
            iso_low, ge->cmos_ge.np_offset[index_lower],
            iso_high, ge->cmos_ge.np_offset[index_upper]) << ISP_GE_MV_BIT, (td_u32)(1 << ge->bit_depth) - 1);
    }
    ge_dyna_reg_cfg->ge_strength  = (td_u16)linear_inter((td_s32)iso, iso_low, ge->cmos_ge.strength[index_lower],
