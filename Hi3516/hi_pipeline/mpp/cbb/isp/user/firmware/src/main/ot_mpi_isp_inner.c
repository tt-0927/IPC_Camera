...
td_s32 mpi_isp_get_pipe_index(ot_vi_pipe vi_pipe)
{
    td_u32 i;
    td_s32 ret;
    vi_pipe_wdr_attr wdr_attr = { 0 };
...
td_s32 mpi_isp_set_isp_stitch_transform_info(ot_vi_pipe vi_pipe,
    const isp_stitch_transform_info *isp_transform_info)
{
    td_s32 ret = TD_SUCCESS;

    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(isp_transform_info);
    isp_mutex_lock(vi_pipe);
    isp_check_open_goto(vi_pipe, ret, exit);
    isp_check_mem_init_goto(vi_pipe, ret, exit);
    isp_check_vreg_permission_goto(vi_pipe, ret, exit);
    ot_ext_system_isp_is_long_edge_stitching_write(vi_pipe, isp_transform_info->is_longedge_stitching);
    ot_ext_system_isp_is_rotating_write(vi_pipe, isp_transform_info->is_rotating180);
exit:
    isp_mutex_unlock(vi_pipe);
    return ret;
}
