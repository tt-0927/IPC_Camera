...
static td_s32 isp_get_block_hw_attr(ot_vi_pipe vi_pipe, isp_block_attr *block)
{
    ...
    if (ret != TD_SUCCESS) {
        isp_err_trace("isp[%d] work_mode init failed, ret:%x!\n", vi_pipe, ret);
        return ret;
    }
    ...
    ot_ext_system_be_total_width_write(vi_pipe,  (td_u16)block->frame_rect.width);
    ot_ext_system_be_total_height_write(vi_pipe, (td_u16)block->frame_rect.height);
...
td_s32 isp_block_init(ot_vi_pipe vi_pipe, isp_block_attr *block)
{
    ...
    if (ret != TD_SUCCESS) {
        isp_err_trace("get isp[%d] block HW attr failed!\n", vi_pipe);
        return ret;
    }
