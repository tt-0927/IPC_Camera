...
td_void isp_sharpen_auto_attr_read(ot_vi_pipe vi_pipe, ot_isp_sharpen_auto_attr *auto_attr)
{
    ...
    for (i = 0; i < OT_ISP_AUTO_ISO_NUM; i++) {
        ...
        for (j = 0; j < OT_ISP_SHARPEN_LUMA_NUM; j++) {
            index = i + j * OT_ISP_AUTO_ISO_NUM;
            auto_attr->luma_wgt[j][i] = (td_u8)ot_ext_system_sharpen_luma_wgt_read(vi_pipe, index);
        }
        ...
        auto_attr->edge_rly_attr.edge_overshoot[i]  = (td_u8)ot_ext_system_sharpen_edge_overshoot_read(vi_pipe, i);
        auto_attr->edge_rly_attr.edge_undershoot[i] = (td_u8)ot_ext_system_sharpen_edge_undershoot_read(vi_pipe, i);

        for (j = 0; j < OT_ISP_SHARPEN_RLYWGT_NUM; j++) {
            index = i + j * OT_ISP_AUTO_ISO_NUM;
            auto_attr->edge_rly_attr.edge_gain_by_rly[j][i] =
                ot_ext_system_sharpen_edge_gain_by_rly_read(vi_pipe, (td_u8)index);
        }

        for (j = 0; j < OT_ISP_SHARPEN_STDGAIN_NUM; j++) {
            index = i + j * OT_ISP_AUTO_ISO_NUM;
            auto_attr->edge_rly_attr.edge_rly_by_mot[j][i] =
                ot_ext_system_sharpen_edge_rly_by_mot_read(vi_pipe, (td_u8)index);
            auto_attr->edge_rly_attr.edge_rly_by_luma[j][i] =
...
td_void isp_sharpen_manual_attr_read(ot_vi_pipe vi_pipe, ot_isp_sharpen_manual_attr *manual_attr)
{
    ...
    manual_attr->edge_rly_attr.edge_overshoot = (td_u8)ot_ext_system_manual_sharpen_edge_overshoot_read(vi_pipe);
    manual_attr->edge_rly_attr.edge_undershoot = (td_u8)ot_ext_system_manual_sharpen_edge_undershoot_read(vi_pipe);

    for (j = 0; j < OT_ISP_SHARPEN_RLYWGT_NUM; j++) {
        manual_attr->edge_rly_attr.edge_gain_by_rly[j] =
            (td_u8)ot_ext_system_manual_sharpen_edge_gain_by_rly_read(vi_pipe, j);
    }

    for (j = 0; j < OT_ISP_SHARPEN_STDGAIN_NUM; j++) {
        manual_attr->edge_rly_attr.edge_rly_by_mot[j] =
            (td_u8)ot_ext_system_manual_sharpen_edge_rly_by_mot_read(vi_pipe, j);
        manual_attr->edge_rly_attr.edge_rly_by_luma[j] =
            (td_u8)ot_ext_system_manual_sharpen_edge_rly_by_luma_read(vi_pipe, j);
    }

    for (j = 0; j < OT_ISP_SHARPEN_MOT_NUM; j++) {
        manual_attr->gain_by_mot_attr.mf_gain_by_mot[j] =
            (td_u8)ot_ext_system_manual_sharpen_mf_gain_by_mot_read(vi_pipe, j);
        manual_attr->gain_by_mot_attr.hf_gain_by_mot[j] =
            (td_u8)ot_ext_system_manual_sharpen_hf_gain_by_mot_read(vi_pipe, j);
        manual_attr->gain_by_mot_attr.lmf_gain_by_mot[j] =
            (td_u8)ot_ext_system_manual_sharpen_lmf_gain_by_mot_read(vi_pipe, j);
    }
}
...
td_void isp_nr_comm_attr_write(ot_vi_pipe vi_pipe, const ot_isp_nr_attr *nr_attr)
{
    ...
    ot_ext_system_bayernr_ref_mode_write(vi_pipe, nr_attr->ref_mode);
    ot_ext_system_bayernr_load_ref_enable_write(vi_pipe, nr_attr->load_ref_en);
...
td_void isp_nr_comm_attr_read(ot_vi_pipe vi_pipe, ot_isp_nr_attr *nr_attr)
{
    ...
    nr_attr->ref_mode         = ot_ext_system_bayernr_ref_mode_read(vi_pipe);
    nr_attr->load_ref_en     = ot_ext_system_bayernr_load_ref_enable_read(vi_pipe);
    for (i = 0; i < OT_ISP_BAYERNR_LUT_LENGTH; i++) {
...
static td_void isp_af_plg_pls_ext_reg_init(ot_vi_pipe vi_pipe, const ot_isp_focus_stats_cfg *af_cfg)
{
    ...
    for (i = 0; i < OT_ISP_AF_PLGS_NUM; i++) {
        ...
        if (i == 0) {
            ot_ext_af_iir_pls_group0_write(vi_pipe, (td_u8)pls);
            ot_ext_af_iir_plg_group0_write(vi_pipe, (td_u8)plg);
        } else {
            ot_ext_af_iir_pls_group1_write(vi_pipe, (td_u8)pls);
            ot_ext_af_iir_plg_group1_write(vi_pipe, (td_u8)plg);
        }
    }
}
...
td_void isp_expander_attr_write(ot_vi_pipe vi_pipe, const ot_isp_expander_attr *expander_attr)
{
    ...
    for (i = 0; i < expander_attr->knee_point_num; i++) {
        ot_ext_system_expander_knee_x_coord_write(vi_pipe, i, (td_u16)expander_attr->knee_point_coord[i].x);
        ot_ext_system_expander_knee_y_coord_write(vi_pipe, i, (td_u32)expander_attr->knee_point_coord[i].y);
    }
}
    ...
    const ot_isp_shading_lut_attr *shading_lut_attr)
{
    ...
    for (i = 0; i < OT_ISP_MLSC_X_HALF_GRID_NUM; i++) {
        ot_ext_system_isp_mesh_shading_xgrid_write(vi_pipe, (td_u8)i, shading_lut_attr->x_grid_width[i]);
    }

    for (i = 0; i < OT_ISP_MLSC_Y_HALF_GRID_NUM; i++) {
        ot_ext_system_isp_mesh_shading_ygrid_write(vi_pipe, (td_u8)i, shading_lut_attr->y_grid_width[i]);
    }
