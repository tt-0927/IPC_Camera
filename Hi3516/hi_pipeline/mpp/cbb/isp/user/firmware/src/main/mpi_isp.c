...
static td_s32 isp_dist_set_fps_vreg(ot_vi_pipe other_pipe, const ot_isp_pub_attr *pub_attr)
{
    td_void *value = TD_NULL;
    td_s32 ret;
    isp_mutex_lock(other_pipe);

    isp_check_open_goto(other_pipe, ret, exit);
    isp_check_vreg_permission_goto(other_pipe, ret, exit);

    value = (td_void *)(&pub_attr->frame_rate);
    ot_ext_system_fps_base_write(other_pipe, *(td_u32 *)value);

    isp_mutex_unlock(other_pipe);
...
static td_s32 isp_dist_set_other_pipe_fps(ot_vi_pipe vi_pipe, ot_vi_pipe other_pipe, const ot_isp_pub_attr *pub_attr)
{
    td_s32 ret;

    isp_check_open_return(other_pipe);

    if (other_pipe == vi_pipe) {
        return TD_SUCCESS;
    }

    ret = isp_check_isp_kernel_init(other_pipe);
    if (ret != TD_SUCCESS) {
        return ret;
    }

    ret = isp_dist_set_fps_vreg(other_pipe, pub_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
static td_s32 isp_dist_set_fps(ot_vi_pipe vi_pipe, const ot_isp_pub_attr *pub_attr, vi_distribute_attr *dist_attr)
{
    td_bool fps_chg_ready = TD_FALSE;
    td_u8 i, j;
    td_void *value = TD_NULL;
    td_s32 ret;
    vi_distribute_attr dist_tmp = {0};

    ret = ioctl(isp_get_fd(vi_pipe), ISP_GET_DIST_FPS_STATUS, &fps_chg_ready);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    if (fps_chg_ready != TD_TRUE) {
        isp_err_trace("ISP[%d] dist fps not ready\n", vi_pipe);
    ...
    if (dist_attr->grp_share_vs == TD_FALSE) {
        for (j = 0; j < OT_VI_MAX_DISTRIBUTE_PIPE_NUM; ++j) {
            ret = isp_dist_set_other_pipe_fps(vi_pipe, dist_attr->pipe_id[j], pub_attr);
            if (ret != TD_SUCCESS) {
                return ret;
            }
        }
    ...
    for (i = 0; i < OT_ISP_MAX_PHY_PIPE_NUM; ++i) {
            isp_check_open_return(i);
        isp_check_isp_kernel_init_return(i);

        ret = ioctl(isp_get_fd(i), ISP_GET_DIST_ATTR, &dist_tmp);
            if (ret != TD_SUCCESS) {
                return ret;
            }

        for (j = 0; j < OT_VI_MAX_DISTRIBUTE_PIPE_NUM; ++j) {
            ret = isp_dist_set_other_pipe_fps(vi_pipe, dist_tmp.pipe_id[j], pub_attr);
            if (ret != TD_SUCCESS) {
                return ret;
            }
        }
...
static td_s32 isp_set_fps(ot_vi_pipe vi_pipe, const ot_isp_pub_attr *pub_attr)
{
    td_void *value = TD_NULL;
#ifdef CONFIG_OT_VI_DISTRIBUTE_GRP
    td_s32 ret;
    ...
    if (dist_attr.distribute_en == TD_TRUE && isp_init == TD_TRUE) {
        fps = ot_ext_system_fps_base_read(vi_pipe);
        value = (td_void *)(&fps);
        if (is_float_equal(pub_attr->frame_rate, (*(td_float *)value)) == TD_TRUE) {
            return TD_SUCCESS;
        }

        return isp_dist_set_fps(vi_pipe, pub_attr, &dist_attr);
    } else {
        value = (td_void *)(&pub_attr->frame_rate);
        ot_ext_system_fps_base_write(vi_pipe, *(td_u32 *)value);
        return TD_SUCCESS;
...
static td_void isp_pub_attr_write(ot_vi_pipe vi_pipe, const ot_isp_pub_attr *pub_attr, isp_usr_ctx *isp_ctx_info)
{
    ...
    ot_ext_sync_total_width_write(vi_pipe, (td_u16)pub_attr->wnd_rect.width);
    ot_ext_sync_total_height_write(vi_pipe, (td_u16)pub_attr->wnd_rect.height);
    ...
    ot_mipi_crop_pos_x_write(vi_pipe, (td_u16)pub_attr->mipi_crop_attr.mipi_crop_offset.x);
    ot_mipi_crop_pos_y_write(vi_pipe, (td_u16)pub_attr->mipi_crop_attr.mipi_crop_offset.y);
    ot_mipi_crop_width_write(vi_pipe, (td_u16)pub_attr->mipi_crop_attr.mipi_crop_offset.width);
    ot_mipi_crop_height_write(vi_pipe, (td_u16)pub_attr->mipi_crop_attr.mipi_crop_offset.height);
}
...
td_s32 mpi_isp_set_pub_attr(ot_vi_pipe vi_pipe, const ot_isp_pub_attr *pub_attr)
{
    ...
    ret = isp_set_fps(vi_pipe, pub_attr);
    if (ret != TD_SUCCESS) {
        isp_err_trace("ISP[%d] set ISP fps failed\n", vi_pipe);
        return ret;
    }
...
static td_s32 isp_set_chn_select(ot_vi_pipe vi_pipe, const td_u32 chn_sel_cur)
{
    ...
    if (chn_sel_pre != chn_sel_cur) {
        ...
        ot_ext_system_top_channel_select_pre_write(vi_pipe, (td_u8)chn_sel_cur);
    }
    return TD_SUCCESS;
}
...
td_s32 mpi_isp_set_dehaze_attr(ot_vi_pipe vi_pipe, const ot_isp_dehaze_attr *dehaze_attr)
{
    ...
    for (i = 0; i < OT_ISP_DEHAZE_LUT_SIZE; i++) {
        ot_ext_system_dehaze_lut_write(vi_pipe, (td_u8)i, dehaze_attr->dehaze_lut[i]);
    }
...
td_s32 mpi_isp_get_dehaze_attr(ot_vi_pipe vi_pipe, ot_isp_dehaze_attr *dehaze_attr)
{
    ...
    for (i = 0; i < OT_ISP_DEHAZE_LUT_SIZE; i++) {
        dehaze_attr->dehaze_lut[i] = ot_ext_system_dehaze_lut_read(vi_pipe, (td_u8)i);
    }
    ...
    vi_pipe_wdr_attr *wdr_attr)
{
    ...
    wdr_chn = (td_u8)(MIN2(wdr_attr->pipe_num, OT_ISP_WDR_MAX_FRAME_NUM));
                                                ...
                                                isp_working_mode *isp_work_mode)
{
    ...
    max_awb_col = (td_u8)(MIN2(OT_ISP_AWB_ZONE_ORIG_COLUMN, width / OT_ISP_AWB_MIN_WIDTH));
    max_awb_row = (td_u8)(MIN2(OT_ISP_AWB_ZONE_ORIG_ROW, height / OT_ISP_AWB_MIN_HEIGHT));
    min_awb_col = (td_u8)(MAX2(isp_work_mode->block_num,
                               (width + OT_ISP_AWB_ZONE_MAX_WIDTH - 1) / OT_ISP_AWB_ZONE_MAX_WIDTH));
    min_awb_row = (td_u8)(MAX2(1, (height + OT_ISP_AWB_ZONE_MAX_HEIGHT - 1) / OT_ISP_AWB_ZONE_MAX_HEIGHT));
...
td_s32 mpi_isp_set_stats_cfg(ot_vi_pipe vi_pipe, const ot_isp_stats_cfg *stat_cfg)
{
    ...
    ret = isp_motion_stats_attr_check(vi_pipe, &stat_cfg->motion_cfg);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
td_s32 mpi_isp_usr_get_mesh_shading_lut_attr(ot_vi_pipe vi_pipe, ot_isp_shading_lut_attr *shading_lut_attr)
{
    ...
    for (i = 0; i < OT_ISP_MLSC_X_HALF_GRID_NUM; i++) {
        shading_lut_attr->x_grid_width[i] = ot_ext_system_isp_mesh_shading_xgrid_read(vi_pipe, (td_u8)i);
    }

    for (i = 0; i < OT_ISP_MLSC_Y_HALF_GRID_NUM; i++) {
        shading_lut_attr->y_grid_width[i] = ot_ext_system_isp_mesh_shading_ygrid_read(vi_pipe, (td_u8)i);
    }
...
td_s32 mpi_isp_get_crosstalk_attr(ot_vi_pipe vi_pipe, ot_isp_cr_attr *cr_attr)
{
    ...
    cr_attr->slope       = (td_u8)ot_ext_system_ge_slope_read(vi_pipe);
    cr_attr->sensi_slope = (td_u8)ot_ext_system_ge_sensitivity_read(vi_pipe);
...
td_s32 mpi_isp_set_ca_attr(ot_vi_pipe vi_pipe, const ot_isp_ca_attr *ca_attr)
{
    ...
    for (i = 0; i < OT_ISP_CA_YRATIO_LUT_LENGTH; i++) {
        ...
        ot_ext_system_ca_cp_lut_write(vi_pipe, (td_u8)i, cp_lut_value);
        ot_ext_system_ca_y_ratio_lut_write(vi_pipe, (td_u8)i, ca_attr->ca.y_ratio_lut[i]);
        ot_ext_system_ca_y_sat_lut_write(vi_pipe, (td_u8)i, ca_attr->ca.y_sat_lut[i]);
    }
    for (i = 0; i < OT_ISP_AUTO_ISO_NUM; i++) {
        ot_ext_system_ca_iso_ratio_lut_write(vi_pipe, (td_u8)i, ca_attr->ca.iso_ratio[i]);
    }
...
td_s32 mpi_isp_get_ca_attr(ot_vi_pipe vi_pipe, ot_isp_ca_attr *ca_attr)
{
    ...
    for (i = 0; i < OT_ISP_CA_YRATIO_LUT_LENGTH; i++) {
        ca_attr->cp.cp_lut_y[i] =
            (ot_ext_system_ca_cp_lut_read(vi_pipe, (td_u8)i) >> 16) & 0xFF; /* bit16~23: cp_lut_y */
        ca_attr->cp.cp_lut_u[i] = (ot_ext_system_ca_cp_lut_read(vi_pipe, (td_u8)i) >> 8) & 0xFF; /* bit8~15: cp_lut_u */
        ca_attr->cp.cp_lut_v[i] = ot_ext_system_ca_cp_lut_read(vi_pipe, (td_u8)i) & 0xFF;         /* bit0~7: cp_lut_v */
    }

    for (i = 0; i < OT_ISP_CA_YRATIO_LUT_LENGTH; i++) {
        ca_attr->ca.y_ratio_lut[i] = ot_ext_system_ca_y_ratio_lut_read(vi_pipe, (td_u8)i);
        ca_attr->ca.y_sat_lut[i] = ot_ext_system_ca_y_sat_lut_read(vi_pipe, (td_u8)i);
    }

    for (i = 0; i < OT_ISP_AUTO_ISO_NUM; i++) {
        ca_attr->ca.iso_ratio[i] = (td_s32)ot_ext_system_ca_iso_ratio_lut_read(vi_pipe, (td_u8)i);
    }
...
static td_s32 isp_trans_vi_err_code(td_s32 err_code)
{
...
static td_s32 isp_get_fpn_calibrate_result(ot_vi_pipe vi_pipe, ot_isp_fpn_calibrate_attr *calibrate,
    vi_fpn_attr *fpn_attr, td_bool bnr_en)
{
    td_s32 i;
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
...
static td_s32 isp_set_calibrate_attr(ot_vi_pipe vi_pipe, ot_isp_fpn_calibrate_attr *calibrate,
    vi_fpn_attr *fpn_attr, td_bool *bnr_en)
{
    td_s32 ret;
    isp_check_open_return(vi_pipe);
    ...
    ret = isp_fpn_calibrate_attr_check(vi_pipe, calibrate);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
td_s32 mpi_isp_fpn_calibrate(ot_vi_pipe vi_pipe, ot_isp_fpn_calibrate_attr *calibrate_attr)
{
    ...
    ret = isp_set_calibrate_attr(vi_pipe, calibrate_attr, &fpn_attr, &bnr_en);
    if (ret != TD_SUCCESS) {
        isp_err_trace("ISP[%d] set isp fpn calibrate failed!\n", vi_pipe);
        goto exit;
    }
    isp_mutex_unlock(vi_pipe);
    ret = mpi_vi_set_fpn_attr(vi_pipe, &fpn_attr); /* online, can wait for 16 frames, so need unlock first */
    if (ret != TD_SUCCESS) {
        isp_mutex_lock(vi_pipe);
        ot_ext_system_fpn_cablibrate_enable_write(vi_pipe, 0);
        fpn_calib_open_bnr(vi_pipe, bnr_en);
        isp_mutex_unlock(vi_pipe);
        return isp_trans_vi_err_code(ret);
    }

    isp_mutex_lock(vi_pipe);
    ret = isp_get_fpn_calibrate_result(vi_pipe, calibrate_attr, &fpn_attr, bnr_en);
exit:
    isp_mutex_unlock(vi_pipe);
    return ret;
}

static td_s32 isp_set_fpn_correction_attr(ot_vi_pipe vi_pipe, const ot_isp_fpn_attr *correction,
    td_bool correct_enable, td_u8 fpn_work_mode)
{
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
    isp_check_vreg_permission_return(vi_pipe);

    return isp_set_correction_attr(vi_pipe, correction, correct_enable, fpn_work_mode);
}

static td_s32 isp_check_prepare_correction_attr(ot_vi_pipe vi_pipe, const ot_isp_fpn_attr *correction,
    vi_fpn_attr *fpn_attr)
{
    td_u8 i;
    td_s32 ret;
    ...
    isp_check_mem_init_return(vi_pipe);
    isp_check_vreg_permission_return(vi_pipe);
    mpi_isp_check_bool_return(correction->enable);
    mpi_isp_check_bool_return(correction->aibnr_mode);
    ret = isp_fpn_correction_attr_check(vi_pipe, correction);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
td_s32 mpi_isp_set_fpn_attr(ot_vi_pipe vi_pipe, const ot_isp_fpn_attr *correction)
{
    ...
    vi_fpn_attr fpn_attr;
    isp_mutex_lock(vi_pipe);
    ret = isp_check_prepare_correction_attr(vi_pipe, correction, &fpn_attr);
    if (ret != TD_SUCCESS) {
        isp_err_trace("ISP[%d] set isp fpn check and prepare correction attr failed!\n", vi_pipe);
        goto exit;
    }
    ...
    isp_mutex_lock(vi_pipe);
    ret = isp_set_fpn_correction_attr(vi_pipe, correction, fpn_attr.correction_attr.enable,
        (td_u8)fpn_attr.fpn_work_mode);
    if (ret != TD_SUCCESS) {
        isp_err_trace("ISP[%d] set isp fpn correction attr failed!\n", vi_pipe);
    }
exit:
    isp_mutex_unlock(vi_pipe);
    return ret;
