...
static td_s32 isp_init_set_sensor_mode(ot_vi_pipe vi_pipe)
{
    ...
    sns_image_mode.width  = (td_u16)ot_ext_top_sensor_width_read(vi_pipe);
    sns_image_mode.height = (td_u16)ot_ext_top_sensor_height_read(vi_pipe);
...
static td_s32 isp_init_sensor_update(ot_vi_pipe vi_pipe, isp_usr_ctx *isp_ctx)
{
    ...
    ret = isp_sensor_init_standby_cfg(vi_pipe);
    if (ret != TD_SUCCESS) {
        isp_err_trace("isp init standby cfg failed 0x%x!\n", ret);
    }
...
static td_void isp_apb_reg_pre_cfg(ot_vi_pipe vi_pipe, const isp_usr_ctx *isp_ctx)
{
    td_s32 ret;
    if (isp_ctx->isp_yuv_mode != TD_TRUE) {
...
td_s32 isp_switch_mode(ot_vi_pipe vi_pipe)
{
    ...
    isp_ctx->sns_image_mode.width    = (td_u16)ot_ext_top_sensor_width_read(vi_pipe);
    isp_ctx->sns_image_mode.height   = (td_u16)ot_ext_top_sensor_height_read(vi_pipe);
...
static td_u8 isp_iso_index_cal(ot_vi_pipe vi_pipe, td_u32 ae_done_frm_cnt)
{
    ...
    if (isp_ctx->linkage.iso_done_frm_cnt > ae_done_frm_cnt) {  /* preview pipe the last frame dont run complete. */
        ...
        index = (td_u8)(index - cnt_diff);
    } else if (isp_ctx->linkage.iso_done_frm_cnt < ae_done_frm_cnt) { /* the preview pipe run first. */
        index = (td_u8)(index + (ae_done_frm_cnt - isp_ctx->linkage.iso_done_frm_cnt - 1));
...
static td_u8 isp_ai_wb_index_cal(ot_vi_pipe vi_pipe, td_u32 ae_done_frm_cnt)
{
    ...
    if (isp_ctx->linkage.iso_done_frm_cnt < ae_done_frm_cnt) {
        index = (td_u8)(ae_done_frm_cnt - isp_ctx->linkage.iso_done_frm_cnt - 1);
...
static td_void isp_ai_process(ot_vi_pipe vi_pipe)
{
    ...
    if (vi_pipe == isp_ctx->ai_info.ai_pipe_id) {
        ...
        isp_ctx->linkage.sensor_iso =
            (td_u32)(((td_u64)isp_ctx->linkage.iso << 0x8) / div_0_to_1(isp_ctx->linkage.isp_dgain));
        ...
        for (i = 0; i < OT_ISP_CCM_MATRIX_SIZE; i++) {
            isp_ctx->linkage.ccm[i] = (td_u16)isp_base_ctx->linkage.sync_ccm[index][i];
        }
...
static td_s32 isp_sync_cfg_config(ot_vi_pipe vi_pipe, const isp_usr_ctx *isp_ctx)
{
    td_s32 ret;
