...
td_s32 isp_sensor_init_standby_cfg(ot_vi_pipe vi_pipe)
{
    ...
    if (sns_exp->pfn_cmos_get_standby_cfg != TD_NULL) {
        sns_exp->pfn_cmos_get_standby_cfg(vi_pipe, &standby_cfg);
    } else {
        return TD_SUCCESS;
    }

    ret = ioctl(isp_get_fd(vi_pipe), ISP_STANDBY_CFG_INIT, &standby_cfg);
    if (ret != TD_SUCCESS) {
        return ret;
    }
