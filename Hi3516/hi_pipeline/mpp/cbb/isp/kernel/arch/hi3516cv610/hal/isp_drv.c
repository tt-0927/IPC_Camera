...
static td_s32 isp_drv_wait_init(isp_drv_ctx *drv_ctx)
{
    ...
    if (osal_wait_init(&drv_ctx->isp_wait_vd_run_end) != TD_SUCCESS) {
        goto out4;
    }
    if (osal_wait_init(&drv_ctx->isp_wait_vd_standby_end) != TD_SUCCESS) {
        goto out5;
    }
#endif
...
out6:
#ifdef CONFIG_OT_SYS_KOL_SUPPORT
    osal_wait_destroy(&drv_ctx->isp_wait_vd_standby_end);
out5:
    osal_wait_destroy(&drv_ctx->isp_wait_vd_run_end);
out4:
#endif
...
#ifdef CONFIG_OT_SYS_KOL_SUPPORT
    if (osal_sem_init(&drv_ctx->isp_sem_run_end_vd, 1) != TD_SUCCESS) {
        goto out4;
    }
    if (osal_sem_init(&drv_ctx->isp_sem_standby_vd, 1) != TD_SUCCESS) {
        goto out5;
    }
#endif
...
out6:
#ifdef CONFIG_OT_SYS_KOL_SUPPORT
    osal_sem_destroy(&drv_ctx->isp_sem_standby_vd);
out5:
    osal_sem_destroy(&drv_ctx->isp_sem_run_end_vd);
out4:
#endif
    ...
    isp_be_offline_stt_buf *be_off_stt_addr, isp_be_offline_stt_attr *be_off_stt_buf)
{
    ...
    for (i = 0; i < blk_num; i++) {
        ...
        be_off_stt_addr->ae_hist_phys_addr[i] =
            be_off_stt_buf->be_stt_buf[working_idx].ae_hist_phys_addr[i] + addr_offset;
        be_off_stt_addr->ae_aver_gb_b_phys_addr[i] =
            be_off_stt_buf->be_stt_buf[working_idx].ae_aver_gb_b_phys_addr[i] + addr_offset;
        be_off_stt_addr->ae_aver_r_gr_phys_addr[i] =
            be_off_stt_buf->be_stt_buf[working_idx].ae_aver_r_gr_phys_addr[i] + addr_offset;
        ...
        be_off_stt_addr->dehaze_min_phys_addr[i] =
            be_off_stt_buf->be_stt_buf[working_idx].dehaze_min_phys_addr[i] + addr_offset;
        be_off_stt_addr->dehaze_max_phys_addr[i] =
            be_off_stt_buf->be_stt_buf[working_idx].dehaze_max_phys_addr[i] + addr_offset;
        be_off_stt_addr->ldci_lpf_phys_addr[i] =
            be_off_stt_buf->be_stt_buf[working_idx].ldci_lpf_phys_addr[i] + addr_offset;
    }
    ...
    isp_be_offline_stt_attr *be_off_stt_buf, td_u32 stat_valid)
{
    ...
    isp_drv_stats_be_end_read(vi_pipe);
    osal_spin_unlock_irqrestore(isp_spin_lock, &flags);
