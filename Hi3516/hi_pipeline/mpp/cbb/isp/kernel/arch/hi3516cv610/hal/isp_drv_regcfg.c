...
static __inline td_void isp_drv_fe_dg2_offset_write(isp_fe_reg_type *fe_reg, const td_u16 *dg_blc)
{
...
{
    fe_reg->isp_dg_blc_offset1.u32 = ((td_u32)dg_blc[OT_ISP_CHN_R] << 16) + dg_blc[OT_ISP_CHN_GR]; /* bit16~30: r */
    fe_reg->isp_dg_blc_offset2.u32 = ((td_u32)dg_blc[OT_ISP_CHN_B] << 16) + dg_blc[OT_ISP_CHN_GB]; /* bit16~30: b */
}
    ...
    const isp_fe_blc_dyna_cfg *fe_blc_reg_cfg)
{
    ...
    if (fe_blc_reg_cfg->resh_dyna == TD_TRUE) {
        isp_drv_fe_dg2_offset_write(fe_reg, fe_blc_reg_cfg->fe_dg_blc[k].blc);
    ...
    ot_isp_bnr_ref_mode bnr_ref_mode)
{
    ...
    isp_bnr_cfg.bits.isp_bnr2d_en = (bnr_ref_mode == OT_ISP_MD_MODE) ? 1 : 0;
    be_reg->isp_bnr_cfg.u32 = isp_bnr_cfg.u32;
}

static td_void isp_drv_bnr_mixing_write(isp_post_be_reg_type *be_reg, ot_isp_bnr_ref_mode bnr_ref_mode)
{
    u_isp_bnr_cfg10 isp_bnr_cfg10;
    isp_bnr_cfg10.u32 = be_reg->isp_bnr_cfg10.u32;
    isp_bnr_cfg10.bits.isp_bnr_en_mixing = (bnr_ref_mode == OT_ISP_PURE_TNR_MODE) ? ISP_MIX_EN_PURE : ISP_MIX_EN_3DNR;
    be_reg->isp_bnr_cfg10.u32 = isp_bnr_cfg10.u32;
}
...
td_void isp_drv_reg_config_bnr_online(ot_vi_pipe vi_pipe, isp_drv_ctx *drv_ctx, td_bool tnr_en,
    td_bool initial_en, ot_isp_bnr_ref_mode bnr_ref_mode)
{
    td_s32 ret;
    ...
    blk_dev = drv_ctx->work_mode.block_dev;
    for (k = 0; k < blk_num; k++) {
        isp_drv_bnr_entmpnr_write(be_reg[k + blk_dev], tnr_en, initial_en, bnr_ref_mode);
        isp_drv_sharpen_mot_en_write(be_reg[k + blk_dev], tnr_en);
        if (tnr_en == TD_FALSE) {
            isp_drv_sharpen_mot_ctrl_adj_en_write(be_reg[k + blk_dev], TD_FALSE);
        }
        isp_drv_bnr_mixing_write(be_reg[k + blk_dev], bnr_ref_mode);
    }
}
...
td_void isp_drv_reg_config_bnr_offline(isp_be_wo_reg_cfg *be_cfg, isp_drv_ctx *drv_ctx)
{
    ...
    for (k = 0; k < drv_ctx->work_mode.block_num; k++) {
        isp_drv_bnr_entmpnr_write(&be_cfg->be_reg_cfg[k].be_reg, tnr_en, is_init, drv_ctx->bnr_tpr_filt.cur.ref_mode);
        ...
        isp_drv_bnr_mixing_write(&be_cfg->be_reg_cfg[k].be_reg, drv_ctx->bnr_tpr_filt.cur.ref_mode);
         /* bnr enable */
...
static td_void isp_drv_drc_var_write(isp_post_be_reg_type *be_reg, td_u8 uisp_drc_var_rng_fine)
{
...
td_void isp_drv_set_drc_var(ot_vi_pipe vi_pipe, isp_drv_ctx *drv_ctx, td_u8 uisp_drc_var_rng_fine)
{
    td_u8   i, block_dev;
    td_s32  ret;
    ...
    for (i = 0; i < drv_ctx->work_mode.block_num; i++) {
        isp_drv_drc_var_write(post_be_reg[i + block_dev], uisp_drc_var_rng_fine);
...
static td_void isp_drv_set_drc_switch(ot_vi_pipe vi_pipe, isp_drv_ctx *drv_ctx, td_bool inner_bypass)
{
    td_u8   i, block_dev;
    td_s32  ret;
    isp_post_be_reg_type *post_be_reg[OT_ISP_STRIPING_MAX_NUM] = { TD_NULL };
    ...
    for (i = 0; i < drv_ctx->work_mode.block_num; i++) {
        if (drv_ctx->drc_freeze == TD_FALSE) {
            ...
            isp_drv_drc_var_write(post_be_reg[i + block_dev], DRC_VAR_RNG_FINE_DEFAULT);
        }
    }
...
static td_void isp_drv_set_aibnr_wdr_gain_cfg(isp_be_sync_para *be_sync_para, isp_aibnr_cfg *ainr_cfg)
{
    ...
    isp_dgain = isp_drv_isp_dgain_blc_compensation(0x100, be_sync_para->be_blc.dg_blc.blc[1]);
    be_sync_para->isp_dgain[0] = clip3(isp_dgain, ISP_DIGITAL_GAIN_MIN, ISP_DIGITAL_GAIN_MAX);
}
#endif
...
static td_void isp_drv_adgamma_cfg_set(isp_be_all_reg_type *be_reg_cfg, td_bool dgain_agamma_en)
{
#if defined(CONFIG_OT_AIISP_SUPPORT) && defined(CONFIG_OT_AIBNR_SUPPORT)
    u_isp_viproc_ispbe_ctrl0 viproc_ispbe_ctrl0;
    ...
    viproc_ispbe_ctrl0.bits.isp_expander_en = dgain_agamma_en;
    ...
    isp_aibnr_cfg *aibnr_cfg)
{
    ...
    dgain_agamma_en = aibnr_cfg->dgain_bypass;  /* if it is bypass, means has cfg agmma */

    for (i = 0; i < drv_ctx->work_mode.block_num; i++) {
        isp_drv_adgamma_cfg_set(&be_reg->be_reg_cfg[i], dgain_agamma_en);
        isp_drv_set_pre_be_sync_para(vi_pipe, &be_reg->be_reg_cfg[i].be_reg, drv_ctx, be_sync_para, aibnr_cfg);
...
td_s32 isp_drv_close_drc_bypass(ot_vi_pipe vi_pipe)
{
    td_u8 k, blk_dev;
    td_s32 ret;
    isp_drv_ctx *drv_ctx = TD_NULL;
...
td_s32 isp_drv_close_drc_var(ot_vi_pipe vi_pipe)
{
    isp_drv_ctx *drv_ctx = TD_NULL;
    osal_spinlock *spin_lock;
    unsigned long flags = 0;
    ...
    drv_ctx->drc_freeze = TD_TRUE;
    isp_drv_set_drc_var(vi_pipe, drv_ctx, 0);
...
td_s32 isp_drv_restore_drc(ot_vi_pipe vi_pipe)
{
    ...
    blk_dev = drv_ctx->work_mode.block_dev;
    for (k = 0; k < drv_ctx->work_mode.block_num; k++) {
        isp_drv_drc_bypass_write(post_be_reg[k + blk_dev], drv_ctx->be_sync_para.drc.inner_bypass_en);
    }
    isp_drv_set_drc_var(vi_pipe, drv_ctx, DRC_VAR_RNG_FINE_DEFAULT);
...
#ifdef CONFIG_OT_ISP_DRC_SWITCH_SUPPORT
    isp_drv_set_drc_switch(vi_pipe, drv_ctx, drv_ctx->be_sync_para.drc.inner_bypass_en);
#endif
...
td_void isp_drv_set_regup_online_yuv(isp_viproc_reg_type *viproc_reg, td_u32 reg_up)
{
    isp_drv_set_be_regup(viproc_reg, reg_up);
}

#if defined(CONFIG_OT_AIISP_SUPPORT) && defined(CONFIG_OT_AIBNR_SUPPORT)
static td_void isp_drv_aibnr_set_bnr_bypass(isp_be_all_reg_type *be_reg_cfg)
{
    u_isp_bnr_cfg isp_bnr_cfg;
    u_isp_viproc_ispbe_ctrl0 viproc_ispbe_ctrl0;

    isp_bnr_cfg.u32 = be_reg_cfg->be_reg.isp_bnr_cfg.u32;
    isp_bnr_cfg.bits.isp_bnr_entmpnr = TD_FALSE;
    be_reg_cfg->be_reg.isp_bnr_cfg.u32 = isp_bnr_cfg.u32;
    /* bnr enable */
    viproc_ispbe_ctrl0.u32 = be_reg_cfg->viproc_reg.viproc_ispbe_ctrl0.u32;
    viproc_ispbe_ctrl0.bits.isp_bnr_en = TD_FALSE;
    be_reg_cfg->viproc_reg.viproc_ispbe_ctrl0.u32 = viproc_ispbe_ctrl0.u32;
}

td_void isp_drv_set_aibnr_bnr_cfg(isp_be_wo_reg_cfg *be_reg, isp_drv_ctx *drv_ctx, isp_aibnr_cfg *aibnr_cfg)
{
    td_u8 i;

    if (aibnr_cfg->bnr_bypass == TD_FALSE) {
        return;
    }
    ...
    if (drv_ctx->work_mode.aiisp_mode == OT_VI_AIISP_MODE_DEFAULT) {
        for (i = 0; i < drv_ctx->work_mode.block_num; i++) {
            isp_drv_sharpen_mot_en_write(&be_reg->be_reg_cfg[i].be_reg, TD_FALSE);
            isp_drv_sharpen_mot_ctrl_adj_en_write(&be_reg->be_reg_cfg[i].be_reg, TD_FALSE);
            isp_drv_aibnr_set_bnr_bypass(&be_reg->be_reg_cfg[i]);
        }
    }
...
static td_void isp_drv_dynamic_be_blc_update_idx(ot_vi_pipe vi_pipe, const isp_drv_ctx *drv_ctx,
    td_u32 *pre_be_idx, td_u32 *post_be_idx)
{
    ...
    if (pipe_low_delay_en || (drv_ctx->frame_int_attr.interrupt_type == OT_FRAME_INTERRUPT_EARLY_END)) {
        /* need_to sync with fe_cfg, but in case of lowdelay or early_end, */
        /* send_frame in advance, update dynamic blc idx is after send_frame */
        *post_be_idx -= 1;
        *pre_be_idx -= 1;
    }
}
    ...
    td_u32 *pre_be_idx, td_u32 *post_be_idx)
{
    ...
    if (is_offline_mode(drv_ctx->work_mode.running_mode) ||
        ...
        *pre_be_idx = 2; /* delay 2 */
        isp_drv_dynamic_be_blc_update_idx(vi_pipe, drv_ctx, pre_be_idx, post_be_idx);
...
td_void isp_drv_get_fe_ae_weight(ot_vi_pipe vi_pipe, td_u32 *fe_ae_weight,
    td_u32 weight_num)
{
    td_s32 i;
    isp_fe_reg_type *fe_reg = TD_NULL;
    ...
    isp_check_pointer_void_return(fe_reg);
    if (weight_num > ISP_FE_WEIGHT_NUM) {
        isp_err_trace("weight_num:%d, is too large\n", weight_num);
        return;
    }
    fe_reg->isp_ae_weight_raddr.u32 = 0;
    for (i = 0; i < weight_num; i++) {
        fe_ae_weight[i] = fe_reg->isp_ae_weight_rdata.u32;
    }
}
td_void isp_drv_set_fe_ae_weight(ot_vi_pipe vi_pipe, td_u32 *fe_ae_weight,
    td_u32 weight_num)
{
    td_s32 i;
    isp_fe_reg_type *fe_reg = TD_NULL;
    ...
    fe_reg->isp_ae_weight_waddr.u32 = 0;
    for (i = 0; i < weight_num; i++) {
        fe_reg->isp_ae_weight_wdata.u32 = fe_ae_weight[i];
    }
    fe_reg->isp_fe_reg_update.u32 = 1;  /* fe_update */
}
#endif
