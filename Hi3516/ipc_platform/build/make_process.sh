...
# @Date         : 2025-06-13 09:47:25
 ...
 # @LastEditTime : 2026-01-22 17:03:28
# @Description  : 编译程序脚本
###
...
check_build() {
    ...
    LAST_BUILD_SENSOR_TYPE=""
    LAST_BUILD_PROJECT_TYPE=""
    ...
    if [ -f "$CHECK_FILE_PATH" ]; then
        ...
        last_sensor=$(cmake_cache_get "SENSOR_TYPE" "$CHECK_FILE_PATH")
        last_project=$(cmake_cache_get "PROJECT_TYPE" "$CHECK_FILE_PATH")
        ...
        [ -n "$last_sensor" ] && LAST_BUILD_SENSOR_TYPE="-DSENSOR_TYPE=${last_sensor}"
        [ -n "$last_project" ] && LAST_BUILD_PROJECT_TYPE="-DPROJECT_TYPE=${last_project}"
    fi
}
...
# 增加镜头型号定义的cmake参数
BUILD_SENSOR_TYPE="-DSENSOR_TYPE=${SENSOR_TYPE}"

# 增加项目类型定义的cmake参数
BUILD_PROJECT_TYPE="-DPROJECT_TYPE=${PROJECT_TYPE}"
...
if [ "$MAKE_MODE" = true ]; then
    ...
    if [ "$LAST_BUILD_VERSION" != "$BUILD_TYPE" ] ||
        [ "$LAST_BUILD_DEVICE_TYPE" != "$BUILD_DEVICE_TYPE" ] ||
        [ "$LAST_BUILD_SENSOR_TYPE" != "$BUILD_SENSOR_TYPE" ] ||
        [ "$LAST_BUILD_PROJECT_TYPE" != "$BUILD_PROJECT_TYPE" ]; then
        trace "重新 cmake"
        ...
        # cmake 输出文件到output
        trace "cmake命令参数 [${BUILD_TYPE} ${BUILD_DEVICE_TYPE} ${BUILD_SENSOR_TYPE} ${BUILD_PROJECT_TYPE} ${EXTRA_PARAM} . -B${CMAKE_TEMP_PATH} -DCMAKE_EXPORT_COMPILE_COMMANDS=ON]"
        if ! cmake "${BUILD_TYPE}" "${BUILD_DEVICE_TYPE}" "${BUILD_SENSOR_TYPE}" "${BUILD_PROJECT_TYPE}" "${EXTRA_PARAM}" . -B"${CMAKE_TEMP_PATH}" -DCMAKE_EXPORT_COMPILE_COMMANDS=ON; then
            error "cmake执行出现错误"
            exit 1
        fi
