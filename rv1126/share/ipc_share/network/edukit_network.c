...
char* get_default_interface() 
{
    ...
    {
        char dev[IFNAMSIZ];
        unsigned long nDest, nGateway, nMask;
        unsigned int nFlags;    
        int nMetric, nRefcnt, nUse;

        if (sscanf(line, "%16s %lx %lx %X %d %d %d %lx",
                   dev, &nDest, &nGateway, &nFlags, &nRefcnt, &nUse, &nMetric, &nMask) != 8) 
        {
            continue;
        }

        /* 检查是否是默认路由（Destination = 0.0.0.0） */ 
        if (nDest == 0) 
        {
            pIface = strdup(dev);
            break;
