...
{
	{command_INIT,          0,     60,  220, ECSmtp::SERVER_NOT_RESPONDING},
	{command_EHLO,          60,  60,  250, ECSmtp::COMMAND_EHLO},
	{command_AUTHPLAIN,     60,  60,  235, ECSmtp::COMMAND_AUTH_PLAIN},
	{command_AUTHLOGIN,     60,  60,  334, ECSmtp::COMMAND_AUTH_LOGIN},
	{command_AUTHCRAMMD5,   60,  60,  334, ECSmtp::COMMAND_AUTH_CRAMMD5},
	{command_AUTHDIGESTMD5, 60,  60,  334, ECSmtp::COMMAND_AUTH_DIGESTMD5},
	{command_DIGESTMD5,     60,  60,  335, ECSmtp::COMMAND_DIGESTMD5},
	{command_USER,          60,  60,  334, ECSmtp::UNDEF_XYZ_RESPONSE},
	{command_PASSWORD,      60,  60,  235, ECSmtp::BAD_LOGIN_PASS},
	{command_MAILFROM,      60,  60,  250, ECSmtp::COMMAND_MAIL_FROM},
	{command_RCPTTO,        60,  60,  250, ECSmtp::COMMAND_RCPT_TO},
	{command_DATA,          60,  60,  354, ECSmtp::COMMAND_DATA},
	{command_DATABLOCK,     60,  0,     0,   ECSmtp::COMMAND_DATABLOCK},	// Here the valid_reply_code is set to zero because there are no replies when sending data blocks
	{command_DATAEND,       60,  60, 250, ECSmtp::MSG_BODY_ERROR},
	{command_QUIT,          60,  60,  221, ECSmtp::COMMAND_QUIT},
	{command_STARTTLS,      60,  60,  220, ECSmtp::COMMAND_EHLO_STARTTLS}
};
...
CSmtp::CSmtp()
{
	...
	char hostname[255];
	if(gethostname((char *) &hostname, 255) == SOCKET_ERROR) dlog_error("ECSmtp抛出异常");
...
bool CSmtp::Send()
{
	...
	dlog_debug("Attachments.size() = %d",Attachments.size());
	dlog_debug("Recipients.size() = %d",Recipients.size());
	//Allocate memory
	if((FileBuf = new char[55]) == NULL)
	{
		dlog_error("ECSmtp抛出异常: [new char[55]失败]");
		return false;
	}
	...
	{
		std::cout << "Recipient " << i + 1 << ": " << (Recipients.at(i).Mail).c_str() << std::endl;
								...
								const char* password/*=NULL*/)
{
	...
	if((hSocket = socket(PF_INET, SOCK_STREAM,0)) == INVALID_SOCKET)
		dlog_error("ECSmtp抛出异常:[hSocket初始化失败]");

	//设置 DSCP 
    int dscp_val = nDscp;         
    int tos = dscp_val << 2;   
    if (setsockopt(hSocket, IPPROTO_IP, IP_TOS, (char*)&tos, sizeof(tos)) < 0) 
    {
        dlog_warn("CSmtp: 设置 DSCP (IP_TOS) 失败: %s", strerror(errno));
    }
	...
	if(GetSecurityType() == USE_TLS || GetSecurityType() == USE_SSL)
	{
		...
		if(GetSecurityType() == USE_SSL)
		{
			if(OpenSSLConnect() != 0)
			{
				dlog_error("ECSmtp抛出异常:[OpenSSLConnect]失败");
				return false;
			}
			else
			{
				dlog_debug("OpenSSLConnect 成功");
			}
		}
	}
	...
	if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
	{
		...
		else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
		{
			...
			//send the response
			if(strstr(RecvBuf, "charset")!=nullptr) snprintf(SendBuf, BUFFER_SIZE, "charset=utf-8,username=\"%s\"", m_sLogin.c_str());
...
void CSmtp::FormatHeader(char* header)
{
	...
	if(!m_sMailFrom.size()) dlog_error("ECSmtp抛出异常");
...
////////////////////////////////////////////////////////////////////////////////
int  CSmtp::ReceiveData(Command_Entry* pEntry)
{
	if(m_ssl != NULL)
	{
		return ReceiveData_SSL(m_ssl, pEntry);
	}
	int res = 0;
	fd_set fdread;
	timeval time;
...
void CSmtp::SendData(Command_Entry* pEntry)
{
	...
	if(SendBuf == NULL)
	{
		dlog_error("ECSmtp抛出异常:[SendBuf为空]");
		return;
	}
		
	// dlog_debug("开始发送 hSocket = %d",hSocket);
	
	while(nLeft > 0)
	{
		// dlog_debug("发送前===========>nLeft[%d]",nLeft);
		FD_ZERO(&fdwrite);
		...
		if(res && FD_ISSET(hSocket,&fdwrite))
		{
			// dlog_debug("发送 SendBuf[%d] = %c",idx,SendBuf[idx]);
			res = send(hSocket,&SendBuf[idx],nLeft,0);
			if(res == SOCKET_ERROR || res == 0)
			{
				FD_CLR(hSocket,&fdwrite);
				dlog_error("ECSmtp抛出异常:发送失败 错误码[%d]",res);
			}
			// dlog_debug("发送返回值[%d]",res);
			nLeft -= res;
			idx += res;
			// dlog_debug("发送后===========>nLeft[%d]",nLeft);
		}
	}
...
int CSmtp::ReceiveData_SSL(SSL* ssl, Command_Entry* pEntry)
{
	int res = 0;
	int offset = 0;
	fd_set fdread;
	...
	while(!bFinish)
	{
		...
		if(FD_ISSET(hSocket,&fdread) || (read_blocked_on_write && FD_ISSET(hSocket,&fdwrite)) )
		{
			while(1)
			{
			...
			#if 0
				dlog_error("ssl_err 错误码[%d]",ssl_err);
				// 打印OpenSSL错误栈
				unsigned long err;
				char err_buf[256];
				while ((err = ERR_get_error()) != 0) 
				{
					...
					dlog_error("OpenSSL error: %s", err_buf);
				}
			#endif
...
int CSmtp::ReceiveResponse(Command_Entry* pEntry)
{
	std::string line;
	int reply_code = 0;
	bool bFinish = false;
	...
	{
		dlog_error("ECSmtp抛出异常:接收响应失败 reply_code[%d] pEntry->valid_reply_code[%d]",reply_code,pEntry->valid_reply_code);
		return -1;
	}
	...
	while(1)
	{
		...
		if(write_blocked || read_blocked)
		{
			...
			if(!res)
			{
				...
				FD_ZERO(&fdwrite);
				FD_ZERO(&fdread);
				dlog_error("SERVER_NOT_RESPONDING");
				break;
			}
		}
		...
		switch(SSL_get_error(m_ssl, res))
		{
		  ...
		  case SSL_ERROR_WANT_WRITE:
		  	dlog_debug("SSL_ERROR_WANT_WRITE");
			write_blocked = 1;
			return 0;
			break;

		  case SSL_ERROR_WANT_READ:
		  	dlog_debug("SSL_ERROR_WANT_READ");
			read_blocked = 1;
			return 0;
			break;
...
int CSmtp::setQosDscp(const int &nDscp)
{
