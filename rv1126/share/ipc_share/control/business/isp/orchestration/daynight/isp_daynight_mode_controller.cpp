...
void CIspDayNightModeController::worker_loop()
{
    ...
    while (true)
    {
        ...
        /* 记录本轮待提交的 filter 起始时间，用于提交前二次校验是否被 cancel 后重建。 */
        std::chrono::steady_clock::time_point stCapturedFilterStart;
        {
            std::lock_guard<std::mutex> stLock(m_mtx);
            if (!m_bRunning)
            {
            ...
            if (m_bInitialized)
            {
                ...
                if (m_bPendingNight && stNow >= m_stFilterDeadline)
                {
                    ...
                    stCapturedFilterStart = m_stFilterStartTime;
                    /* ! 不在锁内清除 m_bPendingNight；保持 pending 使后续 cancel 可命中并写入 m_bPendingNight=false，
                     * 二次校验时即可识别已取消状态，避免 worker-detector cancel 竞态导致误提交。 */
                }
        ...
        if (bNeedSubmit)
        {
            if (bFilterExpired)
            {
                /* lock: 二次校验与清除合并为同一锁域，check-and-clear 原子化。
                 * on_detector_observation 在 cancel 时清除 m_bPendingNight，
                 * 重建候选时同时更新 m_stFilterStartTime；任一变化均表示当前提交已过期。 */
                {
                    std::lock_guard<std::mutex> stVerifyLock(m_mtx);
                    if (!m_bPendingNight || m_stFilterStartTime != stCapturedFilterStart)
                    {
                        dlog_info("AUTO日夜检测提交前校验失败, pending已被cancel或重建, 放弃本次提交");
                        continue;
                    }
                    m_bPendingNight = false;
                }
