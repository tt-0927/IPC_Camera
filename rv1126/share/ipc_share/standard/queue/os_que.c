...
int OS_quePut(OS_QueHndl *hndl, Int64 value, Int32 timeout)
{
	...
	while(1) 
	{
		...
		{
			if (hndl->curRd < 0 || hndl->curWr >= hndl->len || hndl->queue == NULL) 
			{
				printf("OS队列为空 或者 获取数据队列索引hndl->curWr[%d]越界\n",hndl->curWr);
				return OS_EFAIL; 
			}
...
int OS_queGet(OS_QueHndl *hndl, Int64 *value, Int32 timeout)
{
	...
	while(1) 
	{
		if(hndl->count > 0 )
		{
		  	...
		  	if (value!=NULL) 
			{
				if (hndl->curRd < 0 || hndl->curRd >= hndl->len || hndl->queue == NULL) 
				{
					printf("OS队列为空 或者 获取数据队列索引hndl->curRd[%d]越界\n",hndl->curRd);
					return OS_EFAIL; 
				}
