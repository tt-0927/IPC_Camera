/**
 * @FilePath     : dwell_config_adapter.cpp
 * @Author       : zhouzr@kfb.cn
 * @Date         : 2026-08-28
 * @LastEditors  : zhouzr@kfb.cn
 * @LastEditTime : 2026-08-28
 * @Description  : 驻留事件配置适配器实现
 */
...
int CDwellConfigAdapter::adapt(const RawDwellConfig_S &stRawConfig, DwellConfig_S &stOut) const
{
    stOut = DwellConfig_S();
    ...
    {
        DwellRuleConfig_S stRule;
        if (!adapt_rule(stRawRule, stRule))
        {
            dlog_warn("驻留配置规则[%d]无效，跳过该规则", stRawRule.nRuleId);
            continue;
        }
        stRule.bValid = true;
...
bool CDwellConfigAdapter::adapt_rule(const RawDwellRule_S &stRawRule, DwellRuleConfig_S &stOutRule) const
{
    /* 事件类型校验：仅接受徘徊/停车 */
    if (stRawRule.enEventType != Event::Type_E::LOITERING_DETECT &&
        stRawRule.enEventType != Event::Type_E::PARKING_DETECT)
    {
        dlog_warn("驻留规则事件类型非法[%d]", static_cast<int>(stRawRule.enEventType));
        return false;
    }
    ...
    /* 1920×1080 基准归一化 */
    const FrameSize_S stConfigSize{ DWELL_CONFIG_WIDTH, DWELL_CONFIG_HEIGHT };
    ...
    {
        dlog_warn("驻留规则[%d]多边形自交", stRawRule.nRuleId);
        return false;
    }
