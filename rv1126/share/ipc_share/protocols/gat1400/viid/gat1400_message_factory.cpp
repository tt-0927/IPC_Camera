...
// 兼容不同平台json格式
std::vector<std::string> vSystemTime = {
    KEY_SYSTEM_TIME, KEY_SYSTEM_TIME_OBJECT
};
...
#if 0
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(security_person_t, PersonID, InfoKind, SourceID, DeviceID, LeftTopX, LeftTopY, RightBtmX, RightBtmY,
...
SecurityString security_message_factory::makeRegisterMessage(const char* deviceId) {
    ...
    jDeviceID[KEY_DEVICE_ID] = deviceId;
    jRegister[KEY_REGISTER_OBJECT] = jDeviceID;
...
SecurityString security_message_factory::makeUnregisterMessage(const char* deviceId) {
    ...
    jDeviceID[KEY_DEVICE_ID] = deviceId;
    jUnRegister[KEY_UNREGISTER_OBJECT] = jDeviceID;
...
SecurityString security_message_factory::makeKeepaliveMessage(const char* deviceId) {
    ...
    jDeviceID[KEY_DEVICE_ID] = deviceId;
    jKeepalive[KEY_KEEPALIVE_OBJECT] = jDeviceID;
...
SecurityString security_message_factory::makePersonListMessage(const security_persons_t& persons) {
    try {
        nlohmann::json jPersonsObject;
        for (int i = 0; i < persons.size(); i++) {
            nlohmann::json j = persons[i];
            jPersonsObject[KEY_PERSON_LIST_OBJECT][KEY_PERSON_OBJECT].push_back(j);
        }
    
        SecurityString str = jPersonsObject.dump();
        // dlog_debug("%s\n", str.c_str());
        return str;
    } catch (exception e) {
        dlog_debug("%s", e.what());
...
SecurityString security_message_factory::makeFaceListMessage(const security_faces_t& faces) {
    try {
        nlohmann::json jFaceObject;
        for (int i = 0; i < faces.size(); i++) {
            nlohmann::json j = faces[i];
            jFaceObject[KEY_FACE_LIST_OBJECT][KEY_FACE_OBJECT].push_back(j);
        }
        SecurityString str = jFaceObject.dump();
        // dlog_debug("%s\n", str.c_str());
        return str;
    } catch (exception e) {
        dlog_debug("%s", e.what());
...
SecurityString security_message_factory::makeMotorVehicleListMessage(const security_motorvehicles_t& motorVehicles) {
    try {
        nlohmann::json jMotorVehicles;
        for (int i = 0; i < motorVehicles.size(); i++) {
            nlohmann::json j = motorVehicles[i];
            jMotorVehicles[KEY_MOTOR_VEHICLE_LIST_OBJECT][KEY_MOTOR_VEHICLE_OBJECT].push_back(j);
        }

        SecurityString str = jMotorVehicles.dump();
        // dlog_debug("%s", str.c_str());
        return str;
    } catch (exception e) {
        dlog_debug("%s", e.what());
    }
...
SecurityString security_message_factory::makeNonMotorVehicleListMessage(const security_nonmotorvehicles_t& nonMotorVehicles) {
    try {
        nlohmann::json jNonmotorVehicles;
        for (int i = 0; i < nonMotorVehicles.size(); i++) {
            nlohmann::json j = nonMotorVehicles[i];
            jNonmotorVehicles[KEY_NONMOTOR_VEHICLE_LIST_OBJECT][KEY_NONMOTOR_VEHICLE_OBJECT].push_back(j);
        }
        SecurityString str = jNonmotorVehicles.dump();
        // dlog_debug("%s\n", str.c_str());
        return str;
    } catch (exception e) {
        dlog_debug("%s", e.what());
...
int security_message_factory::parseResponseStatusList(const char* body, security_response_statuss_t& statuss) {
    try {
        ...
        json jResponseStatus;
        // 解析 ResponseStatusList 字段，各平台可能不一致
        if (j.contains("ResponseStatusListObject") || j["ResponseStatusListObject"].contains(("ResponseStatusObject"))) {
            jResponseStatus = j["ResponseStatusListObject"]["ResponseStatusObject"];
        } else if (j.contains("ResponseStatusList") || j["ResponseStatusList"].contains(("ResponseStatusObject"))) {
            jResponseStatus = j["ResponseStatusList"]["ResponseStatusObject"];
        } else {
            dlog_debug("ResponseStatusList parse faild\n");
            return -1;
        }

        if (!jResponseStatus.is_array()) {
            dlog_debug("response parse faild.\n");
            return -1;
        }
        for (int i = 0; i < jResponseStatus.size(); i++) {
            statuss.push_back(jResponseStatus[i].get<security_response_status_t>());
        }
        return 0;
    }
    catch (exception e) {
        dlog_debug("parseResponseStatusList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseResponseStatus(const char* body, security_response_status_t& status) {
    try {
        ...
        dlog_debug("parseResponseStatus error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseSystemTime(const char* body, security_system_time_t& time) {
    try {
        json j = json::parse(body);
        for (auto &it : vSystemTime) {
            if (!j.contains(it)) continue;
            time = j[it].get<security_system_time_t>();
        }
        return 0;
    }
    catch (exception e) {
        dlog_debug("parseSystemTime error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseApeList(const char* body, security_apes_t& apes) {
    try {
        ...
        dlog_debug("parseApeList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseApsList(const char* body, security_apss_t& apss) {
    try {
        ...
        dlog_debug("parseApsList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseTollgateList(const char* body, security_tollgates_t& tatollgates) {
    try {
        ...
        dlog_debug("parseTollgateList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseLaneList(const char* body, security_lanes_t& lanes) {
    try {
        ...
        dlog_debug("parseLaneList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseVideoSliceList(const char* body, security_videoslices_t& slices) {
    try {
        ...
        dlog_debug("parseVideoSliceList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseVideoSlice(const char* body, security_videoslice_t& slice) {
    try {
        ...
        dlog_debug("parseVideoSlice error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseVideoSliceInfo(const char* body, security_videoslice_info_t& info) {
    try {
        ...
        dlog_debug("parseVideoSliceInfo error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseFileList(const char* body, security_files_t& files) {
    try {
        ...
        dlog_debug("parseFileList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseFileInfo(const char* body, security_file_info_t& info) {
    try {
        ...
        dlog_debug("parseFileInfo error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parsePersonList(const char* body, security_persons_t& persons) {
    try {
        ...
        // 判断是否存在属性
        if (!j.contains(KEY_PERSON_LIST_OBJECT) || !j[KEY_PERSON_LIST_OBJECT].contains(KEY_PERSON_OBJECT)) {
            dlog_debug("parse faild\n");
            return -1;
        }

        json jPersons = j[KEY_PERSON_LIST_OBJECT][KEY_PERSON_OBJECT];
        // 判断是否是数组类型
        if (!jPersons.is_array()) {
            dlog_debug("format error\n");
            return -1;
        }
        for (int i = 0; i < jPersons.size(); i++) {
            persons.push_back(jPersons[i].get<security_person_t>());
        }
        return 0;
    }
    catch (exception e) {
        dlog_debug("parsePersonList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parsePerson(const char* body, security_person_t& person) {
    try {
        ...
        dlog_debug("parsePerson error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseThingList(const char* body, security_things_t& things) {
    try {
        ...
        dlog_debug("parseThingList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseSceneList(const char* body, security_scenes_t& scenes) {
    try {
        ...
        dlog_debug("parseSceneList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseImageList(const char* body, security_images_t& images) {
    try {
        ...
        dlog_debug("parseImageList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseImageInfo(const char* body, security_image_info_t& info) {
    try {
        ...
        dlog_debug("parseImageInfo error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseCaseList(const char* body, security_cases_t& cases) {
    try {
        ...
        dlog_debug("parseCaseList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseCaseInfo(const char* body, security_caseinfo_t& info) {
    try {
        ...
        dlog_debug("parseCaseInfo error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseDispositionList(const char* body, security_dispositions_t& dispositions) {
    try {
        ...
        dlog_debug("parseDispositionsList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseDisposition(const char* body, security_disposition_t& disposition) {
    try {
        ...
        dlog_debug("parseDisposition error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseDispositionNotificationList(const char* body, security_disposition_notifications_t& notifications) {
    try {
        ...
        dlog_debug("parseDispositionNotificationList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseDispositionNotification(const char* body, security_disposition_notification_t& notification) {
    try {
        ...
        dlog_debug("parseDispositionNotification error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseSubscribeList(const char* body, security_subscribes_t& subscribes) {
    try {
        ...
        dlog_debug("parseSubscribeList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseSubscribe(const char* body, security_subscribe_t& subscribe) {
    try {
        ...
        dlog_debug("parseSubscribe error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseSubscribeNotificationList(const char* body, security_subscribe_notifications_t& notifications) {
    try {
        ...
        dlog_debug("parseSubscribeNotificationList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseSubscribeNotification(const char* body, security_subscribe_notification_t& notification) {
    try {
        ...
        dlog_debug("parseSubscribeNotification error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseAanalysisRuleList(const char* body, security_analysis_rules_t& rules) {
    try {
        ...
        dlog_debug("parseAanalysisRuleList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseAanalysisRule(const char* body, security_analysis_rule_t& rule) {
    try {
        ...
        dlog_debug("parseAanalysisRule error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseVideoLabelList(const char* body, security_videolabels_t& labels) {
    try {
        ...
        dlog_debug("parseVideoLabelList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseVideoLabel(const char* body, security_videolabel_t& label) {
    try {
        ...
        dlog_debug("parseVideoLabel error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseFaceList(const char* body, security_faces_t& faces) {
    try {
        ...
        json j = json::parse(body, nullptr, false);
        // 判断是否存在属性
        if (!j.contains(KEY_FACE_LIST_OBJECT) || !j[KEY_FACE_LIST_OBJECT].contains(KEY_FACE_OBJECT)) {
            dlog_debug("parse faild\n");
            return -1;
        }

        json jFaces = j[KEY_FACE_LIST_OBJECT][KEY_FACE_OBJECT];
        // 判断是否是数组类型
        if (!jFaces.is_array()) {
            dlog_debug("format faild\n");
            return -1;
        }
        for (int i = 0; i < jFaces.size(); i++) {
            faces.emplace_back(jFaces[i].get<security_face_t>());
        }
        return 0;
    }
    catch (exception e) {
...
int security_message_factory::parseMotorVehicleList(const char* body, security_motorvehicles_t& motorvehicles) {
    try {
        ...
        json j = json::parse(body, nullptr, false);
        // 判断是否存在属性
        if (!j.contains(KEY_MOTOR_VEHICLE_LIST_OBJECT)
            || !j[KEY_MOTOR_VEHICLE_LIST_OBJECT].contains(KEY_MOTOR_VEHICLE_OBJECT)) {
                dlog_debug("parse faild");
                return -1;
        }

        json jMotorVehicles = j[KEY_MOTOR_VEHICLE_LIST_OBJECT][KEY_MOTOR_VEHICLE_OBJECT];
        // 判断是否是数组类型
        if (!jMotorVehicles.is_array()) {
            dlog_debug("format error");
            return -1;
        }
        for (int i = 0; i < jMotorVehicles.size(); i++) {
            motorvehicles.push_back(jMotorVehicles[i].get<security_motorvehicle_t>());
        }
        return 0;
    }
    catch (exception e) {
        dlog_debug("parseMotorVehicleList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseMotorVehicle(const char* body, security_motorvehicle_t& motorvehicle) {
    try {
        ...
        dlog_debug("parseMotorVehicle error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseNonMotorVehicleList(const char* body, security_nonmotorvehicles_t& nonmotorvehicles) {
    try {
        ...
        json j = json::parse(body, nullptr, false);
        // 判断是否存在属性
        if (!j.contains(KEY_NONMOTOR_VEHICLE_LIST_OBJECT)
            || !j[KEY_NONMOTOR_VEHICLE_LIST_OBJECT].contains(KEY_NONMOTOR_VEHICLE_OBJECT)) {
                dlog_debug("parse faild\n");
                return -1;
        }
        
        json jNonmotors = j[KEY_NONMOTOR_VEHICLE_LIST_OBJECT][KEY_NONMOTOR_VEHICLE_OBJECT];
        // 判断是否是数组类型
        if (!jNonmotors.is_array()) {
            dlog_debug("format error\n");
            return -1;
        }
        for (int i = 0; i < jNonmotors.size(); i++) {
            nonmotorvehicles.push_back(jNonmotors[i].get<security_nonmotorvehicle_t>());
        }
        return 0;
    }
    catch (exception e) {
        dlog_debug("parseNonMotorVehicleList error %s\n", e.what());
    }
    return -1;
}
...
int security_message_factory::parseNonMotorVehicle(const char* body, security_nonmotorvehicle_t& nonmotorvehicle) {
    try {
        ...
        dlog_debug("parseNonMotorVehicle error %s\n", e.what());
    }
    return -1;
}
