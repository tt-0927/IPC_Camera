...
int security_viid::sys_register(const char* ip, int port, const char* deviceId,
                                const char* username, const char* password, bool bKeepAlive) {
    int ret = checkAddress(ip, port);
    if (ret != 0) {
        dlog_debug("address incorrect");
        return ret;
    }
    if (!deviceId || strlen(deviceId) < 1) {
        dlog_debug("device ID invalid");
        return -1;
    }
...
#endif
    if (auto res = m_Client->Post(SECURITY_URL_PATH_VIID_REGISTER, headers, msg, CONTENT_TYPE)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("register failed %d", res->status);
            return -1;
        }
        ...
        if (0 != ret || SECURITY_STATUS_OK != status.StatusCode) {
            dlog_debug("parseResponseStatus failed %d, %s", status.StatusCode, status.StatusString.c_str());
            return -1;
        }
        this->ip = ip;
        this->port = port;
        ...
        auto err = res.error();
        dlog_debug("register failed %d", err);
        return -1;
    }
    dlog_debug("register success");
    return 0;
}
...
int security_viid::sys_unregister() {
    int ret;
    std::unique_lock<std::mutex> l(m_client_mutex);
    if (!registered || m_Client == nullptr) {
        return 0;
    }
    ...
    Headers headers = get_common_req_header(deviceId);
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
    m_Client->set_digest_auth(this->username.c_str(), this->password.c_str());
#endif
    if (auto res = m_Client->Post(SECURITY_URL_PATH_VIID_UNREGISTER, headers, msg, CONTENT_TYPE)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("unregister failed %d", res->status);
            return -1;
        }
        security_response_status_t status;
        ret = msgFactory.parseResponseStatus(res->body.c_str(), status);
        ...
        auto err = res.error();
        dlog_debug("unregister failed %d", err);
        return -1;
    }
	dlog_debug("unregister success");
	return 0;
}
...
int security_viid::sys_keepalive() {
    ...
    if (auto res = m_Client->Post(SECURITY_URL_PATH_VIID_KEEPALIVE, headers, msg, CONTENT_TYPE)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("keepalive failed %d", res->status);
            return -1;
        }
        security_response_status_t status;
        ret = msgFactory.parseResponseStatus(res->body.c_str(), status);
        ...
        auto err = res.error();
        dlog_debug("keepalive failed %d", err);
        return -1;
    }
    dlog_debug("keepalive success");
    return 0;
}
...
int security_viid::sys_synctime(security_system_time_t& time) {
    ...
    Headers headers = get_common_req_header(this->deviceId);
    if (auto res = m_Client->Get(SECURITY_URL_PATH_VIID_TIME, headers)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("synchorize system time failed %d", res->status);
            return -1;
        }
        ret = msgFactory.parseSystemTime(res->body.c_str(), time);
        if (0 != ret) {
            dlog_debug("parseSystemTime failed");
            return -1;
        }
    }
    else {
        auto err = res.error();
        dlog_debug("synchorize system time failed %d", err);
        return -1;
    }
    dlog_debug("synchorize system time success");
    return 0;
}
...
int security_viid::add_motorvehicles(const security_motorvehicles_t& motorvehicles) {
    ...
    return add_list(msg, SECURITY_URL_PATH_VIID_MOTORVEHIS);
}
...
int security_viid::add_list(const string& msg, const char* viidUri) {
    ...
    if (msg.empty()) {
        dlog_debug("msg invalid");
        return -1;
    }

    Headers headers = get_common_req_header(this->deviceId);
    if (auto res = m_Client->Post(viidUri, headers, msg, CONTENT_TYPE)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("add %s failed %d", viidUri, res->status);
            return -1;
        }
        security_response_statuss_t statuss;
        ret = msgFactory.parseResponseStatusList(res->body.c_str(), statuss);
        if (0 != ret) {
            dlog_debug("parseResponseStatusList failed");
            return -1;
        }
    }
    else {
        auto err = res.error();
        dlog_debug("add %s failed %d", viidUri, err);
        return -1;
    }
    dlog_debug("add %s success", viidUri);
    return 0;
}
...
int security_viid::add_object(const char* id, const string& msg, const char* viidUri, const char* otherUri) {
    ...
    Headers headers = get_common_req_header(this->deviceId);
    if (auto res = m_Client->Post(url.c_str(), headers, msg, CONTENT_TYPE)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("add %s failed %d", url.c_str(), res->status);
            return -1;
        }
        if (otherUri == NULL) {
            security_response_statuss_t statuss;
            ...
            ret = msgFactory.parseResponseStatus(res->body.c_str(), status);
            if (0 != ret) {
                dlog_debug("parseResponseStatus failed");
                return -1;
            }
        }
    }
        ...
        auto err = res.error();
        dlog_debug("add %s failed %d", url.c_str(), err);
        return -1;
    }
    dlog_debug("add %s success", url.c_str());
    return 0;
}
...
int security_viid::query_list(const SecurityParams& params, string& body, const char* viidUri) {
    ...
    Headers headers = get_common_req_header(this->deviceId);
    if (auto res = m_Client->Get(url.c_str(), headers)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("get %s failed %d", url.c_str(), res->status);
            return -1;
        }
        body = res->body;
    }
        ...
        auto err = res.error();
        dlog_debug("get %s failed %d", url.c_str(), err);
        return -1;
    }
    dlog_debug("get %s success", url.c_str());
    return 0;
}
...
int security_viid::update_list(const string& msg, const char* viidUri) {
    ...
    Headers headers = get_common_req_header(this->deviceId);
    if (auto res = m_Client->Put(viidUri, headers, msg, CONTENT_TYPE)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("update %s failed %d", viidUri, res->status);
            return -1;
        }
        security_response_statuss_t statuss;
        ret = msgFactory.parseResponseStatusList(res->body.c_str(), statuss);
        ...
        auto err = res.error();
        dlog_debug("update %s failed %d", viidUri, err);
        return -1;
    }
    dlog_debug("update %s success", viidUri);
    return 0;
}
...
int security_viid::update_object(const char* id, const string& msg, const char* viidUri, const char* otherUri) {
    ...
    Headers headers = get_common_req_header(this->deviceId);
    if (auto res = m_Client->Put(url.c_str(), headers, msg, CONTENT_TYPE)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("update %s failed %d", url.c_str(), res->status);
            return -1;
        }
        if (otherUri == NULL) {
            security_response_statuss_t statuss;
        ...
        auto err = res.error();
        dlog_debug("update %s failed %d", url.c_str(), err);
        return -1;
    }
    dlog_debug("update %s success", url.c_str());
    return 0;
}
...
int security_viid::delete_list(const security_idlist_t& ids, const char* viidUri) {
    ...
    Headers headers = get_common_req_header(this->deviceId);
    if (auto res = m_Client->Delete(url.c_str(), headers)) {
        dlog_debug("response: %s, %d", res->body.c_str(), res->status);
        if (SECURITY_REST_HTTP_RESPONSE_200 != res->status) {
            dlog_debug("delete %s failed %d", url.c_str(), res->status);
            return -1;
        }
        security_response_statuss_t statuss;
        ret = msgFactory.parseResponseStatusList(res->body.c_str(), statuss);
        if (0 != ret) {
            dlog_debug("parseResponseStatuss failed");
            return -1;
        }
    }
    else {
        auto err = res.error();
        dlog_debug("delete %s failed %d", url.c_str(), err);
        return -1;
    }
    dlog_debug("delete %s success", url.c_str());
    return 0;
}
