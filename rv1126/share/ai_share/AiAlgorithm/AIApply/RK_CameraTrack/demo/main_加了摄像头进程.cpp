/*
* File Name      : vo_test.c
* Created on     : 2023 年 3 月 13 日
* Author         : cds
* Mail           : 
* description    : 解码并输出
* Modify date    : 2023 年 3 月 13 日
* Modifier Author: cds
* description    :
*/
#include <chrono>
...
/* 初始化 */
Strategy_CamControl_t* cam  = init_udp_cam(ip, nControlPort);
/* ==================== IP和端口初始化 ========================*/

/* ===================================================== AI需要的全局变量配置 ============================================================= */
SLIDETRACKPARAM Pslidetrack;
AUXILLARY_TRACK auxillary_track(Pslidetrack);
cv::Mat aFrame(1024,1920,CV_8UC3);
std::vector<float> vRtPoints;
...
static int vdec_init(int nVdecChnId)
{
    ...
    g_pVideoHandle->stExParam.bEnableColmv = RK_FALSE;
    g_pVideoHandle->stExParam.eDisPlayMode = VIDEO_DISPLAY_MODE_PREVIEW;
...
void cameraMove()
{
	while (true)
	{
		...
		if(vRtPoints.size()>0)
		{
			...
			float ntotal_cx = (vRtPoints[0] * 1.0 + vRtPoints[2] * 1.0) / 2;
			float ntotal_cy = (vRtPoints[1] * 1.0 + vRtPoints[3] * 1.0) / 2;
			float ntotal_cw = vRtPoints[2] * 1.0 - vRtPoints[0] * 1.0;
			float ntotal_ch = vRtPoints[3] * 1.0 + vRtPoints[1] * 1.0;
			
			float x_size = 100;
			float y_size = 100;
			float nXspeed = 24*abs(ntotal_cx-1920/2)/1920;
			float nXsleep = 300*abs(ntotal_cx-1920/2)/1920*(1+abs(ntotal_cx-1920/2)/1920);
			float nYspeed = 24;
			float nYsleep = 100*abs(ntotal_cy-1024/2)/1024;
			
			/* 左右移动 */
			if (ntotal_cx - 1920 / 2 >= x_size)
			{
				// 向右移动一段时间然后停止
				udp_move_cmd(MOVE_RIGHT, nXspeed, cam);
				usleep(nXsleep*1000);
				udp_move_cmd(STOP_TURN, 20, cam); 
			}
			else if (1920 / 2 - ntotal_cx >= x_size)
			{
				// 向左移动一段时间然后停止
				udp_move_cmd(MOVE_LEFT, nXspeed, cam); 
				usleep(nXsleep*1000);
				udp_move_cmd(STOP_TURN, 20, cam); 
			}
			else
			...
			/* 上下移动 */
			if (ntotal_cy - 1024 / 2 >= y_size)
			{
				// 向上移动一段时间然后停止
				udp_move_cmd(MOVE_DOWN, nYspeed, cam); 
				usleep(nYsleep*1000);
				udp_move_cmd(STOP_TURN, 20, cam); 
			}
			else if (1024 / 2 - ntotal_cy >= y_size)
			{
				udp_move_cmd(MOVE_UP, nYspeed, cam);
				usleep(nYsleep*1000);
				udp_move_cmd(STOP_TURN, 20, cam); 
			}
			else
			...
			/* 聚焦 */
			if (ntotal_cw<1920*0.07)
			{
				udp_move_cmd(MOVE_FAR, 8, cam); // 放大
				usleep(50*1000);
				udp_move_cmd(STOP_ZOOM, 20, cam);
				
			}
			else if (ntotal_cw>1920*0.1)
			{
				udp_move_cmd(MOVE_NEAR, 8, cam); // 缩小
				usleep(50*1000);
				udp_move_cmd(STOP_ZOOM, 20, cam);
			}
			else
			{
				udp_move_cmd(STOP_ZOOM, 20, cam); // udp发送信号
			}
			
			// 结束计时
		  	auto end = std::chrono::high_resolution_clock::now();
		    	std::chrono::duration<double, std::milli> duration = end - start;
		    	std::cout << "摄像头控制一次的时间: " << duration.count() << " ms" << std::endl;
		}
	}	
}
...
void AI()
{
	while (true)
	{
		...
		{
            		vRtPoints.clear();
			auxillary_track.AI((char*)aFrame.data,vRtPoints);
            		if(vRtPoints.size()>0)
			    std::cout<<"输出结果11:"<< vRtPoints[0] <<";"<<vRtPoints[1]<<";"<<vRtPoints[2]<<";"<<vRtPoints[3]<<std::endl;
		}
		// 结束计时
	  	auto end = std::chrono::high_resolution_clock::now();
	    	std::chrono::duration<double, std::milli> duration = end - start;
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    while (!g_bExit)
    {
		...
		memcpy(aFrame.data, pFrame->pData, pFrame->nSize);
        cv::Mat showImg = aFrame.clone();
		/* 用于显示的图片 */
        if(vRtPoints.size()!=0)
		cv::rectangle(showImg, cv::Point(int(vRtPoints[0]),int(vRtPoints[1])), cv::Point(int(vRtPoints[2]),int(vRtPoints[3])), cv::Scalar(0, 0, 255), 2);
		
		/* 绘制分析的区域 */
            	memcpy(stFrameInfo.pData,(void *)showImg.data,pFrame->nSize);
...
int main(int argc, char** argv)
{
    ...
    /* 创建一个摄像头移动线程 */
    std::thread Camera_Demo(cameraMove);
