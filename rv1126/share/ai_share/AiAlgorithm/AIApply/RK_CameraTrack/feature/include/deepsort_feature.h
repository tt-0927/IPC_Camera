...
*  Author: wcp
*  description : 人体特征512关键点提取类接口定义
*       1、通过类.DetectFaceRgb，可以加载大小为64x128x3的cv::Mat格式的图片或者视频流
*          - 返回flaot类型的512特征点vector容器
	2、识别一张160x160图片大概需要1.5ms
*  Modify date: 2023年7月21日
*/

#ifndef __RK_DEEPSORT_FEATURE_H__
#define __RK_DEEPSORT_FEATURE_H__
...
class RK_DEEPSORT_FEATURE
{
	...
	public:
		...
		/******************************** 外部调用函数 ******************************/
		/**
		  * 输入bgr格式的opencv格式图片数据进行识别：
		  * aInputImg：输入的cv::Mat 格式的图片bgr格式数据
		  * vPoints： 是人头检测中，传出来的显示框
		  * vFeatures： 特征提取后的512关键点
		  * return 成功-> 0， 失败-> 1
		* */
		bool GetFeatures(cv::Mat aInputImg, std::vector<float> vPoints, float* vFeatures);
		...
		* */
		    RK_DEEPSORT_FEATURE();
		    RK_DEEPSORT_FEATURE(char* pModelPath);
...
#endif // __RK_DEEPSORT_FEATURE_H__
