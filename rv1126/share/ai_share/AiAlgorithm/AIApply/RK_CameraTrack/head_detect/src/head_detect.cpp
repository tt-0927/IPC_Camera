...
HEAD_DETECT::HEAD_DETECT(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *HEAD_DETECT::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
int HEAD_DETECT::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
int HEAD_DETECT::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int HEAD_DETECT::RknnDetectDestory()
{
...
/* bgr格式视频流的识别 */
int HEAD_DETECT::DetectFaceRgb(cv::Mat aInputImg,std::vector<float> &vPoints)
{
    if (!aInputImg.empty())
    {
    	/* 预处理 */
    	int roiw = aInputImg.cols;
       int roih = aInputImg.rows;
	...
	cv::resize(aInputImg, aSimgResize, cv::Size(int(roiw/fMax),int(roih/fMax)));
	cv::Rect roi(0, 0, aSimgResize.cols, aSimgResize.rows);
       ...
       aSimgResize.copyTo(aSimgPart(roi));
        ...
        aInputs[0].buf = (void*) aSimgPart.data;
        ...
        // std::cout << "head模型推理一次使用的时间:" << duration.count() << "毫秒--"<< std::endl;
       
        // 后处理
        head_post_process((float *)aOutputs[0].buf, (float *)aOutputs[1].buf, (float *)aOutputs[2].buf, nModelWidth, nModelHeight,
