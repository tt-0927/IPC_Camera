...
// 人脸检测
RK_FACES_DETECT faceDetect("./weights/MB_FaceDetector1920x1024.rknn");
// 人脸特征提取
RK_FACE_FEATURE featureNet("./weights/MB_facenet.rknn");
// 人脸识别辅助类
RETINAFACE_FACENET retinafaceFacenet;
/* 人脸库提取的数据 */
std::vector<PEOPLEFEATURES> vAllFeature;
// 存放人脸特征位置
std::vector<int> vTarget;
/* 人脸相识度 */
float fFacial_recognition =0.75;
/* ===================================================== AI需要的全局变量配置 ============================================================= */
...
{
    // 文件夹路径
    char* folderPath = "/sda/RK_test/img";
    DIR *dir;
    struct dirent *entry;
    ...
    while ((entry = readdir(dir)) != NULL) 
    {
            ...
            len >= 4 && strcmp(entry->d_name + len - 4, ".png") == 0)
        {
            ...
            retinafaceFacenet.RetinafaceFacenetBgr((char*)image.data, faceDetect, featureNet ,vOneFeature);
            // 将新的人脸数据插入总库
    	    vAllFeature.insert(vAllFeature.end(), vOneFeature.begin(), vOneFeature.end());
            std::cout << "=^^^^^^^^^^^^^^^^^^^^^^^^" <<vAllFeature.size()<<" ^^^^^^^^^^^^^^^^^^^^^^^\n"<<imagePath<<std::endl;
        }
    }
    // 关闭目录
    closedir(dir);
...
void AI()
{
	while (true)
	{
		...
		/* AI算法 */
		std::vector<PEOPLEFEATURES> vOneFeature;
		retinafaceFacenet.RetinafaceFacenetBgr((char*)aSrcImg.data, faceDetect, featureNet ,vOneFeature);
		std::cout << "=^^^^^^^^^^^^^^^^^^^^^^^^" <<vAllFeature.size()<<" ^^^^^^^^^^^^^^^^^^^^^^^\n"<<std::endl;
		
		/* 识别到的人脸与人脸数据库做余弦相似度，取最大值 */
		vTarget.clear();
		for(int i=0;i<vAllFeature.size();i++)
		{
			vTarget.push_back(-1);
			float fS = -1;
			for(int j=0;j<vOneFeature.size();j++)
			{
				float Similarity = retinafaceFacenet.CosineSimilarity(vAllFeature[i].fFeatures,vOneFeature[j].fFeatures,128);
				std::cout << "=================" <<Similarity<<" ========================\n"<<std::endl;
				if(Similarity>fS)
				{
					fS = Similarity;
			...
			else
			{
				...
				{
					vAllFeature[i].fBoxs[fi] = vOneFeature[vTarget[i]].fBoxs[fi];
				}
			}
		}
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    while (!g_bExit)
    {
		...
		{
			if(vTarget[i]==-1) continue;
			cv::rectangle(imgShow, cv::Point(vAllFeature[i].fBoxs[0],vAllFeature[i].fBoxs[1]), cv::Point(vAllFeature[i].fBoxs[2],vAllFeature[i].fBoxs[3]), classColor[i], 3);
