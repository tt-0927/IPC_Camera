...
/* 显示的相关配置 */
std::vector<char*> classes = {"head" , "head", "head", "head", "js", "zl", "xt", "jz"};
...
// 行人检测
RK_HEAD_SORT demo("./weights/HeadDetect.rknn");
Sort track1(1);
float fConfidence=0.7; // 置信度
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    while (!g_bExit)
    {
		...
		for (int i = 0; i < vPoints.size()/6; i++)
		{
		    ...
		    char conf[10];
		    int ci = 0;
		    if (vPoints[6*i+5] < -0.5)
		        ci = 2;
		    else if(vPoints[6*i+5] > 0.5)
		        ci = 6;
		    // sprintf(conf, "%.1f %s", vPoints[6*i+4], classes[int(vPoints[6*i+5])]);
		    sprintf(conf, "%.0f %.1f", vPoints[6*i+4], vPoints[6*i+5]);
                   rectangle(imgShow, cv::Point(x1, y1), cv::Point(x2, y2), classColor[ci], 2);
                   putText(imgShow, conf, cv::Point(x1, y1 - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, classColor[ci],2);
		}
...
void AI()
{
	while (true)
	{
		...
		demo.HeadSortDetectFaceRgb(aSimgPart,vAIPoints);
		...
		{
            DetectBox detbox;
            detbox.x1 = vAIPoints[6*i]*fMax;
		    detbox.y1 = vAIPoints[6*i+1]*fMax;
		    detbox.x2 = vAIPoints[6*i+2]*fMax;
		    detbox.y2 = vAIPoints[6*i+3]*fMax;
            detbox.confidence = vAIPoints[6*i+4];
            detbox.classID = vAIPoints[6*i+5];
        ...
        // printf("")
        track1.track_process(detboxes);
        printf("track_ids: ");
        // for (auto i : track1.frameTrackingResult)
        for (int i = 0; i < track1.frameTrackingResult.size(); i++)
		{
			// printf("%.1f ", track1.frameTrackingResult[i].trackID);
            vAIPoints[6*i] = track1.frameTrackingResult[i].x1;
		    vAIPoints[6*i+1] = track1.frameTrackingResult[i].y1;
		    vAIPoints[6*i+2] = track1.frameTrackingResult[i].x2;
		    vAIPoints[6*i+3] = track1.frameTrackingResult[i].y2;
		    vAIPoints[6*i+4] = track1.frameTrackingResult[i].trackID;
		    vAIPoints[6*i+5] = track1.frameTrackingResult[i].classID;

		}
        printf("\n");
		auto t2 = std::chrono::high_resolution_clock::now();

        std::chrono::duration<double, std::milli> track_time = t2 - t1;
        std::cout << "TrackTime: " << track_time.count() << " ms" << std::endl;
        // /* 将目标缩放回原始尺寸构建追踪数据结构 */
		// for (int i = 0; i < vAIPoints.size()/6; i++)
		// {
...
int main(int argc, char** argv)
{
    ...
    /*创建视频队列*/
    nRet = OS_queCreate(&g_MediaQue, 5);
    if (nRet != OS_SOK)
    {
        printf("创建队列失败\n");
    }

    nRet = OS_queCreate(&g_videoQue, 5);
    if (nRet != OS_SOK)
    {
        printf("创建队列失败\n");
    }
