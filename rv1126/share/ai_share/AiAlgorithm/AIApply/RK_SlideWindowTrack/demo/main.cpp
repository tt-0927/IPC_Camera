...
void AI()
{
	while (true)
	{
		...
		if(!aFrame.empty())
		{
			auxillary_track.AI((char*)aFrame.data,vAIPoints);
		}
		// 结束计时
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    while (!g_bExit)
    {
		...
		/* 用于显示的图片 */
		float fHead_cx,fHead_cy,fWin_cw,fWin_ch;
		auxillary_track.GetDatas(fHead_cx,fHead_cy,fWin_cw,fWin_ch);
		cv::rectangle(aFrame, cv::Point(int(fHead_cx-fWin_cw/2), int(fHead_cy-fWin_ch/2)), cv::Point(int(fHead_cx+fWin_cw/2), int(fHead_cy+fWin_ch/2)), cv::Scalar(0, 0, 255), 2);
		
		/* 绘制分析的区域 */
            	memcpy(stFrameInfo.pData,(void *)aFrame.data,pFrame->nSize);
