...
#ifndef __RK_HUMAN_DETECT_KPT_H__
#define __RK_HUMAN_DETECT_KPT_H__
...
class RK_HUMAN_DETECT_KPT
{
	private:
	    ...
	    rknn_tensor_attr stOutputAttrs[4];
	    rknn_input aInputs[1];
	    rknn_output aOutputs[4];
	    /* 模型需要数据的宽高和通道  */
	    int nModelChannel;
	    int nModelWidth;
	    int nModelHeight;
	...
	public:
		...
		/**
		  ...
		  * return 成功-> 0， 失败-> 1
		* */
		int DetectKptRgb(cv::Mat aInputImg,std::vector<float> &vPoints);
		...
		* */
		    RK_HUMAN_DETECT_KPT();
		    RK_HUMAN_DETECT_KPT(char* pModelPath);
