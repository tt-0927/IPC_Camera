...
/* 显示的相关配置 */
std::vector<char*> classes = {"person" , "head", "head", "head", "js", "zl", "xt", "jz"};
...
// 行人检测
RK_HUMAN_DETECT_KPT demo("./weights/yolov5kpt.rknn");
// RK_FACE_FEATURE demo("./weights/student_behavior.rknn");

float fConfidence=0.45; // 置信度
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    while (!g_bExit)
    {
		...
		int CoutNum=0;
		for (int i = 0; i < vPoints.size()/57; i++)
		{
		    //if(vPoints[6*i+5] !=1 )continue;
		    CoutNum++;
		    int x1 = vPoints[57*i];
		    int y1 = vPoints[57*i+1];
		    int x2 = vPoints[57*i+2];
		    int y2 = vPoints[57*i+3];
		    char conf[10];
		    sprintf(conf, "%.3f %s", vPoints[57*i+4], classes[int(vPoints[57*i+5])]);
                   rectangle(imgShow, cv::Point(x1, y1), cv::Point(x2, y2), classColor[int(vPoints[57*i+5])], 2);
                   putText(imgShow, conf, cv::Point(x1, y1 - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, classColor[int(vPoints[57*i+5])],2);
            for (int j = 0; j < 17; j++){
                int xj = vPoints[57*i + 6 + j * 3];
                int yj = vPoints[57*i + 6 + j * 3 + 1];
                if (vPoints[57*i + 6 + j * 3 + 2] > 0.7)
                    cv::circle(imgShow, cv::Point(xj, yj), 5, cv::Scalar(255, 0, 0), -1);
            }
		}
...
void AI()
{
	while (true)
	{
		...
		demo.DetectKptRgb(aSimgPart,vAIPoints);
		
		/* 将目标缩放回原始尺寸 */
		for (int i = 0; i < vAIPoints.size()/57; i++)
		{
		    vAIPoints[57*i] = vAIPoints[57*i]*fMax;
		    vAIPoints[57*i+1] = vAIPoints[57*i+1]*fMax;
		    vAIPoints[57*i+2] = vAIPoints[57*i+2]*fMax;
		    vAIPoints[57*i+3] = vAIPoints[57*i+3]*fMax;
            for (int j = 0; j < 17; ++j){
                vAIPoints[57*i+6 + j * 3] =  vAIPoints[57*i + 6 + j *3] * fMax;
                vAIPoints[57*i+6 + j * 3 +1] =  vAIPoints[57*i + 6 + j * 3 + 1] * fMax;
            }
		}
		/* 识别到任务，更新结果容器 */
		if(vAIPoints.size()/57 != 0)
		{
			vPoints = vAIPoints;
		}
		// 结束计时
