...
// 人头检测
RK_FACES_DETECT demo("./weights/MB_FaceDetector1920x1024.rknn");
...
void AI()
{
	while (true)
	{
		...
		/* AI算法 */
		std::vector<int> vAIPoints;
		demo.DetectFaceBgr((char*)aSrcImg.data,vAIPoints);
		/* 识别到任务，更新结果容器 */
		if(vAIPoints.size()/14 != 0)
		{
			vPoints = vAIPoints;
		}
		// 结束计时
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    while (!g_bExit)
    {
		...
		int CoutNum=0;
		for (int i = 0; i < vPoints.size()/14; i++)
		{
			//if(vPoints[6*i+5] !=1 )continue;
			CoutNum++;
			cv::rectangle(imgShow, cv::Point(vPoints[i*14+0],vPoints[i*14+1]), cv::Point(vPoints[i*14+2],vPoints[i*14+3]), cv::Scalar(255, 0, 0), 3);
			cv::circle(imgShow, cv::Point(vPoints[i*14+4],vPoints[i*14+5]), 2, cv::Scalar(0, 255, 0), -1);
			cv::circle(imgShow, cv::Point(vPoints[i*14+6],vPoints[i*14+7]), 2, cv::Scalar(0, 255, 0), -1);
			cv::circle(imgShow, cv::Point(vPoints[i*14+8],vPoints[i*14+9]), 2, cv::Scalar(0, 255, 0), -1);
			cv::circle(imgShow, cv::Point(vPoints[i*14+10],vPoints[i*14+11]), 2, cv::Scalar(0, 255, 0), -1);
			cv::circle(imgShow, cv::Point(vPoints[i*14+12],vPoints[i*14+13]), 2, cv::Scalar(0, 255, 0), -1);
		}
