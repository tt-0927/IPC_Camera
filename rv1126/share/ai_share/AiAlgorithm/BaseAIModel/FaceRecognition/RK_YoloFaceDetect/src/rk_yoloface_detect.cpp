...
*  Modify date: 2024年3月21日
*/
 
#include <chrono>
#include "rk_yoloface_detect.h"

using namespace YoloFaceDetect_NS;
/* 构造函数 -- 初始化变量 */
CRkYoloFaceDetect::CRkYoloFaceDetect()
{
    /* 模型的位置 */
    m_pModelName = "./weights/YoloFace.rknn";
    /* 输入视频流的大小和通道 */
    nImgWidth = 640;
    nImgHeight = 640;
    nImgChannel = 3;
...
CRkYoloFaceDetect::CRkYoloFaceDetect(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *CRkYoloFaceDetect::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
/* 模型初始化 */
int CRkYoloFaceDetect::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
/* 模型相关的参数查询与设置 */
int CRkYoloFaceDetect::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int CRkYoloFaceDetect::RknnDetectDestory()
{
...
/* 人脸马赛克功能-均值滤波实现马赛克效果 */
void CRkYoloFaceDetect::MosaicFace(cv::Mat& aFace, int nX1, int nY1, int nX2, int nY2, int nMosaicSize)
{
    /* 边界检查 */
    if (nX1 < 0)
        nX1 = 0;
    ...
    /* 提取人脸区域ROI */
    cv::Mat faceROI = aFace(roi); 
    if ((nX2 - nX1) < nMosaicSize) {
        nMosaicSize = nX2 - nX1;
    }
    if ((nY2 - nY1) < nMosaicSize) {
        nMosaicSize = nY2 - nY1;
    }
    /* 对人脸区域使用均值滤波实现马赛克效果 */
    cv::blur(faceROI, faceROI, cv::Size(nMosaicSize, nMosaicSize));

    /* 将处理后的人脸区域ROI放回原图像中 */
    faceROI.copyTo(aFace(roi));
}

/* rgb格式视频流的识别 */
int CRkYoloFaceDetect::DetecFaceRgb(cv::Mat aInputImg, std::vector<float> &vPoints)
{
    if (!aInputImg.empty())
    {
        /* 计算函数使用的时间 */
        ...
        std::cout << "人脸检测模型推理一次使用的时间：" << duration.count() << "毫秒--"<< std::endl;
        ...
        std::cout << "人脸检测后处理推理一次使用的时间：" << duration_.count() << "毫秒--"<< std::endl;
