...
RK_FIRE_DETECT::RK_FIRE_DETECT(char* pModelPath)
{
    /* 模型的位置 */
...
/* 载入模型地址， 读取模型数据 */
unsigned char *RK_FIRE_DETECT::LoadModel(const char *filename, int *model_size)
{
    FILE *fp;
...
int RK_FIRE_DETECT::RknnDetectInit()
{
    cout << "Loading mode...\n";
    m_nModelDataSize = 0;
    /* 载入模型，并转为二进制格式 */
...
int RK_FIRE_DETECT::RknnDetectQueryInoutIo()
{
    /* 获取输入tensor的属性信息 */
...
/* 释放模型定义的相关变量内存 */
int RK_FIRE_DETECT::RknnDetectDestory()
{
...
/* bgr格式视频流的识别 */
int RK_FIRE_DETECT::DetectFireRgb(cv::Mat aInputImg,std::vector<float> &vPoints)
{
    if (!aInputImg.empty())
    {
        /* 计算函数使用的时间 */
