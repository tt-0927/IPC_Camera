...
// 行人检测
cRkShipDetect demo("./weights/ShipDetect.rknn");
float fConfidence=0.35; // 置信度
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    while (!g_bExit)
    {
		...
		for (int i = 0; i < vPoints.size()/8; i++)
		{
		    ...
		    CoutNum++;
		    int x1 = vPoints[8*i];
		    int y1 = vPoints[8*i+1];
		    int x2 = vPoints[8*i+2];
		    int y2 = vPoints[8*i+3];
            int pos_id = vPoints[8*i+4];
            std::string text = "ID-" + std::to_string(pos_id);
		    // char conf[10];
		    // sprintf(conf, "%.3f %s", vPoints[6*i+4]);
            rectangle(imgShow, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(0, 255, 0), 2);
            putText(imgShow, text, cv::Point(x1, y1 - 5), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0),2);
		}
		// std::cout<<"船只数："<<CoutNum<<std::endl;
...
void AI()
{
	while (true)
	{
        ...
        // std::cout << "w_s: " << w_s << " h_s: " << h_s << " fMax: " << fMax << " roiw: " << roiw << " roih: " << roih << std::endl;
	       
		cv::Mat aSimgResize;
		cv::Mat aSimgPart(640, 640, CV_8UC3, cv::Scalar(0, 0, 0));
		...
		/* 将目标缩放回原始尺寸 */
		for (int i = 0; i < vAIPoints.size()/8; i++)
		{
		    vAIPoints[8*i] = vAIPoints[8*i]*fMax;
		    vAIPoints[8*i+1] = vAIPoints[8*i+1]*fMax;
		    vAIPoints[8*i+2] = vAIPoints[8*i+2]*fMax;
		    vAIPoints[8*i+3] = vAIPoints[8*i+3]*fMax;
		}
		/* 识别到任务，更新结果容器 */
		vPoints = vAIPoints;
        ...
        std::cout << "ShipTemp中的船只数量为: " << ShipTemp.size() << endl;
        Shiptrack = ShipTemp;
        std::cout << "Shiptrack中的船只数量为: " << Shiptrack.size() << endl;
