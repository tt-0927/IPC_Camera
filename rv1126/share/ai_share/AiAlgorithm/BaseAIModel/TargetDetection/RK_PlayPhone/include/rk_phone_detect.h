...
*  Author: wcp
*  description : 输入一张cv::Mat图片，即可返回flaot类型位置信息vector容器（6个位一组）
*  Modify date: 2023年11月14日
*/

#ifndef _RKNN_PHONE_DETECT_H_
#define _RKNN_PHONE_DETECT_H_
...
class RK_PHONE_DETECT
{
	...
	public:
		...
		/**
		  ...
		  * return 成功-> 0， 失败-> 1
		* */
		int DetectPhoneRgb(cv::Mat aInputImg,std::vector<float> &vPoints);
		...
		* */
		    RK_PHONE_DETECT();
		    RK_PHONE_DETECT(char* pModelPath);
