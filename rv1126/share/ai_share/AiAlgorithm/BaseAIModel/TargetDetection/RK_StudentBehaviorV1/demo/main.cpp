...
// 行人检测
RK_STUDENT_BEHAVIOR_V1 demo("./weights/student_behavior.rknn");
    ...
    while (!g_bExit)
    {
		...
		for (int i = 0; i < vPoints.size()/6; i++)
		{
		    ...
		    sprintf(conf, "%.3f %s", vPoints[6*i+4], classes[int(vPoints[6*i+5])]);
                   rectangle(aSrcImg, cv::Point(x1, y1), cv::Point(x2, y2), classColor[int(vPoints[6*i+5])], 2);
                   putText(aSrcImg, conf, cv::Point(x1, y1 - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, classColor[int(vPoints[6*i+5])],2);
		}
...
void AI()
{
	while (true)
	{
		...
		demo.DetectFaceRgb(aSimgPart,vAIPoints);
