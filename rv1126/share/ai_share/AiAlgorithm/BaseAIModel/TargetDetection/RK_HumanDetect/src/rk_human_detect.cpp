...
/* bgr格式视频流的识别 */
int RK_FACE_FEATURE::DetectHumanRgb(cv::Mat aInputImg,std::vector<float> &vPoints)
{
    if (!aInputImg.empty())
    {
        /* 计算函数使用的时间 */
        ...
        // std::cout << "人脸特征点分析模型推理一次使用的时间：" << duration.count() << "毫秒--"<< std::endl;
