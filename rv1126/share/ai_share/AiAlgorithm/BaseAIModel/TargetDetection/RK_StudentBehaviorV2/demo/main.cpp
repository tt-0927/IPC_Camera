...
/* 显示的相关配置 */
// std::vector<char*> classes = {"dx" , "dk", "tt", "zt", "js", "zl", "xt", "jz"};
std::vector<char*> classes = {"dt" , "tt", "zt", "js", "js", "zl", "xt", "jz"};
...
// 行人检测
RK_STUDENT_BEHAVIOR_V2 demo("./weights/stuface_best.rknn");
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    while (!g_bExit)
    {
	 naddr = 0;
        ...
        // std::vector<int> cnt {0, 0, 0, 0, 0, 0, 0, 0};
		for (int i = 0; i < vPoints.size()/6; i++)
		{
		    //if(vPoints[6*i+5] !=1 )continue;
		    CoutNum++;
            int cid = int(vPoints[6*i+5]);
            // cnt[cid]++;
		    int x1 = vPoints[6*i];
		    ...
		    char conf[10];
		    sprintf(conf, "%.3f %s", vPoints[6*i+4], classes[cid]);
            rectangle(imgShow, cv::Point(x1, y1), cv::Point(x2, y2), classColor[cid], 2);
            putText(imgShow, conf, cv::Point(x1, y1 - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, classColor[int(vPoints[6*i+5])],2);
		}
        ...
        // char msg[10];
        // // cv::Scalar(255, 128, 0)
        // putText(imgShow, "数据统计", cv::Point(xx, yy), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255),2);
        // sprintf(msg, "低头写字：%d", cnt[0]);
        // putText(imgShow, msg, cv::Point(xx, yy + dx * 1), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255),2);
        // sprintf(msg, "低头看书：%d", cnt[1]);
        // putText(imgShow, msg, cv::Point(xx, yy + dx * 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255),2);
        // sprintf(msg, "抬头听课：%d", cnt[2]);
        // putText(imgShow, msg, cv::Point(xx, yy + dx * 3), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255),2);
        // sprintf(msg, "抬头听课：%d", cnt[3]);
        // putText(imgShow, msg, cv::Point(xx, yy + dx * 4), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255),2);
        // sprintf(msg, "转头：%d", cnt[4]);
        // putText(imgShow, msg, cv::Point(xx, yy + dx * 5), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255),2);
        // sprintf(msg, "举手：%d", cnt[5]);
        // putText(imgShow, msg, cv::Point(xx, yy + dx * 6), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255),2);
...
void AI()
{
	while (true)
	{
		...
		demo.fBoxThreshold=0.25;
		demo.DetectFaceRgb((char*)aSrcImg.data,vAIPoints);
