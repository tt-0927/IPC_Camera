...
{
public:
	cSTrack( std::vector<float> tlwh_, float score);
	~cSTrack();

	std::vector<float> static tlbr_to_tlwh( std::vector<float> &tlbr);
	void static multi_predict( std::vector<cSTrack*> &stracks, byte_kalman::ByteKalmanFilter &kalman_filter);
	void static_tlwh();
	void static_tlbr();
	...
	void re_activate(cSTrack &new_track, int frame_id, bool new_id = false);
	void update(cSTrack &new_track, int frame_id);
