...
Scenario_NS::CFaceExpressV1_0::CFaceExpressV1_0(AiScenario_NS::InParam_S stInParam)
    : CScenarioBase(stInParam)
{
}

Scenario_NS::CFaceExpressV1_0::~CFaceExpressV1_0()
{
    unInit();
}

/* 初始化 */
bool Scenario_NS::CFaceExpressV1_0::init()
{
    bool        bRet = false;
    std::string strModel;
    if (m_stInParam.stNeedParam.vstrModelPath.size() > 1)
    {
        ...
        m_pFaceDetectInference = new InferenceV1_0_NS::CYoloFaceDetect(
...
/* 反初始化 */
bool Scenario_NS::CFaceExpressV1_0::unInit()
{
    if (m_pFaceDetectInference)
    {
...
/* 处理数据 */
bool Scenario_NS::CFaceExpressV1_0::process(
    ...
    int&                    nDataSize)
{
    ...
    {
        dlog(LOG_ERROR, "人脸检测模型需要的通道数和输入图片的通道数不一致 inMat[%d] != m_nFaceDetectLimitChannel[%d]",
...
/* 释放处理结果 */
bool Scenario_NS::CFaceExpressV1_0::releaseData(char*& pchOutData)
{
    if (!pchOutData)
    {
        return false;
...
bool Scenario_NS::CFaceExpressV1_0::convertToJson(
    std::vector<float> vecBoxPos,
    std::vector<float> vecExpressPos,
    char**             pchOutData,
    int&               nDataSize)
