/*
 * @FilePath     : HeadCountV1_0.cpp
 * @Author       : 严泽辉 yanzeh@kfb.cn
 * @Date         : 2024-05-17 17:39:27
 * @LastEditors: 李辉 lihui@kfb.cn
 * @LastEditTime: 2024-10-31 16:30:39
 * @Description  : 人多场景，人流检测（地铁人数）
 */
...
Scenario_NS::CHeadCountV1_0::CHeadCountV1_0(AiScenario_NS::InParam_S stInParam)
    : CScenarioBase(stInParam)
{
}

Scenario_NS::CHeadCountV1_0::~CHeadCountV1_0()
{
    unInit();
}

/* 初始化 */
bool Scenario_NS::CHeadCountV1_0::init()
{
    bool bRet = false;
    ...
    if (m_stInParam.stNeedParam.vstrModelPath.size() > 0)
    {
        ...
        m_pInference = new InferenceV1_0_NS::CHumanCount(
...
/* 反初始化 */
bool Scenario_NS::CHeadCountV1_0::unInit()
{
    if (m_pInference)
    {
        delete m_pInference;
...
/* 处理数据 */
bool Scenario_NS::CHeadCountV1_0::process(
    ...
    int&                    nDataSize)
{
    ...
    /* 前处理 - 模型输入(m_nLimitWidth,m_nLimitHeight) 直接缩放 */
    ...
    cv::cvtColor(stInData.inMat, stInData.inMat, cv::COLOR_RGB2BGR);
...
/* 释放处理结果 */
bool Scenario_NS::CHeadCountV1_0::releaseData(char*& pchOutData)
{
    if (!pchOutData)
    {
        return false;
...
bool Scenario_NS::CHeadCountV1_0::convertToJson(
    ...
    int&               nDataSize)
{
    ...
    for (int nIndex = 0; nIndex < vPointsXY.size(); nIndex++)
    {
        if (vPointsXY.size() >= ((nIndex + 1) * DATA_GROUP_SIZE))
        {
            ...
            Json::Array::add(pArrayBox, -1);
            Json::Array::add(pArrayBox, -1);
            Json::add(pItem, "Box", pArrayBox);
            Json::add(pItem, "Confidence", -1);
