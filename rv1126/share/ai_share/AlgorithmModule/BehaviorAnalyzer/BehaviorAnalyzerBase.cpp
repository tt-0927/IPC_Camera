/*
 * @FilePath     : BehaviorAnalyzerBase.cpp
 * @Author       : yanzeh yanzeh@kfb.cn
 * @Date         : 2024-01-11 10:08:07
 * @LastEditors  : 严泽辉 yanzeh@kfb.cn
 * @LastEditTime : 2024-02-21 16:34:29
 * @Description  :
 */
...
CBehaviorAnalyzerBase::CBehaviorAnalyzerBase(BehaviorAnalyzerInParam_S stInParam)
    : m_stInParam(stInParam)
{
    if (nullptr == m_pDataQueue)
    {
        m_pDataQueue = new CDataQueue<MediaDataInfo_S, std::list<BehaviorAnalyzerResult_S>>(freeMediaData, nullptr, stInParam.stExParam.nMaxQueue);
    }

    /* 创建线程 */
    m_threadObj = std::thread(&CBehaviorAnalyzerBase::run, this);
}

CBehaviorAnalyzerBase::~CBehaviorAnalyzerBase()
{
    /* 结束线程 */
...
/* 发送分析数据 */
BlError_E CBehaviorAnalyzerBase::send_dataAnalysis(MediaDataInfo_S stMediaDataInfo)
{
...
/* 读取分析数据 */
BlError_E CBehaviorAnalyzerBase::read_analysisResult(std::list<BehaviorAnalyzerResult_S>& listOutInfo)
{
    if (nullptr == m_pDataQueue)
    {
        dlog(LOG_ERROR, "未初始化队列操作");
...
/* 实时分析数据 */
BlError_E CBehaviorAnalyzerBase::realTime_dataAnalysis(
...
/* 线程函数 */
void CBehaviorAnalyzerBase::run()
{
    BlError_E enRetCode = OK;
    ...
    std::list<BehaviorAnalyzerResult_S> listResultInfo;
    ...
    while (m_bRunning.load())
    {
        ...
        {
            dlog(LOG_INFO, "行为分析待分析队列[%d]", m_pDataQueue->getSize_pendingQueue());
        }
