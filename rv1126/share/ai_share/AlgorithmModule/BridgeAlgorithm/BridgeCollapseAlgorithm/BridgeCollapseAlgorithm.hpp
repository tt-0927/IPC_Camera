/*
 * @FilePath     : BridgeCollapseAlgorithm.hpp
 * @Author       : 严泽辉 yanzeh@kfb.cn
 * @Date         : 2024-03-08 10:56:16
 * @LastEditors  : 严泽辉 yanzeh@kfb.cn
 * @LastEditTime : 2024-03-14 15:56:45
 * @Description  : 桥梁坍塌分析模块
 */
#pragma once

#include "BridgeAlgorithmBase.hpp"

namespace BDGA_NS
{
    class CBridgeCollapseAlgorithm : public CBridgeAlgorithmBase
    {
    public:

        CBridgeCollapseAlgorithm(InParam_S stInParam);
        ~CBridgeCollapseAlgorithm();

    protected:

        /**
         * @brief 分析数据
         * @param [MediaDataInfo_S&] stMediaDataInfo: 媒体数据信息
         * @param [AnalyzerResult_S&] stOutInfo: 分析后的结果信息
         * @return [*] 成功 >= BlError_E::OK   其他失败
         * @note 如果设置了stMediaDataInfo.freeDataFunc; 内部使用完会自动释放stMediaDataInfo.pchData。
         */
        BlError_E dataAnalysis(MediaDataInfo_S&  stMediaDataInfo,
                               AnalyzerResult_S& stOutInfo);


        /**
         * @brief 设置桥梁基准线
         * @param [list<PosInfo_S>] listPos: 基准线坐标
         * @return [*] 成功 >= BlError_E::OK   其他失败
         * @note
         */
        BlError_E setDatumLine(std::list<PosInfo_S> listPos);

    private:

        /* 算法实例 */
        void* m_pAlgorithm = nullptr;
    };

}    // namespace BDGA_NS
