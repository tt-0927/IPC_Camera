...
/* 结果容器 */
std::vector<PMNMDetect_NS::Result_S> vGlobalOutData; /* 全局的结果 */
/* ===================================================== AI需要的全局变量配置 ============================================================= */
...
void showImg(cv::Mat &aImgShow)
{
    for (int i = 0; i < vGlobalOutData.size(); i++)
    {
        ...
        cv::putText(aImgShow, conf, cv::Point(vGlobalOutData[i].fX1, vGlobalOutData[i].fY1 - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, getColor(0), 2);
        std::cerr << "ID: " << vGlobalOutData[i].nID << "; 种类: " << vGlobalOutData[i].sClassName << std::endl;
    }
}
