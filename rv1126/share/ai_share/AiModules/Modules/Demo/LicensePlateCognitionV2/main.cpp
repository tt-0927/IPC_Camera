 ...
 */
#include "FFmedia.hpp"
#include "LicensePlateCognitionV2_0.hpp"
...
/* 结果容器 */
std::vector<LicensePlateCognition_NS::Result_S> vGlobalOutData; /* 全局的结果 */
/* ===================================================== AI需要的全局变量配置 ============================================================= */
...
void AI()
{
    ...
    stInParam.strModelPath1 = "./LicensePlateDetect.rknn";
    stInParam.strModelPath2 = "./LicensePlateRec.rknn";
    /* 初始化车牌识别检测 */
    LicensePlateCognition_NS::CLicensePlateCognitionV2_0 *demo = new LicensePlateCognition_NS::CLicensePlateCognitionV2_0(stInParam);
...
void showImg(cv::Mat &aImgShow)
{
    for (int i = 0; i < vGlobalOutData.size(); i++)
    {
        cv::rectangle(aImgShow,
                      cv::Point(vGlobalOutData[i].fX, vGlobalOutData[i].fY),
                      cv::Point(vGlobalOutData[i].fX+vGlobalOutData[i].fWidth, vGlobalOutData[i].fY+vGlobalOutData[i].fHeight),
                      cv::Scalar(255, 0, 0), 2);
        // char conf[10];
		// sprintf(conf, "%d %d %d",
        //         vGlobalOutData[i].licensePlateNumber, 
        //         vGlobalOutData[i].licensePlateColor, 
        //         vGlobalOutData[i].licensePlateType);
        std::string text = vGlobalOutData[i].licensePlateNumber + " " + vGlobalOutData[i].licensePlateColor + " " + vGlobalOutData[i].licensePlateType;
        cv::putText(aImgShow, text, cv::Point(vGlobalOutData[i].fX, vGlobalOutData[i].fY - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, getColor(0), 2);
        std::cerr << "ID: " << vGlobalOutData[i].licensePlateNumber << std::endl;
    }
}
