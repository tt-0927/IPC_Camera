...
static int clamp(float fVal, int nMin, int nMax)
{
...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    stInParam.strModelPath = "./RK3588_Face_Detect_V2.0.rknn";
    FaceAttribute_NS::InParam_S stInAttributeParam;
    stInAttributeParam.strModelPath = "./RK3588_Face_Attribute_V1.0.rknn";
    /* 初始人脸检测检测 */
    FaceDetect_NS::CFaceDetectV2_0 *demo = new FaceDetect_NS::CFaceDetectV2_0(stInParam);
    ...
    /* 初始化属性识别模型 */
    FaceAttribute_NS::CFaceAttributeV1_0 *demoAttribute = new FaceAttribute_NS::CFaceAttributeV1_0(stInAttributeParam);
    ...
    FaceAttribute_NS::InData_S stInAttributeData;
    ...
    while (!g_bExit)
    {
        ...
        if (nstatus == 0 && naddr != 0)
        {
            ...
            { /*---------------------------------- 模型推理 ------------------------------------ */
                ...
                // cv::Mat copiedImage = aSrcImg.clone();
                std::cerr << "1、人脸推理开始！！！" << std::endl;
                bFlag = demo->process(stInData, vOutData);
                std::cerr << "2、人脸推理结束！！！" << std::endl;
                if (!bFlag)
                {
                    printf("推理失败\n");
                }
            ...
            { /* ---------------------------------- 显示函数 ----------------------------------  */
                ...
                {
                    float originalWidth = vOutData[i].fX2 - vOutData[i].fX1;
                    float originalHeight = vOutData[i].fY2 - vOutData[i].fY1;
                    float newWidth = originalWidth * 1.5f;
                    float newHeight = originalHeight * 1.5f;
                    float centerX = (vOutData[i].fX1 + vOutData[i].fX2) / 2;
                    float centerY = (vOutData[i].fY1 + vOutData[i].fY2) / 2;
                    float fX1 = clamp(centerX - newWidth / 2, 0, aSrcImg.cols);
                    float fY1 = clamp(centerY - newHeight / 2, 0, aSrcImg.rows);
                    float fX2 = clamp(centerX + newWidth / 2, 0, aSrcImg.cols);
                    float fY2 = clamp(centerY + newHeight / 2, 0, aSrcImg.rows);

                    cv::Rect rect(cv::Point(fX1, fY1),
                                  cv::Point(fX2, fY2));
                    std::cout << "框: " << fX1 << ";" << fY1 << ";" << fX2 << ";" << fY2 << std::endl;
                    cv::Mat NonMotorizedImage = aSrcImg(rect);
                    // cv::cvtColor(NonMotorizedImage, NonMotorizedImage, cv::COLOR_BGR2RGB);
                    stInAttributeData.inMat = NonMotorizedImage;
                    std::vector<FaceAttribute_NS::Result_S> vOutAttributeData;
                    bFlag = demoAttribute->process(stInAttributeData, vOutAttributeData);
                    if (!bFlag)
                    {
                        printf("属性识别推理失败\n");
                    }
                    cv::rectangle(aSrcImg,
                                  cv::Point(fX1, fY1),
                                  cv::Point(fX2, fY2),
                                  cv::Scalar(0, 0, 255), 2);
                    float fY = 20.0;
                    for (int j = 0; j < 6; j++)
                    {
                        std::string conf = std::string(vOutAttributeData[j].strName) + ":" + std::to_string(vOutAttributeData[j].fConfidence);
                        cv::putText(aSrcImg, conf, cv::Point(fX2, fY1 + fY), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 0, 0), 2);
                        fY += 20.0;
                    }
                    std::cerr << "ID: " << vOutData[i].nID << "; 种类: " << vOutData[i].sClassName << std::endl;
                }
            }
