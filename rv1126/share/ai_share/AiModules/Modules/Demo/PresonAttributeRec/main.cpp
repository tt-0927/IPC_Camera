...
/* 结果容器 */
std::vector<PMNMDetect_NS::Result_S> vDetectOutData;                      /* 全局的结果 */
std::vector<std::vector<PresonAttribute_NS::Result_S>> vAttributeOutData; /* 全局的结果 */
...
{
    /* ---------------------- 模型初始化 ------------------------ */
    PMNMDetect_NS::InParam_S stIn0Param;
    stIn0Param.strModelPath = "./RK3588_PersonMotorNomotor_Detect_V1.0.rknn";
    /* 初始化车牌识别检测 */
    PMNMDetect_NS::CPMNMDetectV1_0 *demo_detect = new PMNMDetect_NS::CPMNMDetectV1_0(stIn0Param);
    bool bFlag = demo_detect->init();
    if (!bFlag)
    {
        printf("检测模型初始化失败\n");
        exit(0);
    }
    PresonAttribute_NS::InParam_S stIn1Param;
    stIn1Param.strModelPath = "./RK3588_Person_Attribute_V1.0.rknn";
    /* 初始化车牌识别检测 */
    PresonAttribute_NS::CPresonAttributeV1_0 *demo_attribute = new PresonAttribute_NS::CPresonAttributeV1_0(stIn1Param);
    bFlag = demo_attribute->init();
    if (!bFlag)
    {
        printf("属性模型初始化失败\n");
        exit(0);
    ...
    PresonAttribute_NS::InData_S stIn1Data;
    stInData.stParam.fBoxThreshold = 0.35;
    ...
    while (true)
    {
        ...
        std::vector<PMNMDetect_NS::Result_S> vOutData0;
        std::vector<std::vector<PresonAttribute_NS::Result_S>> vOutData1;
        bFlag = demo_detect->process(stInData, vOutData0);
        if (!bFlag)
        {
            printf("推理失败\n");
        }
        std::chrono::duration<double, std::milli> t_detect = std::chrono::high_resolution_clock::now() - start;
        std::cout << "InferDetectTime: " << t_detect.count() << " ms" << std::endl;
        auto t = 0.0f;
        auto tpfb = std::chrono::high_resolution_clock::now();
        for (int idx = 0; idx < vOutData0.size(); idx++)
        {
            if (vOutData0[idx].nID != 0) continue;
            float fX1 = vOutData0[idx].fX1; /* 左上角x坐标 */
            float fY1 = vOutData0[idx].fY1; /* 左上角y坐标 */
            float fX2 = vOutData0[idx].fX2; /* 右下角x坐标 */
            float fY2 = vOutData0[idx].fY2; /* 右下角y坐标 */
            fX1 = std::max(0.0f, std::min(fX1, float(stInData.inMat.cols)));
            fY1 = std::max(0.0f, std::min(fY1, float(stInData.inMat.rows)));
            fX2 = std::max(0.0f, std::min(fX2, float(stInData.inMat.cols)));
            fY2 = std::max(0.0f, std::min(fY2, float(stInData.inMat.rows)));
            // std::cerr << "X1: " << fX1 << "; Y1: " << fY1 << "; W:" << fX2 - fX1 << "; H:" << fY2 - fY1 << std::endl;
            stIn1Data.inMat = stInData.inMat(cv::Rect(fX1, fY1, fX2 - fX1, fY2 - fY1)).clone();
            // cv::imwrite("test.jpg", stIn1Data.inMat);
            // std::cerr << "图片裁剪完成" << std::endl;
            std::vector<PresonAttribute_NS::Result_S> vOutData2;
            bFlag = demo_attribute->process(stIn1Data, vOutData2);
            if (!bFlag)
            {
                printf("推理失败\n");
            }
            // std::cerr << "模型推理完成" << std::endl;
            vOutData1.push_back(vOutData2);
            // for (int attr_num=0; attr_num<vOutData2.size(); attr_num++)
            // {
            //     std::cerr << "属性: " << vOutData2[attr_num].strName << "; 置信度: " << vOutData2[attr_num].fConfidence << std::endl;
            // }

           std::chrono::duration<double, std::milli> tpf = std::chrono::high_resolution_clock::now() - tpfb; 
           std::cout << "InferPerBoxTime: " << tpf.count() << " ms" << std::endl;
           t += tpf.count();
           tpfb = std::chrono::high_resolution_clock::now();
        }
        if (vOutData1.size() > 0)
        {    
            std::cout << "InferPerBoxAvgTime: " << t/vOutData1.size() << " ms" << std::endl;
        }
        if (vOutData0.size() != 0)
        {
            vDetectOutData = vOutData0;
            vAttributeOutData = vOutData1;
        }
...
void showImg(cv::Mat &aImgShow)
{
    ...
    if (!vDetectOutData.empty())
    {
        // std::vector<PMNMDetect_NS::Result_S> vDetectOutData = vDetectOutDatas.front();
        // vDetectOutDatas.pop(); /* 移除队列前端的元素 */

        // std::vector<std::vector<PresonAttribute_NS::Result_S>> vAttributeOutData = vAttributeOutDatas.front();
        // vAttributeOutDatas.pop(); /* 移除队列前端的元素 */


        // printf("===================[%d]-[%d]======================\n", vDetectOutData.size(), vAttributeOutData.size());
        int idx = 0;
        
        for (int i = 0; i < vDetectOutData.size(); i++)
        {
            if (vDetectOutData[i].nID != 0)
            {
                continue;
            }

            cv::rectangle(aImgShow,
                          cv::Point(vDetectOutData[i].fX1, vDetectOutData[i].fY1),
                          cv::Point(vDetectOutData[i].fX2, vDetectOutData[i].fY2),
                          cv::Scalar(255, 0, 0), 2);
            char conf[10];
            std::string text;
            sprintf(conf, "%d %.2f", vDetectOutData[i].nID, vDetectOutData[i].fBoxConfidence);
            cv::putText(aImgShow, conf, cv::Point(vDetectOutData[i].fX1, vDetectOutData[i].fY1 - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, getColor(0), 2);
            // std::cerr << "ID: " << vDetectOutData[i].nID << "; 种类: " << vDetectOutData[i].sClassName << std::endl;
            if (idx < vAttributeOutData.size())
            {
                for (int j = 0; j < vAttributeOutData[idx].size(); j++)
                {
                    // sprintf(conf, "%s %.2f", vAttributeOutData[idx][j].strName, vAttributeOutData[idx][j].fConfidence);
                    text = vAttributeOutData[idx][j].strName + std::string(" ") + std::to_string(vAttributeOutData[idx][j].fConfidence);
                    cv::putText(aImgShow, text, cv::Point(vDetectOutData[i].fX2 + 2, vDetectOutData[i].fY1 + 10 + j * 20), cv::FONT_HERSHEY_SIMPLEX, 0.7, getColor(0), 2);
                    // std::cerr << "属性: " << vAttributeOutData[idx][j].strName << "; 置信度: " << vAttributeOutData[idx][j].fConfidence << std::endl;
                }
                idx++;
            }
        }
