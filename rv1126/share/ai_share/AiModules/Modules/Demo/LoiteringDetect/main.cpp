...
/* 结果容器 */
std::vector<LoiteringDetect_NS::Result_S> vGlobalOutData; /* 全局的结果 */
/* ===================================================== AI需要的全局变量配置 ============================================================= */
...
{
    /* ---------------------- 模型初始化 ------------------------ */
    LoiteringDetect_NS::InParam_S stInParam;
    stInParam.strModelPath = "./HumanDetect.rknn";
    /* 初始化徘徊检测 */
    LoiteringDetect_NS::CLoiteringDetectV1_0 *demo = new LoiteringDetect_NS::CLoiteringDetectV1_0(stInParam);
    ...
    LoiteringDetect_NS::InData_S stInData;
    ...
    while (true)
    {
        ...
        std::vector<LoiteringDetect_NS::Result_S> vOutData;
...
void showImg(cv::Mat &aImgShow)
{
    for (int i = 0; i < vGlobalOutData.size(); i++)
    {
        cv::rectangle(aImgShow,
                      ...
                      cv::Scalar(0, 255, 0), 2);
        char conf[10];
		sprintf(conf, "%d",  vGlobalOutData[i].nId);
        cv::putText(aImgShow, conf, cv::Point(vGlobalOutData[i].fX, vGlobalOutData[i].fY - 2), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 0, 0), 2);
        // std::cerr << "ID: " << vGlobalOutData[i].nId << std::endl;
    }
}
