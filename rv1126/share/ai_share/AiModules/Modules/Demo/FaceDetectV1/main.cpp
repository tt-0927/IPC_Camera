...
/* 结果容器 */
std::vector<FaceDetect_NS::Result_S> vGlobalOutData; /* 全局的结果 */
/* ===================================================== AI需要的全局变量配置 ============================================================= */
...
void AI()
{
    ...
    stInParam.strModelPath = "./MB_FaceDetector1920x1024.rknn";
    // stInParam.strOriginalDataPath = "./images/";
    /* 初始化车牌识别检测 */
    FaceDetect_NS::CFaceDetectV1_0 *demo = new FaceDetect_NS::CFaceDetectV1_0(stInParam);
...
void showImg(cv::Mat &aImgShow)
{
    for (int i = 0; i < vGlobalOutData.size(); i++)
    {
        cv::rectangle(aImgShow,
                      ...
                      cv::Scalar(255, 0, 0), 2);
        // printf("[%f,%f][%f,%f]\n", vGlobalOutData[i].fX1, vGlobalOutData[i].fY1,vGlobalOutData[i].fX2,vGlobalOutData[i].fY2);
