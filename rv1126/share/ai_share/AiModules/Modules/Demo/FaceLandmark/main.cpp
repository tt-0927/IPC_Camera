...
/* 结果容器 */
std::vector<FaceLandmark_NS::OutData_S> vGlobalOutData; /* 全局的结果 */
/* ===================================================== AI需要的全局变量配置 ============================================================= */
...
{
    /* ---------------------- 模型初始化 ------------------------ */
    FaceLandmark_NS::InParam_S stInParam;
    stInParam.strDetectModelPath = "./Scrfd.rknn";
    stInParam.strLandmarkModelPath = "./FaceLandmark.rknn";
    /* 初始化车牌识别检测 */
    // FaceLandmark_NS::CFaceLandmarkV1_0 *demo = new FaceLandmark_NS::CFaceLandmarkV1_0(stInParam);
    FaceLandmark_NS::CFaceLandmarkV2_0 *demo = new FaceLandmark_NS::CFaceLandmarkV2_0(stInParam);
    ...
    FaceLandmark_NS::InData_S stInData;
    ...
    while (true)
    {
        ...
        std::vector<FaceLandmark_NS::OutData_S> vOutData;
...
void showImg(cv::Mat &aImgShow)
{
    for (int i = 0; i < vGlobalOutData.size(); i++)
    {
        cv::rectangle(aImgShow,
                      ...
                      cv::Scalar(255, 0, 0), 2);
        for (int nP = 0; nP < vGlobalOutData[i].vvPoints.size(); nP++)
        {
            int nPx = int(vGlobalOutData[i].vvPoints[nP][0]);
            int nPy = int(vGlobalOutData[i].vvPoints[nP][1]);
            cv::circle(aImgShow,
                        cv::Point(nPx, nPy), 5, cv::Scalar(255, 0, 0), -1);
        }
        for (int nP = 0; nP < vGlobalOutData[i].vvLandmarks.size(); nP++)
        {
            int nPx = int(vGlobalOutData[i].vvLandmarks[nP][0]);
            int nPy = int(vGlobalOutData[i].vvLandmarks[nP][1]);
            cv::circle(aImgShow,
                       cv::Point(nPx, nPy), 2, cv::Scalar(0, 255, 0), -1);
        }
    }
}
