...
static void *videoFrameHandlThr(void *pParam)
{
    ...
    /* ---------------------- 模型初始化 ------------------------ */
    HandLandmark_NS::InParam_S stInParam;
    stInParam.strDetectModelPath = "./Hand_aiConfig.json";
    stInParam.strLandmarkModelPath = "./HandPose_aiConfig.json";
    /* 初始化检测 */
    HandLandmark_NS::CHandLandmarkV1_0 *demo = new HandLandmark_NS::CHandLandmarkV1_0(stInParam);
    ...
    HandLandmark_NS::InData_S stInData;
    /* ---------------------------------------------------------- */
    ...
    while (!g_bExit)
    {
        ...
        if (nstatus == 0 && naddr != 0)
        {
            ...
            std::vector<HandLandmark_NS::OutData_S> vOutData;
            { /*---------------------------------- 模型推理 ------------------------------------ */
                /* ================================ 开始计时 ===================================== */
            ...
            { /* ---------------------------------- 显示函数 ----------------------------------  */
                for (int i = 0; i < vOutData.size(); i++)
                {
                    cv::rectangle(aSrcImg,
                                  ...
                                  cv::Scalar(255, 0, 0), 2);
                    for (int nP = 0; nP < vOutData[i].vvLandmarks.size(); nP++)
                    {
                        int nPx = int(vOutData[i].vvLandmarks[nP][0]);
                        int nPy = int(vOutData[i].vvLandmarks[nP][1]);
                        cv::circle(aSrcImg,
                                   cv::Point(nPx, nPy), 4, cv::Scalar(0, 0, 255), -1);
                    }

                    std::string label = vOutData[i].nLabel==0? "Right":"Left";
                    int baseLine = 0;
                    int fontFace = cv::FONT_HERSHEY_SIMPLEX;
                    double fontScale = 0.5;
                    int textThickness = 1;
                    cv::Size textSize = cv::getTextSize(
                        label, fontFace, fontScale, textThickness, &baseLine
                    ); 
                    int textX = vOutData[i].fX1;
                    int textY = vOutData[i].fY1 - baseLine - 2;
                    if (textY < textSize.height) {
                        // 如果上方空间不足，则放到框下方
                        textY = vOutData[i].fY1 + (vOutData[i].fY2-vOutData[i].fY1) + textSize.height + 2;
                    }
                    cv::Scalar cloro_ = vOutData[i].nLabel==0?cv::Scalar(0, 255, 0):cv::Scalar(0, 255, 255);
                    cv::putText(
                        aSrcImg,
                        label,
