...
{
    /* 全局的图片 */
    cv::Mat aSrcImg(1080, 1920, CV_8UC3);
    /* ---------------------- 模型初始化 ------------------------ */
    MoveDetect_NS::InParam_S stInParam;
    /* 初始化车牌识别检测 */
    MoveDetect_NS::CMoveDetectV1_0 *demo = new MoveDetect_NS::CMoveDetectV1_0(stInParam);
    ...
    while (!g_bExit)
    {
        ...
        if (nstatus == 0 && naddr != 0)
        {
            ...
            /* =============================== AI算法 ======================================== */
            if (demo->LastFrameCache.empty())
            {
                demo->LastFrameCache = aSrcImg.clone();
            }
            
            if (nCount == stInData.stParam.nSkipFrames)
            {
                stInData.inMat = aSrcImg.clone();
                ...
                nCount = 0;
                demo->LastFrameCache = stInData.inMat;
            }
            nCount++;
            /* =============================== 输出推理事件耗时 ================================== */
            ...
            /* ---------------------------------- 显示函数 ----------------------------------  */
            for (int i = 0; i < stOutData.size(); i++)
            {
                // printf("%d,%d,%d,%d\n", stOutData[i][0], stOutData[i][1], stOutData[i][2], stOutData[i][3]);

                cv::rectangle(aSrcImg, cv::Point(stOutData[i][0], stOutData[i][1]), cv::Point(stOutData[i][0] + stOutData[i][2], stOutData[i][1] + stOutData[i][3]), cv::Scalar(0, 255, 0), 4);
            }
