/**
 * @file ReflectiveDetectV1_0.cpp
 * @author tianl (tianl@kfb.cn)
 * @date 2025-11-13
 * 
 * @brief 反光衣识别
 */
#include "ReflectiveDetectV1_0.hpp"
...
using namespace ReflectiveDetect_NS;

ReflectiveDetect_NS::CReflectiveDetectV1_0::CReflectiveDetectV1_0(InParam_S stInParam)
    : m_stInParam(stInParam)
{
}

ReflectiveDetect_NS::CReflectiveDetectV1_0::~CReflectiveDetectV1_0()
{
    unInit();
}

/* 初始化 */
bool ReflectiveDetect_NS::CReflectiveDetectV1_0::init()
{
    CStatisticsTimer runTime("反光衣检测初始化耗时");
    bool bRet = false;
...
/* 反初始化 */
bool ReflectiveDetect_NS::CReflectiveDetectV1_0::unInit()
{
    CStatisticsTimer runTime("反光衣检测反初始化耗时");
...
/* 处理数据 */
bool ReflectiveDetect_NS::CReflectiveDetectV1_0::process(
    InData_S stInData,
    ...
    OutData_S *stOutData)
{
    ...
    if (access("/test_ReflectiveDetect", F_OK) == 0)
	{
        for (int i = 0; i < vBoxDatas.size(); i++)
        {
		
            printf("===================>反光衣识别  当前目标[%d] 类别class[%d]  置信度[%.2f] <===================\n",
            ...
            // Modules_NS::saveImage(stInData.inMat, "/mnt/Reflectivey_test");
		
	    }	
    }
    
	// 标记当前帧是否识别到了反光衣
	bool bDetectReflectivey = false;
    for (const auto& box : vBoxDatas)
    {
        DetectResult_S result;
        result.fConfidence = box.fConfidence;
        result.nClassId = box.nLabel;
        // printf("===================>反光衣检测  类别class[%d]  置信度[%.2f] 反光衣置信度阈值[%.2f]<===================\n",
        // result.nClassId,result.fConfidence,stInData.stParam.stReflectiveyParam.fConfidence);
		if (result.fConfidence > stInData.stParam.stReflectiveyParam.fConfidence) 
		{
			bDetectReflectivey = true; 
		} 
    }
	int enType = 0;
	if(stInData.stParam.stReflectiveyParam.bEnable)
	{
		if (bDetectReflectivey) 
		{
            m_nReflectiveyFrameCount++;
            if (m_nReflectiveyFrameCount >= stInData.stParam.stReflectiveyParam.nDetectFrame) 
            {
                Result_S stResult;
                stResult.bReflectivey = true;
                vecResult.push_back(stResult);

                printf("【报警】识别到了反光衣！\n");
                enType |= 0x02;
                m_nReflectiveyFrameCount = 0;
            }
		} 
