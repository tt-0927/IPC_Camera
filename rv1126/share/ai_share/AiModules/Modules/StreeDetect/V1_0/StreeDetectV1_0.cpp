...
using namespace StreeDetect_NS;

StreeDetect_NS::CStreeDetectV1_0::CStreeDetectV1_0(InParam_S stInParam)
    : m_stInParam(stInParam)
{
}

StreeDetect_NS::CStreeDetectV1_0::~CStreeDetectV1_0()
{
    unInit();
}

/* 初始化 */
bool StreeDetect_NS::CStreeDetectV1_0::init()
{
    CStatisticsTimer runTime("道路积水井盖检测初始化耗时");
    bool bRet = false;
...
/* 反初始化 */
bool StreeDetect_NS::CStreeDetectV1_0::unInit()
{
    CStatisticsTimer runTime("道路积水井盖,检测反初始化耗时");
...
/* 处理数据 */
bool StreeDetect_NS::CStreeDetectV1_0::process(
    InData_S stInData,
    ...
    OutData_S *stOutData)
{
	...
	// 标记当前帧是否检测到井盖异常或道路积水
	bool bDetectManhole = false;
	bool bDetectRoad = false;
    if (access("/test_streedetect", F_OK) == 0)
	{
        for (int i = 0; i < vBoxDatas.size(); i++)
        {
		
		printf("===================>街道检测  当前目标[%d] 类别class[%d]  置信度[%.2f] <===================\n",
    ...
    for (const auto& box : vBoxDatas)
    {
		...
		if ((result.nClassId == DAMAGED || result.nClassId == MISSING || result.nClassId == UNCOVERED || result.nClassId == EDGE_DAMAGED) && result.fConfidence > 0.5) 
		{
			bDetectManhole = true; 
		} 
		else if (result.nClassId == ROAD_FLOODED && result.fConfidence > 0.5) 
		{
			bDetectRoad = true;
		}
	...
	if(stInData.stParam.stRoadPondingParam.bEnable)
	{
		...
		{
			m_nRoadFrameCount++; 
			if (m_nRoadFrameCount >= stInData.stParam.stRoadPondingParam.nDetectFrame) 
			{
				Result_S stResult;
				stResult.bRoadPonding = true;
				vecResult.push_back(stResult);

				//printf("【报警】连续%d帧检测到道路积水！\n", m_nRoadFrameCount);
				enType |= 0x02;
				m_nRoadFrameCount = 0;
			}
		} 
	...
	if(stInData.stParam.stManholeCoverAbnormalParam.bEnable)
	{
		if (bDetectManhole) 
		{
			m_nManholeFrameCount++; 
			if (m_nManholeFrameCount >= stInData.stParam.stManholeCoverAbnormalParam.nDetectFrame) 
			{
				Result_S stResult;
				stResult.bManholeCoverAbnormal = true;
				vecResult.push_back(stResult);

				//printf("【报警】连续%d帧检测到井盖异常！\n", m_nManholeFrameCount);
				m_nManholeFrameCount = 0; 
				enType |= 0x04;
			}
		} 
	}
