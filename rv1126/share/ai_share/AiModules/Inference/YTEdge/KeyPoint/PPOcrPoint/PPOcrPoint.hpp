...
{
    class CPPOcrPoint : public CCVInferenceYT
    {
    public:

        CPPOcrPoint(std::string strConfigPath);
        ~CPPOcrPoint();
    ...
    private:
        ...
        /* 后处理 */
        PostProcess_NS::cPPOCRDetectPostProcess* m_postProcess;

        /* yolo的阈值 */
        float m_fMaskThreshold = 0.5;   /* 掩码阈值 */
        float m_fBoxThreshold  = 0.5;    /* 文本框阈值 */ 
        bool m_bPolyType = true;          /* 是否使用多边形处理 */
        float m_fUnclipRatio = 2;           /* 文本扩充系数 */
    };
