...
void TokenizerClip::tokenize(std::string token, std::vector<int> &idx)
{
    idx.push_back(49406);
    {
...
bool TokenizerClip::encode_text(std::string text, std::vector<int> &idx)
{
...
bool TokenizerClipChinese::encode_text(std::string text, std::vector<int> &idx)
{
    if (tokenizer_token2idx.size() == 0)
    {
        printf("词表未初始化\n");
