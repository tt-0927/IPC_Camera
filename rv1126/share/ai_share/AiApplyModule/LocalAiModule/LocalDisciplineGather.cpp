/*
 * @FilePath     : LocalDisciplineGather.cpp
 * @Author       : weigl 308241951@qq.com
 * @Date         : 2024-05-31 17:17:57
 * @LastEditors: weigl 308241951@qq.com
 * @LastEditTime: 2025-03-10 19:56:18
 * @Description  : 课堂纪律分析
 */
...
AiLocal_NS::CLocalDisciplineGather::CLocalDisciplineGather(returnDataFunc pReturn)
    : m_pReturn(pReturn)
{
}

AiLocal_NS::CLocalDisciplineGather::~CLocalDisciplineGather()
{
    /* 结束线程 */
...
/* 添加分析数据 */
BlError_E AiLocal_NS::CLocalDisciplineGather::addData(AiManage_NS::CommDataInfo_S* pstData)
{
    if (pstData && m_pAlgorithmHndl)
    {
        ...
        dlog(LOG_TRACE, "本地课堂纪律分析，添加数据队列[%ld]", m_pendingQueue.size());
    ...
    else
    {
        if (!pstData)
        {
            dlog(LOG_ERROR, "添加课堂纪律分析数据-失败, 传入的数据为空");
        }

        if (!m_pAlgorithmHndl)
        {
            dlog(LOG_ERROR, "添加课堂纪律分析数据-失败, 本类未初始化");
        }
    }
...
/* 清空数据 */
BlError_E AiLocal_NS::CLocalDisciplineGather::clearData()
{
    std::unique_lock<std::mutex> lock(m_queueMutex);
...
/* 初始化分析功能 */
BlError_E AiLocal_NS::CLocalDisciplineGather::init()
{
    if (m_pAlgorithmHndl)
    {
        dlog(LOG_ERROR, "课堂纪律分析功能已初始化, 不用再次初始化");
        return ERR_INI_ERR;
    }

    AiScenario_NS::InParam_S stInParam;
    stInParam.clear();
    stInParam.stNeedParam.enType       = AiScenario_NS::DISCIPLINE;
    ...
    stInParam.stExParam.fBoxThreshold = 0.5;

    stInParam.stExParam.bDebug              = false;
    stInParam.stExParam.strOriginalDataPath = "/opt/course/old_stDiscipline";
    stInParam.stExParam.strAnalyzeDataPath  = "/opt/course/stDiscipline";
    if (stInParam.stExParam.bDebug)
    {
        system("rm /opt/course/old_stDiscipline -r;rm /opt/course/stDiscipline -r");
    }

    m_pAlgorithmHndl = new Scenario_NS::CClassroomMoveDetectV1_0(stInParam);
    if (!m_pAlgorithmHndl)
    {
        dlog(LOG_ERROR, "创建课堂纪律分析句柄-失败");
        return ERR_INI_ERR;
    }

    if (!m_pAlgorithmHndl->init())
    {
        dlog(LOG_ERROR, "初始化课堂纪律分析句柄-失败");
        return ERR_INI_ERR;
    }

    /* 创建线程 */
    m_bRunning.store(true);
    m_threadObj = std::thread(&CLocalDisciplineGather::run, this);

    dlog(LOG_TRACE, "课堂纪律分析功能初始化成功");

    return OK;
}

/* 线程函数 */
void AiLocal_NS::CLocalDisciplineGather::run()
{
    ...
    while (m_bRunning.load())
    {
        ...
        if (m_pAlgorithmHndl && pstDataInfo)
        {
            ...
            {
                // dlog(LOG_USER,"%s",pchProcessData);
                /* 重新组装JSON数据 */
