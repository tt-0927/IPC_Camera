...
typedef struct package
{
	...
	long long int len;
	int   type;       // 用于区分 AI模型:1 或 APP程序:0 
	int reserves;
} Upgrade_Package_t;
#pragma pack()
#define CLIENT 1
...
int upgrade_makePacket(char *path, char *out_name, char *version, char *buildType, char *upgrade_id_index, int type,int upgrade_type)
{
	int nRet;
	DIR *pDir = NULL;
	int8_t cmd[1024] = {0};
	...
	printf("the pack.version=[%s], pack.len=[%lld], pack.id=[%s], pack.type=[%d]\n", version, pack.len, pack.id,pack.type);
...
int main(int argc, char **argv)
{
	if (argc < 6)
	{
		printf("用法: %s  <升级文件根目录><软件包名称><版本号 eg: V1.01><构建类型(itc/中性)><升级ID索引(6124 or 204 or 304)><类型(客户端为:1 服务端:0)><升级内容(系统程序:0 AI模型:1)>\n", argv[0]);
	...
	{
		type = atoi(argv[6]);
		upgrade_type  = atoi(argv[7]);
		upgrade_makePacket(argv[1], argv[2], argv[3], argv[4], argv[5], type,upgrade_type);
	}
	else if (7 == argc)
	{
		type = atoi(argv[5]);
		upgrade_type  = atoi(argv[6]);
		upgrade_makePacket(argv[1], argv[2], argv[3], NULL, argv[4], type,upgrade_type);
	}
