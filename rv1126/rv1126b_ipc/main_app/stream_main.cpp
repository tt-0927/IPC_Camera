 ...
 * @Author       : huangjunda
 * @Date         : 2025-03-12 16:21:37
 * @LastEditors  : cyc
 * @LastEditTime : 2025-03-27 20:58:48
 * @Description  :
 */
#include <atomic>
#include <stdio.h>
...
/* 日志记录最大保留的日志文件数量 */
#define MAX_LOG_FILES (6)

/* 进程是否捕获信号而退出标志 */
static std::atomic<bool> g_bExit(false);
 
// 定义全局文件描述符
static int g_fd_spk_val = -1;
static int g_fd_lo_val = -1;
extern std::atomic<bool> g_bExit;

// Audio静音函数
static void emergencyMute() 
{
    if (g_fd_spk_val >= 0) write(g_fd_spk_val, "0", 1);
    if (g_fd_lo_val >= 0) write(g_fd_lo_val, "0", 1);
}

static void exitHandler(int nSigno)
{
    //正常的退出请求 (SIGINT, SIGTERM)
    if (SIGINT == nSigno || SIGTERM == nSigno)
    {
        g_bExit.store(true);
        emergencyMute();
    }
    else if (SIGSEGV == nSigno || SIGABRT == nSigno || 
             SIGILL == nSigno || SIGFPE == nSigno)
    {
        emergencyMute();
...
void initSignals()
{
    g_fd_spk_val = open("/sys/class/gpio/gpio4/value", O_WRONLY);
    g_fd_lo_val = open("/sys/class/gpio/gpio199/value", O_WRONLY);
