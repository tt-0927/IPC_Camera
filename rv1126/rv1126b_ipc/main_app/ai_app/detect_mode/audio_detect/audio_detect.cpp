 ...
 * @LastEditTime : 2025-11-12 09:51:57
 * @Description  : 音频异常侦测
 */
...
void CAudioDetect::run()
{
    ...
    while (m_bRunning.load())
    {
        ...
        if (stMediaData.nSize == 0 || stMediaData.pData == nullptr)
        {
            continue;
        }
        ...
        if (m_stAudioAnomalyCfg.bEnable)
        {
            ...
            /* 音频异常侦测后处理函数 */
            processAudioAnomaly(pData, stMediaData.nSize);
...
void CAudioDetect::processAudioAnomaly(char *pData, int nLength)
{
    if (!pData || nLength <= 0)
    {
        return;
