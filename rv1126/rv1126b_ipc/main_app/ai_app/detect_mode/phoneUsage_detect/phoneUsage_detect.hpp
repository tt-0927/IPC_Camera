...
class CPhoneUsageDetect : public CAlgorithm
{
public:
...
private:
     ...
     * @param minConfidence 最低置信度（默认 0.4f，float 类型）
     * @param maxConfidence 最高置信度（默认 0.7f，float 类型）
     * @return float 置信度（范围：minConfidence ~ maxConfidence）
     */
    float sensitivityToConfidence(int sensitivity, float minConfidence = 0.4f, float maxConfidence = 0.7f);
...
private:
    /* 句柄 */
    PlayPhoneDetect_NS::CPlayPhoneDetectV1_0* m_pHandle = nullptr;
