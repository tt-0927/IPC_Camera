...
CPhoneUsageDetect::CPhoneUsageDetect()
    : m_dateQueue(QUEUE_MAX)
{
    m_bRunning.store(true);
    m_thread = std::thread(&CPhoneUsageDetect::run, this);
}
...
/* 接受媒体数据 */
void CPhoneUsageDetect::recvMediaData(MediaData_S stMediaData)
{
    if (!m_stPhoneUsageCfg.bEnable)
    {
        dlog_debug("ai_app:  玩手机识别-开关未启用");
        return;
    }
    ...
    if (m_RecvManager.handleEvent(stMediaData.stMediaParam.nChannel))
    {
        ...
        {
            dlog_error(" 玩手机识别-数据队列满了 [%d]" ,m_dateQueue.size());
        }
        m_dateQueue.pushOrReplace(stMediaData);
    }
}

float CPhoneUsageDetect::sensitivityToConfidence(int sensitivity, float minConfidence, float maxConfidence) 
{
...
int CPhoneUsageDetect::sensitivityToFrames(int sensitivity, int minFrames, int maxFrames) 
{
...
bool CPhoneUsageDetect::init()
{
    if (!m_pHandle)
    {
        ...
        m_pHandle = new PlayPhoneDetect_NS::CPlayPhoneDetectV1_0(stInParam);
        if (m_pHandle)
        {
            if (m_pHandle->init())
            {
...
/* 事件转换函数 */
int CPhoneUsageDetect::convertMaskToType( int eventFlags)
{
    
    { if (eventFlags & 0x02) return 31; /* Event::Type::PHONE_USAGE */ }
 ...
 * @param stAlgoConfig 
 */
void CPhoneUsageDetect::setAlgoEnCfg(const Event::AlgorithmConfig &stAlgoConfig)
{
...
void CPhoneUsageDetect::setAlgoParamCfg(const Alarm::PhoneUsageDetection_S &stAlgoCfg,Event::Type_E enType)
{
    dlog_debug("ai_app: 设置玩手机识别参数");
    m_stPhoneUsageCfg = stAlgoCfg;
}

void CPhoneUsageDetect::run()
{
    MediaData_S      stMediaData;   
    ...
    while (m_bRunning.load())
    {
        ...
        CStatisticsTimer runTime(" 玩手机识别完整耗时");

        /* 送分析 */
        if (1)
        {
            frameRate(" 玩手机识别-分析数据", 5);
                ...
                /* 分析数据 */
                {
                    ...
                    if (stOutData.validResult)
                    {
                        for (const auto& result : vecResult)
                        {
                            ...
                            {
                                /* 上报事件 */
                                dlog_debug("ai_app:  玩手机识别报警触发 类型[%d]",nType);
        ...
        {
            dlog_error("ai_app:  玩手机识别-获取虚拟地址失败");
        }
    }
}