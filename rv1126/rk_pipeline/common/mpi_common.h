 ...
 * @Date         : 2025-12-02 15:43:00
 * @LastEditors  : zhouzr@kfb.cn
 * @LastEditTime : 2025-12-02 16:55:26
...
#define mpi_tde_log(fmt, ...)       mpi_module_log("TDE", fmt, ##__VA_ARGS__)
#define mpi_vdec_log(fmt, ...)      mpi_module_log("VDEC", fmt, ##__VA_ARGS__)
...
#define mpi_vo_log(fmt, ...)        mpi_module_log("VO", fmt, ##__VA_ARGS__)
...
#define CHECK_API_RETURN(func) \
    do { \
        int result = func; \
        if (result != 0) { \
            printf("\033[0;31m[ROCKIT][Func]:%s [Line]:%d %s failed, error code: 0x%08X\033[0;39m\n", __FUNCTION__, __LINE__, #func, (unsigned int)result); \
            fflush(stdout); \
            return result; \
        } \
    } while (0)
