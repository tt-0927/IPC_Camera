...
typedef struct rk_smart_ir_params_s {
    float d2n_envL_th;        /* ⽇转夜亮度阈值 */
    float n2d_envL_th;        /* 夜转⽇亮度阈值 */
    float rggain_base;        /* ⿊夜切⽩天的Rgain/Ggain基准值 */
    float bggain_base;        /* ⿊夜切⽩天的Bgain/Ggain基准值 */
    float awbgain_rad;        /* ⿊夜切⽩天的awbgain滤波半径 */
    float awbgain_dis;        /* ⿊夜切⽩天的awbgain离散度阈值 */
    uint16_t switch_cnts_th;  /* 切换阈值， 保持相同状态次数⼤于该阈值时才允许状态切换。*/
...
typedef struct rk_smart_ir_attr_s {
    RK_SMART_IR_STATUS_t init_status;       /* 配置⽇夜状态 */
    RK_SMART_IR_SWTICH_MODE_t switch_mode;  /* ⽇夜转化模式 */
    RK_SMART_IR_LIGHT_MODE_t light_mode;    /* 补光灯亮度调节模式，包括⾃动亮度补光模式和固定亮度补光模式。 */
    RK_SMART_IR_LIGHT_TYPE_t light_type;    /* 补光灯光源类型，包括可⻅光、红外光、⽆补光、混合光四种类型。 */
    uint8_t light_value;                    /* 补光灯亮度值 */
    rk_smart_ir_params_t params;            /* ⽇夜切换的配置参数 */
    bool en_quick_switch;                   /* 是否加快⽇夜转化，对转化速度有要求的可以将此参数配置为true,切换速度提升了，相应的误切概率也会增加 */
    bool en_grid_weight;                    /* 是否启⽤带分块权重的⽇夜转化，该权重值与AE模块中的GridWeights参数是⼀致的。 */
    bool en_auto_n2dth;                     /* 补光灯类型配置为⾃动，⿊夜切⽩天时，是否⾃动调整的⿊切⽩阈值。true为 ⾃动调整，false为不调整。*/
