...
private:
     ...
     * @brief 接收数据
     * 
     * @param pSession 指向 TcpAdapter 的指针
     */
    void receive(TcpAdapter* pSession);
...
private:
    /**
     * @brief 存储 TCP 服务器参数配置
     */
    Param_S m_stParam;
    
    /**
     * @brief TCP 适配器的共享指针，用于管理服务器实例
     */
    std::shared_ptr<TcpAdapter> m_server;
     ...
     * @brief 存储所有活动会话的唯一集合
     */
    std::unordered_set<TcpAdapter*> m_sessions;
