...
void CWifiManager::monitorLoop() {
    ...
    int consecutiveScanningCount = 0;
    const int scanningReconnectThreshold = 3;
    const std::chrono::seconds normalMonitorInterval(30);
    ...
    while (is_running.load()) {
        ...
        if (status.find("wpa_state=DISCONNECTED") != std::string::npos) {
            consecutiveScanningCount = 0;
            is_connected.store(false);
            shouldReconnect = true;
            dlog_warn("[WiFi监控] 检测到DISCONNECTED，立即启动重连");
        }
        else if (status.find("wpa_state=SCANNING") != std::string::npos) {
            ++consecutiveScanningCount;
            dlog_warn("[WiFi监控] 连续检测到SCANNING: %d/%d",
                      consecutiveScanningCount, scanningReconnectThreshold);

            if (consecutiveScanningCount >= scanningReconnectThreshold) {
                consecutiveScanningCount = 0;
                is_connected.store(false);
                shouldReconnect = true;
                dlog_warn("[WiFi监控] SCANNING连续达到阈值，启动重连");
            }
        }
        ...
        else if (status.find("wpa_state=ASSOCIATING") != std::string::npos ||
                 status.find("wpa_state=ASSOCIATED") != std::string::npos ||
                 status.find("wpa_state=4WAY_HANDSHAKE") != std::string::npos ||
                 status.find("wpa_state=GROUP_HANDSHAKE") != std::string::npos) {
            /* 正在关联或握手属于正常建链过程，不主动调用重连。 */
            consecutiveScanningCount = 0;
            is_connected.store(false);
            dlog_info("[WiFi监控] WiFi正在关联或握手，不干预当前连接过程");
        }
        else {
            /* 未知状态打断连续SCANNING计数，避免跨状态累计造成误触发。 */
            consecutiveScanningCount = 0;
        }
        ...
        monitorLock.lock();
        /*
         * 普通状态每30秒检查一次；SCANNING防抖期间每2秒复查一次，
         * 避免连续3次确认耗时90秒。SCANNING达到阈值并触发重连后，
         * 计数会清零，失败重试仍保持30秒间隔。
         */
        const std::chrono::seconds monitorInterval =
            consecutiveScanningCount > 0 ? scanningConfirmInterval : normalMonitorInterval;
