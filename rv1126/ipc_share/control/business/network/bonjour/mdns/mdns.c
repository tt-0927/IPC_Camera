			   ...
			   size_t record_length, void *user_data)
{
	...
	if (rtype == MDNS_RECORDTYPE_PTR)
	{
		...
		dlog_info("%.*s : %s %.*s PTR %.*s rclass 0x%x ttl %u length %d\n",
	...
	else if (rtype == MDNS_RECORDTYPE_SRV)
	{
		mdns_record_srv_t srv = mdns_record_parse_srv(data, size, record_offset, record_length,
													  namebuffer, sizeof(namebuffer));
		dlog_info("%.*s : %s %.*s SRV %.*s priority %d weight %d port %d\n",
			   MDNS_STRING_FORMAT(fromaddrstr), entrytype, MDNS_STRING_FORMAT(entrystr),
	...
	{
		dlog_info("%.*s : %s %.*s type %u rclass 0x%x ttl %u length %d\n",
...
open_client_sockets(int *sockets, int max_sockets, int port)
{
	...
	{
		free(adapter_address);
		dlog_error("Failed to get network adapter addresses");
		return num_sockets;
	}
			...
			if (unicast->Address.lpSockaddr->sa_family == AF_INET)
			{
					...
					(saddr->sin_addr.S_un.S_un_b.s_b4 != 1))
				{
					...
					if (log_addr)
					{
						...
						dlog_info("Local IPv4 address: %.*s\n", MDNS_STRING_FORMAT(addr));
					}
				}
			}
			else if (unicast->Address.lpSockaddr->sa_family == AF_INET6)
			{
					...
					memcmp(saddr->sin6_addr.s6_addr, localhost_mapped, 16))
				{
					...
					if (log_addr)
					{
						...
						dlog_info("Local IPv6 address: %.*s\n", MDNS_STRING_FORMAT(addr));
					}
				}
			}
		}
	...
	if (getifaddrs(&ifaddr) < 0)
		dlog_info("Unable to get interface addresses");
...
send_dns_sd(void)
{
	...
	if (num_sockets <= 0)
	{
		dlog_error("Failed to open any client sockets");
		return -1;
	}
	dlog_info("Opened %d socket%s for DNS-SD\n", num_sockets, num_sockets > 1 ? "s" : "");

	dlog_info("Sending DNS-SD discovery");
	for (int isock = 0; isock < num_sockets; ++isock)
	{
		if (mdns_discovery_send(sockets[isock]))
			dlog_error("Failed to send DNS-DS discovery: %s\n", strerror(errno));
	}
	...
	int res;
	dlog_info("Reading DNS-SD replies");
	do
	{
...
send_mdns_query(mdns_query_t *query, size_t count)
{
	...
	dlog_info("Opened %d socket%s for mDNS query\n", num_sockets, num_sockets > 1 ? "s" : "");

	size_t capacity = 2048;
	void *buffer = malloc(capacity);
	void *user_data = 0;

	dlog_info("Sending mDNS query");
	for (size_t iq = 0; iq < count; ++iq)
	{
		...
		dlog_info(" : %s %s", query[iq].name, record_name);
	}
	dlog_info("");
	for (int isock = 0; isock < num_sockets; ++isock)
	{
		...
		if (query_id[isock] < 0)
			dlog_error("Failed to send mDNS query: %s\n", strerror(errno));
	}

	// This is a simple implementation that loops for 5 seconds or as long as we get replies
	int res;
	dlog_info("Reading mDNS query replies");
	int records = 0;
	do
	{
		struct timeval timeout;
...
{
	int sockets[MDNS_SOCKETS_COUNT];
	...
	dlog_info("Opened %d socket%s for mDNS service\n", num_sockets, num_sockets > 1 ? "s" : "");

	size_t service_name_length = strlen(service_name);
	if (!service_name_length)
	{
		dlog_info("Invalid service name");
		return -1;
	}
	...
	dlog_info("Service mDNS: %s:%d\n", service_name, service_port);
	dlog_info("Hostname: %s\n", hostname);
...
dump_mdns(void)
{
	...
	dlog_info("Opened %d socket%s for mDNS dump\n", num_sockets, num_sockets > 1 ? "s" : "");
...
#define MAX_PASSES (1024 * 1024 * 1024)
	...
	for (int ipass = 0; ipass < MAX_PASSES; ++ipass)
	{
		...
		if (ipass && !(ipass % 10000))
			dlog_info("Completed fuzzing pass %d\n", ipass);
	}
...
int main(int argc, const char *const *argv)
{
	...
	{
		dlog_error("Failed to initialize WinSock");
		return -1;
	}
