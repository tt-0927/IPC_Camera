...
void CPlatformManager::on_mqtt_message(const std::string &strTopic, const std::string &strPayload)
{
    ...
    dlog_info("MQTT 原始报文：%s", strPayload.c_str());
...
void CPlatformManager::process_mqtt_command(const std::string &strTopic, const std::string &strPayload)
{
    ...
    /* 记录设备实际收到的 Data，便于核对平台发送字段是否在 MQTT 链路中丢失。 */
    if (normalize_mqtt_command(strCommand) == "NET_TV_ADD_FACE_INFO" ||
        normalize_mqtt_command(strCommand) == "NET_TV_SET_FACE_INFO")
    {
        dlog_info("MQTT 人脸命令实际 Data：%s", strData.c_str());
    }
...
bool CPlatformManager::ensure_face_nv21_local(cJSON *pData, std::string &strError)
{
    ...
    std::string strNv21Url;
    std::string strPicPath;
    get_json_string(pData, "Nv21Url", strNv21Url);
    get_json_string(pData, "PicPath", strPicPath);
    const std::string strDownloadUrl = strNv21Url;

    /* 记录字段与最终选择结果，禁止将 JPG 的 PicPath 误用为 NV21 下载地址。 */
    dlog_info("人脸 NV21 下载源：Nv21Url[%s] PicPath[%s] selected[%s]",
              strNv21Url.c_str(), strPicPath.c_str(), strDownloadUrl.c_str());
    if (strNv21Url.empty())
    {
        strError = "Nv21Url is empty";
        return false;
    }
