# 库路径
set(PUGIXML_LIBRARY_DIR "${CMAKE_CURRENT_LIST_DIR}/lib")
set(PUGIXML_INCLUDE_DIR "${CMAKE_CURRENT_LIST_DIR}/include")

link_directories(${PUGIXML_LIBRARY_DIR})

# 使用 file(GLOB ...) 查找所有库文件
file(GLOB PUGIXML_LIBRARY
    "${PUGIXML_LIBRARY_DIR}/lib*.so*"
)

# 创建一个命名的 target（目标），方便在 CMake 中使用
add_library(libpugixml INTERFACE)
target_link_libraries(libpugixml INTERFACE ${PUGIXML_LIBRARY})
target_include_directories(libpugixml INTERFACE ${PUGIXML_INCLUDE_DIR})

