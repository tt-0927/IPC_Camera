...
public:
    ...
    static RTSPServer* createNew(UsageEnvironment& env,
                                 ...
                                 Authenticator::AuthAlgorithm authAlgorithm = Authenticator::ALGO_MD5); // 添加算法参数
    // info ------------------ zhouzr ------------------
    ...
    // 'Register' the stream represented by "serverMediaSession" with the given remote client (specifed by name and port
    // number). This is done using our custom "REGISTER" RTSP command. The function returns a unique number that can be
    // used to identify the request; this number is also passed to "responseHandler". When a response is received from
    // the remote client (or the "REGISTER" request fails), the specified response handler
    ...
    // If "receiveOurStreamViaTCP" is True, then we're requesting that the remote client access our stream using
    // RTP/RTCP-over-TCP.
    ...
    //   It tells the proxy server the suffix that it should use in its "rtsp://" URL (when front-end clients access the
    //   stream)

    // info ------------------ zhouzr ------------------
    /* 客户端连接握手认证回调函数 */
    HandshakeAuthCallback handshakeAuthCallback = nullptr;
    // info ------------------ zhouzr ------------------
...
public:
    ...
    // Changes the server's authentication database to "newDB", returning a pointer to the old database (if there was
    // one). "newDB" may be NULL (you can use this to disable authentication at runtime, if desired).
    ...
    // Returns True iff the specified port can be used in this way (i.e., it's not already being used for a separate
    // HTTP server). Note: RTSP-over-HTTP tunneling is described in
    ...
    void setTLSState(char const* certFileName,
...
protected:
    ...
    RTSPServer(UsageEnvironment& env,
               ...
               Authenticator::AuthAlgorithm authAlgorithm); // 添加算法参数
...
public: // should be protected, but some old compilers complain otherwise
    ...
    protected:
        ...
        // You probably won't need to subclass/reimplement this function; reimplement
        // "RTSPServer::allowedCommandNames()" instead.
    ...
    // The state of an individual client session (using one or more sequential TCP connections) handled by a RTSP
    // server:
...
private:
    ...
    HashTable*
        fClientConnectionsForHTTPTunneling; // maps client-supplied 'session cookie' strings to "RTSPClientConnection"s
    ...
    // info ------------------ zhouzr ------------------
    Authenticator::AuthAlgorithm fAuthAlgorithm; // 在服务器级别存储算法
                                                 // info ------------------ zhouzr ------------------

    // info ------------------ lianghy ------------------
public:
    std::map<std::string, std::list<RTSPClientConnection*>> fClientConnectionsByIP;
    void registerClientConnection(const char* clientIP, RTSPClientConnection* conn);
    void unregisterClientConnection(const char* clientIP, RTSPClientConnection* conn);
    void closeConnectionsByIP(const char* targetIP);
    // info ------------------ lianghy ------------------
};
...
public:
    ...
    static RTSPServerWithREGISTERProxying* createNew(
        ...
        Authenticator::AuthAlgorithm authAlgorithm = Authenticator::ALGO_MD5);

protected:
    RTSPServerWithREGISTERProxying(UsageEnvironment& env,
                                   int ourSocketIPv4,
                                   int ourSocketIPv6,
                                   Port ourPort,
                                   UserAuthenticationDatabase* authDatabase,
