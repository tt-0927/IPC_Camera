...
/**
 * @brief V2 告警回调函数。
 * @details 对基础、规则、AI、人脸比对、车牌和统计告警，pAlarmInfo 指向对应的
 *          NET_TV_ALARM_*_V2_S，图片通过 NET_TV_IMAGE_DATA_S 的 pData/dwLen 提供。
 *          图片内存仅在回调执行期间有效；异步使用时，调用方必须自行复制。
 */
typedef void(STDCALL *NET_TV_AlarmCallBackV2)(OUT INT64 lCommand,
                                              OUT NET_TV_ALARMER_S *pAlarmer,
                                              OUT CHAR* pAlarmInfo,
                                              OUT INT32* dwBufLen,
                                              OUT LPVOID lpUserData);
 ...
 * @note 对已定义 V2 结构的告警优先调用该回调；其它历史命令仍可由 V1 回调承接。
 */
