...
/*
 * V2 告警图片仅在回调期间借用 vector 的数据，避免恢复为 V1 的固定大数组。
 * Base64 解析失败时保留告警元数据并将该图片置空，防止单张坏图中断监听线程。
 */
void DecodeAlarmImage(Json::Object* pAlarmInfo,
                      ...
                      NET_TV_IMAGE_DATA_S& stImage)
{
    ...
    if (!pAlarmInfo || !Json::get(pAlarmInfo, pszJsonField, strBase64) || strBase64.empty())
    {
        return;
    }
    ...
    {
        NSDK_LOG_WARN("[DIAG-ALARM] V2 image Base64 decode failed, field=%s", pszJsonField);
        return;
    }

    if (vecDecoded.size() > static_cast<size_t>(std::numeric_limits<UINT32>::max()))
    {
        NSDK_LOG_WARN("[DIAG-ALARM] V2 image exceeds UINT32 length, field=%s, size=%zu",
                      pszJsonField,
                      vecDecoded.size());
        return;
    }

    vecImage.swap(vecDecoded);
    if (!vecImage.empty())
    {
        stImage.pData = vecImage.data();
        stImage.dwLen = static_cast<UINT32>(vecImage.size());
    }
}

template <typename T>
void InvokeAlarmCallbackV2(NET_TV_AlarmCallBackV2 cb,
                           INT64 lCommand,
                           NET_TV_ALARMER_S& stAlarmer,
                           T& stAlarmInfo,
                           void* pUserData)
{
    INT32 nBufLen = static_cast<INT32>(sizeof(T));
    cb(lCommand,
       &stAlarmer,
...
/* 返回 true 表示该命令已按 V2 动态图片结构分发。 */
bool DispatchAlarmV2(INT64 lCommand,
                     Json::Object* pAlarmInfo,
                     NET_TV_ALARMER_S& stAlarmer,
                     NET_TV_AlarmCallBackV2 cb,
                     void* pUserData)
{
    if (!pAlarmInfo || !cb)
    {
    ...
    if (lCommand == NET_TV_ALARM_FACE_COMPARE)
    {
        ...
        std::vector<BYTE> vecLibFaceImage;
        std::vector<BYTE> vecCapFaceImage;
        SDKConvert::deal(pAlarmInfo, stInfo, true);
        DecodeAlarmImage(pAlarmInfo, "LibFaceImgBase64", vecLibFaceImage, stInfo.stLibFaceImg);
        DecodeAlarmImage(pAlarmInfo, "CapFaceImgBase64", vecCapFaceImage, stInfo.stCapFaceImg);
        InvokeAlarmCallbackV2(cb, lCommand, stAlarmer, stInfo, pUserData);
        return true;
    }

    if (nAlarmBase == NET_TV_ALARM_BASE_BASIC)
    {
        NET_TV_ALARM_BASIC_INFO_V2_S stInfo = {};
        std::vector<BYTE> vecPanoramaImage;
        SDKConvert::deal(pAlarmInfo, stInfo, true);
        DecodeAlarmImage(pAlarmInfo, "PanoramaImgBase64", vecPanoramaImage, stInfo.stPanoramaImg);
        InvokeAlarmCallbackV2(cb, lCommand, stAlarmer, stInfo, pUserData);
        return true;
    }

    if (nAlarmBase == NET_TV_ALARM_BASE_RULE)
    {
        NET_TV_ALARM_RULE_INFO_V2_S stInfo = {};
        std::vector<BYTE> vecPanoramaImage;
        std::vector<BYTE> vecTargetImage;
        SDKConvert::deal(pAlarmInfo, stInfo, true);
        DecodeAlarmImage(pAlarmInfo, "PanoramaImgBase64", vecPanoramaImage, stInfo.stPanoramaImg);
        DecodeAlarmImage(pAlarmInfo, "TargetImgBase64", vecTargetImage, stInfo.stTargetImg);
        InvokeAlarmCallbackV2(cb, lCommand, stAlarmer, stInfo, pUserData);
        return true;
    }

    if (nAlarmBase == NET_TV_ALARM_BASE_AI)
    {
        ...
        SDKConvert::deal(pAlarmInfo, stInfo, true);
        DecodeAlarmImage(pAlarmInfo, "PanoramaImgBase64", vecPanoramaImage, stInfo.stPanoramaImg);
        DecodeAlarmImage(pAlarmInfo, "ImgDataBase64", vecTargetImage, stInfo.stImgData);
        InvokeAlarmCallbackV2(cb, lCommand, stAlarmer, stInfo, pUserData);
        return true;
    }

    if (nAlarmBase == NET_TV_ALARM_BASE_TRAFFIC)
    {
        NET_TV_ALARM_PLATE_INFO_V2_S stInfo = {};
        std::vector<BYTE> vecPlateImage;
        SDKConvert::deal(pAlarmInfo, stInfo, true);
        DecodeAlarmImage(pAlarmInfo, "PlateImgBase64", vecPlateImage, stInfo.stPlateImg);
        InvokeAlarmCallbackV2(cb, lCommand, stAlarmer, stInfo, pUserData);
        return true;
    }

    if (nAlarmBase == NET_TV_ALARM_BASE_STATISTICS)
    {
        NET_TV_ALARM_STATISTICS_INFO_V2_S stInfo = {};
        std::vector<BYTE> vecPanoramaImage;
        std::vector<BYTE> vecTargetImages[NET_TV_ALARM_STATISTICS_TARGET_MAX_NUM];
        SDKConvert::deal(pAlarmInfo, stInfo, true);

        Json::Object* pTargets = Json::get(pAlarmInfo, "Targets");
        const UINT32 nTargetCount = std::min<UINT32>(
            stInfo.dwTargetCount,
            NET_TV_ALARM_STATISTICS_TARGET_MAX_NUM);
        for (UINT32 i = 0; pTargets && i < nTargetCount; ++i)
        {
            Json::Object* pTarget = Json::Array::get(pTargets, static_cast<int>(i));
            DecodeAlarmImage(pTarget,
                             "ImgDataBase64",
                             vecTargetImages[i],
                             stInfo.stTargets[i].stImgData);
        }
        DecodeAlarmImage(pAlarmInfo, "PanoramaImgBase64", vecPanoramaImage, stInfo.stPanoramaImg);
        InvokeAlarmCallbackV2(cb, lCommand, stAlarmer, stInfo, pUserData);
        return true;
...
void CClientAlarmManager::AlarmLoop()
{
    ...
    auto dispatch_alarm = [&](const std::string& jsonBody) -> bool {
        ...
        if (alarmCbV2_ && DispatchAlarmV2(lCommand,
                                           alarmInfoObj,
                                           alarmer,
                                           alarmCbV2_,
                                           alarmUserDataV2_))
        ...
        // V2 仅覆盖动态图片告警；其它历史命令继续由 V1 回调承接。
        if (!alarmCb_)
        {
            Json::deinit(root);
            return true;
        ...
        {
            auto pCaptureInfo = std::make_unique<NET_FaceCapturePushInfo_S>();
            SDKConvert::deal(alarmInfoObj, *pCaptureInfo, true);
            INT32 nLength = (INT32)sizeof(*pCaptureInfo);
            alarmCb_(lCommand, &alarmer, (CHAR*)pCaptureInfo.get(), &nLength, alarmUserData_);
        }
        else if (lCommand == NET_PUSH_PERSON_CAPTURE_INFO)
        {
            auto pCaptureInfo = std::make_unique<NET_PersonCapturePushInfo_S>();
            SDKConvert::deal(alarmInfoObj, *pCaptureInfo, true);
            INT32 nLength = (INT32)sizeof(*pCaptureInfo);
            alarmCb_(lCommand, &alarmer, (CHAR*)pCaptureInfo.get(), &nLength, alarmUserData_);
        }
        ...
        {
            auto pCaptureInfo = std::make_unique<NET_MotorvehicleCapturePushInfo_S>();
            SDKConvert::deal(alarmInfoObj, *pCaptureInfo, true);
            INT32 nLength = (INT32)sizeof(*pCaptureInfo);
            alarmCb_(lCommand, &alarmer, (CHAR*)pCaptureInfo.get(), &nLength, alarmUserData_);
        }
        ...
        {
            auto pCaptureInfo = std::make_unique<NET_NonMotorvehicleCapturePushInfo_S>();
            SDKConvert::deal(alarmInfoObj, *pCaptureInfo, true);
            INT32 nLength = (INT32)sizeof(*pCaptureInfo);
            alarmCb_(lCommand, &alarmer, (CHAR*)pCaptureInfo.get(), &nLength, alarmUserData_);
        }
