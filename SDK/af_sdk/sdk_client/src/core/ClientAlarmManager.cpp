 ...
 *
 * @brief ClientAlarmManager 模块实现
 * 功能说明：
 * 1. 实现 ClientAlarmManager 模块核心逻辑
 * 2. 校验输入参数并管理模块资源生命周期
 * 3. 向上层提供可复用的 SDK 能力
 */
...
/**
 * @author tianl (tianl@kfb.cn)
 * @brief 执行 ContainsNoCase 定义的内部处理。
 * @param [in] haystack 函数处理参数。
 * @param [in] needle 函数处理参数。
 * @return 返回该处理的状态或结果。
 */
...
/**
 * @author tianl (tianl@kfb.cn)
 * @brief 查询或校验 GetMultipartHeaderValue 对应的数据。
 * @return 返回该处理的状态或结果。
 */
...
/**
 * @author tianl (tianl@kfb.cn)
 * @brief 查询或校验 ParseContentLength 对应的数据。
 * @param [in] headers 函数处理参数。
 * @param [in,out] contentLength 函数处理参数。
 * @return 返回该处理的状态或结果。
 */
...
/**
 * @author tianl (tianl@kfb.cn)
 * @brief 执行 TrimTrailingCrlf 定义的内部处理。
...
/**
 * @author tianl (tianl@kfb.cn)
 * @brief 查询或校验 IsCompleteJsonBody 对应的数据。
 * @param [in] body 函数处理参数。
 * @return 返回该处理的状态或结果。
 */
...
/**
 * @author tianl (tianl@kfb.cn)
 * @brief 执行 StartListen 对应的处理。
 * @param [in,out] userHandle 函数处理参数。
...
void CClientAlarmManager::AlarmLoop()
{
    ...
    auto dispatch_alarm = [&](const std::string& jsonBody) -> bool {
        ...
        if (msgType == "heartbeat") {
            ...
            if (m_nReceivedAlarmCount > 0) {
                auto noAlarmSec = std::chrono::duration_cast<std::chrono::seconds>(
                    std::chrono::steady_clock::now() - m_stLastAlarmTime).count();
                if (noAlarmSec >= 300) {
                    NETSDK_LOG_MESSAGE_WARN("[DIAG-ALARM] User-%p ZOMBIE: no alarm for %llds (hb=%d, alarms=%d), force reconnect",
                                  m_hUser, (long long)noAlarmSec,
        ...
        {
            std::unique_ptr<NET_ALARM_FACE_COMPARE_INFO_S> info = std::make_unique<NET_ALARM_FACE_COMPARE_INFO_S>();
        ...
        {
            auto info = std::make_unique<NET_ALARM_RULE_INFO_S>();
            SDKConvert::deal(alarmInfoObj, *info, true);
        ...
        {
            auto info = std::make_unique<NET_ALARM_AI_OBJECT_INFO_S>();
            SDKConvert::deal(alarmInfoObj, *info, true);
        ...
        {
            NET_ALARM_EXCEPTION_INFO_S info = {0};
            SDKConvert::deal(alarmInfoObj, info, true);
            INT32 len = (INT32)sizeof(info);
            m_fnAlarmCallback(lCommand, &alarmer, (CHAR*)&info, &len, m_pAlarmUserData);
        }
        ...
        {
            auto info = std::make_unique<NET_ALARM_STATISTICS_INFO_S>();
            SDKConvert::deal(alarmInfoObj, *info, true);
            INT32 len = (INT32)sizeof(*info);
            m_fnAlarmCallback(lCommand, &alarmer, (CHAR*)info.get(), &len, m_pAlarmUserData);
        }
        ...
        {
            NET_RECORD_DOWNLOAD_PROGRESS_S info = {0};
            SDKConvert::deal(alarmInfoObj, info, true);
            INT32 len = (INT32)sizeof(info);
            m_fnAlarmCallback(lCommand, &alarmer, (CHAR*)&info, &len, m_pAlarmUserData);
        }
...
/**
 * @author tianl (tianl@kfb.cn)
 * @brief 执行 HealthMonitorLoop 定义的内部处理。
 * @return 无返回值。
 */
...
void CClientAlarmManager::HealthMonitorLoop()
{
    ...
    while (m_bHealthMonitorRunning.load()) {
        ...
        if (lastMs > 0 && elapsedSec > timeoutSec) {
            ...
            /*    Stop() 会 join(m_stHealthMonitorThread)，而此时正是 HealthMonitor 自己 */
            /*    在等待，导致线程等待自身——死锁。 */
            if (m_fnSessionExpiredCallback) {
                NETSDK_LOG_MESSAGE_INFO("[DIAG-ALARM] User-%p [HEALTH] Dispatching m_fnSessionExpiredCallback async",
                              m_hUser);
                SessionExpiredCallback cb = m_fnSessionExpiredCallback; /* 值拷贝，防止 this 提前析构 */
