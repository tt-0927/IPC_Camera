...
{
	NETSDK_LOG_MESSAGE_INFO("~CSessionManager() ");
    Cleanup();
}
 ...
 */
LPUSER_HANDLE CSessionManager::Login(const std::string& host, int port,
 ...
 */
bool CSessionManager::Logout(LPUSER_HANDLE pHandle)
{
 ...
 */
int CSessionManager::GenerateUserId()
{
 ...
 */
LPUSER_HANDLE CSessionManager::GenerateHandle()
{
...
std::shared_ptr<CUserSession> CSessionManager::GetSession(LPUSER_HANDLE pHandle)
{
	...
	auto it = m_stSessions.find(pHandle);
    if (it != m_stSessions.end()) {
        NETSDK_LOG_MESSAGE_INFO("[SessionManager] Session found for handle=%p", pHandle);
        return it->second;
    } else {
        NETSDK_LOG_MESSAGE_WARN("[SessionManager] Session NOT found for handle=%p", pHandle);
        return nullptr;
    }
}
 ...
 */
void CSessionManager::SetGlobalRevTimeout(int timeoutSec)
{
 ...
 */
void CSessionManager::SetGlobalConnectTime(int waitTime, int tryTimes)
{
 ...
 */
void CSessionManager::OnSessionLost(LPUSER_HANDLE pHandle)
{
    /* 仅记录日志，不删除 session */
