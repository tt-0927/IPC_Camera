...
static void PrintMenu()
{
    ...
    printf("184 - 获取人脸抓拍叠加配置 (NET_TV_GET_FACECAPTUREOVERLAYINFO)\n");
    printf("185 - 设置人脸抓拍叠加配置 (NET_TV_SET_FACECAPTUREOVERLAYINFO)\n");
...
static void PrintVoiceComAudioCfg(const NET_TV_VOICECOM_AUDIO_CFG_S* pCfg)
{
    if (!pCfg)
    {
        return;
...
{
    NET_TV_VOICECOM_AUDIO_CFG_S stCfg;
    memset(&stCfg, 0, sizeof(stCfg));
...
static void DoSetVoiceComAudioCfg()
{
    ...
    stCfg.enFormat = NET_TV_AUDIO_FORMAT_AAC;
    stCfg.dwSampleRate = 16000;
    stCfg.dwBitDepth = 16;
    stCfg.dwChannels = 1;
    stCfg.dwFrameIntervalMs = 20;
    stCfg.dwFrameBytes = 640;
    stCfg.dwBitRate = 64000;
...
static void PrintAlarmSchedule(const NET_AlarmSchedule_S* pSchedule)
{
    ...
    for (INT32 nDay = 0; nDay < NET_ALARM_SCHEDULE_DAY_COUNT; ++nDay)
    {
        INT32 nSectionCount = pSchedule->dwTimeSectionCount[nDay];
        if (nSectionCount < 0)
        {
            nSectionCount = 0;
        }
        if (nSectionCount > NET_TV_PLAN_SECTION_NUM)
        {
            nSectionCount = NET_TV_PLAN_SECTION_NUM;
        }
        ...
        {
            const NET_TV_SCHED_TIME_S* pTime = &pSchedule->astTimeSection[nDay][nSection];
...
static void PrintAlarmLinkage(const NET_LinkageList_S* pLinkage)
{
    ...
    printf("    联动报警输出=%d, 联动录像=%d, 联动抓拍=%d\n",
           pLinkage->dwAlarmOutputCount,
           pLinkage->dwRecordChannelCount,
           pLinkage->dwSnapshotChannelCount);
}
...
static void DoGetAudibleAlarmInfo()
{
    ...
    if (NET_TV_GetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_GET_AUDIBLE_ALARM_INFO,
...
static void DoSetAudibleAlarmInfo()
{
    ...
    nBytesReturned = 0;
    if (NET_TV_SetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_SET_AUDIBLE_ALARM_INFO,
                            &stInfo, (INT32)sizeof(stInfo), &nBytesReturned))
    {
...
static void PrintAlarmInputInfoList(const NET_AlarmInputInfoList_S* pInfo)
{
    ...
    {
        nInputCount = NET_TV_MAX_ALARM_IN_NUM;
    }
...
static void DoGetAlarmInputInfo()
{
    ...
    if (NET_TV_GetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_GET_ALARM_INPUT_INFO,
                            &stInfo, (INT32)sizeof(stInfo), &nBytesReturned))
    {
...
static void DoSetAlarmInputInfo()
{
    ...
    nBytesReturned = 0;
    if (NET_TV_SetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_SET_ALARM_INPUT_INFO,
                            &stInfo, (INT32)sizeof(stInfo), &nBytesReturned))
    {
...
static void PrintAlarmOutputInfoList(const NET_AlarmOutputInfoList_S* pInfo)
{
    ...
    {
        nOutputCount = NET_TV_MAX_ALARM_OUT_NUM;
    }
...
static void DoGetAlarmOutputInfo()
{
    ...
    if (NET_TV_GetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_GET_ALARM_OUTPUT_INFO,
                            &stInfo, (INT32)sizeof(stInfo), &nBytesReturned))
    {
...
static void DoSetAlarmOutputInfo()
{
    ...
    nBytesReturned = 0;
    if (NET_TV_SetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_SET_ALARM_OUTPUT_INFO,
                            &stInfo, (INT32)sizeof(stInfo), &nBytesReturned))
    {
...
static void DoGetFlashingLightAlarmInfo()
{
    ...
    if (NET_TV_GetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_GET_FLASHING_LIGHT_ALARM_INFO,
                            &stInfo, (INT32)sizeof(stInfo), &nBytesReturned))
    {
...
static void DoSetFlashingLightAlarmInfo()
{
    ...
    nBytesReturned = 0;
    if (NET_TV_SetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_SET_FLASHING_LIGHT_ALARM_INFO,
                            &stInfo, (INT32)sizeof(stInfo), &nBytesReturned))
    {
...
static void DoGetPirAlarmInfo()
{
    ...
    if (NET_TV_GetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_GET_PIR_ALARM_INFO,
                            &stInfo, (INT32)sizeof(stInfo), &nBytesReturned))
    {
...
static void DoSetPirAlarmInfo()
{
    ...
    nBytesReturned = 0;
    if (NET_TV_SetDevConfig(g_lpUserID, DEMO_CONFIG_CHANNEL_ID, NET_SET_PIR_ALARM_INFO,
                            &stInfo, (INT32)sizeof(stInfo), &nBytesReturned))
    {
 ...
 * @brief 打印人脸抓拍图片叠加配置。
 * @author ITC
 * @param [in] pInfo 人脸抓拍叠加配置指针。
 * @param [out] 无。
 * @return 无。
 */
static void PrintFaceCaptureOverlayInfo(const NET_TV_FACE_CAPTURE_OVERLAY_INFO_S* pInfo)
{
    if (!pInfo)
    {
        return;
    ...
    printf("\n[Client] ===== 人脸抓拍叠加配置 =====\n");
    printf("  DeviceID                    : %d\n", pInfo->nDeviceID);
    printf("  MonitoryPointInfo           : %s\n", pInfo->strMonitoryPointInfo);
    printf("  OverlayDeviceID             : %s\n", pInfo->bOverlayDeviceID ? "ON" : "OFF");
    printf("  OverlayCaptureTime          : %s\n", pInfo->bOverlayCaptureTime ? "ON" : "OFF");
    printf("  OverlayMonitoryPointInfo    : %s\n", pInfo->bOverlayMonitoryPointInfo ? "ON" : "OFF");
    printf("  FontColor                   : %d\n", pInfo->enFontColor);
    printf("  CustomFontColor             : %s\n", pInfo->strFontColor);
    printf("=================================\n");
}
...
{
    NET_TV_FACE_CAPTURE_OVERLAY_INFO_S stInfo;
    INT32 nBytesReturned = 0;
    BOOL bResult = FALSE;

    memset(&stInfo, 0, sizeof(stInfo));
    bResult = NET_TV_GetDevConfig(g_lpUserID,
                                   1,
                                   NET_TV_GET_FACECAPTUREOVERLAYINFO,
                                   &stInfo,
                                   (INT32)sizeof(stInfo),
                                   &nBytesReturned);
    if (bResult)
    {
        printf("[Client] 获取人脸抓拍叠加配置成功，返回字节数=%d\n", nBytesReturned);
        PrintFaceCaptureOverlayInfo(&stInfo);
        return;
    }

    printf("[Client] 获取人脸抓拍叠加配置失败，错误码=%d\n", NET_TV_GetLastError());
}
...
static void DoSetFaceCaptureOverlayInfo()
{
    ...
    memset(&stInfo, 0, sizeof(stInfo));
    stInfo.nDeviceID = 1;
    strncpy(stInfo.strMonitoryPointInfo,
            "Demo monitoring point",
            sizeof(stInfo.strMonitoryPointInfo) - 1);
    stInfo.bOverlayDeviceID = TRUE;
    stInfo.bOverlayCaptureTime = TRUE;
    stInfo.bOverlayMonitoryPointInfo = TRUE;
    stInfo.enFontColor = NET_TV_OSD_COLOR_WHITE;
    strncpy(stInfo.strFontColor, "#FFFFFF", sizeof(stInfo.strFontColor) - 1);

    bResult = NET_TV_SetDevConfig(g_lpUserID,
                                   1,
    ...
    if (bResult)
    {
        printf("[Client] 设置人脸抓拍叠加配置成功，返回字节数=%d\n", nBytesReturned);
        PrintFaceCaptureOverlayInfo(&stInfo);
        return;
    }

    printf("[Client] 设置人脸抓拍叠加配置失败，错误码=%d\n", NET_TV_GetLastError());
}
