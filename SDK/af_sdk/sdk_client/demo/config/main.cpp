...
static void NET_STDCALL OnChannelStatus(NET_ChannelInfo_S *pInfo, LPVOID pUserData)
 {
    (void)pUserData;
    if (!pInfo) {
...
int main() {
    ...
    mkdir(logDir, 0755);
#endif
    if (!NET_SetLogToFile(0, (char*)logDir, 5 * 1024 * 1024, 10)) {
        printf("NET_SetLogToFile failed!\n");
    } else {
    ...
    if (!NET_SetChannelStatusCallBack(lpUserID, OnChannelStatus, NULL)) {
        printf("NET_SetChannelStatusCallBack failed!\n");
        NET_Logout(lpUserID);
        NET_Cleanup();
        return -1;
    }
