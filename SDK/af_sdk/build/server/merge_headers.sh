...
# 检查参数数量是否正确
if [ $# -ne 3 ]; then
    echo -e "\033[31m用法错误！正确用法：\033[0m $0 <NetTVSDKCommon.h路径> <NetTVSDKServerInterface.h路径> <目标输出目录>"
    echo -e "示例：$0 ./src/NetTVSDKCommon.h ./src/NetTVSDKServerInterface.h ./output"
    exit 1
fi
...
TARGET_DIR="$3"        # 目标输出目录
TARGET_H="${TARGET_DIR}/NetTVSDKServer.h"      # Linux 侧原始生成头文件
TARGET_ITC_H="${TARGET_DIR}/ItcNetTVSDKServer.h"  # 对外交付的 Itc 版本头文件
