...
public:
    ...
    /**
     * @brief 创建带 session 校验的 HTTP 回调处理器
     * @details 在 HTTP 认证通过后，额外从 URL 参数中提取 session_id，验证会话存在且处于登录状态。
     *          验证成功后会刷新会话的最近活跃时间，与 HttpCommandKeepAlive 的语义一致；
     *          验证失败返回与 Logout 一致的错误响应。
     * @param Func 业务处理函数
     * @return HTTP 回调函数
     */
    static std::function<void(const httplib::Request&, httplib::Response&)> MakeHttpCallbackHandlerWithSession(
        const std::function<std::string(const std::string&, const std::string&)>& Func)
    {
        return [Func](const httplib::Request& req, httplib::Response& res)
        {
            /* 1. HTTP 鉴权 */
            ...
            /* 2. 会话校验：参考 SessionManager::HttpCommandKeepAlive 语义 */
            std::string sessionId = req.get_param_value("session_id");
            auto session = CSessionManager::instance()->GetSession(sessionId);
            if (!session || !session->IsLogined())
            {
                NETSDK_LOG_MESSAGE_WARN("[CHttpBasicCommand] session invalid or not logined, sessionId=%s, url=%s",
                              sessionId.c_str(), req.target.c_str());
                res.status = NET_HTTP_RESP_CODE_SUCCESS;
