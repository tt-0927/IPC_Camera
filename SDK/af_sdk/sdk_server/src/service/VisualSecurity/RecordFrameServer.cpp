 ...
 */
NET_COMMON_ECODE_E RecordFrameServer::open_stream(const NET_RecordFrameStreamCond_S& cond,
 ...
 * @return 错误码，NET_E_SUCCEED表示成功，其他值表示失败
 */
NET_COMMON_ECODE_E RecordFrameServer::close_stream(const std::string& stream_id) {
    if (stream_id.empty()) {
...
bool RecordFrameServer::send_packet(socket_fd_t fd,
    ...
    header.uTimestamp = to_be32(frame_info.uTimestamp);
    header.uSsrc = to_be32(ssrc);
    header.uPayloadLen = to_be32(frame_info.uPayloadLen);
    header.uFlags = to_be32(frame_info.uFlags);
