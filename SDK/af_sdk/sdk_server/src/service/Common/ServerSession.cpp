...
void CServerSession::EnqueueMessage(const AlarmData_S& data)
{
    std::lock_guard<std::mutex> lock(m_stMutex);
...
bool CServerSession::DequeueMessage(AlarmData_S& outMsg)
{
    std::lock_guard<std::mutex> lock(m_stMutex);
    if (m_stMessageQueue.empty()) {
        return false;
    }
    outMsg = m_stMessageQueue.front();
