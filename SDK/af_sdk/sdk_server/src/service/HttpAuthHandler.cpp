...
// #define HTTP_AUTH_DEBUG 1
#define HTTP_AUTH_DEBUG 0
...
{
    NSDK_LOG_DEBUG("设置HTTP鉴权信息 realm： %s user：%s passwd:%s", realm.c_str(),user.c_str(),passwd.c_str());
...
{
    //NSDK_LOG_DEBUG("开始处理认证请求，客户端 IP: %s", req.remote_addr.c_str());
    NSDK_LOG_DEBUG("[%s] 开始处理认证请求，客户端 IP: %s", GetCurrentTimeString().c_str(), req.remote_addr.c_str());
    ...
    NSDK_LOG_DEBUG("检测到认证类型: %d, 客户端 IP: %s", static_cast<int>(auth_type), req.remote_addr.c_str());
    
    // 根据认证类型进行处理
    switch (auth_type) 
	{
        case AuthType::BASIC:
            NSDK_LOG_DEBUG("使用 Basic 认证，客户端 IP: %s", req.remote_addr.c_str());
            return handle_basic_auth(req, res);
            
        case AuthType::DIGEST:
            NSDK_LOG_DEBUG("使用 Digest 认证，客户端 IP: %s", req.remote_addr.c_str());
...
{
    NSDK_LOG_DEBUG("收到认证请求，客户端 IP: %s", req.remote_addr.c_str());
	...
	{
        NSDK_LOG_DEBUG("请求缺少 Authorization 头");
        send_challenge(res);
        return false;
    }
	...
	{
        NSDK_LOG_DEBUG("Authorization 头不是 Digest 类型: %s", auth_header.substr(0, 20).c_str());
        send_challenge(res);
        return false;
    }
	...
	{
        NSDK_LOG_ERROR("解析 Digest 头失败");
        send_challenge(res);
        return false;
    }
    
    NSDK_LOG_DEBUG("解析 Digest 参数: 用户=%s, realm=%s, uri=%s", 
               params.username.c_str(), params.realm.c_str(), params.uri.c_str());
    
    // 验证认证参数
    if (verify_digest_auth(req, params)) 
	{
        NSDK_LOG_INFO("[%s] Digest 认证成功: 用户=%s, 客户端 IP=%s", GetCurrentTimeString().c_str(), params.username.c_str(), req.remote_addr.c_str());
        //NSDK_LOG_INFO("Digest 认证成功: 用户=%s, 客户端 IP=%s", params.username.c_str(), req.remote_addr.c_str());
        return true;
    }
	else 
	{
        NSDK_LOG_INFO("[%s] Digest 认证失败: 用户=%s, 客户端 IP=%s", GetCurrentTimeString().c_str(), params.username.c_str(), req.remote_addr.c_str());
        //NSDK_LOG_WARN("Digest 认证失败: 用户=%s, 客户端 IP=%s", params.username.c_str(), req.remote_addr.c_str());
...
bool CHttpAuthHandler::handle_basic_auth(const httplib::Request& req, httplib::Response& res) 
{
    try {
		...
		{
            NSDK_LOG_DEBUG("请求缺少 Basic Authorization 头，客户端 IP: %s", req.remote_addr.c_str());
        ...
        if (colon_pos == std::string::npos) {
            NSDK_LOG_ERROR("Basic认证格式错误，客户端 IP: %s", req.remote_addr.c_str());
            send_basic_challenge(res);
            return false;
        }
        ...
        {
            NSDK_LOG_ERROR("Basic认证失败: 用户不存在=%s, 客户端 IP=%s", username.c_str(), req.remote_addr.c_str());
            send_basic_challenge(res);
            return false;
        }
        ...
        if (!authenticated) {
            NSDK_LOG_ERROR("Basic认证失败: 密码错误，用户=%s, 客户端 IP=%s", username.c_str(), req.remote_addr.c_str());
            send_basic_challenge(res);
            return false;
        }
        
        NSDK_LOG_INFO("Basic认证成功: 用户=%s, 客户端 IP=%s", username.c_str(), req.remote_addr.c_str());
        return authenticated;
        
    } catch (const std::exception& e) {
        NSDK_LOG_ERROR("Basic认证解析异常: %s, 客户端 IP=%s", e.what(), req.remote_addr.c_str());
        send_basic_challenge(res);
        return false;
    }
}
...
void CHttpAuthHandler::send_challenge(httplib::Response& res, const std::string& stale) 
{
    ...
    NSDK_LOG_DEBUG("发送 401 质询响应: nonce=%s, opaque=%s", nonce.c_str(), opaque.c_str());
}
...
void CHttpAuthHandler::send_basic_challenge(httplib::Response& res) 
{
    ...
    NSDK_LOG_DEBUG("发送 Basic Authentication 质询");
}
...
bool CHttpAuthHandler::verify_digest_auth(const httplib::Request& req, const DigestParams& params) 
{
    ...
    if (user_it == user_passwords_.end()) 
	{
        NSDK_LOG_ERROR("用户不存在: %s", params.username.c_str());
        return false;
    }
    
    // 2. 验证 nonce 和会话
    if (!verify_session(params.nonce)) {
        NSDK_LOG_ERROR("无效的 nonce: %s", params.nonce.c_str());
        return false;
    }
    ...
    if (params.response != expected_response) {
        NSDK_LOG_ERROR("Response 不匹配. 期望: %s, 实际: %s", 
    ...
    if (params.uri != req.path) {
        NSDK_LOG_WARN("URI 不匹配. 期望: %s, 实际: %s", req.path.c_str(), params.uri.c_str());
        // 在某些实现中这可能不是致命错误
    }
...
bool CHttpAuthHandler::verify_session(const std::string& nonce) {
    ...
    if (it == sessions_.end()) {
        NSDK_LOG_DEBUG("nonce 不存在: %s", nonce.c_str());
    ...
    { // 5 分钟
        sessions_.erase(it);
        NSDK_LOG_DEBUG("nonce 已过期: %s", nonce.c_str());
        return false;
    }
    
    // 更新最后活动时间
    it->second.last_active = now;
    NSDK_LOG_DEBUG("nonce 验证成功: %s", nonce.c_str());
    return true;
}

#define SESSION_CLEANUP_THRESHOLD 100
...
void CHttpAuthHandler::save_session(const std::string& nonce, const std::string& opaque) {
    ...
    NSDK_LOG_DEBUG("保存会话: nonce=%s, 当前会话数=%zu", nonce.c_str(), sessions_.size());
    
    
    if (sessions_.size() > SESSION_CLEANUP_THRESHOLD){
...
void CHttpAuthHandler::cleanup_expired_sessions() 
{
    ...
    if (sessions_.size() != initial_size) {
        NSDK_LOG_DEBUG("清理过期会话: 清理前=%zu, 清理后=%zu", initial_size, sessions_.size());
    }
}