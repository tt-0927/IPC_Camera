                               ...
                               INT32 dwBufLen)
{
    ...
    // 添加AlarmInfo（按命令码选择结构体）
    {
        ...
        {
            if (dwBufLen < (INT32)sizeof(NET_FaceCapturePushInfo_S))
            {
                Json::add(pInfoJson, "AlarmType", (long long)lCommand);
            }
            else
            {
                NET_FaceCapturePushInfo_S& stCaptureInfo = *(NET_FaceCapturePushInfo_S*)pAlarmInfo;
                SDKConvert::deal(pInfoJson, stCaptureInfo, false);
            }
        }
        else if (lCommand == NET_PUSH_PERSON_CAPTURE_INFO)
        {
            if (dwBufLen < (INT32)sizeof(NET_PersonCapturePushInfo_S))
            {
                Json::add(pInfoJson, "AlarmType", (long long)lCommand);
            }
            else
            {
                NET_PersonCapturePushInfo_S& stCaptureInfo = *(NET_PersonCapturePushInfo_S*)pAlarmInfo;
                SDKConvert::deal(pInfoJson, stCaptureInfo, false);
            }
        }
        else if (lCommand == NET_PUSH_MOTORVEHICLE_CAPTURE_INFO)
        {
            if (dwBufLen < (INT32)sizeof(NET_MotorvehicleCapturePushInfo_S))
            {
                Json::add(pInfoJson, "AlarmType", (long long)lCommand);
            }
            else
            {
                NET_MotorvehicleCapturePushInfo_S& stCaptureInfo = *(NET_MotorvehicleCapturePushInfo_S*)pAlarmInfo;
                SDKConvert::deal(pInfoJson, stCaptureInfo, false);
            }
        }
        else if (lCommand == NET_PUSH_NONMOTORVEHICLE_CAPTURE_INFO)
        {
            if (dwBufLen < (INT32)sizeof(NET_NonMotorvehicleCapturePushInfo_S))
            {
                Json::add(pInfoJson, "AlarmType", (long long)lCommand);
            }
            else
            {
                NET_NonMotorvehicleCapturePushInfo_S& stCaptureInfo = *(NET_NonMotorvehicleCapturePushInfo_S*)pAlarmInfo;
                SDKConvert::deal(pInfoJson, stCaptureInfo, false);
            }
        }
...
BOOL AlarmModule::PushAlarmInfoV2(NET_TV_ALARMER_S* pAlarmer,
                                  INT32 lCommand,
                                  LPVOID pAlarmInfo,
                                  INT32 dwBufLen)
{
    if (!pAlarmer || !pAlarmInfo || dwBufLen <= 0)
    {
        NSDK_LOG_ERROR("PushAlarmInfoV2: Invalid parameters");
        return FALSE;
    }
    ...
    if (!pRoot)
    {
        NSDK_LOG_ERROR("PushAlarmInfoV2: Failed to init JSON");
        return FALSE;
    }

    const auto now = std::chrono::steady_clock::now();
    const long long enqueueTs = std::chrono::duration_cast<std::chrono::milliseconds>(
        now.time_since_epoch()).count();
    Json::add(pRoot, "enqueue_ts", enqueueTs);
    Json::add(pRoot, "Command", static_cast<long long>(lCommand));
    {
        Json::Object* pAlarmerJson = Json::init();
        NET_TV_ALARMER_S alarmer = *pAlarmer;
        SDKConvert::deal(pAlarmerJson, alarmer, false);
        Json::add(pRoot, "Alarmer", pAlarmerJson);
    }
    ...
    if (lCommand == NET_TV_ALARM_FACE_COMPARE)
    {
        if (dwBufLen < static_cast<INT32>(sizeof(NET_TV_ALARM_FACE_COMPARE_INFO_V2_S)))
        {
            bSupported = false;
        }
        else
        {
            SDKConvert::deal(pInfoJson,
                             *static_cast<NET_TV_ALARM_FACE_COMPARE_INFO_V2_S*>(pAlarmInfo),
                             false);
        }
    }
    else if (nAlarmBase == NET_TV_ALARM_BASE_BASIC)
    {
        if (dwBufLen < static_cast<INT32>(sizeof(NET_TV_ALARM_BASIC_INFO_V2_S)))
        {
            bSupported = false;
        }
        else
        {
            SDKConvert::deal(pInfoJson,
                             *static_cast<NET_TV_ALARM_BASIC_INFO_V2_S*>(pAlarmInfo),
                             false);
        }
    }
    else if (nAlarmBase == NET_TV_ALARM_BASE_RULE)
    {
        if (dwBufLen < static_cast<INT32>(sizeof(NET_TV_ALARM_RULE_INFO_V2_S)))
        {
            bSupported = false;
        }
        else
        {
            SDKConvert::deal(pInfoJson,
                             *static_cast<NET_TV_ALARM_RULE_INFO_V2_S*>(pAlarmInfo),
                             false);
        }
    }
    else if (nAlarmBase == NET_TV_ALARM_BASE_AI)
    {
        if (dwBufLen < static_cast<INT32>(sizeof(NET_TV_ALARM_AI_OBJECT_INFO_V2_S)))
        {
            bSupported = false;
        }
        else
        {
            SDKConvert::deal(pInfoJson,
                             *static_cast<NET_TV_ALARM_AI_OBJECT_INFO_V2_S*>(pAlarmInfo),
                             false);
        }
    }
    else if (nAlarmBase == NET_TV_ALARM_BASE_TRAFFIC)
    {
        if (dwBufLen < static_cast<INT32>(sizeof(NET_TV_ALARM_PLATE_INFO_V2_S)))
        {
            bSupported = false;
        }
        else
        {
            SDKConvert::deal(pInfoJson,
                             *static_cast<NET_TV_ALARM_PLATE_INFO_V2_S*>(pAlarmInfo),
                             false);
        }
    }
    else if (nAlarmBase == NET_TV_ALARM_BASE_STATISTICS)
    {
        if (dwBufLen < static_cast<INT32>(sizeof(NET_TV_ALARM_STATISTICS_INFO_V2_S)))
        {
            bSupported = false;
        }
        else
        {
            SDKConvert::deal(pInfoJson,
                             *static_cast<NET_TV_ALARM_STATISTICS_INFO_V2_S*>(pAlarmInfo),
                             false);
        }
    }
    else
    ...
    if (!bSupported)
    {
        Json::deinit(pInfoJson);
        Json::deinit(pRoot);
        NSDK_LOG_ERROR("PushAlarmInfoV2: Unsupported command or invalid buffer, cmd=0x%x, bufLen=%d",
                       lCommand,
                       dwBufLen);
        return FALSE;
    }
    ...
    Json::add(pRoot, "AlarmInfo", pInfoJson);
    const std::string jsonStr = Json::to_string(pRoot);
    Json::deinit(pRoot);

    const size_t pushCount = CSessionManager::instance()->PushToAll(jsonStr);
    if (pushCount == 0)
    {
        NSDK_LOG_WARN("AlarmModule::PushAlarmInfoV2: No eligible clients, cmd=0x%x, device=%s, bufLen=%d",
                      lCommand,
                      pAlarmer->szDeviceIP,
                      dwBufLen);
    }
    ...
    {
        NSDK_LOG_INFO("AlarmModule::PushAlarmInfoV2: Forwarded to %zu client(s), cmd=0x%x, device=%s, jsonLen=%zu",
                      pushCount,
                      lCommand,
                      pAlarmer->szDeviceIP,
                      jsonStr.size());
