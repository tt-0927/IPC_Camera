...
static int ValidateRecordFrameCond(NET_RECORD_FRAME_STREAM_COND_S& stCond)
{
    if (stCond.dwSize == 0)
    {
        stCond.dwSize = sizeof(NET_RECORD_FRAME_STREAM_COND_S);
    }
    ...
    {
        stCond.dwMediaType = NET_RECORD_FRAME_MEDIA_VIDEO;
    }

    if (stCond.dwMediaType != NET_RECORD_FRAME_MEDIA_VIDEO &&
        stCond.dwMediaType != NET_RECORD_FRAME_MEDIA_AUDIO)
    {
        return NET_E_INVALID_PARAM;
    }
...
std::string CRecordFrameBusiness::StartRecordFrameStream(const std::string& req_data, const std::string& url_param)
{
    ...
    NET_RECORD_FRAME_STREAM_COND_S stCond;
    ...
    NET_RECORD_FRAME_STREAM_INFO_S stInfo;
    ...
    if (nRespCode == NET_E_SUCCEED && stInfo.dwTcpPort == 0)
    {
...
std::string CRecordFrameBusiness::StopRecordFrameStream(const std::string& req_data, const std::string& url_param)
{
    ...
    NET_RECORD_FRAME_STOP_INFO_S stInfo;
    std::memset(&stInfo, 0, sizeof(stInfo));
    ...
    if (stInfo.dwSize == 0)
    {
        stInfo.dwSize = sizeof(NET_RECORD_FRAME_STOP_INFO_S);
    }
