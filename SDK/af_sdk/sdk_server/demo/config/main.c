...
static NET_TV_FACE_CAPTURE_OVERLAY_INFO_S g_stFaceCaptureOverlayInfo;
...
static void ConfigDemoInitAlarmSchedule(NET_AlarmSchedule_S* pSchedule)
{
    ...
    {
        pSchedule->dwTimeSectionCount[nDay] = 1;
...
static void InitDefaultConfig(void)
{
    ...
    /* 人脸抓拍叠加信息默认值。 */
    g_stFaceCaptureOverlayInfo.nDeviceID = 1;
    strncpy(g_stFaceCaptureOverlayInfo.strMonitoryPointInfo,
            "Demo monitoring point",
            sizeof(g_stFaceCaptureOverlayInfo.strMonitoryPointInfo) - 1);
    g_stFaceCaptureOverlayInfo.bOverlayDeviceID = TRUE;
    g_stFaceCaptureOverlayInfo.bOverlayCaptureTime = TRUE;
    g_stFaceCaptureOverlayInfo.bOverlayMonitoryPointInfo = TRUE;
    g_stFaceCaptureOverlayInfo.enFontColor = NET_TV_OSD_COLOR_WHITE;
    strncpy(g_stFaceCaptureOverlayInfo.strFontColor,
            "#FFFFFF",
            sizeof(g_stFaceCaptureOverlayInfo.strFontColor) - 1);
...
/* OSD能力集配置 Get 回调，对应命令 NET_TV_GET_OSDCAPCFG */
...
/* OSD能力集配置 Set 回调，对应命令 NET_TV_SET_OSDCAPCFG */
static NET_TV_COMMON_ECODE_E MySetOSDCapCfgCb(INT32 dwChannelID, LPVOID lpInBuffer)
{
    (void)dwChannelID;
 ...
 */
static NET_TV_COMMON_ECODE_E ConfigDemoGetAudibleAlarmInfo(INT32 nChannelId, LPVOID pOutBuffer)
{
 ...
 * @return 成功返回 NET_TV_E_SUCCEED；参数为空返回 NET_TV_E_INVALID_PARAM。
 */
static NET_TV_COMMON_ECODE_E ConfigDemoSetAudibleAlarmInfo(INT32 nChannelId, LPVOID pInBuffer)
{
    pNET_AudibleAlarmInfo_S pAlarmInfo = NULL;
 ...
 * @return 成功返回 NET_TV_E_SUCCEED；参数为空返回 NET_TV_E_INVALID_PARAM。
 */
static NET_TV_COMMON_ECODE_E ConfigDemoGetAlarmInputInfo(INT32 nChannelId, LPVOID pOutBuffer)
{
 ...
 */
static NET_TV_COMMON_ECODE_E ConfigDemoSetAlarmInputInfo(INT32 nChannelId, LPVOID pInBuffer)
{
    ...
    if (nInputCount < 0 || nInputCount > NET_TV_MAX_ALARM_IN_NUM)
    {
        return NET_TV_E_INVALID_PARAM;
    }
    ...
    if (nInputCount >= NET_TV_MAX_ALARM_IN_NUM)
    {
        return NET_TV_E_INVALID_PARAM;
    }
 ...
 * @return 成功返回 NET_TV_E_SUCCEED；参数为空返回 NET_TV_E_INVALID_PARAM。
 */
static NET_TV_COMMON_ECODE_E ConfigDemoGetAlarmOutputInfo(INT32 nChannelId, LPVOID pOutBuffer)
{
 ...
 * @return 成功返回 NET_TV_E_SUCCEED；参数或通道数量异常返回 NET_TV_E_INVALID_PARAM。
 */
static NET_TV_COMMON_ECODE_E ConfigDemoSetAlarmOutputInfo(INT32 nChannelId, LPVOID pInBuffer)
{
    ...
    if (nOutputCount < 0 || nOutputCount > NET_TV_MAX_ALARM_OUT_NUM)
    {
        return NET_TV_E_INVALID_PARAM;
    }
    ...
    if (nOutputCount >= NET_TV_MAX_ALARM_OUT_NUM)
    {
        return NET_TV_E_INVALID_PARAM;
    }
 ...
 * @return 成功返回 NET_TV_E_SUCCEED；参数为空返回 NET_TV_E_INVALID_PARAM。
 */
static NET_TV_COMMON_ECODE_E ConfigDemoGetFlashingLightAlarmInfo(INT32 nChannelId, LPVOID pOutBuffer)
{
 ...
 * @return 成功返回 NET_TV_E_SUCCEED；参数为空返回 NET_TV_E_INVALID_PARAM。
 */
static NET_TV_COMMON_ECODE_E ConfigDemoSetFlashingLightAlarmInfo(INT32 nChannelId, LPVOID pInBuffer)
{
    pNET_FlashingLightAlarmInfo_S pAlarmInfo = NULL;
 ...
 * @return 成功返回 NET_TV_E_SUCCEED；参数为空返回 NET_TV_E_INVALID_PARAM。
 */
static NET_TV_COMMON_ECODE_E ConfigDemoGetPirAlarmInfo(INT32 nChannelId, LPVOID pOutBuffer)
{
 ...
 * @return 成功返回 NET_TV_E_SUCCEED；参数为空返回 NET_TV_E_INVALID_PARAM。
 */
static NET_TV_COMMON_ECODE_E ConfigDemoSetPirAlarmInfo(INT32 nChannelId, LPVOID pInBuffer)
{
    pNET_PirAlarmInfo_S pAlarmInfo = NULL;
 ...
 * @param [out] lpOutBuffer 人脸抓拍叠加配置输出缓冲区。
 * @return NET_TV_E_SUCCEED 表示成功，其他值表示参数异常。
 */
static NET_TV_COMMON_ECODE_E MyGetFaceCaptureOverlayInfoCb(INT32 dwChannelID, LPVOID lpOutBuffer)
{
    (void)dwChannelID;
    ...
    LPNET_TV_FACE_CAPTURE_OVERLAY_INFO_S pOut = (LPNET_TV_FACE_CAPTURE_OVERLAY_INFO_S)lpOutBuffer;
    *pOut = g_stFaceCaptureOverlayInfo;

    printf("[ConfigServerDemo] GetFaceCaptureOverlayInfo callback, Channel=%d\n", dwChannelID);
    printf("  DeviceID=%d, DeviceIDOverlay=%d, CaptureTimeOverlay=%d, PointOverlay=%d\n",
           g_stFaceCaptureOverlayInfo.nDeviceID,
           g_stFaceCaptureOverlayInfo.bOverlayDeviceID,
           g_stFaceCaptureOverlayInfo.bOverlayCaptureTime,
           g_stFaceCaptureOverlayInfo.bOverlayMonitoryPointInfo);
 ...
 * @author ITC
 * @param [in] dwChannelID 通道编号。
 * @param [in] lpInBuffer 人脸抓拍叠加配置输入缓冲区。
 * @return NET_TV_E_SUCCEED 表示成功，其他值表示参数异常。
 */
static NET_TV_COMMON_ECODE_E MySetFaceCaptureOverlayInfoCb(INT32 dwChannelID, LPVOID lpInBuffer)
{
    (void)dwChannelID;
    ...
    LPNET_TV_FACE_CAPTURE_OVERLAY_INFO_S pIn = (LPNET_TV_FACE_CAPTURE_OVERLAY_INFO_S)lpInBuffer;
    g_stFaceCaptureOverlayInfo = *pIn;

    printf("[ConfigServerDemo] SetFaceCaptureOverlayInfo callback, Channel=%d\n", dwChannelID);
    printf("  DeviceID=%d, DeviceIDOverlay=%d, CaptureTimeOverlay=%d, PointOverlay=%d\n",
           g_stFaceCaptureOverlayInfo.nDeviceID,
           g_stFaceCaptureOverlayInfo.bOverlayDeviceID,
           g_stFaceCaptureOverlayInfo.bOverlayCaptureTime,
...
static void RegisterCallbacks(void)
{
    ...
    if (NET_TV_SERVER_RegisterCb_SetOsdCapCfg(MySetOSDCapCfgCb))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_GetAudibleAlarmInfo(ConfigDemoGetAudibleAlarmInfo))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_SetAudibleAlarmInfo(ConfigDemoSetAudibleAlarmInfo))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_GetAlarmInputInfo(ConfigDemoGetAlarmInputInfo))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_SetAlarmInputInfo(ConfigDemoSetAlarmInputInfo))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_GetAlarmOutputInfo(ConfigDemoGetAlarmOutputInfo))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_SetAlarmOutputInfo(ConfigDemoSetAlarmOutputInfo))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_GetFlashingLightAlarmInfo(ConfigDemoGetFlashingLightAlarmInfo))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_SetFlashingLightAlarmInfo(ConfigDemoSetFlashingLightAlarmInfo))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_GetPirAlarmInfo(ConfigDemoGetPirAlarmInfo))
    {
    ...
    if (NET_TV_SERVER_RegisterCb_SetPirAlarmInfo(ConfigDemoSetPirAlarmInfo))
    {
