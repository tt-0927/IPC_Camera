 ...
 * @return 无。
 */
void PushFaceCaptureInfo()
{
    NET_TV_ALARMER_S stAlarmer = {0};
    static NET_FaceCapturePushInfo_S s_stCaptureInfo;

    InitAlarmInfo(&stAlarmer, "Camera-Face", "192.168.1.105", "SN202312120006", NULL);
    memset(&s_stCaptureInfo, 0, sizeof(s_stCaptureInfo));
    s_stCaptureInfo.bMale = TRUE;
    s_stCaptureInfo.nAgeLabel = 2;
    s_stCaptureInfo.bGlasses = FALSE;
    s_stCaptureInfo.bBeard = FALSE;
    s_stCaptureInfo.bMask = FALSE;
    s_stCaptureInfo.nEmotionLabel = 8;
    s_stCaptureInfo.stFaceRegion.uPointCount = 4;
    s_stCaptureInfo.stFaceRegion.afPointX[0] = 0.20f;
    s_stCaptureInfo.stFaceRegion.afPointY[0] = 0.20f;
    s_stCaptureInfo.stFaceRegion.afPointX[1] = 0.40f;
    s_stCaptureInfo.stFaceRegion.afPointY[1] = 0.20f;
    s_stCaptureInfo.stFaceRegion.afPointX[2] = 0.40f;
    s_stCaptureInfo.stFaceRegion.afPointY[2] = 0.50f;
    s_stCaptureInfo.stFaceRegion.afPointX[3] = 0.20f;
    s_stCaptureInfo.stFaceRegion.afPointY[3] = 0.50f;
    strncpy(s_stCaptureInfo.strTimestamp, "2026-08-04 16:00:00", sizeof(s_stCaptureInfo.strTimestamp) - 1);

    if (NET_SERVER_PushFaceCaptureInfo(&stAlarmer, &s_stCaptureInfo))
    {
        printf("[Demo] Face capture info pushed successfully!\n");
    }
    else
    {
        printf("[Demo] Failed to push face capture info!\n");
    }
}
 ...
 * @return 无。
 */
void PushPersonCaptureInfo()
{
    NET_TV_ALARMER_S stAlarmer = {0};
    static NET_PersonCapturePushInfo_S s_stCaptureInfo;

    InitAlarmInfo(&stAlarmer, "Camera-Person", "192.168.1.106", "SN202312120007", NULL);
    memset(&s_stCaptureInfo, 0, sizeof(s_stCaptureInfo));
    s_stCaptureInfo.bMale = TRUE;
    s_stCaptureInfo.nAgeLabel = 3;
    s_stCaptureInfo.bBag = TRUE;
    s_stCaptureInfo.nTopColorLabel = 2;
    s_stCaptureInfo.nBottomColorLabel = 6;
    strncpy(s_stCaptureInfo.strTimestamp, "2026-08-04 16:00:01", sizeof(s_stCaptureInfo.strTimestamp) - 1);

    if (NET_SERVER_PushPersonCaptureInfo(&stAlarmer, &s_stCaptureInfo))
    {
        printf("[Demo] Person capture info pushed successfully!\n");
    }
    else
    {
        printf("[Demo] Failed to push person capture info!\n");
    }
}
 ...
 * @return 无。
 */
void PushMotorvehicleCaptureInfo()
{
    NET_TV_ALARMER_S stAlarmer = {0};
    static NET_MotorvehicleCapturePushInfo_S s_stCaptureInfo;

    InitAlarmInfo(&stAlarmer, "Camera-Motor", "192.168.1.107", "SN202312120008", NULL);
    memset(&s_stCaptureInfo, 0, sizeof(s_stCaptureInfo));
    strncpy(s_stCaptureInfo.strVehicleBrand, "DemoBrand", sizeof(s_stCaptureInfo.strVehicleBrand) - 1);
    s_stCaptureInfo.nVehicleType = 8;
    s_stCaptureInfo.nVehicleColor = 9;
    strncpy(s_stCaptureInfo.strLicensePlateNumber, "A12345", sizeof(s_stCaptureInfo.strLicensePlateNumber) - 1);
    strncpy(s_stCaptureInfo.strTimestamp, "2026-08-04 16:00:02", sizeof(s_stCaptureInfo.strTimestamp) - 1);

    if (NET_SERVER_PushMotorvehicleCaptureInfo(&stAlarmer, &s_stCaptureInfo))
    {
        printf("[Demo] Motorvehicle capture info pushed successfully!\n");
    }
    else
    {
        printf("[Demo] Failed to push motorvehicle capture info!\n");
    }
}
 ...
 * @return 无。
 */
void PushNonMotorvehicleCaptureInfo()
{
    NET_TV_ALARMER_S stAlarmer = {0};
    static NET_NonMotorvehicleCapturePushInfo_S s_stCaptureInfo;

    InitAlarmInfo(&stAlarmer, "Camera-NonMotor", "192.168.1.108", "SN202312120009", NULL);
    memset(&s_stCaptureInfo, 0, sizeof(s_stCaptureInfo));
    s_stCaptureInfo.nVehicleType = 1;
    s_stCaptureInfo.nVehicleColor = 3;
    strncpy(s_stCaptureInfo.strTimestamp, "2026-08-04 16:00:03", sizeof(s_stCaptureInfo.strTimestamp) - 1);

    if (NET_SERVER_PushNonMotorvehicleCaptureInfo(&stAlarmer, &s_stCaptureInfo))
    {
        printf("[Demo] Non-motorvehicle capture info pushed successfully!\n");
    }
    else
    {
        printf("[Demo] Failed to push non-motorvehicle capture info!\n");
    }
}
...
void PushRandomAlarm()
{
    ...
    switch (alarmCounter % 10)
    {
        ...
        case 6:
            ...
            break;
        case 7:
            PushPersonCaptureInfo();
            break;
        case 8:
            PushMotorvehicleCaptureInfo();
            break;
        case 9:
            PushNonMotorvehicleCaptureInfo();
            break;
        default:
            PushMotionDetectAlarm();
            break;
