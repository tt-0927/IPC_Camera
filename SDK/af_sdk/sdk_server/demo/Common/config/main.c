 ...
 * 3. 启动 SDK 服务端，等待客户端通过 NET_clientGetDevConfig/NET_clientSetDevConfig 访问配置
 *
 * 说明：
...
static void InitDefaultConfig(void)
{
    ...
    /* 通用运行状态（只读）默认值 */
    g_stDeviceBasicInfo.fCPULoadRatio = 12.5f;
    g_stDeviceBasicInfo.fMemoryUsage = 35.8f;
    g_stDeviceBasicInfo.nBootTime    = 86400;  /* 运行时长-秒，示例 1 天 */

    /* 设备存储信息默认值（NVR/录播等有硬盘的设备专用，无存储设备不初始化即可） */
    g_stDeviceStorageInfo.nHardDiskStatus = 0;  /* 0-正常 */
    strncpy(g_stDeviceStorageInfo.strDiskTotal,       "500GB", sizeof(g_stDeviceStorageInfo.strDiskTotal) - 1);
    strncpy(g_stDeviceStorageInfo.strDiskAvailable,   "320GB", sizeof(g_stDeviceStorageInfo.strDiskAvailable) - 1);
    strncpy(g_stDeviceStorageInfo.strDiskUsedSpace,   "180GB", sizeof(g_stDeviceStorageInfo.strDiskUsedSpace) - 1);
...
/* 设备基本信息 Get 回调，对应命令 NET_GET_DEVICECFG（走通用设备回调 executeGetDeviceBasicInfoCb） */
static NET_COMMON_ECODE_E MyGetDeviceBasicInfoCb(pNET_DeviceBasicInfo_S pInfo)
{
    if (!pInfo)
    {
        return NET_E_INVALID_PARAM;
    ...
    *pInfo = g_stDeviceBasicInfo;

    printf("[ConfigServerDemo] GetDeviceBasicInfo callback\n");
    printf("  DevModel=%s\n", g_stDeviceBasicInfo.strDevModel);
    printf("  SerialNum=%s\n", g_stDeviceBasicInfo.strSerialNum);
    printf("  DeviceName=%s\n", g_stDeviceBasicInfo.strDeviceName);
    printf("  CPULoadRatio=%.1f%%\n", g_stDeviceBasicInfo.fCPULoadRatio);
    printf("  MemoryUsage=%.1f%%\n", g_stDeviceBasicInfo.fMemoryUsage);
    printf("  BootTime=%ds\n", g_stDeviceBasicInfo.nBootTime);

    return NET_E_SUCCEED;
}

/* 设备存储信息 Get 回调，对应命令 NET_GET_STORAGE_INFO（NVR/录播等有硬盘的设备专用） */
static NET_COMMON_ECODE_E MyGetDeviceStorageInfoCb(pNET_DeviceStorageInfo_S pInfo)
{
    if (!pInfo)
    {
        return NET_E_INVALID_PARAM;
    ...
    *pInfo = g_stDeviceStorageInfo;

    printf("[ConfigServerDemo] GetDeviceStorageInfo callback\n");
    printf("  DiskTotal=%s\n", g_stDeviceStorageInfo.strDiskTotal);
    printf("  DiskAvailable=%s\n", g_stDeviceStorageInfo.strDiskAvailable);

    return NET_E_SUCCEED;
}

/* 设备基本信息 Set 回调，对应命令 NET_SET_DEVICECFG（走通用设备回调 executeSetDeviceBasicInfoCb）
 * 仅设备名 strDeviceName 可写，其余身份字段(型号/序列号/固件/MAC/厂商)只读，不予应用 */
static NET_COMMON_ECODE_E MySetDeviceBasicInfoCb(pNET_DeviceBasicInfo_S pInfo)
{
    if (!pInfo)
    {
        return NET_E_INVALID_PARAM;
    ...
    /* 身份字段只读，仅应用设备名 strDeviceName */
    strncpy(g_stDeviceBasicInfo.strDeviceName, pInfo->strDeviceName, NET_LEN_64 - 1);
    g_stDeviceBasicInfo.strDeviceName[NET_LEN_64 - 1] = '\0';

    printf("[ConfigServerDemo] SetDeviceBasicInfo callback (only DeviceName writable)\n");
...
static NET_COMMON_ECODE_E MyDownloadRecordFileCb(INT32 dwChannelID, LPVOID lpInBuffer)
{
    ...
    NET_serverPushAlarmInfo(&stAlarmer,
...
static NET_COMMON_ECODE_E MyGetChannelInfoCb(INT32 dwChannelID, LPVOID lpOutBuffer)
{
    ...
    if (dwChannelID >= 0)
    {
        ...
        pOut->stChannels[0].uChannel = (UINT32)dwChannelID;
    ...
    printf("[ConfigServerDemo] GetChannelInfo callback, Channel=%d, count=%u\n",
           dwChannelID, pOut->uChannelCount);
...
static void RegisterCallbacks(void)
{
    ...
    /* 设备基本信息配置回调（通用身份信息，归设备回调，对应 NET_GET/SET_DEVICECFG） */
    if (NET_serverRegisterGetDeviceBasicInfoCb(MyGetDeviceBasicInfoCb))
    {
        printf("[ConfigServerDemo] RegisterCb_GetDeviceBasicInfo SUCCESS\n");
    }
    else
    {
        printf("[ConfigServerDemo] RegisterCb_GetDeviceBasicInfo FAILED\n");
    }

    if (NET_serverRegisterSetDeviceBasicInfoCb(MySetDeviceBasicInfoCb))
    {
        printf("[ConfigServerDemo] RegisterCb_SetDeviceBasicInfo SUCCESS\n");
    }
    else
    {
        printf("[ConfigServerDemo] RegisterCb_SetDeviceBasicInfo FAILED\n");
    }

    /* 设备存储信息回调（NVR/录播等有硬盘的设备专用，编码器不注册即可） */
    if (NET_serverRegisterGetDeviceStorageInfoCb(MyGetDeviceStorageInfoCb))
    {
        printf("[ConfigServerDemo] RegisterCb_GetDeviceStorageInfo SUCCESS\n");
    }
    else
    {
        printf("[ConfigServerDemo] RegisterCb_GetDeviceStorageInfo FAILED\n");
    }

    /* 网络配置回调 */
    if (NET_serverRegisterGetNetworkConfigCb(MyGetNetworkCfgCb))
    {
    ...
    if (NET_serverRegisterSetNetworkConfigCb(MySetNetworkCfgCb))
    {
    ...
    if (NET_serverRegisterSetNtpConfigCb(MySetNtpCfgCb))
    {
    ...
    /* 音频配置回调 */
    if (NET_serverRegisterGetAudioConfigCb(MyGetAudioCfgCb))
    {
    ...
    if (NET_serverRegisterSetAudioConfigCb(MySetAudioCfgCb))
    {
    ...
    /* 视频码流配置回调 */
    if (NET_serverRegisterGetStreamConfigCb(MyGetStreamCfgCb))
    {
    ...
    if (NET_serverRegisterSetStreamConfigCb(MySetStreamCfgCb))
    {
    ...
    /* WIFI/4G/热点配置回调 */
    if (NET_serverRegisterSetConfigWifiStaCb(MySetConfigWifiStaCb))
    {
    ...
    if (NET_serverRegisterConnectWifiStaCb(MyConnectWifiStaCb))
    {
    ...
    if (NET_serverRegisterDisconnectWifiStaCb(MyDisconnectWifiStaCb))
    {
    ...
    if (NET_serverRegisterGet4GInfoCb(MyGet4GInfoCb))
    {
    ...
    if (NET_serverRegisterSet4GInfoCb(MySet4GInfoCb))
    {
    ...
    if (NET_serverRegisterSetHotspotInfoCb(MySetHotspotInfoCb))
    {
    ...
    if (NET_serverRegisterGetHotspotConnCb(MyGetHotspotConnCb))
    {
    ...
    if (NET_serverRegisterGetSecurityServicesInfoCb(MyGetSecurityServicesInfoCb))
    {
    ...
    if (NET_serverRegisterSetSecurityServicesInfoCb(MySetSecurityServicesInfoCb))
    {
    ...
    if (NET_serverRegisterGetSshCountdownCb(MyGetSshCountdownCb))
    {
    ...
    if (NET_serverRegisterFindLogCb(MyFindLogCb))
    {
    ...
    if (NET_serverRegisterExportLogCb(MyExportLogCb))
    {
    ...
    if (NET_serverRegisterGetLogServerCb(MyGetLogServerCb))
    {
    ...
    if (NET_serverRegisterSetLogServerCb(MySetLogServerCb))
    {
    ...
    if (NET_serverRegisterTestLogServerCb(MyTestLogServerCb))
    {
    ...
    if (NET_serverRegisterControlRecordInfoCb(MyControlRecordInfoCb))
    {
    ...
    if (NET_serverRegisterGetRecordStatusCb(MyGetRecordStatusCb))
    {
    ...
    if (NET_serverRegisterGetRecordScheduleCb(MyGetRecordScheduleCb))
    {
    ...
    if (NET_serverRegisterSetRecordScheduleCb(MySetRecordScheduleCb))
    {
    ...
    if (NET_serverRegisterGetRecordAdvancedParamCb(MyGetRecordAdvancedParamCb))
    {
    ...
    if (NET_serverRegisterSetRecordAdvancedParamCb(MySetRecordAdvancedParamCb))
    {
    ...
    if (NET_serverRegisterFindRecordFileInfoCb(MyFindRecordFileInfoCb))
    {
    ...
    if (NET_serverRegisterDownloadRecordFileCb(MyDownloadRecordFileCb))
    {
    ...
    /* OSD能力集配置回调 */
    if (NET_serverRegisterGetOsdCapConfigCb(MyGetOSDCapCfgCb))
    {
    ...
    if (NET_serverRegisterSetOsdCapConfigCb(MySetOSDCapCfgCb))
    {
    ...
    /* 移动侦测配置回调 */
    if (NET_serverRegisterGetMotionAlarmCb(MyGetMotionAlarmCb))
    {
    ...
    if (NET_serverRegisterSetMotionAlarmCb(MySetMotionAlarmCb))
    {
    ...
    /* 隐私遮盖配置回调 */
    if (NET_serverRegisterGetPrivacyMaskConfigCb(MyGetPrivacyMaskCfgCb))
    {
    ...
    if (NET_serverRegisterSetPrivacyMaskConfigCb(MySetPrivacyMaskCfgCb))
    {
    ...
    /* 遮挡报警配置回调 */
    if (NET_serverRegisterGetTamperAlarmCb(MyGetTamperAlarmCb))
    {
    ...
    if (NET_serverRegisterSetTamperAlarmCb(MySetTamperAlarmCb))
    {
    ...
    /* 越界检测配置回调 */
    if (NET_serverRegisterGetCrossLineAlarmCb(MyGetCrossLineAlarmCb))
    {
    ...
    if (NET_serverRegisterSetCrossLineAlarmCb(MySetCrossLineAlarmCb))
    {
    ...
    /* 入侵检测配置回调 */
    if (NET_serverRegisterGetIntrusionAlarmCb(MyGetIntrusionAlarmCb))
    {
    ...
    /* 进入区域侦测配置回调 */
    if (NET_serverRegisterGetEnterRegionAlarmCb(MyGetEnterRegionAlarmCb))
    {
    ...
    if (NET_serverRegisterSetEnterRegionAlarmCb(MySetEnterRegionAlarmCb))
    {
    ...
    /* 离开区域侦测配置回调 */
    if (NET_serverRegisterGetLeaveRegionAlarmCb(MyGetLeaveRegionAlarmCb))
    {
    ...
    if (NET_serverRegisterSetLeaveRegionAlarmCb(MySetLeaveRegionAlarmCb))
    {
    ...
    /* 徘徊侦测配置回调 */
    if (NET_serverRegisterGetLoiteringAlarmCb(MyGetLoiteringAlarmCb))
    {
    ...
    if (NET_serverRegisterSetLoiteringAlarmCb(MySetLoiteringAlarmCb))
    {
    ...
    /* 场景变更侦测配置回调 */
    if (NET_serverRegisterGetSceneChangeAlarmCb(MyGetSceneChangeAlarmCb))
    {
    ...
    if (NET_serverRegisterSetSceneChangeAlarmCb(MySetSceneChangeAlarmCb))
    {
    ...
    /* 人员聚集侦测配置回调 */
    if (NET_serverRegisterGetCrowGatheringAlarmCb(MyGetCrowdGatheringAlarmCb))
    {
    ...
    if (NET_serverRegisterSetCrowGatheringAlarmCb(MySetCrowdGatheringAlarmCb))
    {
    ...
    /* 垃圾暴露配置回调 */
    if (NET_serverRegisterGetGarbageExposureConfigCb(MyGetGarbageExposureCfgCb))
    {
    ...
    if (NET_serverRegisterSetGarbageExposureConfigCb(MySetGarbageExposureCfgCb))
    {
    ...
    /* 垃圾满溢配置回调 */
    if (NET_serverRegisterGetGarbageOverflowConfigCb(MyGetGarbageOverflowCfgCb))
    {
    ...
    if (NET_serverRegisterSetGarbageOverflowConfigCb(MySetGarbageOverflowCfgCb))
    {
    ...
    /* 人流统计配置回调 */
    if (NET_serverRegisterGetPeopleFlowStatisticsConfigCb(MyGetPeopleFlowStatisticsCfgCb))
    {
    ...
    if (NET_serverRegisterSetPeopleFlowStatisticsConfigCb(MySetPeopleFlowStatisticsCfgCb))
    {
    ...
    if (NET_serverRegisterResetPeopleFlowStatisticsCb(MyResetPeopleFlowStatisticsCb))
    {
    ...
    /* 人员密度检测配置回调 */
    if (NET_serverRegisterGetPeopleDensityDetectionConfigCb(MyGetPeopleDensityDetectionCfgCb))
    {
    ...
    if (NET_serverRegisterSetPeopleDensityDetectionConfigCb(MySetPeopleDensityDetectionCfgCb))
    {
    ...
    /* 井盖异常检测配置回调 */
    if (NET_serverRegisterGetManholeCoverAbnormalConfigCb(MyGetManholeCoverAbnormalCfgCb))
    {
    ...
    if (NET_serverRegisterSetManholeCoverAbnormalConfigCb(MySetManholeCoverAbnormalCfgCb))
    {
    ...
    /* 睡岗识别配置回调 */
    if (NET_serverRegisterGetSleepOnDutyConfigCb(MyGetSleepOnDutyCfgCb))
    {
    ...
    if (NET_serverRegisterSetSleepOnDutyConfigCb(MySetSleepOnDutyCfgCb))
    {
    ...
    /* 电瓶车进电梯识别配置回调 */
    if (NET_serverRegisterGetElectricVehicleInElevatorConfigCb(MyGetElectricVehicleInElevatorCfgCb))
    {
    ...
    if (NET_serverRegisterSetElectricVehicleInElevatorConfigCb(MySetElectricVehicleInElevatorCfgCb))
    {
    ...
    /* 人员倒地识别配置回调 */
    if (NET_serverRegisterGetPersonFallDownConfigCb(MyGetPersonFallDownCfgCb))
    {
    ...
    if (NET_serverRegisterSetPersonFallDownConfigCb(MySetPersonFallDownCfgCb))
    {
    ...
    /* 施工占道识别配置回调 */
    if (NET_serverRegisterGetConstructionOccupyRoadConfigCb(MyGetConstructionOccupyRoadCfgCb))
    {
    ...
    if (NET_serverRegisterSetConstructionOccupyRoadConfigCb(MySetConstructionOccupyRoadCfgCb))
    {
    ...
    /* 拥堵识别配置回调 */
    if (NET_serverRegisterGetCongestionConfigCb(MyGetCongestionCfgCb))
    {
    ...
    if (NET_serverRegisterSetCongestionConfigCb(MySetCongestionCfgCb))
    {
    ...
    /* 车牌识别配置回调 */
    if (NET_serverRegisterGetLicensePlateRecognitionConfigCb(MyGetLicensePlateRecognitionCfgCb))
    {
    ...
    if (NET_serverRegisterSetLicensePlateRecognitionConfigCb(MySetLicensePlateRecognitionCfgCb))
    {
    ...
    /* 高空安全带识别配置回调 */
    if (NET_serverRegisterGetHighAltitudeSeatbeltConfigCb(MyGetHighAltitudeSeatbeltCfgCb))
    {
    ...
    if (NET_serverRegisterSetHighAltitudeSeatbeltConfigCb(MySetHighAltitudeSeatbeltCfgCb))
    {
    ...
    /* 安全帽识别配置回调 */
    if (NET_serverRegisterGetSafetyHelmetConfigCb(MyGetSafetyHelmetCfgCb))
    {
    ...
    if (NET_serverRegisterSetSafetyHelmetConfigCb(MySetSafetyHelmetCfgCb))
    {
    ...
    /* 摔倒识别配置回调 */
    if (NET_serverRegisterGetPersonFallConfigCb(MyGetPersonFallCfgCb))
    {
    ...
    if (NET_serverRegisterSetPersonFallConfigCb(MySetPersonFallCfgCb))
    {
    ...
    /* 玩手机识别配置回调 */
    if (NET_serverRegisterGetPhoneUsageConfigCb(MyGetPhoneUsageCfgCb))
    {
    ...
    if (NET_serverRegisterSetPhoneUsageConfigCb(MySetPhoneUsageCfgCb))
    {
    ...
    /* 抽烟识别配置回调 */
    if (NET_serverRegisterGetSmokingConfigCb(MyGetSmokingCfgCb))
    {
    ...
    if (NET_serverRegisterSetSmokingConfigCb(MySetSmokingCfgCb))
    {
    ...
    /* 明火识别配置回调 */
    if (NET_serverRegisterGetOpenFlameConfigCb(MyGetOpenFlameCfgCb))
    {
    ...
    if (NET_serverRegisterSetOpenFlameConfigCb(MySetOpenFlameCfgCb))
    {
    ...
    /* 黄土裸露识别配置回调 */
    if (NET_serverRegisterGetBareSoilConfigCb(MyGetBareSoilCfgCb))
    {
    ...
    if (NET_serverRegisterSetBareSoilConfigCb(MySetBareSoilCfgCb))
    {
    ...
    /* 洞口防护栏识别配置回调 */
    if (NET_serverRegisterGetHoleProtectionBarConfigCb(MyGetHoleProtectionBarCfgCb))
    {
    ...
    if (NET_serverRegisterSetHoleProtectionBarConfigCb(MySetHoleProtectionBarCfgCb))
    {
    ...
    /* 反光衣识别配置回调 */
    if (NET_serverRegisterGetReflectiveClothingConfigCb(MyGetReflectiveClothingCfgCb))
    {
    ...
    if (NET_serverRegisterSetReflectiveClothingConfigCb(MySetReflectiveClothingCfgCb))
    {
    ...
    /* 宠物识别配置回调 */
    if (NET_serverRegisterGetPetRecognitionInfoCb(MyGetPetRecognitionInfoCb))
    {
    ...
    if (NET_serverRegisterSetPetRecognitionInfoCb(MySetPetRecognitionInfoCb))
    {
    ...
    /* 翻越围栏配置回调 */
    if (NET_serverRegisterGetClimbFenceInfoCb(MyGetClimbFenceInfoCb))
    {
    ...
    if (NET_serverRegisterSetClimbFenceInfoCb(MySetClimbFenceInfoCb))
    {
    ...
    /* 离岗配置回调 */
    if (NET_serverRegisterGetDimissionInfoCb(MyGetDimissionInfoCb))
    {
    ...
    if (NET_serverRegisterSetDimissionInfoCb(MySetDimissionInfoCb))
    {
    ...
    /* 违规变道配置回调 */
    if (NET_serverRegisterGetIllegalLaneInfoCb(MyGetIllegalLaneInfoCb))
    {
    ...
    if (NET_serverRegisterSetIllegalLaneInfoCb(MySetIllegalLaneInfoCb))
    {
    ...
    /* 逆行配置回调 */
    if (NET_serverRegisterGetRetrogradeInfoCb(MyGetRetrogradeInfoCb))
    {
    ...
    if (NET_serverRegisterSetRetrogradeInfoCb(MySetRetrogradeInfoCb))
    {
    ...
    /* 非机动车闯入配置回调 */
    if (NET_serverRegisterGetNonmotorVehicleIntrusionInfoCb(MyGetNonmotorVehicleIntrusionInfoCb))
    {
    ...
    if (NET_serverRegisterSetNonmotorVehicleIntrusionInfoCb(MySetNonmotorVehicleIntrusionInfoCb))
    {
    ...
    /* 应急车道占用识别配置回调 */
    if (NET_serverRegisterGetOccupationEmergencyInfoCb(MyGetOccupationEmergencyInfoCb))
    {
    ...
    if (NET_serverRegisterSetOccupationEmergencyInfoCb(MySetOccupationEmergencyInfoCb))
    {
    ...
    /* 行人闯入配置回调 */
    if (NET_serverRegisterGetPedestrianIntrusionInfoCb(MyGetPedestrianIntrusionInfoCb))
    {
    ...
    if (NET_serverRegisterSetPedestrianIntrusionInfoCb(MySetPedestrianIntrusionInfoCb))
    {
    ...
    /* 烟火识别配置回调 */
    if (NET_serverRegisterGetSmokeFireConfigCb(MyGetSmokeFireCfgCb))
    {
    ...
    if (NET_serverRegisterSetSmokeFireConfigCb(MySetSmokeFireCfgCb))
    {
    ...
    /* 道路积水检测配置回调 */
    if (NET_serverRegisterGetRoadPondingConfigCb(MyGetRoadPondingCfgCb))
    {
    ...
    if (NET_serverRegisterSetRoadPondingConfigCb(MySetRoadPondingCfgCb))
    {
    ...
    /* 停车侦测配置回调 */
    if (NET_serverRegisterGetParkingAlarmCb(MyGetParkingAlarmCb))
    {
    ...
    if (NET_serverRegisterSetParkingAlarmCb(MySetParkingAlarmCb))
    {
    ...
    /* 物品遗留侦测配置回调 */
    if (NET_serverRegisterGetUnattendedObjectAlarmCb(MyGetUnattendedObjectAlarmCb))
    {
    ...
    if (NET_serverRegisterSetUnattendedObjectAlarmCb(MySetUnattendedObjectAlarmCb))
    {
    ...
    /* 物品拿取侦测配置回调 */
    if (NET_serverRegisterGetObjectRemovalAlarmCb(MyGetObjectRemovalAlarmCb))
    {
    ...
    if (NET_serverRegisterSetObjectRemovalAlarmCb(MySetObjectRemovalAlarmCb))
    {
    ...
    /* 音频异常侦测配置回调 */
    if (NET_serverRegisterGetAudioAnomalyAlarmCb(MyGetAudioAnomalyAlarmCb))
    {
    ...
    if (NET_serverRegisterSetAudioAnomalyAlarmCb(MySetAudioAnomalyAlarmCb))
    {
    ...
    if (NET_serverRegisterSetAudibleAlarmInfoCb(ConfigDemoSetAudibleAlarmInfo))
    {
    ...
    if (NET_serverRegisterGetAlarmInputInfoCb(ConfigDemoGetAlarmInputInfo))
    {
    ...
    if (NET_serverRegisterSetAlarmInputInfoCb(ConfigDemoSetAlarmInputInfo))
    {
    ...
    if (NET_serverRegisterGetAlarmOutputInfoCb(ConfigDemoGetAlarmOutputInfo))
    {
    ...
    if (NET_serverRegisterSetAlarmOutputInfoCb(ConfigDemoSetAlarmOutputInfo))
    {
    ...
    if (NET_serverRegisterGetFlashingLightAlarmInfoCb(ConfigDemoGetFlashingLightAlarmInfo))
    {
    ...
    if (NET_serverRegisterSetFlashingLightAlarmInfoCb(ConfigDemoSetFlashingLightAlarmInfo))
    {
    ...
    if (NET_serverRegisterGetPirAlarmInfoCb(ConfigDemoGetPirAlarmInfo))
    {
    ...
    if (NET_serverRegisterSetPirAlarmInfoCb(ConfigDemoSetPirAlarmInfo))
    {
    ...
    /* 图像配置回调 */
    if (NET_serverRegisterGetImageConfigCb(MyGetImageCfgCb))
    {
    ...
    if (NET_serverRegisterSetImageConfigCb(MySetImageCfgCb))
    {
    ...
    /* 预览信息配置回调 */
    if (NET_serverRegisterGetPreviewInfoCb(MyGetPreviewInfoCb))
    {
    ...
    if (NET_serverRegisterSetPreviewInfoCb(MySetPreviewInfoCb))
    {
    ...
    /*通道信息*/
    if (NET_serverRegisterGetChannelInfoCb(MyGetChannelInfoCb))
    {
    ...
    /* 系统升级相关回调*/
    if(NET_serverRegisterGetUpgradeStatusCb(MyGetUpgradeStatus))
    {
    ...
    if(NET_serverRegisterGetUpgradeVersionCb(MyGetUpgradeVersion))
    {
    ...
    if(NET_serverRegisterSetUpgradeCb(MySetUpgrade))
    {
    ...
    /* 抓图参数回调 */
    if (NET_serverRegisterGetCaptureParamInfoCb(MyGetCaptureParamInfo))
    {
    ...
    if (NET_serverRegisterSetCaptureParamInfoCb(MySetCaptureParamInfo))
    {
    ...
    /* 抓图任务回调 */
    if (NET_serverRegisterGetCapturePlanInfoCb(MyGetCapturePlanInfo))
    {
    ...
    if (NET_serverRegisterSetCapturePlanInfoCb(MySetCapturePlanInfo))
    {
    ...
    /* 曝光信息回调 */
    if(NET_serverRegisterGetExposureInfoCb(MyGetExposureInfo))
    {
    ...
    if(NET_serverRegisterSetExposureInfoCb(MySetExposureInfo))
    {
    ...
    /* 日夜转换信息回调*/
    if(NET_serverRegisterGetDayNightInfoCb(MyGetDayNightInfo))
    {
    ...
    if(NET_serverRegisterSetDayNightInfoCb(MySetDayNightInfo))
    {
    ...
    /* 背光信息回调 */
    if(NET_serverRegisterGetBackLightInfoCb(MyGetBackLightInfo))
    {
    ...
    if(NET_serverRegisterSetBackLightInfoCb(MySetBackLightInfo))
    {
    ...
    /* 降噪信息回调 */
    if(NET_serverRegisterGetDenoiseInfoCb(MyGetDenoiseInfo))
    {
    ...
    if(NET_serverRegisterSetDenoiseInfoCb(MySetDenoiseInfo))
    {
    ...
    /* 白平衡回调*/
    if(NET_serverRegisterGetWhiteBalanceInfoCb(MyGetWhiteBalanceInfo))
    {
    ...
    if(NET_serverRegisterSetWhiteBalanceInfoCb(MySetWhiteBalanceInfo))
    {
    ...
    /* 对讲相关*/
    if (NET_serverRegisterSetTalkbackStateCb(MySetTalkbackStateCb))
    {
    ...
    if (NET_serverRegisterSetTalkbackToStreamCb(MySetTalkbackToStreamCb))
    {
    ...
    if (NET_serverRegisterGetTalkbackFromStreamCb(MyGetTalkbackFromStreamCb))
    {
    ...
    if (NET_serverRegisterGetReplayUrlCb(MyGetReplayUrlCb))
    {
    ...
    if (NET_serverRegisterControlReplayCb(MyControlReplayCb))
    {
    ...
    if (NET_serverRegisterGetReplayRecordListCb(MyGetReplayRecordListCb))
    {
    ...
    if (NET_serverRegisterSetReplayTalkbackCb(MySetReplayTalkbackCb))
    {
    ...
    /* 人脸抓拍配置回调 */
    if (NET_serverRegisterGetFaceCaptureInfoCb(MyGetFaceCaptureInfoCb))
    {
    ...
    if (NET_serverRegisterSetFaceCaptureInfoCb(MySetFaceCaptureInfoCb))
    {
    ...
    /* 人脸比对配置回调 */
    if (NET_serverRegisterSetFaceCompareInfoCb(MySetFaceCompareInfoCb))
    {
    ...
    /* 目标库配置回调 */
    if (NET_serverRegisterAddTargetLibCb(MyAddTargetLibCb))
    {
    ...
    if (NET_serverRegisterDelTargetLibCb(MyDelTargetLibCb))
    {
    ...
    if (NET_serverRegisterSetTargetLibCb(MySetTargetLibCb))
    {
    ...
    if (NET_serverRegisterGetTargetLibCb(MyGetTargetLibCb))
    {
    ...
    /* 人脸信息配置回调 */
    if (NET_serverRegisterAddFaceInfoCb(MyAddFaceInfoCb))
    {
    ...
    if (NET_serverRegisterDelFaceInfoCb(MyDelFaceInfoCb))
    {
    ...
    if (NET_serverRegisterSetFaceInfoCb(MySetFaceInfoCb))
    {
    ...
    if (NET_serverRegisterGetFaceInfoCb(MyGetFaceInfoCb))
    {
...
int main(int argc, char* argv[])
{
    ...
    printf("   - 支持 NET_clientGetDevConfig/NET_clientSetDevConfig\n");
    ...
    {
        printf("[ConfigServerDemo] NET_serverInit FAILED!\n");
        return -1;
    }
    ...
    if (NET_serverRegisterVoiceComPlayCb(MyVoiceComPlayCb) &&
        NET_serverRegisterVoiceComCaptureCb(MyVoiceComCaptureCb, NULL) &&
        NET_serverStartVoiceComServer(DEMO_VOICECOM_PORT))
    {
