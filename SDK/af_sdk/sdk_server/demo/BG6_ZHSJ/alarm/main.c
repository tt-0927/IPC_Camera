...
void PushMotionDetectAlarm()
{
    ...
    if (NET_serverPushAlarmInfo(&stAlarmInfo, NET_ALARM_BASE_BASIC, &stBasic, (INT32)sizeof(stBasic)))
    {
...
void PushIntrusionAlarm()
{
    ...
    if (NET_serverPushAlarmInfo(&stAlarmInfo, NET_ALARM_BASE_RULE, pRule, (INT32)sizeof(*pRule)))
    {
...
void PushVideoLossAlarm()
{
    ...
    if (NET_serverPushAlarmInfo(&stAlarmInfo, NET_ALARM_BASE_EXCEPTION, &stEx, (INT32)sizeof(stEx)))
    {
...
void PushPeopleFlowStatisticsAlarm()
{
    ...
    if (NET_serverPushAlarmInfo(&stAlarmInfo, NET_ALARM_PEOPLE_FLOW_STATISTICS, &stStat, (INT32)sizeof(stStat)))
    {
...
void PushFaceCompareAlarm()
{
    ...
    if (NET_serverPushAlarmInfo(&stAlarmInfo, NET_ALARM_FACE_COMPARE, &stCompare, (INT32)sizeof(stCompare)))
    {
...
void PushPeopleCaptureAlarm()
{
    NET_Alarmer_S stAlarmInfo;
    NET_AlarmCaptureInfo_S stCapture = {0};

    printf("[Demo] Pushing People Capture Alarm...\n");

    InitAlarmInfo(&stAlarmInfo, "Camera-06", "192.168.1.105",
                  "SN202312120006", NULL);

    stCapture.uAlarmType = NET_ALARM_CAPTURE_PEOPLE;
    stCapture.uChannel = 1;
    stCapture.uCaptureType = NET_CAPTURE_TYPE_PEOPLE;
    stCapture.llTimestampMs = (INT64)time(NULL) * 1000;
    stCapture.uPanoramaWidth = 1920;
    stCapture.uPanoramaHeight = 1080;
    ...
    stCapture.stPanoramaImg.pData = (BYTE*)malloc(sizeof(demoJpeg));
    if (stCapture.stPanoramaImg.pData)
    {
        memcpy(stCapture.stPanoramaImg.pData, demoJpeg, sizeof(demoJpeg));
        stCapture.stPanoramaImg.uDataLen = (UINT32)sizeof(demoJpeg);
    }
    else
    {
        stCapture.stPanoramaImg.uDataLen = 0;
    }

    /* 构造 2 张裁剪小图 */
    stCapture.uCropCount = 2;

    /* 小图 1：行人全身 */
    stCapture.stCropImages[0].uCropX = 200;
    stCapture.stCropImages[0].uCropY = 100;
    stCapture.stCropImages[0].uCropWidth = 160;
    stCapture.stCropImages[0].uCropHeight = 400;
    stCapture.stCropImages[0].uTargetType = 1;
    stCapture.stCropImages[0].fConfidence = 0.95f;
    stCapture.stCropImages[0].nTrackID = 2001;
    stCapture.stCropImages[0].stImage.pData = (BYTE*)malloc(sizeof(demoJpeg));
    if (stCapture.stCropImages[0].stImage.pData)
    {
        memcpy(stCapture.stCropImages[0].stImage.pData, demoJpeg, sizeof(demoJpeg));
        stCapture.stCropImages[0].stImage.uDataLen = (UINT32)sizeof(demoJpeg);
    }
    else
    {
        stCapture.stCropImages[0].stImage.uDataLen = 0;
    }

    /* 小图 2：行人半身 */
    stCapture.stCropImages[1].uCropX = 220;
    stCapture.stCropImages[1].uCropY = 100;
    stCapture.stCropImages[1].uCropWidth = 120;
    stCapture.stCropImages[1].uCropHeight = 200;
    stCapture.stCropImages[1].uTargetType = 1;
    stCapture.stCropImages[1].fConfidence = 0.92f;
    stCapture.stCropImages[1].nTrackID = 2001;
    stCapture.stCropImages[1].stImage.pData = (BYTE*)malloc(sizeof(demoJpeg));
    if (stCapture.stCropImages[1].stImage.pData)
    {
        memcpy(stCapture.stCropImages[1].stImage.pData, demoJpeg, sizeof(demoJpeg));
        stCapture.stCropImages[1].stImage.uDataLen = (UINT32)sizeof(demoJpeg);
    }
    else
    {
        stCapture.stCropImages[1].stImage.uDataLen = 0;
    }

    if (NET_SERVER_PushAlarmInfo(&stAlarmInfo, NET_ALARM_CAPTURE_PEOPLE, &stCapture, (INT32)sizeof(stCapture)))
    {
        printf("[Demo] People Capture Alarm pushed successfully! cropCount=%u panorama=%ux%u\n",
               stCapture.uCropCount, stCapture.uPanoramaWidth, stCapture.uPanoramaHeight);
    }
    else
    {
        printf("[Demo] Failed to push People Capture Alarm!\n");
    }

    /* 释放图片内存 */
    if (stCapture.stPanoramaImg.pData) free(stCapture.stPanoramaImg.pData);
    for (UINT32 i = 0; i < stCapture.uCropCount; ++i)
    {
        if (stCapture.stCropImages[i].stImage.pData) free(stCapture.stCropImages[i].stImage.pData);
    }
}
...
void AddRegisterCb()
{
    NET_serverRegisterGetDeviceInfoCb(MyDeviceInfoCb);
}
...
int main(int argc, char* argv[])
{
    ...
    if (!NET_serverInit(dwPort, szUserName, szPassword, NULL))
    {
