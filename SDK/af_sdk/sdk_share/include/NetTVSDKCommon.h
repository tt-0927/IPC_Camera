/*
 * 文件名：NetTVSDKCommon.h
 * 创建日期：2026-05-13
 * 创建作者：qinjt
 * 功能描述：定义网络摄像机 SDK 客户端与服务端共用的公共命令和数据结构。
 * 修改记录：
 * 2026-08-04 ITC：新增声音报警、报警输入、报警输出、闪光报警和人体红外报警配置结构。
 */
...
typedef enum tagNETTVCfgCmd
{
    ...
    /* 获取声音报警配置，参见 NET_AudibleAlarmInfo_S。 */
    NET_GET_AUDIBLE_ALARM_INFO            = 494,
    /* 设置声音报警配置，参见 NET_AudibleAlarmInfo_S。 */
    NET_SET_AUDIBLE_ALARM_INFO            = 495,
    /* 获取报警输入配置，参见 NET_AlarmInputInfoList_S。 */
    NET_GET_ALARM_INPUT_INFO              = 496,
    /* 设置报警输入配置，参见 NET_AlarmInputInfo_S。 */
    NET_SET_ALARM_INPUT_INFO              = 497,
    /* 获取报警输出配置，参见 NET_AlarmOutputInfoList_S。 */
    NET_GET_ALARM_OUTPUT_INFO             = 498,
    /* 设置报警输出配置，参见 NET_AlarmOutputInfo_S。 */
    NET_SET_ALARM_OUTPUT_INFO             = 499,
    /* 获取闪光报警灯配置，参见 NET_FlashingLightAlarmInfo_S。 */
    NET_GET_FLASHING_LIGHT_ALARM_INFO     = 500,
    /* 设置闪光报警灯配置，参见 NET_FlashingLightAlarmInfo_S。 */
    NET_SET_FLASHING_LIGHT_ALARM_INFO     = 501,
    /* 获取人体红外（PIR）报警配置，参见 NET_PirAlarmInfo_S。 */
    NET_GET_PIR_ALARM_INFO                = 502,
    /* 设置人体红外（PIR）报警配置，参见 NET_PirAlarmInfo_S。 */
    NET_SET_PIR_ALARM_INFO                = 503,
    /* 推送人脸抓拍信息，参见 NET_FaceCapturePushInfo_S。 */
    NET_PUSH_FACE_CAPTURE_INFO             = 504,
    /* 推送行人抓拍信息，参见 NET_PersonCapturePushInfo_S。 */
    NET_PUSH_PERSON_CAPTURE_INFO           = 505,
    /* 推送机动车抓拍信息，参见 NET_MotorvehicleCapturePushInfo_S。 */
    NET_PUSH_MOTORVEHICLE_CAPTURE_INFO     = 506,
    /* 推送非机动车抓拍信息，参见 NET_NonMotorvehicleCapturePushInfo_S。 */
    NET_PUSH_NONMOTORVEHICLE_CAPTURE_INFO  = 507,
    /* 触发声光报警联动事件，参见 NET_SoundLightAlarmTrigger_S。 */
    NET_TV_TRIGGER_SOUND_LIGHT_ALARM       = 508,
...
typedef enum tagNETTVAlarmLightControlCmd
{
    ...
    NET_TV_ALARM_LIGHT_CTRL_SET_MODE = 3,       /* 设置声光模式，参数由 dwParam1/dwParam2 或 szExt 承载 */
...
/**
 * @struct tagNETTVVoiceComAudioCfg
 * @brief 对讲音频参数配置
 * @note 用于NET_TV_GET_VOICECOM_AUDIO_CFG/NET_TV_SET_VOICECOM_AUDIO_CFG
 */
typedef struct tagNETTVVoiceComAudioCfg
{
    ...
    INT32   dwFrameIntervalMs;    /* 单帧时长, ms */
    INT32   dwFrameBytes;         /* 单帧字节数 */
    INT32   dwBitRate;            /* 比特率, bps */
    BOOL    bLittleEndian;        /* 是否小端序 */
    BYTE    byRes[128];           /* 保留字段 */
} NET_TV_VOICECOM_AUDIO_CFG_S, *LPNET_TV_VOICECOM_AUDIO_CFG_S;
...
/**
 * @struct tagNETTVUserPasswordInfo
 * @brief 修改用户密码参数结构体
 * @note 用于 NET_TV_SERVER_RegisterCb_SetUserPassword 回调
 */
typedef struct tagNETTVUserPasswordInfo
{
    CHAR szUserName[NET_TV_USERNAME_LEN];              /* 用户名 */
    CHAR szOldPassword[NET_TV_PASSWORD_LEN];            /* 旧密码 */
    CHAR szNewPassword[NET_TV_PASSWORD_LEN];            /* 新密码 */
    BYTE byRes[128];                                    /* 保留字段 */
}NET_TV_USER_PASSWORD_INFO_S, *LPNET_TV_USER_PASSWORD_INFO_S;
...
/*
 * 动态图片视图。图片数据由调用方持有，SDK 仅在本次同步推送或回调期间访问。
 * 使用该结构可避免每条告警预分配 1 MiB 的固定图片数组。
 */
typedef struct tagNETTVImageData
{
    const BYTE  *pData;      /* JPEG 二进制数据，空图片时为 NULL */
    UINT32       dwLen;      /* JPEG 实际字节数 */
    UINT32       dwWidth;    /* 图片宽度，未知时为 0 */
    UINT32       dwHeight;   /* 图片高度，未知时为 0 */
} NET_TV_IMAGE_DATA_S, *LPNET_TV_IMAGE_DATA_S;

/* 动态图片 V2 告警结构。V1 结构保留，保证已有 SDK 使用方 ABI 不受影响。 */
 ...
 * 该请求不修改设备长期配置，设备仅按本次请求携带的联动列表模拟触发事件。
 */
