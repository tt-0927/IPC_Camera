...
// 端口复用控制宏：定义此宏可禁用端口复用，防止多个服务绑定同一端口
// Port reuse control macro: Define this to disable port reuse, preventing multiple servers from binding to the same port
#ifndef SDK_HTTP_DISABLE_PORT_REUSE
  // 默认启用端口复用 (Enable port reuse by default)
  #define SDK_HTTP_ENABLE_PORT_REUSE 1
#else
  // 禁用端口复用 (Disable port reuse)
  #define SDK_HTTP_ENABLE_PORT_REUSE 0
#endif

inline void default_socket_options(socket_t sock) {
#if SDK_HTTP_ENABLE_PORT_REUSE
  // 启用端口复用：允许快速重启或多进程负载均衡
  // Enable port reuse: Allow fast restart or multi-process load balancing
  detail::set_socket_opt(sock, SOL_SOCKET,
                         ...
                         1);
#else
  // 禁用端口复用：防止多个服务意外绑定同一端口
  // Disable port reuse: Prevent multiple services from accidentally binding to the same port
  detail::set_socket_opt(sock, SOL_SOCKET,
#ifdef SO_REUSEPORT
                         SO_REUSEPORT,
#else
