...
protected:
    ...
    {
        auto time = msg.time;
        /* Change by zjc. 2022-08-23，2022-08-30 */
        auto diff = time - rotation_tp_;
        auto day = std::chrono::nanoseconds(std::chrono::hours(24));
        bool should_rotate = time >= rotation_tp_ || diff <= -day;
        if (should_rotate)
        {
